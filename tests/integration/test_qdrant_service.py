...
def qdrant_service(mock_qdrant_client):
    ...
    with patch("telegram_bot.services.qdrant.AsyncQdrantClient") as mock_class:
        ...
        ***REMOVED*** Expose mock for assertions
        service._mock_client = mock_qdrant_client
        yield service


***REMOVED*** =============================================================================
***REMOVED*** TestQdrantServiceInit - Initialization and Quantization Defaults
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** TestHybridSearchRRFQuantization - Quantization in hybrid search
***REMOVED*** =============================================================================
...
class TestHybridSearchRRFQuantization:
    ...
    async def test_search_with_quantization_enabled(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient") as mock_class:
            ...
            ***REMOVED*** Verify search_params has quantization config
            assert call_kwargs["search_params"] is not None
            quant_params = call_kwargs["search_params"].quantization
            assert quant_params.ignore is False  ***REMOVED*** Use quantization
    ...
    async def test_search_with_quantization_disabled_at_init(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient") as mock_class:
            ...
            call_kwargs = mock_client.query_points.call_args[1]
            ***REMOVED*** Should not have search_params when quantization disabled
    ...
    async def test_search_with_quantization_ignore_true(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient") as mock_class:
            ...
            ***REMOVED*** Service has quantization enabled by default
            service = QdrantService(
                url="http://localhost:6333",
                collection_name="test",
                use_quantization=True,
            ...
            ***REMOVED*** But we override per-request to ignore quantization
            await service.hybrid_search_rrf(
                dense_vector=[0.1] * 1024,
                top_k=10,
                quantization_ignore=True,  ***REMOVED*** A/B test: skip quantization
            )

            call_kwargs = mock_client.query_points.call_args[1]

            ***REMOVED*** Should have search_params with ignore=True
            assert call_kwargs["search_params"] is not None
    ...
    async def test_search_with_quantization_ignore_false(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient") as mock_class:
            ...
            ***REMOVED*** Service has quantization disabled
            service = QdrantService(
                url="http://localhost:6333",
                collection_name="test",
                use_quantization=False,
            ...
            ***REMOVED*** But we override per-request to force quantization
            await service.hybrid_search_rrf(
                dense_vector=[0.1] * 1024,
                top_k=10,
                quantization_ignore=False,  ***REMOVED*** A/B test: force quantization
            )

            call_kwargs = mock_client.query_points.call_args[1]

            ***REMOVED*** Should have search_params with quantization enabled
            assert call_kwargs["search_params"] is not None
    ...
    async def test_search_with_quantization_ignore_none_uses_default(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient") as mock_class:
            ...
            service = QdrantService(
                ...
                collection_name="test",
                use_quantization=True,
                quantization_oversampling=4.0,  ***REMOVED*** Custom value
            )

            await service.hybrid_search_rrf(
                dense_vector=[0.1] * 1024,
                top_k=10,
                quantization_ignore=None,  ***REMOVED*** Use default
            )

            call_kwargs = mock_client.query_points.call_args[1]

            ***REMOVED*** Should use service defaults
            assert call_kwargs["search_params"].quantization.oversampling == 4.0


***REMOVED*** =============================================================================
***REMOVED*** TestBinaryQuantization - enable_binary_quantization and get_collection_info
***REMOVED*** =============================================================================
...
class TestBinaryQuantization:
    ...
    async def test_enable_binary_quantization_success(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient") as mock_class:
            mock_client = AsyncMock()
            mock_client.update_collection.return_value = None  ***REMOVED*** Success
            mock_class.return_value = mock_client
    ...
    async def test_enable_binary_quantization_always_ram_false(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient") as mock_class:
            ...
            assert result is True
            call_kwargs = mock_client.update_collection.call_args[1]
            ***REMOVED*** Verify BinaryQuantization config
...
***REMOVED*** =============================================================================
***REMOVED*** Original tests (TestQdrantServiceUnit)
***REMOVED*** =============================================================================
...
class TestMMRRerank:
    ...
    def test_mmr_rerank_selects_diverse_results(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient"):
            ...
            ***REMOVED*** Create mock points with embeddings
            ***REMOVED*** Points 0 and 1 are very similar, point 2 is different
            points = [
            ...
            ***REMOVED*** Embeddings: 0 and 1 are similar, 2 is different
            embeddings = [
                [1.0, 0.0, 0.0],
                [0.99, 0.1, 0.0],  ***REMOVED*** Very similar to 0
                [0.0, 1.0, 0.0],  ***REMOVED*** Different
            ]

            result = service.mmr_rerank(
                points=points,
                embeddings=embeddings,
                lambda_mult=0.5,  ***REMOVED*** Balanced
                top_k=2,
            )

            ***REMOVED*** Should select point 0 (highest score) first
            assert result[0]["id"] == "0"
            ***REMOVED*** Second should be point 2 (more diverse) not point 1
    ...
    def test_mmr_rerank_with_high_lambda_prefers_relevance(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient"):
            ...
            result = service.mmr_rerank(
                points=points,
                embeddings=embeddings,
                lambda_mult=1.0,  ***REMOVED*** Only relevance
                top_k=2,
            )

            ***REMOVED*** Should select by score only: 0, then 1
            assert result[0]["id"] == "0"
