...
async def test_postgres_connection():
    ***REMOVED*** Defaults from docker-compose
...
def test_qdrant_health():
    ***REMOVED*** Qdrant client uses sync HTTP by default or async
...
async def test_bm42_health():
    async with aiohttp.ClientSession() as session:
        async with session.get("http://localhost:8002/health") as resp:
            ***REMOVED*** Note: BM42 service in docker-compose.dev.yml maps 8002:8000
            assert resp.status == 200
            data = await resp.json()
            assert data["status"] == "ok"
