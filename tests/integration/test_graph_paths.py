...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Path 2b: classify(GENERAL) → cache_check(MISS, allowlist bypass) → retrieve
***REMOVED***          Semantic cache skipped for non-allowlisted types (***REMOVED***163)
***REMOVED*** ---------------------------------------------------------------------------


@pytest.mark.integration
async def test_path_general_bypasses_semantic_cache():
    """GENERAL query type bypasses semantic cache check and store (***REMOVED***163)."""
    mocks = _make_graph_mocks(
        cache_semantic="should not be used",  ***REMOVED*** would be a hit if checked
    ...
    ***REMOVED*** GENERAL query — not in CACHEABLE_QUERY_TYPES
    state = make_initial_state(
    ...
    assert result["cache_hit"] is False
    ***REMOVED*** Semantic cache NOT checked (allowlist guard)
    mocks["cache"].check_semantic.assert_not_awaited()
