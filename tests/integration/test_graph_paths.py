...
def _make_llm_completion(content: str) -> MagicMock:
    ...
    ***REMOVED*** Set serializable values to avoid MagicMock in state (needed for checkpointer)
...
def _make_mock_graph_config(llm_mock: MagicMock) -> MagicMock:
    ...
    ***REMOVED*** Needed by _create_summarize_model when checkpointer is active (***REMOVED***154)
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Path 1b: classify(GENERAL) → guard(BLOCKED) → respond → END
***REMOVED*** ---------------------------------------------------------------------------
...
async def test_path_guard_blocked():
    ...
    ***REMOVED*** State assertions
    ...
    assert result["response"]  ***REMOVED*** canned blocked response
    assert result["cache_hit"] is False
    assert result["documents"] == []
    ...
    ***REMOVED*** respond_node sent the blocked message
    mocks["message"].answer.assert_awaited_once()


***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Path 2: classify(FAQ) → cache_check(HIT) → respond → END
***REMOVED*** ---------------------------------------------------------------------------
...
async def test_path_cache_hit():
    ...
    ***REMOVED*** FAQ query (allowlisted for semantic cache) — triggers cache_check_node
    state = make_initial_state(
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Path 2b: classify(GENERAL) → cache_check(MISS) → retrieve → generate
***REMOVED***          GENERAL is in CACHEABLE_QUERY_TYPES (threshold 0.08, ***REMOVED***477)
***REMOVED*** ---------------------------------------------------------------------------


@pytest.mark.integration
async def test_path_general_uses_semantic_cache():
    """GENERAL query type uses semantic cache (threshold 0.08, ***REMOVED***477)."""
    mocks = _make_graph_mocks(
        cache_semantic=None,  ***REMOVED*** cache miss — pipeline continues to retrieve/generate
    ...
    ***REMOVED*** GENERAL query — now in CACHEABLE_QUERY_TYPES (***REMOVED***477)
    state = make_initial_state(
    ...
    assert result["cache_hit"] is False
    ***REMOVED*** Semantic cache IS checked (GENERAL now in allowlist)
    mocks["cache"].check_semantic.assert_awaited_once()
    ***REMOVED*** Semantic cache IS stored after generate
...
async def test_path_happy_retrieve_rerank_generate():
    ...
    ***REMOVED*** ENTITY query (allowlisted for semantic cache)
    state = make_initial_state(
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Path 7: classify → cache_check(MISS) → retrieve → grade(irrelevant)
***REMOVED***        → rewrite → retrieve → grade(irrelevant, score not improved)
***REMOVED***        → generate (score_improved=False) → cache_store → respond → END
***REMOVED*** ---------------------------------------------------------------------------
...
async def test_path_rewrite_stopped_by_score_guard():
    ...
    ***REMOVED*** First retrieve: score=0.003 (not relevant, prev=0.0 → improved=True → rewrite)
    first_docs = [
        {"text": "Нерелевантный текст", "score": 0.003, "id": "x1", "metadata": {}},
    ]
    ***REMOVED*** Second retrieve: score=0.0031 (delta=0.0001 < 0.001 → improved=False → generate)
    second_docs = [
    ...
    ***REMOVED*** Qdrant: 1st call → low score, 2nd call → barely higher (delta < threshold)
    _ok_meta = {"backend_error": False, "error_type": None, "error_message": None}
    mocks["qdrant"].hybrid_search_rrf = AsyncMock(
        side_effect=[(first_docs, _ok_meta), (second_docs, _ok_meta)]
    )

    ***REMOVED*** LLM: 1st call → rewrite, 2nd call → generate answer
    ...
    mock_gc = _make_mock_graph_config(mocks["llm"])
    mock_gc.max_rewrite_attempts = 3  ***REMOVED*** allow many retries
    ...
    state["max_rewrite_attempts"] = 3  ***REMOVED*** allow many retries

    with traced_pipeline(session_id="test-score-guard", user_id="integration"):
        with _patch_graph_configs(mock_gc):
            result = await graph.ainvoke(state)

    ***REMOVED*** State assertions: rewrite happened once, then stopped by score guard
    assert result["rewrite_count"] == 1
    assert result["documents_relevant"] is False
    ...
    ***REMOVED*** Service call counts
    assert mocks["qdrant"].hybrid_search_rrf.await_count == 2  ***REMOVED*** retrieve twice
    assert mocks["llm"].chat.completions.create.await_count == 2  ***REMOVED*** rewrite + generate
    mocks["reranker"].rerank.assert_not_awaited()  ***REMOVED*** rerank skipped (never relevant)


***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Path 8: START → transcribe → classify → cache_check(MISS) → retrieve
***REMOVED***        → grade(relevant) → rerank → generate → cache_store → respond → END
***REMOVED*** ---------------------------------------------------------------------------
...
async def test_path_voice_transcribe_full_rag():
    ...
    ***REMOVED*** Add audio transcription mock to LLM
    ...
    ***REMOVED*** Voice state: voice_audio present → route_start → transcribe
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Conversation Memory: checkpointer persists messages across invocations
***REMOVED*** ---------------------------------------------------------------------------
...
class TestConversationMemory:
    ...
    async def test_second_query_sees_first_in_messages(self):
        ...
        ***REMOVED*** Build without message mock — avoids MagicMock serialization in state
        ...
        ***REMOVED*** Reset mocks for second invocation
        ...
        ***REMOVED*** Second query — should see first query in messages via checkpointer
        ...
        ***REMOVED*** Verify messages contain history from first query
    ...
    async def test_summarize_observation_setup_failure_still_runs_summarize(self):
        ...
        ***REMOVED*** Response still returned (pipeline not broken by observation failure)
        assert result["response"]
        ***REMOVED*** Summarize was still invoked despite Langfuse observation setup failure
        summarize_node.ainvoke.assert_awaited_once()
        ***REMOVED*** Latency still recorded
        assert "summarize" in result.get("latency_stages", {})
    ...
    async def test_summarize_observation_update_failure_preserves_result(self):
        ...
        ***REMOVED*** Response still returned (pipeline not broken by update failure)
        assert result["response"]
        ***REMOVED*** Summarize was invoked
        summarize_node.ainvoke.assert_awaited_once()
        ***REMOVED*** Latency still recorded
        assert "summarize" in result.get("latency_stages", {})
        ***REMOVED*** Update was attempted and failed (but that didn't break anything)
        mock_observation.update.assert_called_once()


***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Path 8b: GENERAL coverage query → grouped RRF, bypass ColBERT
***REMOVED*** ---------------------------------------------------------------------------
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Path 9: classify → cache_check(ColBERT embed) → retrieve(ColBERT search)
***REMOVED***        → grade(relevant, rerank_applied=True) → generate → cache_store → respond
***REMOVED*** ---------------------------------------------------------------------------
...
async def test_path_retrieve_colbert_skips_rerank():
    ...
    ***REMOVED*** Embeddings supports aembed_hybrid_with_colbert → returns 3-tuple
    colbert_vecs = [[0.2] * 1024] * 4  ***REMOVED*** 4 token vectors
    mocks["embeddings"].aembed_hybrid_with_colbert = AsyncMock(
        return_value=([0.1] * 1024, {"indices": [1, 5], "values": [0.5, 0.3]}, colbert_vecs)
    )

    ***REMOVED*** Qdrant supports hybrid_search_rrf_colbert → 3-stage server-side search
    _ok_meta = {"backend_error": False, "error_type": None, "error_message": None}
    colbert_docs = [
        {
            "text": "Квартира в Несебр, 85000 евро",
            "score": 85.0,  ***REMOVED*** ColBERT MaxSim score (different scale)
            "id": "1",
            "metadata": {"title": "Квартира", "city": "Несебр", "price": 85000},
        },
        {
    ...
    ***REMOVED*** State assertions
    assert result["cache_hit"] is False
    assert result["documents_relevant"] is True
    assert result["rerank_applied"] is True  ***REMOVED*** set by retrieve_node (server-side ColBERT)
    assert result["response"] == "Найдено 2 варианта квартир с ColBERT."

    ***REMOVED*** ColBERT search path used (NOT regular hybrid_search_rrf)
    mocks["qdrant"].hybrid_search_rrf_colbert.assert_awaited_once()
    mocks["qdrant"].hybrid_search_rrf.assert_not_awaited()

    ***REMOVED*** Rerank node SKIPPED (server-side ColBERT already reranked)
    mocks["reranker"].rerank.assert_not_awaited()

    ***REMOVED*** Generate ran
    mocks["llm"].chat.completions.create.assert_awaited_once()
