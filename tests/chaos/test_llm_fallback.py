...
class TestLLMResponseParsing:
    ...
    async def test_malformed_json_response_handled(self):
        ...
        ***REMOVED*** Instructor failure falls through to generic exception → fallback
        assert isinstance(result, ConfidenceResult)
        assert result.confidence == 0.0
        assert result.is_low_confidence is True
    ...
    async def test_missing_confidence_field_handled(self):
        ...
        assert isinstance(result, ConfidenceResult)
        assert result.confidence == 0.5  ***REMOVED*** Pydantic default when missing
    ...
    async def test_invalid_confidence_value_clamped(self):
        ...
        ***REMOVED*** 1.5 is clamped to 1.0 by the ConfidenceResponse field_validator
        assert result.confidence == 1.0  ***REMOVED*** Clamped to max
...
class TestLLMFallbackChain:
    ...
    async def test_fallback_includes_search_results(self, httpx_mock: HTTPXMock):
        ...
        ***REMOVED*** Should show first 3 results
