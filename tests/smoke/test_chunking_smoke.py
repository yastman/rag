...
except ModuleNotFoundError as exc:  ***REMOVED*** pragma: no cover - legacy smoke helper is optional
    pytest.skip(f"legacy chunking helper unavailable: {exc}", allow_module_level=True)


***REMOVED*** Test file path
CRIMINAL_CODE_PATH = (
...
class TestChunkingQuality:
    ...
    def test_all_chunks_have_article_number(self, chunks):
        ...
        ***REMOVED*** Should be 100% for legal documents
    ...
    def test_reasonable_chunk_count(self, chunks):
        ...
        ***REMOVED*** Criminal Code has 564 articles
        ***REMOVED*** Expected: ~500-600 chunks (close to 1:1 ratio)
        ***REMOVED*** Docling was creating 1294 (way too many!)
    ...
    def test_articles_are_sequential(self, chunks):
        ...
        ***REMOVED*** Get all unique articles in order they appear
        articles_in_order = []
        for chunk in chunks:
        ...
        ***REMOVED*** Check that articles are mostly sequential (allowing some gaps for notes, etc)
        gaps = []
        for i in range(len(articles_in_order) - 1):
            gap = articles_in_order[i + 1] - articles_in_order[i]
            if gap > 10:  ***REMOVED*** Large gap
        ...
        ***REMOVED*** Should have close to 564 unique articles
        assert 500 <= len(articles_in_order) <= 600, (
            f"Found {len(articles_in_order)} unique articles, expected ~564"
        )

        ***REMOVED*** Should not have too many large gaps (indicates scattered articles)
        assert len(gaps) < 50, (
    ...
    def test_no_duplicate_article_splitting(self, chunks):
        ...
        ***REMOVED*** Check for the specific bug: "Article 3" appearing in 15 scattered locations
        article_positions = {}
        ...
        ***REMOVED*** Find articles that are scattered (non-consecutive chunk indices)
        scattered_articles = {}
        for article, positions in article_positions.items():
            if len(positions) > 1:
                ***REMOVED*** Check if positions are consecutive
                gaps = [positions[i + 1] - positions[i] for i in range(len(positions) - 1)]
                max_gap = max(gaps) if gaps else 0
                if max_gap > 5:  ***REMOVED*** Not consecutive
        ...
        ***REMOVED*** Should have very few scattered articles (long articles can legitimately span chunks)
    ...
    def test_chunk_metadata_completeness(self, chunks):
        ...
        for i, chunk in enumerate(chunks[:10]):  ***REMOVED*** Check first 10
            ...
            ***REMOVED*** Check text is not empty
            assert len(chunk["text"].strip()) > 0, f"Chunk {i} has empty text"

            ***REMOVED*** Check article_number is valid integer
    ...
    def test_chunk_sizes_reasonable(self, chunks):
        ...
        token_counts = [len(chunk["text"]) // 4 for chunk in chunks]  ***REMOVED*** Rough token estimate
        ...
        ***REMOVED*** Most chunks should be within target range (400-800)
        ...
        ***REMOVED*** At least 70% should be in range
