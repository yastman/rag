...
except ModuleNotFoundError as exc:  ***REMOVED*** pragma: no cover - legacy benchmark helper is optional
...
def _run_docling_approach():
    ...
    ***REMOVED*** Initialize tokenizer (same as for embeddings)
    print("\n⚙️  Creating HybridChunker with HuggingFace tokenizer...")
    EMBED_MODEL_ID = "sentence-transformers/all-MiniLM-L6-v2"
    tokenizer = HuggingFaceTokenizer(
        tokenizer=AutoTokenizer.from_pretrained(EMBED_MODEL_ID),
        max_tokens=800,  ***REMOVED*** Match PyMuPDF max
    )
    ...
    ***REMOVED*** Analyze metadata - Docling chunks have different structure
    print("📊 Metadata Analysis:")
    print(f"   Total chunks: {len(chunks)}")

    ***REMOVED*** Docling chunks have meta.headings instead of article_number
    ...
    ***REMOVED*** Try to find article numbers in text
    chunks_with_article_in_text = []
    for chunk in chunks:
        if "Стаття" in chunk.text[:50]:  ***REMOVED*** Check if starts with "Стаття"
