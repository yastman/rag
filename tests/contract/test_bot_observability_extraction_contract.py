...
``telegram_bot/observability/trace.py`` (card_2a71ec058138).
...
* ``telegram_bot.observability.trace`` exists and exposes ``_build_trace_metadata``.
...
HELPERS_PY = REPO_ROOT / "telegram_bot" / "observability" / "trace.py"
...
def test_bot_observability_helpers_module_exists() -> None:
    assert HELPERS_PY.exists(), (
        "card_2a71ec058138: telegram_bot/observability/trace.py is the new home "
...
def test_extracted_helpers_module_has_no_aiogram_or_fastapi_imports() -> None:
    ...
    for forbidden in ("aiogram", "langgraph", "langchain", "fastapi"):
        assert f"import {forbidden}" not in text and f"from {forbidden}" not in text, (
            f"telegram_bot/observability/trace.py must not import {forbidden!r}; "
            "it is a pure observability dict/score helper module."
        )


def test_bot_observability_module_exposes_build_trace_metadata() -> None:
    module = importlib.import_module("telegram_bot.observability.trace")
    assert hasattr(module, "_build_trace_metadata"), (
        "telegram_bot.observability.trace missing required helper '_build_trace_metadata'."
    )
...
def test_bot_py_re_exports_build_trace_metadata_with_runtime_parity() -> None:
    ...
    helpers = importlib.import_module("telegram_bot.observability.trace")
