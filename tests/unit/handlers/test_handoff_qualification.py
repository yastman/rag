...
def test_build_goal_keyboard():
    ***REMOVED*** Pass a mock i18n that returns the key as-is.
    kb = build_goal_keyboard(i18n=None)
    ***REMOVED*** Should have 3 buttons in one row.
...
def test_build_budget_keyboard():
    ...
    assert len(kb.inline_keyboard) == 2  ***REMOVED*** 2 rows: [3 buttons] + [1 button]
...
def test_build_contact_keyboard():
    ...
    assert len(kb.inline_keyboard) == 1  ***REMOVED*** 1 row: 2 buttons
