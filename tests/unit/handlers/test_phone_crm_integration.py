...
def mock_kommo():
    ...
    kommo.create_lead = AsyncMock(return_value=Lead(id=101, name="Заявка: test_source — Iван"))
...
async def test_creates_contact_and_lead(mock_kommo, mock_message, mock_state):
...
async def test_creates_manager_task(mock_kommo, mock_message, mock_state):
...
async def test_graceful_when_kommo_disabled(mock_message, mock_state):
...
async def test_graceful_on_kommo_error(mock_kommo, mock_message, mock_state):
...
async def test_source_tracking_in_lead_name(mock_kommo, mock_message, mock_state):
    """Lead name includes source from FSM state data."""
    from telegram_bot.handlers.phone_collector import on_phone_received
    ...
    assert "test_source" in lead_arg.name
