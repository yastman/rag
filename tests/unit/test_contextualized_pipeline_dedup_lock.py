"""Regression lock: contextualized pipeline test was duplicated (closes ***REMOVED***1782).
...
copies were skipped collection-wide before ***REMOVED***1775 because they imported
...
surface. ***REMOVED***1782 calls for "remove or guard duplicate".
...
def test_integration_duplicate_was_removed() -> None:
    """The integration-tier duplicate must stay deleted (***REMOVED***1782)."""
    assert not DUPLICATE_PATH.exists(), (
        ...
        f"***REMOVED***1782. If a real integration test for the contextualized "
...
def test_unit_test_file_still_exists() -> None:
    ...
    assert UNIT_PATH.exists(), (
        ...
        f"integration-tier duplicate was deleted (***REMOVED***1782)."
    )
    text = UNIT_PATH.read_text(encoding="utf-8")
    ***REMOVED*** Smoke check: file remains a non-trivial test for the pipeline.
    assert len(text) > 1000, "unit test shrank unexpectedly; ***REMOVED***1782 lock"
    assert "ContextualizedEmbeddingService" in text, (
        "unit test no longer references ContextualizedEmbeddingService; ***REMOVED***1782 / ***REMOVED***1773 contract"
    )
