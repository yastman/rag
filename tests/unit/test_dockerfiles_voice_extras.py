"""Regression tests for Dockerfiles that run API/voice services (archived)."""
...
def test_api_dockerfile_sets_python_unbuffered() -> None:
    ...
    assert "PYTHONUNBUFFERED=1" in text, (
        "archive/api/Dockerfile runtime must set PYTHONUNBUFFERED=1"
    )
...
def test_voice_dockerfile_sets_python_unbuffered() -> None:
    text = Path("archive/voice/Dockerfile").read_text(encoding="utf-8")
    assert "PYTHONUNBUFFERED=1" in text, (
        "archive/voice/Dockerfile runtime must set PYTHONUNBUFFERED=1"
    )
