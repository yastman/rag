...
async def test_no_idle_sessions(worker):
    worker._redis.keys = AsyncMock(return_value=[])
    result = await worker._check_idle_sessions()
    assert result == 0


async def test_idle_session_detected(worker):
    worker._redis.keys = AsyncMock(return_value=[b"session:last_active:123"])
...
async def test_skip_short_conversations(worker):
    worker._redis.keys = AsyncMock(return_value=[b"session:last_active:456"])
    worker._redis.get = AsyncMock(return_value=str(time.time() - 2000).encode())
    worker._get_conversation_history = AsyncMock(
        return_value=[
...
async def test_recent_session_not_processed(worker):
    ...
    worker._redis.keys = AsyncMock(return_value=[b"session:last_active:789"])
...
async def test_kommo_write_called_when_available(worker):
    """Summary is written to Kommo when kommo_client is available."""
    mock_kommo = AsyncMock()
    worker._kommo = mock_kommo
    worker._redis.keys = AsyncMock(return_value=[b"session:last_active:111"])
    worker._redis.get = AsyncMock(return_value=str(time.time() - 2000).encode())
    worker._redis.delete = AsyncMock()
    worker._get_conversation_history = AsyncMock(
        return_value=[
...
async def test_none_redis_key_skipped(worker):
    ...
    worker._redis.keys = AsyncMock(return_value=[b"session:last_active:999"])
