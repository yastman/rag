...
class TestLLMServiceInit:
    ...
    def test_init_custom_values(self):
        ...
        assert service.base_url == "https://custom.api.com/v1"  ***REMOVED*** Trailing slash stripped
    ...
    def test_init_strips_trailing_slash(self):
        ...
        ***REMOVED*** rstrip("/") removes ALL trailing slashes
...
class TestLLMServiceStreamAnswer:
    ...
    async def test_stream_answer_yields_chunks(self, sample_chunks):
        ...
        ***REMOVED*** Mock streaming response as async iterator
...
class TestLLMServiceObserveInstrumentation:
    """Tests for @observe instrumentation on LLMService public methods (***REMOVED***1660).
    ...
    CRITICAL (audit correction on ***REMOVED***1660): the wrapper must NOT use
    ...
    ***REMOVED*** ------------------------------------------------------------------
    ***REMOVED*** Decorator kwargs (one test per method)
    ***REMOVED*** ------------------------------------------------------------------
    ...
    def test_generate_answer_decorator_kwargs(self, monkeypatch):
        ...
        correction on ***REMOVED***1660). The class uses ``langfuse.openai.AsyncOpenAI``
        ...
        assert "as_type" not in kwargs, (
            "Wrapper must NOT use as_type='generation' (audit correction on ***REMOVED***1660): "
    ...
    def test_stream_answer_decorator_kwargs(self, monkeypatch):
        ...
        assert "as_type" not in kwargs, (
            ...
            "correction on ***REMOVED***1660): langfuse.openai handles the streaming "
    ...
    def test_generate_decorator_kwargs(self, monkeypatch):
        ...
        assert kwargs.get("capture_output") is False
        assert "as_type" not in kwargs, (
            "generate wrapper must NOT use as_type='generation' (audit correction on ***REMOVED***1660)."
        )

    ***REMOVED*** ------------------------------------------------------------------
    ***REMOVED*** Behavior: generate_answer
    ***REMOVED*** ------------------------------------------------------------------
    ...
    async def test_generate_answer_curated_input_no_full_prompt(self, monkeypatch):
        ...
        Full prompt MUST NOT appear in span input (***REMOVED***1660 Forbidden section).
        """
        ...
        ***REMOVED*** Forbidden: full question MUST NOT appear in span input.
    ...
    async def test_generate_answer_curated_output_response_len(self, monkeypatch):
        """``generate_answer`` records response_len after LLM (***REMOVED***1660 plan)."""
        self._disable_observe(monkeypatch)
        mock_lf = self._patched_lf(monkeypatch)
        ...
        assert result == long_response  ***REMOVED*** behavior unchanged
        ...
        ***REMOVED*** Forbidden: full response text MUST NOT appear in span output.
    ...
    async def test_generate_answer_error_path_records_error_level(self, monkeypatch):
        ...
        path must still be recorded on the span (***REMOVED***1660 plan step 4).
        """
        self._disable_observe(monkeypatch)
        mock_lf = self._patched_lf(monkeypatch)
        ...
        assert error_calls, (
            ...
            "on LLMService.generate_answer (***REMOVED***1660 plan step 4)"
        )
    ...
    ***REMOVED*** ------------------------------------------------------------------
    ***REMOVED*** Behavior: stream_answer
    ***REMOVED*** ------------------------------------------------------------------
    ...
    async def test_stream_answer_error_path_records_error_level(self, monkeypatch):
        ...
        assert error_calls, (
            "Failure path must call update_current_span(level='ERROR', ...) "
            "on LLMService.stream_answer (***REMOVED***1660 plan step 4)"
        )
        status = error_calls[0].get("status_message", "")
    ...
    async def test_stream_options_include_usage_preserved(self, monkeypatch):
        ...
        The audit on ***REMOVED***1660 explicitly notes the existing call already passes
        ...
        assert kwargs.get("stream_options") == {"include_usage": True}, (
            ...
            "(audit on ***REMOVED***1660: required for langfuse.openai to capture token "
            "usage from the final streaming chunk)"
        )

    ***REMOVED*** ------------------------------------------------------------------
    ***REMOVED*** Behavior: generate
    ***REMOVED*** ------------------------------------------------------------------
    ...
    async def test_generate_curated_input_no_full_prompt(self, monkeypatch):
        ...
        assert len(captured_input["prompt_preview"]) <= 120
        assert captured_input.get("model") == "gpt-4o-mini"
        ***REMOVED*** Forbidden: full prompt MUST NOT appear in span input.
        assert long_prompt not in str(captured_input)

    async def test_generate_curated_output_response_len(self, monkeypatch):
        """``generate`` records response_len after LLM (***REMOVED***1660 plan)."""
        self._disable_observe(monkeypatch)
        mock_lf = self._patched_lf(monkeypatch)
    ...
    async def test_generate_error_path_records_error_level_and_reraises(self, monkeypatch):
        ...
        assert error_calls, (
            "Failure path must call update_current_span(level='ERROR', ...) "
            "on LLMService.generate (***REMOVED***1660 plan step 4)"
        )
        status = error_calls[0].get("status_message", "")
