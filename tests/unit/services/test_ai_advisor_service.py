"""Tests for AIAdvisorService — LLM-powered CRM prioritization (***REMOVED***697)."""
...
async def test_get_prioritized_tasks_calls_llm_with_tasks():
    ...
    tasks = [
        ...
        Task(id=2, text="Отправить КП", complete_till=int(time.time()) - 3600),  ***REMOVED*** overdue
    ]
    kommo = MagicMock()
...
async def test_get_stale_deals_returns_active_message_when_no_stale():
    ...
    ***REMOVED*** All leads updated recently (within 5 days)
    recent_ts = int(time.time()) - (2 * 86400)  ***REMOVED*** 2 days ago
    leads = [
...
async def test_get_stale_deals_filters_leads_older_than_5_days():
    ...
    now = int(time.time())
    leads = [
        Lead(id=1, name="Old Deal", updated_at=now - (7 * 86400)),  ***REMOVED*** 7 days stale
        Lead(id=2, name="Fresh Deal", updated_at=now - (1 * 86400)),  ***REMOVED*** 1 day fresh
    ]
    kommo = MagicMock()
    kommo.search_leads = AsyncMock(return_value=leads)
    ...
    ***REMOVED*** LLM was called (there IS a stale deal)
    llm.chat.completions.create.assert_called_once()
    ***REMOVED*** The call args should reference the stale lead
...
async def test_get_full_briefing_uses_cache_on_hit():
    ...
    ***REMOVED*** LLM should NOT have been called
...
async def test_get_full_briefing_stores_result_in_cache():
    ...
    ***REMOVED*** Cache should have been populated
...
async def test_get_full_briefing_cache_key_includes_manager_id():
    ...
    ***REMOVED*** Cache get/set should include manager_id in the key
...
async def test_get_prioritized_leads_handles_llm_error():
    ...
    assert isinstance(result, str)
    ***REMOVED*** Should return error message, not raise
