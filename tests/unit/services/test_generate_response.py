...
async def test_streaming_placeholder_and_llm_called_in_parallel() -> None:
    ...
    ***REMOVED*** Both placeholder send and LLM create must have been called
...
async def test_generate_response_uses_provided_llm_without_creating_new() -> None:
    ...
    ***REMOVED*** Build a separate explicit client
    mock_choice = MagicMock()
    ...
    ***REMOVED*** config.create_llm must NOT have been called — singleton was provided
...
async def test_drift_below_threshold_does_not_warn() -> None:
    ...
    ***REMOVED*** Stream that produces a small TTFT drift (well below 500ms)
    stream = _AsyncStream([_StreamChunk("Ответ без дрифта")])
    client.chat.completions.create = AsyncMock(return_value=stream)
    config.create_llm.return_value = client

    lf = MagicMock()
    ***REMOVED*** Reset call tracking to check update_current_span calls
    ...
    ***REMOVED*** No update_current_span call with level="WARNING" should have occurred
...
async def test_streaming_placeholder_telegram_retry_after_falls_through_to_non_streaming() -> None:
    ...
    ***REMOVED*** Fallback non-streaming path taken; response delivered but not via streaming
    assert result["response"] == "Фолбэк после rate-limit"
    assert result["response_sent"] is False
    ***REMOVED*** Stream was cleaned up despite placeholder failure (requires return_exceptions=True)
    stream.aclose.assert_awaited_once()
    ***REMOVED*** LLM called twice: stream object + non-streaming fallback
    assert client.chat.completions.create.await_count == 2
