...
class TestFilterExtractorPrice:
    ...
    ***REMOVED*** Range pattern - Note: current implementation checks 'до' first, so range only works
    ***REMOVED*** when no individual lt/gt patterns match. Testing actual behavior here.
    def test_price_range_pattern_order(self, extractor: FilterExtractor) -> None:
        ...
        """
        ***REMOVED*** 'до 100000' matches first, so we get lt instead of range
        result = extractor.extract_filters("квартира от 50000 до 100000")
        ***REMOVED*** Current behavior: 'до' pattern matches first
        assert result["price"] == {"lt": 100000}
    ...
    def test_price_range_using_dashes(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** No valid pattern matches
        assert "price" not in result

    ***REMOVED*** K suffix - Note: current regex doesn't capture 'к', only digits
    def test_price_k_suffix_not_captured_by_regex(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** Only '100' is captured, not '100к', so result is 100, not 100000
        assert result == {"price": {"lt": 100}}

    ***REMOVED*** Numbers with spaces
...
class TestFilterExtractorRooms:
    ...
    ***REMOVED*** Word patterns - Note: regex is (одно|дву|трех|четырех|пяти)комнатн
    ***REMOVED*** This means 'двухкомнатная' doesn't match - it needs 'двукомнатная'
    ...
    def test_rooms_dvuhkomnatnaya_not_matched(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** Regex (одно|дву|трех|четырех|пяти) doesn't include 'двух'
...
class TestFilterExtractorDistanceToSea:
    ...
    ***REMOVED*** First line / near sea - BUG: patterns use \s+ but condition checks literal string
    def test_distance_pervaya_liniya_bug(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** BUG: pattern matches but condition fails, so no distance_to_sea
    ...
    def test_distance_u_morya_bug(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** BUG: pattern matches but condition fails
        assert "distance_to_sea" not in result

    ***REMOVED*** No distance filter
...
class TestFilterExtractorBathrooms:
    ...
    def test_bathrooms_word_odin(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** Pattern r"(один|два|три)\\s*санузл" requires санузл prefix
...
class TestFilterExtractorCombined:
    ...
    def test_combined_with_distance_meters(self, extractor: FilterExtractor) -> None:
        ...
        assert result["city"] == "Варна"
        ***REMOVED*** Note: "до 500" matches price lt pattern, so we only get distance
        assert result["distance_to_sea"] == {"lte": 500}
...
class TestFilterExtractorEdgeCases:
    ...
    def test_price_zero(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** _parse_number returns 0 for "0", but "if price:" is False for 0
        assert "price" not in result
    ...
    def test_unicode_spaces(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** Non-breaking space \xa0
    ...
    def test_multiple_cities_first_in_list_wins(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** Бургас comes before Варна in the cities list, but the query has Варна first
        ***REMOVED*** Actually, the code iterates through cities list and checks if city is in query
        ***REMOVED*** So Бургас (earlier in list) will be found first
        assert result["city"] == "Бургас"
    ...
    def test_price_lt_pattern_priority(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** "до 100000" should match lt pattern first
    ...
    def test_partial_word_no_match(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** Partial city name should not match full city
    ...
    def test_distance_meters_without_sea_context(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** Should not match distance_to_sea as it's not about sea/beach
        assert "distance_to_sea" not in result
...
class TestParseNumberMethod:
    ...
    def test_parse_number_with_k_suffix_and_spaces(self, extractor: FilterExtractor) -> None:
        ...
        ***REMOVED*** After removing spaces: "100к"
        assert result == 100000
