***REMOVED*** tests/unit/test_bot_observability.py
...
class TestHandleQueryObservability:
    ...
    def bot_handler(self):
        ...
        ***REMOVED*** Avoid running PropertyBot.__init__ in unit tests (aiogram + real services)
        ...
        handler.config = BotConfig(
            ...
            cesc_enabled=False,  ***REMOVED*** keep handle_query on the simplest path
        )
        handler._cache_initialized = True

        ***REMOVED*** Mock services used by handle_query
        ...
        ***REMOVED*** Query preprocessor and HyDE (required by handle_query pipeline)
        ...
        ***REMOVED*** Router decision is external; make it deterministic
    ...
    async def test_handle_query_updates_trace(self, bot_handler, mock_message):
        ...
        with (
            ...
            ***REMOVED*** Session ID format: chat-{hash}-{YYYYMMDD}
