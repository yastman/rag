...
class TestBotConfig:
    ...
    def reload_config(self):
        ...
        def _reload(env_vars: dict):
            ***REMOVED*** Remove the module from cache so it reloads with new env
            if "telegram_bot.config" in sys.modules:
                del sys.modules["telegram_bot.config"]

            ***REMOVED*** Mock load_dotenv to prevent loading actual .env file
        ...
        ***REMOVED*** Cleanup: remove module from cache
        if "telegram_bot.config" in sys.modules:
            del sys.modules["telegram_bot.config"]
    ...
    def test_cesc_configuration_defaults(self, reload_config):
        ...
        assert config.user_context_ttl == 30 * 24 * 3600  ***REMOVED*** 30 days
