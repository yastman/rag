...
class TestDotenvLoading:
    ...
    def test_voyage_api_key_loaded_from_dotenv(self, tmp_path: Path, monkeypatch):
        ...
        ***REMOVED*** Arrange: create .env file
        env_file = tmp_path / ".env"
        env_file.write_text("VOYAGE_API_KEY=test-key-from-dotenv\n")

        ***REMOVED*** Clear any existing env var
        monkeypatch.delenv("VOYAGE_API_KEY", raising=False)

        ***REMOVED*** Act: load dotenv and create config
        from dotenv import load_dotenv

        load_dotenv(env_file, override=True)

        ***REMOVED*** Import AFTER load_dotenv to get fresh defaults
        ***REMOVED*** Force reimport to pick up new env
    ...
    def test_explicit_env_var_takes_precedence(self, tmp_path: Path, monkeypatch):
        ...
        ***REMOVED*** Arrange: .env has one value
        env_file = tmp_path / ".env"
        env_file.write_text("VOYAGE_API_KEY=from-dotenv-file\n")

        ***REMOVED*** Set explicit env var (should take precedence)
        monkeypatch.setenv("VOYAGE_API_KEY", "from-explicit-export")

        ***REMOVED*** Act: load dotenv WITHOUT override (default behavior)
        from dotenv import load_dotenv
        ...
        ***REMOVED*** Assert: explicit env var wins
