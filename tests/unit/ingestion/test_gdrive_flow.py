...
class TestGDriveFlowConfig:
    ...
    def test_default_config_values(self):
        ...
        ***REMOVED*** Clear GDRIVE_SYNC_DIR so we test the actual default (dotenv may set it)
        env = os.environ.copy()
        env.pop("GDRIVE_SYNC_DIR", None)
        with patch.dict("os.environ", env, clear=True):
            config = GDriveFlowConfig()

            ***REMOVED*** Default is ~/drive-sync expanded to user's home
...
class TestComputeFileId:
    ...
    def test_compute_file_id_uses_relative_path(self):
        ...
        ***REMOVED*** Two different sync dirs, same relative path
        ...
        ***REMOVED*** Same relative path -> same ID
        assert id1 == id2
...
class TestNeedsProcessing:
    ...
    def test_needs_processing_unchanged_file(self, tmp_path):
        ...
        ***REMOVED*** Record as already processed with same hash
    ...
    def test_needs_processing_changed_file(self, tmp_path):
        ...
        ***REMOVED*** File content changed (different hash)
