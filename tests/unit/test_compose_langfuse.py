...
***REMOVED*** Сервисы где дефолт ДОЛЖЕН быть непустым (dev-ready)
SERVICES_WITH_DEV_DEFAULTS = ["bot", "litellm", "rag-api", "voice-agent", "ingestion"]

***REMOVED*** Паттерн для пустого дефолта: ${VAR:-} или ${VAR:-""} или просто ""
_EMPTY_PATTERNS = ("${", "")


class TestLangfuseDevDefaults:
    """Services must have non-empty dev defaults for LANGFUSE keys ([REDACTED-LANGFUSE-KEY]
    ...
    def test_public_key_has_dev_default(self, compose: dict, service: str):
        ...
        ***REMOVED*** Проверяем что дефолт содержит [REDACTED-LANGFUSE-KEY]
        assert "[REDACTED-LANGFUSE-KEY] in val, (
            f"compose.yml: {service}.LANGFUSE_PUBLIC_KEY must default to [REDACTED-LANGFUSE-KEY] got: {val!r}"
        )
    ...
    def test_secret_key_has_dev_default(self, compose: dict, service: str):
        ...
        assert "[REDACTED-LANGFUSE-KEY] in val, (
            f"compose.yml: {service}.LANGFUSE_SECRET_KEY must default to [REDACTED-LANGFUSE-KEY] got: {val!r}"
        )
