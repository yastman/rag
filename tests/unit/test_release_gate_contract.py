...
REMOVED_PUBLIC_DEPLOY_FILES = [
    ROOT / "compose.vps.yml",
    ROOT / "scripts" / "deploy-vps.sh",
    ROOT / "deploy" / "telegram-bot.service",
]
...
def test_public_repo_does_not_ship_manual_vps_deploy_files() -> None:
    """VPS deploy wiring is intentionally outside the public repository."""
    for path in REMOVED_PUBLIC_DEPLOY_FILES:
        assert not path.exists(), f"public repo should not include {path.relative_to(ROOT)}"
