...
***REMOVED*** Skip entire module if opentelemetry.instrumentation is not installed
...
def fresh_otel_setup_module():
    ...
    """
    ***REMOVED*** Clear only our module, not opentelemetry itself
...
***REMOVED*** Pre-mock opentelemetry to avoid import side effects
reset_otel_mocks()


def test_setup_opentelemetry():
    """Test OpenTelemetry setup creates providers and configures exporters."""
    ***REMOVED*** Import INSIDE test to get fresh module after fixture clears cache
    ***REMOVED*** Then patches on module attributes will take effect
    from src.observability.otel_setup import setup_opentelemetry

    ***REMOVED*** Use patching on the module's imported names
    with (
...
async def test_traced_pipeline_query():
    ...
    ***REMOVED*** Mock internal methods using patches on instance
    with patch.object(pipeline, "_embed", new_callable=AsyncMock) as mock_embed:
        with patch.object(pipeline, "_search", new_callable=AsyncMock) as mock_search:
            with patch.object(pipeline, "_rerank", new_callable=AsyncMock) as mock_rerank:
                ...
                ***REMOVED*** Setup tracer mock context managers
                mock_span = MagicMock()
                ...
                ***REMOVED*** Check metrics recorded
