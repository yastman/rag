...
***REMOVED*** ── pgrep self-match safety (***REMOVED***1510) ────────────────────────────────────────
...
def test_voice_agent_compose_healthcheck_is_self_match_safe() -> None:
    ...
    so the pgrep command line does not match itself (***REMOVED***1510)."""
    import re
    ...
    voice = compose["services"]["voice-agent"]
    health_test = " ".join(voice["healthcheck"]["test"])
    ***REMOVED*** A self-match-safe pgrep -f pattern uses brackets: [s]rc.voice.agent
...
def test_voice_dockerfile_healthcheck_is_self_match_safe() -> None:
    """src/voice/Dockerfile HEALTHCHECK must use a bracketed pgrep regex (***REMOVED***1510)."""
    import re
