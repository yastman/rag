"""Preflight test for ***REMOVED***1665: verify langfuse.openai already captures response.model.

Issue ***REMOVED***1665 proposed adding ``update_current_generation(model=response.model)``
...
***REMOVED***1665 asked for a preflight test FIRST, because if ``langfuse.openai``
...
implementation work for ***REMOVED***1665 is not needed and the issue can be closed
...
class TestLangfuseOpenAISourceCodeContract:
    ...
    these tests fail loudly so the team can re-evaluate whether ***REMOVED***1665's
    ...
    def test_langfuse_openai_reads_response_model_field(self):
        ...
        ***REMOVED*** As of langfuse>=4.0 the SDK reads response.model in two places:
        ***REMOVED*** - line ~761 in `_extract_chat_response` (sync chat completions)
        ***REMOVED*** - line ~616 / ~642 in `_extract_streamed_openai_response` (streaming)
        assert source.count('response.get("model"') >= 2, (
            ...
            "API may have changed and ***REMOVED***1665's manual override may be needed. "
            "Re-validate the response.model auto-capture contract before "
            "closing ***REMOVED***1665."
        )
    ...
    def test_langfuse_openai_passes_model_to_generation_update(self):
        ...
        ***REMOVED*** The SDK calls generation.update(model=model, ...) in chat-completion
        ***REMOVED*** finalization (sync + async + responses-API + streaming variants).
        ***REMOVED*** Count >= 4 covers all canonical paths.
        assert source.count("model=model") >= 4, (
            ...
            "SDK API may have changed; re-evaluate ***REMOVED***1665."
        )
...
class TestLangfuseOpenAIRuntimeContract:
    ...
    def test_create_langfuse_update_guards_on_model_field(self):
        ...
        ***REMOVED*** The SDK's `_create_langfuse_update(generation, completion, model, ...)`
        ***REMOVED*** is the single point that decides whether the generation observation
        ***REMOVED*** gets a `model` field. We assert the guard expression and the
        ***REMOVED*** assignment exist verbatim — a future refactor that drops the guard
        ***REMOVED*** or renames the parameter must surface here so we re-evaluate ***REMOVED***1665.
        ...
        assert update_fn is not None, (
            ...
            "internal API renamed — re-evaluate ***REMOVED***1665."
        )
        ...
        assert "if model is not None" in fn_source, (
            ...
            "with `if model is not None:` — re-evaluate ***REMOVED***1665."
        )
        assert 'update["model"] = model' in fn_source, (
            "_create_langfuse_update no longer assigns the model parameter "
            "to the generation update payload — re-evaluate ***REMOVED***1665."
        )
    ...
    def test_response_model_extraction_function_signature(self):
        ...
        assert extract_fn is not None, (
            ...
            "renamed — re-evaluate ***REMOVED***1665."
        )
        ...
        assert params == ["resource", "response"], (
            ...
            f"{params!r}; re-evaluate ***REMOVED***1665 against the new shape."
        )

        ***REMOVED*** The function source should still read response.get("model").
        ...
        assert 'response.get("model"' in fn_source, (
            ...
            "response.get('model', ...) — re-evaluate ***REMOVED***1665."
        )
