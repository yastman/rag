...
class TestQdrantServiceMMR:
    ...
    def test_mmr_rerank_basic(self, service):
        ...
        assert len(result) == 3
        ***REMOVED*** First should be highest score
    ...
    def test_mmr_rerank_diversity(self, service):
        ...
        points = [
            {"id": "1", "text": "doc1", "score": 0.9},
            {"id": "2", "text": "doc2", "score": 0.85},  ***REMOVED*** High score but similar to 1
            {"id": "3", "text": "doc3", "score": 0.7},  ***REMOVED*** Lower score but very different
        ]
        ***REMOVED*** Doc 2 is very similar to doc 1, doc 3 is orthogonal
        embeddings = [
            [1.0, 0.0],  ***REMOVED*** doc1
            [0.99, 0.01],  ***REMOVED*** doc2 - nearly identical to doc1
            [0.0, 1.0],  ***REMOVED*** doc3 - completely different
        ]

        result = service.mmr_rerank(points=points, embeddings=embeddings, lambda_mult=0.5, top_k=2)

        ***REMOVED*** First should be highest score (doc1)
        assert result[0]["id"] == "1"
        ***REMOVED*** Second should prefer diversity - doc3 over doc2 despite lower score
    ...
    def test_mmr_rerank_lambda_1_relevance_only(self, service):
        ...
        embeddings = [
            [1.0, 0.0],
            [0.99, 0.01],  ***REMOVED*** Very similar to doc1
            [0.0, 1.0],  ***REMOVED*** Very different
        ]

        result = service.mmr_rerank(points=points, embeddings=embeddings, lambda_mult=1.0, top_k=3)

        ***REMOVED*** With lambda=1.0, should be pure relevance order
        assert result[0]["id"] == "1"
    ...
    def test_mmr_rerank_lambda_0_diversity_only(self, service):
        ...
        embeddings = [
            [1.0, 0.0],  ***REMOVED*** doc1
            [0.99, 0.01],  ***REMOVED*** doc2 - similar to doc1
            [0.0, 1.0],  ***REMOVED*** doc3 - different
        ]

        result = service.mmr_rerank(points=points, embeddings=embeddings, lambda_mult=0.0, top_k=2)

        ***REMOVED*** First is still highest score
        assert result[0]["id"] == "1"
        ***REMOVED*** With lambda=0, should strongly prefer diversity
        assert result[1]["id"] == "3"
    ...
    def test_mmr_rerank_fewer_points_than_top_k(self, service):
        ...
        ***REMOVED*** Should return all points unchanged
        assert len(result) == 2
    ...
    def test_mmr_rerank_preserves_point_structure(self, service):
        ...
        ***REMOVED*** Should preserve all fields
        for point in result:
            assert "id" in point
...
class TestQdrantServiceScoreBoosting:
    ...
    async def test_score_boosting_with_fresh_document(self, service):
        ...
        ***REMOVED*** Create document with recent timestamp
        ...
        assert len(results) == 1
        ***REMOVED*** Fresh documents should have boosted score
        ***REMOVED*** Base score is 0.7, boost adds up to 0.1
    ...
    async def test_score_boosting_with_old_document(self, service):
        ...
        ***REMOVED*** Create document with old timestamp
    ...
    async def test_score_boosting_reorders_by_freshness(self, service):
        ...
        ***REMOVED*** Old doc with higher base score
        old_point = MagicMock()
        old_point.id = "old"
        ...
        ***REMOVED*** New doc with slightly lower base score
        new_point = MagicMock()
        new_point.id = "new"
        ...
        ***REMOVED*** With freshness boost, newer doc may come first
    ...
    async def test_score_boosting_handles_missing_date(self, service):
        ...
        mock_point.payload = {
            ...
            "metadata": {},  ***REMOVED*** No created_at
        }
        ...
        assert len(results) == 1
        ***REMOVED*** Should return without error, using base score
    ...
    async def test_score_boosting_handles_invalid_date(self, service):
        ...
        assert len(results) == 1
        ***REMOVED*** Should return without error
...
class TestQdrantServiceFormatResults:
    ...
    def test_format_results_missing_fields(self, service):
        ...
        mock_point.id = "doc_1"
        mock_point.score = 0.8
        mock_point.payload = {}  ***REMOVED*** No page_content or metadata

        results = service._format_results([mock_point])

        assert len(results) == 1
        assert results[0]["text"] == ""  ***REMOVED*** Empty string for missing content
        assert results[0]["metadata"] == {}  ***REMOVED*** Empty dict for missing metadata
...
class TestQdrantServiceInit:
    ...
    def test_init_with_api_key(self):
        ...
        with patch("telegram_bot.services.qdrant.AsyncQdrantClient"):
            ...
            ***REMOVED*** Verify service was created with expected state
