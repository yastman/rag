...
class TestGenerateNodeStreaming:
    ...
    async def test_streaming_sends_placeholder_and_edits(self) -> None:
        ...
        ***REMOVED*** Final edit_text called with Markdown (last call)
    ...
    async def test_streaming_empty_response_falls_back(self) -> None:
        ...
        ***REMOVED*** First call returns empty stream, second call returns normal response
        mock_client.chat.completions.create = AsyncMock(
    ...
    async def test_stream_error_before_visible_output(self) -> None:
        ...
        mock_client = MagicMock()
        ***REMOVED*** Stream fails immediately (no chunks delivered), then fallback succeeds
        mock_client.chat.completions.create = AsyncMock(
    ...
    async def test_stream_error_after_visible_output(self) -> None:
        ...
        mock_client = MagicMock()
        ***REMOVED*** Stream yields real chunks then fails, then fallback succeeds
        mock_client.chat.completions.create = AsyncMock(
            side_effect=[
        ...
        ***REMOVED*** KEY: response_sent=True because partial content was already visible
        assert result["response_sent"] is True
        ***REMOVED*** Fallback was edited into existing message (last edit_text call)
    ...
    async def test_stream_error_partial_and_edit_fails_falls_back_to_respond_node(self) -> None:
        ...
        sent_msg = AsyncMock()
        ***REMOVED*** First call may happen during stream; subsequent fallback edits fail.
...
class TestGenerateNodeProviderMetadata:
    ...
    async def test_non_streaming_captures_provider_model(self) -> None:
        ...
        assert result["llm_ttft_ms"] == 0.0  ***REMOVED*** non-streaming has no TTFT
...
class TestGenerateNodeResponseStyle:
    ...
    async def test_style_budget_capped_by_generate_max_tokens(self) -> None:
        ...
        ***REMOVED*** "сколько стоит" -> short/easy baseline budget=50, capped to 40
        state = _make_state_with_docs(query="сколько стоит студия")
    ...
    async def test_shadow_mode_keeps_legacy_prompt(self) -> None:
        ...
        ***REMOVED*** But legacy max_tokens is used
        call_kwargs = mock_client.chat.completions.create.call_args
