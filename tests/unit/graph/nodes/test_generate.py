"""Tests for _generate_streaming parallelization in voice path (***REMOVED***685).

Issue ***REMOVED***685: parallelize placeholder send + LLM stream creation via asyncio.gather.
"""
...
async def test_placeholder_and_stream_parallel() -> None:
    ...
    assert mock_gather.called, "asyncio.gather must be used for parallel execution (***REMOVED***685)"
    ***REMOVED*** gather should receive exactly 2 coroutines: LLM stream create + placeholder answer
...
async def test_stream_failure_raises() -> None:
    ...
    ***REMOVED*** Placeholder must be cleaned up when stream creation fails (***REMOVED***685 safety)
