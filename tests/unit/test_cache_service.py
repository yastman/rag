...
***REMOVED*** Create mock modules for redisvl to avoid heavy imports
...
class TestCacheServiceInitialize:
    ...
    async def test_initialize_handles_connection_error(self, cache_service, mock_redisvl_modules):
        ...
        with patch("telegram_bot.services.cache.redis") as mock_redis_module:
            ...
            ***REMOVED*** Should not crash, redis_client should be None
    ...
    async def test_initialize_sets_up_semantic_cache_with_api_key(
        ...
        ***REMOVED*** Reset the mock to track calls
...
class TestSemanticCache:
    ...
    async def test_check_semantic_cache_with_user_filter(self, cache_service):
        ...
        ***REMOVED*** Verify acheck was called with filter expression
...
class TestSearchCacheKeys:
    ...
    async def test_search_cache_key_no_double_version(self, cache_service):
        ...
        ***REMOVED*** Should be search:v2:v1:hash:hash, not search:v1:v1:hash:hash
...
class TestCacheMetrics:
    ...
    async def test_get_full_metrics_includes_redis_stats(self, cache_service):
        ...
        cache_service.redis_client.info = AsyncMock(
            side_effect=[
                {"used_memory_human": "10M", "maxmemory_human": "512M"},  ***REMOVED*** memory
                {"evicted_keys": 5, "keyspace_hits": 100, "keyspace_misses": 20},  ***REMOVED*** stats
            ]
        )
...
class TestConversationHistoryExtended:
    ...
    async def test_store_conversation_message_with_pipeline(self, cache_service):
        ...
        ***REMOVED*** Verify all pipeline operations were called
        cache_service.redis_client.lpush.assert_called_once()
...
class TestCacheServiceInitExtended:
    ...
    def test_init_default_ttls(self):
        ...
        assert service.semantic_cache_ttl == 48 * 3600  ***REMOVED*** 48 hours
        assert service.embeddings_cache_ttl == 7 * 24 * 3600  ***REMOVED*** 7 days
        assert service.analyzer_cache_ttl == 24 * 3600  ***REMOVED*** 24 hours
        assert service.search_cache_ttl == 2 * 3600  ***REMOVED*** 2 hours
        assert service.distance_threshold == 0.20  ***REMOVED*** Relaxed for RU paraphrases
