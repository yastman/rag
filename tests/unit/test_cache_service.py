...
class TestCacheServiceInit:
    ...
    async def test_initialize_creates_connections(self):
        ...
        ***REMOVED*** SemanticCache requires VOYAGE_API_KEY
...
class TestSemanticCache:
    ...
    async def test_threshold_override(self, cache_service):
        ...
        ***REMOVED*** With strict threshold, similar but not identical should miss
        result = await cache_service.check_semantic_cache(
            "slightly different query text",
            threshold_override=0.01,  ***REMOVED*** Very strict
        )

        ***REMOVED*** May or may not match depending on embedding similarity
        ***REMOVED*** Just verify no exception
...
class TestTier2Caches:
    ...
    async def test_search_cache_store_get(self, cache_service):
        ...
        embedding = [0.1] * 10  ***REMOVED*** Only first 10 used for hash
        filters = {"city": "Варна"}
...
class TestSemanticMessageHistory:
    ...
    async def test_add_message_no_error(self, cache_service):
        ...
        user_id = 999999  ***REMOVED*** Test user

        ***REMOVED*** Add messages - should not raise
        ...
        ***REMOVED*** Test passes if no exception raised
    ...
    async def test_get_relevant_history_returns_list(self, cache_service):
        ...
        user_id = 888888  ***REMOVED*** Different test user

        ***REMOVED*** Get history - may be empty but should be a list
...
class TestCacheServiceVectorizer:
    ...
    async def test_cache_service_uses_voyage_vectorizer_for_semantic_cache(self):
        ...
        with (
            ...
            ***REMOVED*** Setup mock Redis client
            mock_redis_client = AsyncMock()
            mock_redis_client.ping = AsyncMock()
            mock_redis.from_url.return_value = mock_redis_client

            ***REMOVED*** Setup mock vectorizer
            ...
            ***REMOVED*** Verify VoyageAITextVectorizer was created with correct parameters
            ***REMOVED*** First call is for SemanticCache, second is for SemanticMessageHistory
            assert mock_vectorizer_class.call_count >= 1

            ***REMOVED*** Check first call (SemanticCache vectorizer)
            ...
            ***REMOVED*** Verify SemanticCache was created with the vectorizer
    ...
    async def test_cache_service_uses_voyage_vectorizer_for_message_history(self):
        ...
        with (
            ...
            ***REMOVED*** Setup mock vectorizers
            ...
            ***REMOVED*** Verify VoyageAITextVectorizer was created twice
            assert mock_vectorizer_class.call_count == 2

            ***REMOVED*** Check second call (SemanticMessageHistory vectorizer)
            ...
            ***REMOVED*** Verify SemanticMessageHistory was created with the vectorizer
    ...
    async def test_cache_service_no_vectorizer_without_voyage_key(self):
        ...
        with (
            ...
            ***REMOVED*** VoyageAITextVectorizer should NOT be called without API key
            mock_vectorizer_class.assert_not_called()

            ***REMOVED*** SemanticCache should be None
            assert service.semantic_cache is None
            ***REMOVED*** MessageHistory should be None
    ...
    async def test_vectorizer_model_is_voyage_3_lite(self):
        ...
        with (
            ...
            ***REMOVED*** Both vectorizer creations should use voyage-3-lite
