...
class TestThrottlingMiddleware:
    ...
    async def test_menu_buttons_use_shorter_rate_limit(self):
        ...
        event = MagicMock(spec=Message)
        event.answer = AsyncMock()
        ***REMOVED*** Simulate aiogram flags: get_flag(data, "menu_nav") returns True
        data = {"event_from_user": user, "handler": MagicMock(flags={"menu_nav": True})}

        ***REMOVED*** First menu button press
        with patch(
            ...
            ***REMOVED*** Second rapid press — should be throttled (within 0.3s)
            result2 = await middleware(handler, event, data)
            assert result2 is None
    ...
    async def test_menu_buttons_not_throttled_by_message_cache(self):
        ...
        ***REMOVED*** First: regular message (fills message cache)
        ...
        ***REMOVED*** Second: menu button press — should NOT be throttled by message cache
