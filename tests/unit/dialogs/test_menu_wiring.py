...
async def test_handle_search_starts_funnel_dialog():
    """_handle_search starts FunnelSG.complex dialog when dialog_manager available (***REMOVED***658, ***REMOVED***697)."""
    with patch("telegram_bot.bot.PropertyBot.__init__", return_value=None):
        from telegram_bot.bot import PropertyBot
...
def test_client_menu_dialog_not_in_register_handlers():
    """client_menu_dialog should NOT be registered on the dispatcher (***REMOVED***658)."""
    import ast
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == ".dialogs.client_menu":
            ***REMOVED*** If import exists, check it's not used in include_router
            for name in node.names:
                if name.name == "client_menu_dialog":
                    ***REMOVED*** Search for include_router(client_menu_dialog) call
