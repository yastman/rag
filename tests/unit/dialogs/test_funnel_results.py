"""Tests for funnel filter building and results getter (***REMOVED***660)."""
...
***REMOVED*** --- get_results_data integration (mocked svc) ---
...
async def test_get_results_data_no_service():
    ...
    result = await get_results_data(dialog_manager=manager)
    assert result["results_text"]  ***REMOVED*** non-empty fallback
...
async def test_get_results_data_malformed_payload():
    ...
    mock_svc.search = AsyncMock(
        return_value=[
            {"id": "apt-bad"},  ***REMOVED*** missing "payload" key
        ],
    )
    ...
    result = await get_results_data(dialog_manager=manager)
    ***REMOVED*** Must not crash — falls through to no_results_text via exception handler
