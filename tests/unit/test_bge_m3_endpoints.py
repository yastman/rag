...
***REMOVED*** ── Mock heavy dependencies BEFORE importing app ──
mock_flag = MagicMock()
mock_prom = MagicMock()
sys.modules["FlagEmbedding"] = mock_flag
sys.modules["prometheus_client"] = mock_prom

***REMOVED*** pydantic_settings is available in test env, but config.py imports it;
***REMOVED*** also re-export make_asgi_app so app.py can mount metrics
mock_prom.make_asgi_app = MagicMock(return_value=MagicMock())

***REMOVED*** Add service directory to path
