"""Unit tests for client menu integration in bot.py (***REMOVED***628).
...
***REMOVED*** ─── Task A: handle_menu_button routing ──────────────────────────────────────
...
***REMOVED*** ─── Task B: cmd_start role routing ──────────────────────────────────────────
...
class TestCmdStartRoleRouting:
    ...
    async def test_cmd_start_client_sends_welcome_keyboard(self, mock_config):
        ...
        mock_config.manager_ids = []  ***REMOVED*** no manager elevation
        bot = _create_bot(mock_config)
        ...
        ***REMOVED*** reply_markup keyword arg must be set (ReplyKeyboardMarkup)
        assert call_args.kwargs.get("reply_markup") is not None
        ***REMOVED*** welcome text contains "FortNoks" (from services.yaml)
    ...
    async def test_cmd_start_manager_with_dialog(self, mock_config):
        ...
        ***REMOVED*** Positional arg is the state (ManagerMenuSG.main)
    ...
    async def test_cmd_start_manager_without_dialog(self, mock_config):
        ...
        message.answer.assert_awaited_once()
        ***REMOVED*** No reply_markup for manager text fallback
        call_args = message.answer.call_args
        assert isinstance(call_args.args[0], str)


***REMOVED*** ─── Task C: handle_service_callback ─────────────────────────────────────────
...
class TestHandleServiceCallback:
    ...
    async def test_handle_service_callback_show_card(self, mock_config):
        ...
        ***REMOVED*** edit_text called once (card render), then answer() called
    ...
    async def test_handle_service_callback_unknown(self, mock_config):
        ...
        bot = _create_bot(mock_config)
        ***REMOVED*** Neither "svc:" nor "cta:" prefix → parse_service_callback returns None
...
***REMOVED*** ─── Task D: handle_favorite_callback ────────────────────────────────────────
...
class TestHandleFavoriteCallback:
    ...
    async def test_handle_favorite_no_service(self, mock_config):
        ...
        bot = _create_bot(mock_config)
        ***REMOVED*** _favorites_service not set (only initialized in start(), not __init__)
...
***REMOVED*** ─── Task E: handle_cta_callback ─────────────────────────────────────────────
...
***REMOVED*** ─── Task F: handle_menu_action_text ─────────────────────────────────────────
...
***REMOVED*** ─── Task G: _handle_bookmarks display ───────────────────────────────────────
...
class TestHandleBookmarks:
    ...
    async def test_handle_bookmarks_with_items(self, mock_config):
        ...
        ***REMOVED*** At least 2 answer calls: card + footer
        assert message.answer.await_count >= 2
        ***REMOVED*** Footer text contains the item count
