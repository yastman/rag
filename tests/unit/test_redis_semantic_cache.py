...
***REMOVED*** Pre-mock dependencies to avoid runtime overhead/connection attempts
...
async def test_get_response_hit(cache, mock_redis):
    ...
    ***REMOVED*** Assert
    ...
    assert cache.get_stats()["cache_hits"] == 1
    ***REMOVED*** Check key format contains version and top_k
    call_args = mock_redis.get.call_args[0][0]
