"""Regression tests for handle_results_callback pagination bugs (***REMOVED***948).
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Bug ***REMOVED***1: start_from parameter name
***REMOVED*** ---------------------------------------------------------------------------


class TestScrollStartFromParameter:
    async def test_scroll_with_filters_called_with_start_from_not_offset(self):
        """scroll_with_filters must use start_from=, not offset= (***REMOVED***948 bug 1)."""
        bot = _create_bot()
        mock_svc = MagicMock()
        ...
        ***REMOVED*** State: current page full → triggers lazy fetch
        ...
        ***REMOVED*** Bug ***REMOVED***1: must use start_from=, not offset=
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Bug ***REMOVED***2: 4-tuple unpack (page_ids captured)
***REMOVED*** ---------------------------------------------------------------------------


class TestScrollFourTupleUnpack:
    async def test_page_ids_saved_to_state_after_scroll(self):
        """page_ids from 4-tuple must be saved to state as apartment_scroll_seen_ids (***REMOVED***948 bug 2)."""
        bot = _create_bot()
        mock_svc = MagicMock()
        ...
        ***REMOVED*** Find the call that stores apartment data
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Bug ***REMOVED***3: exclude_ids passed from state
***REMOVED*** ---------------------------------------------------------------------------


class TestScrollExcludeIds:
    async def test_exclude_ids_passed_from_state(self):
        """exclude_ids must be passed from state's apartment_scroll_seen_ids (***REMOVED***948 bug 3)."""
        bot = _create_bot()
        mock_svc = MagicMock()
        ...
        mock_svc.scroll_with_filters.assert_awaited_once()
        call_kwargs = mock_svc.scroll_with_filters.call_args.kwargs
        ***REMOVED*** Bug ***REMOVED***3: exclude_ids must be passed
    ...
    async def test_exclude_ids_is_none_when_seen_ids_empty(self):
        ...
        call_kwargs = mock_svc.scroll_with_filters.call_args.kwargs
        ***REMOVED*** Empty list → None (falsy guard)
