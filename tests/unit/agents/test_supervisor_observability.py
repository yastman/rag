"""Tests for supervisor Langfuse observability (***REMOVED***240 Task 7, ***REMOVED***242)."""
...
async def test_supervisor_writes_agent_used_score(supervisor_config):
    ...
    ***REMOVED*** Find the agent_used score call
...
def test_supervisor_node_has_observe_decorator():
    """supervisor_node inside build_supervisor_graph is decorated with @observe (***REMOVED***242)."""

    ***REMOVED*** Verify the module imports observe and get_client for span creation
...
def test_rag_agent_tool_has_observe_decorator():
    """rag_search tool is decorated with @observe (***REMOVED***242)."""
...
def test_history_tool_has_observe_decorator():
    """history_search tool uses @observe via tools module (***REMOVED***408)."""
...
def test_direct_response_tool_has_observe_decorator():
    """direct_response tool is decorated with @observe (***REMOVED***242)."""
    import telegram_bot.agents.tools as tools_mod
...
async def test_supervisor_propagate_attributes_called_with_supervisor_tag(supervisor_config):
    """Supervisor path calls propagate_attributes with 'supervisor' tag (***REMOVED***242)."""
    bot = _create_bot_patched(supervisor_config)
    ...
    ***REMOVED*** Verify propagate_attributes was called with supervisor tag
...
async def test_supervisor_curated_span_metadata_on_routing(supervisor_config):
    """Supervisor node writes curated span metadata for routing decision (***REMOVED***242)."""
    bot = _create_bot_patched(supervisor_config)
    ...
    ***REMOVED*** Verify update_current_trace was called (trace-level metadata)
    trace_call = mock_lf.update_current_trace.call_args
    assert trace_call is not None
