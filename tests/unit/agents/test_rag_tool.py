"""Tests for rag_search tool with config-based context DI (***REMOVED***413)."""
...
async def test_rag_search_writes_langfuse_scores(bot_context):
    """rag_search tool calls write_langfuse_scores with full pipeline result (***REMOVED***425)."""
    from telegram_bot.agents.rag_tool import rag_search
...
async def test_rag_search_writes_input_type_text(bot_context):
    """rag_search result has input_type defaulting to 'text' for scoring (***REMOVED***425)."""
    from telegram_bot.agents.rag_tool import rag_search
    ...
    ***REMOVED*** input_type defaults to "text" in write_langfuse_scores when not in result
