...
def test_default_system_prompt_contains_safety_instructions():
    """DEFAULT_SYSTEM_PROMPT must include safety/refusal instructions (***REMOVED***439)."""
...
def test_history_trimmer_removes_old_messages_when_over_limit():
    """_create_history_trimmer returns RemoveMessage for oldest messages (***REMOVED***519)."""
    from unittest.mock import MagicMock
    ...
    ***REMOVED*** Oldest 2 messages should be removed (keep last 4 starting on human)
    removed_ids = {m.id for m in remove_msgs}
    assert "h1" in removed_ids
    assert "a1" in removed_ids
    ***REMOVED*** Kept messages should not be in remove list
...
def test_history_trimmer_respects_start_on_human():
    ...
    ***REMOVED*** Window must start on h2 or later — both h2, a2, h3 should be kept
    assert "h2" not in removed_ids and "a2" not in removed_ids and "h3" not in removed_ids
    ***REMOVED*** First kept message must be a HumanMessage (start_on constraint)
