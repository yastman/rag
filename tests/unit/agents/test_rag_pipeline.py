...
async def test_pipeline_cache_hit_via_original_query(
    ...
    ***REMOVED*** Cache has an entry stored under the original query
    ...
    result = await rag_pipeline(
        "apartments in Nesebar under 80000 EUR",  ***REMOVED*** agent-reformulated query
        original_query="квартиры в Несебре до 80000",  ***REMOVED*** original user query
        user_id=42,
        session_id="test",
        query_type="FAQ",
        cache=mock_cache,
...
async def test_pipeline_cache_uses_original_query_as_key(
    ...
    ***REMOVED*** Cache miss — proceed to retrieval
    ...
    await rag_pipeline(
        "apartments in Nesebar",  ***REMOVED*** reformulated
        original_query="квартиры в Несебре",  ***REMOVED*** original
        user_id=42,
        session_id="test",
        query_type="FAQ",
        cache=mock_cache,
...
async def test_rag_pipeline_recomputes_colbert_for_reformulated_query(mock_cache, mock_sparse):
    ...
    ***REMOVED*** First call is cache_key embedding (miss), second is reformulated query warm embedding (hit).
