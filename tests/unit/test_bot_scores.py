...
class TestScoreWriting:
    ...
    async def _run_handle_query(self, mock_config, graph_result, mock_lf_client):
        ...
        with (
            ...
            ***REMOVED*** propagate_attributes is a context manager
    ...
    async def test_scores_written_full_pipeline(self, mock_config):
        ...
        ***REMOVED*** Extract all score names from calls
        ...
        expected_names = [
            ...
            ***REMOVED*** Latency breakdown (***REMOVED***147): streaming path
            "streaming_enabled",
            "llm_timeout",
            "llm_stream_recovery",
            "llm_decode_ms",
    ...
    async def test_score_values_full_pipeline(self, mock_config):
        ...
        ***REMOVED*** embeddings/search cache misses, confidence from grade
        ...
        assert scores["response_style_applied"] == 1.0  ***REMOVED*** balanced
    ...
    async def test_empty_style_without_policy_mode_does_not_emit_style_applied(self, mock_config):
        ...
        legacy_result = {
            **CACHE_HIT_RESULT,
            "response_style": "",
            ***REMOVED*** response_policy_mode intentionally missing
        }
    ...
    async def test_scores_written_even_on_null_client(self, mock_config):
        ...
        with (
            ...
            ***REMOVED*** Should not raise — NullClient silently ignores
            await bot.handle_query(_make_message())
...
class TestLatencyBreakdownScores:
    ...
    async def test_streaming_path_writes_numeric_and_boolean_scores(self, mock_config):
        ...
        ***REMOVED*** NUMERIC scores written
        ...
        ***REMOVED*** queue_unavailable because queue_ms is None
        assert score_map["llm_queue_unavailable"]["value"] == 1
        assert score_map["llm_queue_unavailable"]["data_type"] == "BOOLEAN"
        ***REMOVED*** No llm_decode_unavailable (decode_ms was written)
    ...
    async def test_non_streaming_writes_unavailable_flags(self, mock_config):
        ...
        ***REMOVED*** streaming_enabled = False
...
class TestVoiceScores:
    ...
    async def test_voice_scores_written(self, mock_config):
        ...
        ***REMOVED*** Voice-specific scores must be present
    ...
    async def test_text_scores_omit_voice_metrics(self, mock_config):
        ...
        ***REMOVED*** input_type always written (as "text")
        assert "input_type" in score_names
        ***REMOVED*** Voice-only scores NOT written for text input
...
class TestMemoryScores:
    ...
    async def test_memory_messages_count_zero_when_no_messages(self, mock_config):
        ...
        result.pop("messages", None)  ***REMOVED*** no messages key
...
class TestHistoryScores:
    """Test history-related Langfuse scores (***REMOVED***239)."""
...
class TestCheckpointerOverheadScore:
    ...
    async def test_checkpointer_overhead_proxy_score_written(self, mock_config):
        ...
        ***REMOVED*** FULL_PIPELINE_RESULT stages sum to 862ms (0.862s)
        ***REMOVED*** Simulate ainvoke taking 0.901s (901ms) → overhead proxy = 39ms
        perf_values = iter(
            [
                0.0,  ***REMOVED*** pipeline_start
                1.0,  ***REMOVED*** invoke_start
                1.901,  ***REMOVED*** after ainvoke → ainvoke_wall_ms = 901ms
                2.0,  ***REMOVED*** pipeline_wall_ms
            ]
        )
