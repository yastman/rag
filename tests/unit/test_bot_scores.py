...
class TestScoreWriting:
    ...
    def test_scores_written_even_on_null_client(self):
        """When Langfuse disabled, _NullLangfuseClient.create_score is called (no-op, ***REMOVED***435)."""
        from telegram_bot.observability import _NullLangfuseClient

        mock_lf = _NullLangfuseClient()
        ***REMOVED*** _NullLangfuseClient.get_current_trace_id() returns "" → write_langfuse_scores
        ***REMOVED*** returns early. Pass explicit trace_id to exercise the scoring code path.
...
class TestExtractCurrentTurn:
    """Test _extract_current_turn helper (***REMOVED***507)."""

    def test_extracts_current_turn_from_history(self):
        """Only messages after last HumanMessage should be returned."""
        from telegram_bot.bot import _extract_current_turn

        ***REMOVED*** Simulate checkpointer history: 2 old turns + 1 current turn
        ...
        assert len(result) == 3  ***REMOVED*** current_human + current_ai + current_tool
        assert result[0] is current_human
        assert result[1] is current_ai
        assert result[2] is current_tool

    def test_single_turn_returns_all(self):
        """Single turn (no history) should return all messages."""
        from telegram_bot.bot import _extract_current_turn
    ...
    def test_no_human_message_returns_all(self):
        """If no HumanMessage found, return all (safety fallback)."""
        from telegram_bot.bot import _extract_current_turn
    ...
    def test_tool_calls_count_excludes_history(self):
        """tool_calls_total should count only current turn, not history (***REMOVED***507)."""
        from telegram_bot.bot import _extract_current_turn

        ***REMOVED*** Old turn: 2 tool calls
        ...
        ***REMOVED*** Current turn: 1 tool call
        ...
        ***REMOVED*** Count tool calls only in current turn
        tool_calls = sum(
            len(m.tool_calls)
            for m in current
            if hasattr(m, "tool_calls") and isinstance(m.tool_calls, list) and m.tool_calls
        )
        assert tool_calls == 1  ***REMOVED*** NOT 3 (which includes old turn)
