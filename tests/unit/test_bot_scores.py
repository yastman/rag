...
class TestExtractCurrentTurn:
    """Test _extract_current_turn helper (#507)."""
    ...
    def test_extracts_current_turn_from_history(self):
        ...
        # Simulate checkpointer history: 2 old turns + 1 current turn
        ...
        assert len(result) == 3  # current_human + current_ai + current_tool
    ...
    def test_tool_calls_count_excludes_history(self):
        """tool_calls_total should count only current turn, not history (#507)."""
        from telegram_bot.bot import _extract_current_turn

        # Old turn: 2 tool calls
        ...
        # Count tool calls only in current turn
        tool_calls = sum(
            len(m.tool_calls)
            for m in current
            if hasattr(m, "tool_calls") and isinstance(m.tool_calls, list) and m.tool_calls
        )
        assert tool_calls == 1  # NOT 3 (which includes old turn)
