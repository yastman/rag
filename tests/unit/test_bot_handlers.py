...
class TestPostgresPoolInit:
    """Pool is not created when DB doesn't exist (***REMOVED***570)."""
...
class TestPreAgentCacheCheck:
    ...
    async def test_pre_agent_uses_hybrid_colbert_when_available(self, mock_config):
        """When aembed_hybrid_with_colbert exists, all three embeddings are stashed (***REMOVED***571)."""
        bot, _ = _create_bot(mock_config)
        ...
        ***REMOVED*** hybrid_with_colbert must be called; aembed_query must NOT be called
        ...
        ***REMOVED*** all three embeddings must be stashed
    ...
    async def test_pre_agent_fallback_to_aembed_query_when_no_hybrid_colbert(self, mock_config):
        """Fallback to aembed_query when aembed_hybrid_with_colbert is not available (***REMOVED***571)."""
        bot, _ = _create_bot(mock_config)
        test_embedding = [0.5] * 10
    ...
    async def test_pre_agent_hybrid_colbert_stash_on_cache_hit(self, mock_config):
        """On cache HIT with hybrid_with_colbert, sparse is stored in cache (***REMOVED***571)."""
        bot, _ = _create_bot(mock_config)
