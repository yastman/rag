...
class TestCommandHandlers:
    ...
    async def test_cmd_stats(
        ...
        assert "75" in call_args  ***REMOVED*** Overall hit rate
...
class TestGetSparseVector:
    ...
    async def test_get_sparse_vector_error_fallback(
        ...
        ***REMOVED*** Should return empty sparse vector
...
class TestHandleQuery:
    ...
    async def test_handle_query_chitchat_routing(
        ...
        message.answer.assert_called_once()
        ***REMOVED*** RAG pipeline should not be called (voyage.embed_query)
    ...
    async def test_handle_query_semantic_cache_hit(
        ...
        ***REMOVED*** Cached answer should be returned
        call_args = message.answer.call_args[0][0]
    ...
    async def test_handle_query_no_results(
        ...
        ***REMOVED*** Mock _get_sparse_vector
        ...
        ***REMOVED*** Should show "nothing found" message
        call_args = message.answer.call_args[0][0]
...
class TestBotStart:
    ...
    async def test_start_skips_reinit_if_already_initialized(
        ...
        bot = PropertyBot(mock_config)
        bot._cache_initialized = True  ***REMOVED*** Already initialized
        bot.dp = MagicMock()
        bot.dp.start_polling = AsyncMock()

        await bot.start()

        ***REMOVED*** Should not reinitialize
...
class TestRegisterHandlers:
    ...
    def test_handlers_registered(
        ...
        ***REMOVED*** Dispatcher should have message handlers registered
        ***REMOVED*** We can't easily check the exact handlers without more complex mocking,
        ***REMOVED*** but we can verify the bot has the expected methods
...
class TestCESCIntegration:
    ...
    async def test_personalization_applied_to_cached_answer(
        ...
        ***REMOVED*** Setup CESC personalizer
        ...
        ***REMOVED*** CESC should personalize
        cesc_instance.personalize.assert_called_once()

        ***REMOVED*** Personalized answer should be sent
        call_args = message.answer.call_args[0][0]
