...
async def test_model(provider: str, model: str, label: str, chunks: list[dict]) -> dict:
    ...
    ***REMOVED*** Initialize contextualizer
    if provider == "openai":
...
def print_comparison_table(all_results: dict):
    ...
    ***REMOVED*** Collect data for ranking
    model_scores = []
    ...
    for tier in ["budget", "mid", "premium"]:
        ...
        for result in all_results[tier]:
            ...
            ***REMOVED*** Calculate weighted score (higher is better)
            ***REMOVED*** Speed: inverse (faster = better)
            ***REMOVED*** Cost: inverse (cheaper = better)
            ***REMOVED*** Accuracy: direct (higher = better)
            ***REMOVED*** Success: direct (higher = better)
            score = (
                (1.0 / speed) * 0.2  ***REMOVED*** Speed 20%
                + (1.0 / (cost + 0.00001)) * 0.3  ***REMOVED*** Cost 30%
                + accuracy["overall_score"] * 0.4  ***REMOVED*** Accuracy 40%
                + success * 0.1  ***REMOVED*** Success 10%
            )
    ...
    ***REMOVED*** Find best speed/cost ratio in budget tier
    ...
    ***REMOVED*** Find best accuracy with success rate
    quality_models = [
    ...
    ***REMOVED*** Find model with good accuracy but not most expensive
    ...
    ***REMOVED*** Cost comparison for full Criminal Code (448 chunks)
    print("\n" + "=" * 100)
...
async def main():
    ...
    ***REMOVED*** Load chunks once
    chunks = load_test_chunks(pdf_path, num_chunks)

    ***REMOVED*** Test all models
    all_results = {}
    ...
    for tier, models in MODELS_TO_TEST.items():
        ...
        for model_config in models:
            ...
            ***REMOVED*** Small delay between models
