...
def load_test_chunks(pdf_path: str, num_chunks: int = 10) -> list[dict]:
    ...
    ***REMOVED*** Select diverse chunks (beginning, middle, end)
...
async def test_openai(chunks: list[dict], model: str = "gpt-4o-mini") -> dict:
    ...
    contextualizer = ContextualRetrievalOpenAIAsync(
        model=model,
        max_concurrent=5,  ***REMOVED*** Conservative limit
    )
...
async def test_groq(chunks: list[dict], model: str = "llama-3.1-8b-instant") -> dict:
    ...
    contextualizer = ContextualRetrievalGroqAsync(
        model=model,
        max_concurrent=10,  ***REMOVED*** Groq can handle more
    )
...
def compare_results(openai_result: dict, groq_result: dict) -> dict:
    ...
    ***REMOVED*** Calculate scores (simple weighted scoring)
    openai_score = (
        (1.0 / openai_result["avg_time_per_chunk"]) * 0.3  ***REMOVED*** Speed (30%)
        + (1.0 / (openai_cost + 0.0001)) * 0.3  ***REMOVED*** Cost (30%)
        + (openai_acc[0] / openai_acc[3]) * 0.4  ***REMOVED*** Accuracy (40%)
    )

    groq_score = (
        (1.0 / groq_result["avg_time_per_chunk"]) * 0.3  ***REMOVED*** Speed (30%)
        + (1.0 / (groq_cost + 0.0001)) * 0.3  ***REMOVED*** Cost (30%)
        + (groq_acc[0] / groq_acc[3]) * 0.4  ***REMOVED*** Accuracy (40%)
    )
    ...
    ***REMOVED*** Return comparison data
    return {
...
async def main():
    ...
    ***REMOVED*** Models to test
    openai_model = "gpt-4o-mini"  ***REMOVED*** Cheapest option
    groq_model = "llama-3.1-8b-instant"  ***REMOVED*** Fastest, cheapest
    ...
    ***REMOVED*** Save results
    output_file = "/srv/api_comparison_results.json"
