...
Usage (orchestrator):
    ***REMOVED*** Init: discover window, generate ID, rename, save state
    ORCH_ID=$(python scripts/tmux_orch_identity.py init)

    ***REMOVED*** Get saved ORCH_ID later
    ORCH_ID=$(python scripts/tmux_orch_identity.py get)

    ***REMOVED*** Get webhook target for worker prompts
    TARGET=$(python scripts/tmux_orch_identity.py webhook-target)
    ***REMOVED*** → claude:ORCH-a1b2c3d4
...
def _main_repo_root() -> Path:
    ...
    try:
        ...
        ***REMOVED*** common = /path/to/main-repo/.git → parent = /path/to/main-repo
        return Path(common).parent
    except (subprocess.CalledProcessError, FileNotFoundError):
        ***REMOVED*** Fallback: current git toplevel (might be worktree root)
        try:
...
***REMOVED*** Keep for backward compat with tests that reference it
...
class OrchIdentity:
    ...
    def discover(self) -> None:
        ...
        session_result = subprocess.run(
            ["tmux", "display-message", *target_args, "-p", "***REMOVED***{session_name}"],
            capture_output=True,
            text=True,
        )
        ...
        window_result = subprocess.run(
            ["tmux", "display-message", *target_args, "-p", "***REMOVED***{window_index}"],
            capture_output=True,
            text=True,
        )
...
def _cli() -> None:
    ...
    if cmd == "init":
        ...
        ***REMOVED*** Generate webhook bash snippet for worker prompt injection
        if len(sys.argv) < 3:
        ...
        ident = OrchIdentity.load()
        ***REMOVED*** Print 3-call webhook snippet
        ...
        ***REMOVED*** Worker sends text to orchestrator (call 1 of 3)
        if len(sys.argv) < 3:
        ...
        ***REMOVED*** Worker sends Enter to orchestrator (call 3 of 3)
        try:
            ident = OrchIdentity.load()
        except FileNotFoundError:
            print("No orchestrator session active (missing .claude/orch-identity.json)")
