***REMOVED***!/bin/bash
***REMOVED*** Auto-monitor workers: read logs, close completed, notify when all done
    ...
    for log_name in "${!WINDOW_MAP[@]}"; do
        ...
        if [[ -f "$log_file" ]]; then
            if grep -q '\[COMPLETE\]' "$log_file" 2>/dev/null; then
                ***REMOVED*** Check if window still exists
                if tmux list-windows -F '***REMOVED***{window_name}' | grep -q "^${window}$"; then
