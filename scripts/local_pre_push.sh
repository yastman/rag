...
***REMOVED*** Fast local pre-push sanity gate: lightweight checks before code leaves the machine.
***REMOVED*** Called by pre-commit pre-push hook and can be run manually via:
***REMOVED***   make local-pre-push
set -euo pipefail
