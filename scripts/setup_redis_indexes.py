...
Usage:
    ...
    python scripts/setup_redis_indexes.py --force  ***REMOVED*** Drop and recreate
...
def get_redis_url() -> str:
    ...
    ***REMOVED*** Check for full URL first
    redis_url = os.environ.get("REDIS_URL")
    if redis_url:
        return redis_url

    ***REMOVED*** Build URL from individual components
...
def check_redisearch_module(client: redis.Redis) -> bool:
    ...
    try:
        ...
        for m in modules:
            ***REMOVED*** Handle both bytes and str keys (depends on decode_responses)
        ...
        ***REMOVED*** Try FT._LIST as fallback check
        try:
            client.execute_command("FT._LIST")
            return True
        except redis.ResponseError as e:
            ***REMOVED*** If "unknown command" - module not available, otherwise it exists
...
def drop_index(client: redis.Redis, index_name: str) -> bool:
    """Drop existing index."""
    try:
        ***REMOVED*** DD flag drops documents, without DD only index is dropped
...
def create_semantic_cache_index(client: redis.Redis) -> dict:
    ...
    ***REMOVED*** FT.CREATE command for semantic cache index
    ***REMOVED*** Using FLAT index for exact KNN search (good for smaller cache sizes)
    ***REMOVED*** BGE-M3 model outputs 1024-dimensional vectors
    client.execute_command(
        ...
        "VECTOR",
        "FLAT",
        "6",  ***REMOVED*** Number of following args for FLAT
        "TYPE",
        "FLOAT32",
        "DIM",
        "1024",
    ...
    ***REMOVED*** Get index info after creation
    ...
    while i < len(info):
        ...
        if i < len(info):
            ...
            if isinstance(value, bytes):
                ...
                ***REMOVED*** Handle nested lists (like attributes)
                value = str(value)
            result[key] = value
            i += 1
...
def print_index_info(info: dict, index_name: str):
    ...
    ***REMOVED*** Key metrics to display
    display_keys = [
        ("num_docs", "Documents"),
...
Examples:
  ...
  REDIS_URL=redis://localhost:6379 python scripts/setup_redis_indexes.py
    ...
    ***REMOVED*** Connect to Redis
    try:
        client = redis.from_url(
            redis_url,
            decode_responses=False,  ***REMOVED*** Keep bytes for proper handling
    ...
    ***REMOVED*** Check for RediSearch module
    ...
    ***REMOVED*** Create semantic cache index
    index_name = "idx:rag:semantic_cache"
    ...
    if exists:
        if args.force:
            ...
            ***REMOVED*** Show current index info
            info = client.execute_command("FT.INFO", index_name)
