...
***REMOVED*** Deploy to VPS via git pull + selective Docker rebuild
***REMOVED*** Usage: ./scripts/deploy-vps.sh [--dry-run] [service...]
***REMOVED***
***REMOVED*** Examples:
***REMOVED***   ./scripts/deploy-vps.sh                    ***REMOVED*** Auto-detect changes, rebuild affected
***REMOVED***   ./scripts/deploy-vps.sh vps-bot            ***REMOVED*** Force rebuild specific service
...
***REMOVED*** Verify SSH key
[[ -f "$VPS_KEY" ]] || error "SSH key not found: $VPS_KEY"

***REMOVED*** Step 1: Push local changes
log "Pushing to GitHub..."
if ! $DRY_RUN; then
    git push origin main 2>&1 | tail -3
fi

***REMOVED*** Step 2: Pull on VPS
log "Pulling on VPS..."
if ! $DRY_RUN; then
    ssh_cmd "cd $VPS_DIR && git pull origin main" 2>&1 | tail -5
fi

***REMOVED*** Step 3: Determine services to rebuild
SERVICES_TO_BUILD=()

if [[ ${***REMOVED***FORCE_SERVICES[@]} -gt 0 ]]; then
    SERVICES_TO_BUILD=("${FORCE_SERVICES[@]}")
    info "Force rebuild: ${SERVICES_TO_BUILD[*]}"
else
    ***REMOVED*** Auto-detect from git diff
...
***REMOVED*** Step 4: Build and restart
if [[ ${***REMOVED***SERVICES_TO_BUILD[@]} -gt 0 ]]; then
...
if ! $DRY_RUN; then
    ...
    ***REMOVED*** Restart LiteLLM if config changed
