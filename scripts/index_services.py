...
def build_chunks(services: list[dict]) -> list[dict]:
    ...
    for svc in services:
        ...
        ***REMOVED*** Deterministic ID: same key → same UUID across runs (idempotency)
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** PointStruct construction
***REMOVED*** ---------------------------------------------------------------------------
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** BGE-M3 helpers (same pattern as index_local_docs.py)
***REMOVED*** ---------------------------------------------------------------------------
...
def _create_bge_client(bge_url: str):
    ...
    try:
        from telegram_bot.services.bge_m3_client import (
            BGEM3SyncClient,  ***REMOVED*** type: ignore[import-not-found]
        )
...
def encode_hybrid_sdk(client: object, texts: list[str]) -> dict[str, list]:
    ...
    result = client.encode_hybrid(texts)  ***REMOVED*** type: ignore[union-attr]
    return {
...
def main() -> int:
    ...
    ***REMOVED*** 2. Build chunks
    chunks = build_chunks(services)
    texts = [c["text"] for c in chunks]

    ***REMOVED*** 3. Qdrant preflight (skip in dry-run)
    ...
    ***REMOVED*** 5. Build points
    points = build_points(chunks, embeddings)

    if args.dry_run:
        print(f"[dry-run] Would upsert {len(points)} points to '{args.collection}'")
        for p in points:
            print(f"  id={p.id}  key={p.payload['metadata']['service_key']}")  ***REMOVED*** type: ignore[index]
        return 0

    ***REMOVED*** 6. Upsert (idempotent — same IDs overwrite existing points)
