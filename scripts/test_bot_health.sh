...
resolve_qdrant_collection() {
  ...
  if [ -f "$PROJECT_ROOT/.env" ]; then
    dotenv_value=$(
      sed -nE "s/^[[:space:]]*(export[[:space:]]+)?QDRANT_COLLECTION[[:space:]]*=[[:space:]]*['\"]?([^'\"***REMOVED***[:space:]]+)['\"]?.*$/\\2/p" "$PROJECT_ROOT/.env" \
        | tail -n1
    )
...
***REMOVED*** Qdrant: target collection exists (match bot's suffix rules)
