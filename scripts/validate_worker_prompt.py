...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "launch_opencode_worker.sh resolves ORCH placeholders."
        )
    )
...
def opencode_field(text: str, field: str) -> str:
    match = re.search(rf"(?im)^\s*OpenCode {re.escape(field)}:\s*(.+?)\s*$", text)
    if not match:
        return ""
...
def validate_canonical_worker_route(text: str, errors: list[str]) -> None:
    ...
    actual_agent = opencode_field(text, "agent")
    actual_model = opencode_field(text, "model")
    ...
    errors.append(
        ...
        f"{task_type}: expected OpenCode agent {expected_agent} with model {expected_model}; "
...
def validate_secretary_route(text: str, errors: list[str]) -> None:
    ...
    errors.append(
        ...
        f"expected OpenCode agent {actual_agent} with model {expected_model}; "
...
_CONTENT_SAFETY_METADATA_PATTERN = re.compile(
    r"(?im)^\s*(WORKER_NAME|REPORT_FILE|Worker type|OpenCode agent|OpenCode model|SWARM_CONTRACT):\s*"
)
...
def validate(text: str, *, contract: str) -> list[str]:
    ...
    if re.search(r"(?im)^\s*OpenCode variant:\s*empty\s*$", text):
        errors.append('OpenCode variant must be "" or "empty string", not literal "empty"')
    ...
    if any(pattern.search(text) for pattern in CJ_SUBMIT_PATTERNS):
        errors.append(
            "prompt source must not use C-j as submit; Codex TUI treats Ctrl+J as newline"
        )
    if any(pattern.search(text) for pattern in ENTER_SUBMIT_PATTERNS):
        errors.append("prompt source must not use Enter as submit; Codex TUI requires isolated C-m")
