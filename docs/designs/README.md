...
- [`monolith-core-plan.md`](monolith-core-plan.md) — **единый план** монолитного
  ...
  `Stabilize Core Monolith` и issues `CORE-001…CORE-017`. Опирается на детальные
  companion-документы ниже.

- [`monolith-core-audit-implementation-plan.md`](monolith-core-audit-implementation-plan.md) —
  расширенный аудит архитектуры и подробный инкрементальный план реализации:
  runtime coupling, перенос grounding/generation/RAG, подключение Telegram как
  тонкого адаптера и E2E/contract gates.

- [`monolith-core-issue-backlog.md`](monolith-core-issue-backlog.md) —
  детальный backlog шаблонов `CORE-001` … `CORE-010` (milestone, правила
  выполнения без лишних зависимостей, week-1 focus).
...
- [`lean-core-test-guardrail-audit.md`](lean-core-test-guardrail-audit.md) —
  аудит тестов и guardrails и план среза лишнего (issues `CORE-011…CORE-017`).
...
- Аудит и детальный план: [`monolith-core-audit-implementation-plan.md`](monolith-core-audit-implementation-plan.md)
...
- Аудит тестов и guardrails: [`lean-core-test-guardrail-audit.md`](lean-core-test-guardrail-audit.md)
