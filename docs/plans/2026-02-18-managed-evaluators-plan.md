...
> **Review note (2026-02-18):** План скорректирован по фактическому SDK `langfuse==3.14.x` и текущему коду репозитория (методы dataset items, сигнатуры experiment task, judge thresholds, тестовые команды).
...
Run: `uv run pytest tests/unit/test_validate_queries.py tests/unit/test_validate_aggregates.py -v -n auto --dist=worksteal`
Expected: PASS
  ...
  "evaluators": [
    {
      ...
      "variable_mapping": {
        "input": "$.output.eval_query",
        "output": "$.output.eval_answer",
        "context": "$.output.eval_context"
      },
      "notes": "Требует полей eval_* из Task 3."
    },
    {
...
**Решение:** Добавить отдельные поля `eval_*` в curated metadata, содержащие данные для evaluators (с ограничением длины для cost control).
...
lf.update_current_span(output={
    ...
    "eval_docs": "\n\n".join(
        ...
        for doc in _build_retrieved_context(results)[:5]
    ),
})
```
...
Run: `uv run pytest tests/unit/graph/test_generate_node.py tests/unit/graph/test_retrieve_node.py -v -n auto --dist=worksteal`
Expected: PASS
...
Run: `uv run pytest tests/unit/agents/ tests/unit/test_bot_handlers.py -v -n auto --dist=worksteal`
...
Run: `uv run pytest tests/unit/ -n auto --dist=worksteal --timeout=30 -q`
...
Run: `grep -rn "judge_sample_rate\|judge_model" telegram_bot scripts --include="*.py" | grep -v __pycache__ | grep -v ".pyc"`
Expected: Никаких ссылок в runtime-коде (`telegram_bot/**`, `scripts/evaluate_judge.py`); `scripts/e2e/**` может содержать независимый `judge_model`.
...
def sync_to_langfuse(
    ...
    Uses deterministic item IDs for idempotent re-runs (upsert semantics).
    ...
    for sample in samples:
        item_id = f"{dataset_name}-{sample['id']}"
...
def build_rag_task(graph: Any) -> Any:
    ...
    async def task(*, item: Any, **kwargs: Any) -> dict[str, str]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--collection",
        default=None,
        help="Qdrant collection for RAG graph (default: BotConfig().get_collection_name())",
    )
    args = parser.parse_args()
    ...
    from scripts.validate_traces import init_services

    collection = args.collection or BotConfig().get_collection_name()
    services = asyncio.run(init_services(collection=collection))
...
def test_compute_aggregates_includes_judge_scores():
    ...
    assert "judge_faithfulness_mean" in agg["cold"]
    assert agg["cold"]["judge_faithfulness_mean"] == 0.8
...
def test_go_no_go_fails_on_low_judge_scores():
    ...
    criteria = evaluate_go_no_go(agg, results=[])
    assert criteria["judge_faithfulness_mean_gte"]["passed"] is False
```

**Step 3: Запустить тест, убедиться что падает**

Run: `uv run pytest tests/unit/test_validate_aggregates.py -v -k "judge" -n auto --dist=worksteal`
Expected: FAIL
...
if phase == "cold":
    for judge_name in ("judge_faithfulness", "judge_answer_relevance", "judge_context_relevance"):
        vals = [r.scores.get(judge_name) for r in phase_results if r.scores.get(judge_name) is not None]
        if vals:
...
```python
threshold_cfg = yaml.safe_load(Path("tests/baseline/thresholds.yaml").read_text())
judge_thresholds = threshold_cfg.get("judge", {})
for metric, key in [
    ...
    value = aggregates.get("cold", {}).get(metric)
    if threshold is not None and value is not None:
        criteria[f"{metric}_gte"] = {
            "target": f">= {threshold}",
            "actual": f"{value:.3f}",
...
Run: `uv run pytest tests/unit/test_validate_aggregates.py -v -n auto --dist=worksteal`
Expected: PASS
...
**Step 1: Required gate — lint/type-check**

Run: `make check`
Expected: PASS

**Step 2: Required gate — unit tests (AGENTS baseline)**

Run: `PYTEST_ADDOPTS='-n auto --dist=worksteal' make test-unit`
Expected: PASS

**Step 3: Integration тесты (targeted)**

Run: `uv run pytest tests/integration/test_graph_paths.py -v -n auto --dist=worksteal`
Expected: PASS
