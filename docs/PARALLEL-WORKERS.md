***REMOVED*** Parallel Claude Workers (tmux + spawn-claude)
...
***REMOVED******REMOVED*** Быстрый старт (3 шага)

**1. Открыть tmux сессию в проекте**
```bash
cd /mnt/c/Users/user/Documents/Сайты/rag-fresh
tmux new -s claude
***REMOVED*** Или через WezTerm (Ctrl+Shift+M) → выбрать проект
```
...
***REMOVED******REMOVED*** Переключение между воркерами (tmux)
...
***REMOVED******REMOVED*** Синтаксис spawn-claude
...
***REMOVED******REMOVED*** Правила параллелизации (ВАЖНО)
...
***REMOVED******REMOVED*** Шаблон промпта для воркера
...
***REMOVED******REMOVED*** Пример: 4 воркера
...
***REMOVED******REMOVED*** Короткий синтаксис для оркестратора
...
***REMOVED******REMOVED*** Мониторинг прогресса

```bash
***REMOVED*** git log (обновляется каждые 2 сек)
watch -n 2 "git log --oneline -10"

***REMOVED*** Какие файлы изменены
git diff --name-only HEAD~5

***REMOVED*** Финальная проверка
. venv/bin/activate && pytest tests/unit/ -q
```

***REMOVED******REMOVED*** Обработка ошибок
...
***REMOVED******REMOVED*** Шпаргалка tmux
...
***REMOVED******REMOVED*** Когда использовать
