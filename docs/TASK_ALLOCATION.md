***REMOVED*** Task Allocation - Server vs Local Development
...
***REMOVED******REMOVED*** 📊 Current Environment
...
***REMOVED******REMOVED******REMOVED*** Production VPS Server
```
Location: /srv/contextual_rag
...
***REMOVED******REMOVED*** 🖥️ SERVER TASKS (Требуют VPS)

***REMOVED******REMOVED******REMOVED*** Priority 1: Must Deploy on Server 🔴

***REMOVED******REMOVED******REMOVED******REMOVED*** S1.1 BGE-M3 Model Deployment (2-3 hours)
**Why Server:** Requires 2-3GB RAM, runs continuously

```bash
***REMOVED*** On VPS: /srv/contextual_rag
Task:
...
***REMOVED******REMOVED******REMOVED******REMOVED*** S1.2 MLflow Server Deployment (1 hour)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** S1.3 Langfuse Server Deployment (1 hour)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** S1.4 Prometheus + Grafana Stack (2-3 hours)
...
***REMOVED******REMOVED******REMOVED*** Priority 2: Should Deploy on Server 🟡

***REMOVED******REMOVED******REMOVED******REMOVED*** S2.1 Load Testing & Performance Tuning (4-5 hours)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** S2.2 Backup & Disaster Recovery (2-3 hours)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** S2.3 Production Environment Variables Audit (1 hour)
...
***REMOVED******REMOVED******REMOVED*** Priority 3: Nice to Have on Server 🟢

***REMOVED******REMOVED******REMOVED******REMOVED*** S3.1 SSL/TLS for Internal Services (2 hours)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** S3.2 Resource Monitoring & Alerts (3 hours)
...
***REMOVED******REMOVED*** 💻 LOCAL TASKS (Можно делать на WSL)

***REMOVED******REMOVED******REMOVED*** Priority 1: Critical for CI/CD 🔴

***REMOVED******REMOVED******REMOVED******REMOVED*** L1.1 docker-compose.yml Creation (2-3 hours)
**Why Local:** Configuration file, can test locally

```yaml
***REMOVED*** Create: docker-compose.yml
Services:
- qdrant:
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L1.2 GitHub Actions CI/CD Pipeline (3-4 hours)
**Why Local:** YAML configuration files

```yaml
***REMOVED*** Create: .github/workflows/ci.yml
name: CI/CD Pipeline
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L1.3 pytest Configuration & Fixtures (2-3 hours)
**Why Local:** Test infrastructure setup

```python
***REMOVED*** Create: pytest.ini, conftest.py
Files:
...
***REMOVED******REMOVED******REMOVED*** Priority 2: Testing Implementation 🟡

***REMOVED******REMOVED******REMOVED******REMOVED*** L2.1 Unit Tests - Cache Service (4-5 hours)
**Why Local:** Mock Redis, no real services needed

```python
***REMOVED*** Create: tests/unit/test_cache_service.py
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L2.2 Unit Tests - LLM Service (3-4 hours)
**Why Local:** Mock httpx client

```python
***REMOVED*** Create: tests/unit/test_llm_service.py
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L2.3 Unit Tests - Retriever Service (3 hours)
**Why Local:** Mock Qdrant client

```python
***REMOVED*** Create: tests/unit/test_retriever_service.py
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L2.4 Integration Tests - RAG Pipeline (4-5 hours)
**Why Local:** Use docker-compose for real services

```python
***REMOVED*** Create: tests/integration/test_rag_pipeline.py
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L2.5 Integration Tests - Graceful Degradation (3 hours)
**Why Local:** Test service failures with docker-compose

```python
***REMOVED*** Create: tests/integration/test_graceful_degradation.py
Coverage: 8 test cases
...
***REMOVED******REMOVED******REMOVED*** Priority 3: Documentation & Quality 🟢

***REMOVED******REMOVED******REMOVED******REMOVED*** L3.1 Code Quality Tools Setup (2 hours)
**Why Local:** Configuration files

```bash
***REMOVED*** Create: .pre-commit-config.yaml, pyproject.toml
Tools:
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L3.2 API Documentation Generation (2-3 hours)
**Why Local:** Extract docstrings → docs

```bash
***REMOVED*** Create: docs/api/ using Sphinx or mkdocs
Tools:
- Sphinx with autodoc
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L3.3 Load Testing Scripts (2-3 hours)
**Why Local:** Scripts can be written locally, run on server

```python
***REMOVED*** Create: tests/load/locustfile.py
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L3.4 User Feedback Loop - Telegram Inline Buttons (3-4 hours)
**Why Local:** Bot code modification

```python
***REMOVED*** Modify: telegram_bot/bot.py
Add:
...
***REMOVED******REMOVED******REMOVED*** Priority 4: Performance Optimizations 🟢

***REMOVED******REMOVED******REMOVED******REMOVED*** L4.1 Connection Pooling Implementation (3-4 hours)
**Why Local:** Code modification

```python
***REMOVED*** Modify: telegram_bot/services/retriever.py, cache.py
Add:
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L4.2 AsyncQdrantClient Migration (3-4 hours)
**Why Local:** Code refactoring

```python
***REMOVED*** Migrate: telegram_bot/services/retriever.py
Change:
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L4.3 Prometheus Metrics Endpoint (2-3 hours)
**Why Local:** Code addition

```python
***REMOVED*** Create: telegram_bot/metrics.py
Add:
...
***REMOVED******REMOVED******REMOVED******REMOVED*** L4.4 Distributed Lock for Semantic Cache (2 hours)
**Why Local:** Redis lock implementation

```python
***REMOVED*** Modify: telegram_bot/services/cache.py
Add:
...
***REMOVED******REMOVED*** 📋 HYBRID TASKS (Нужны оба места)

***REMOVED******REMOVED******REMOVED*** H1. End-to-End Testing (4-5 hours)
**Phase 1 (Local):** Write tests with mocks
**Phase 2 (Server):** Run tests against real services

```python
***REMOVED*** Local: Write tests/e2e/test_full_pipeline.py
***REMOVED*** Server: Run with real Qdrant, Redis, BGE-M3, LLM
...
***REMOVED******REMOVED******REMOVED*** H2. Documentation Updates (Ongoing)
**Local:** Write docs, commit to git
**Server:** Verify accuracy against production

```markdown
***REMOVED*** Update after each feature:
- README.md
- CHANGELOG.md
...
***REMOVED******REMOVED******REMOVED*** Week 1: Local Development
```bash
Priority 1 - Local:
...
***REMOVED******REMOVED******REMOVED*** Week 2: Server Deployment
```bash
Priority 1 - Server:
...
***REMOVED******REMOVED******REMOVED*** Week 3: CI/CD & Monitoring
```bash
...
***REMOVED******REMOVED*** ⚖️ TASK DISTRIBUTION SUMMARY

***REMOVED******REMOVED******REMOVED*** Server Tasks (Must deploy there)
- **Total:** 8 tasks
- **Time:** ~15-20 hours
- **Critical:** BGE-M3, MLflow, Langfuse, Load testing

***REMOVED******REMOVED******REMOVED*** Local Tasks (Can do in WSL)
- **Total:** 14 tasks
- **Time:** ~40-50 hours
- **Critical:** docker-compose, CI/CD, Unit tests

***REMOVED******REMOVED******REMOVED*** Hybrid Tasks (Both places)
- **Total:** 2 tasks
- **Time:** ~5-10 hours
...
***REMOVED******REMOVED******REMOVED*** To Start Local Development:
```bash
cd /mnt/c/Users/user/Documents/Сайты/Раг
git pull origin main

***REMOVED*** Pick first task:
***REMOVED*** L1.1 - Create docker-compose.yml
touch docker-compose.yml
***REMOVED*** Start writing services...
```

***REMOVED******REMOVED******REMOVED*** To Access Server:
```bash
***REMOVED*** SSH to VPS
...
***REMOVED*** Check running services
docker ps
systemctl status telegram-bot

***REMOVED*** Deploy new code
./deploy.sh
```
