***REMOVED*** Quick Start: DBSF + ColBERT Search
...
***REMOVED*** 2. Run A/B test (depends on query count, ~5-10 minutes)
cd evaluation
python run_ab_test.py

***REMOVED*** 3. View results
...
***REMOVED******REMOVED******REMOVED*** Example Good Result
...
SCORE_THRESHOLD_HYBRID = 0.2  ***REMOVED*** Was 0.3
```

**Option 2: Increase stage 1 candidates**
```python
***REMOVED*** In config.py
RETRIEVAL_LIMIT_STAGE1 = 200  ***REMOVED*** Was 100
```

**Option 3: Higher HNSW precision**
```python
***REMOVED*** In config.py
HNSW_EF_HIGH_PRECISION = 512  ***REMOVED*** Was 256
```

***REMOVED******REMOVED******REMOVED*** If Search is Too Slow (> 2s per query)

**Option 1: Lower HNSW ef**
```python
***REMOVED*** In config.py
HNSW_EF_HIGH_PRECISION = 128  ***REMOVED*** Was 256
```

**Option 2: Reduce stage 1 candidates**
```python
***REMOVED*** In config.py
RETRIEVAL_LIMIT_STAGE1 = 50  ***REMOVED*** Was 100
```

**Option 3: Use only minimal payload fields**
```python
***REMOVED*** Already configured in search_engines.py
self.payload_fields = PAYLOAD_FIELDS_MINIMAL  ***REMOVED*** ["article_number", "text"]
```
...
***REMOVED*** Existing Payload Indexes:
***REMOVED***   ✓ article_number: integer
***REMOVED***   ✓ chapter_number: integer
***REMOVED***   ✓ section_number: integer
***REMOVED***   ✓ book_number: integer
```

***REMOVED******REMOVED******REMOVED*** Test 2: Quick Search Test
...
***REMOVED*** Create DBSF+ColBERT engine
...
***REMOVED******REMOVED******REMOVED*** Test 3: Compare All 3 Engines
...
python ingestion_contextual_kg_fast.py  ***REMOVED*** Re-ingest data
```

***REMOVED******REMOVED******REMOVED*** Issue 2: DBSF search returns empty results

**Cause:** Score threshold too high.

**Quick test:**
```python
engine.score_threshold = 0.0  ***REMOVED*** Disable threshold
results = engine.search(query, top_k=10)
```

If this works, lower threshold in config.py from 0.3 → 0.2.

***REMOVED******REMOVED******REMOVED*** Issue 3: Slow performance (> 5s per query)
