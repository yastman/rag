***REMOVED*** Docling → Qdrant → n8n (BGE‑M3) — чек‑лист задач (обновлено 2025-10-22 06:41:41)
...
***REMOVED******REMOVED*** 0) Предусловия и версии
...
***REMOVED******REMOVED*** 1) Конвертация документов в DoclingDocument
...
***REMOVED******REMOVED*** 2) Чанкование: Hierarchical → Hybrid
...
***REMOVED******REMOVED*** 3) Сериализация (Markdown/JSON/YAML) + DPK (опционально)
...
***REMOVED******REMOVED*** 4) Эмбеддинги (BGE‑M3: dense + sparse + colbert/multivector)
...
***REMOVED******REMOVED*** 5) Qdrant: коллекция и загрузка
...
***REMOVED******REMOVED*** 6) Поисковый запрос (retrieve → rerank) + фильтры
...
***REMOVED******REMOVED*** 7) n8n оркестрация и Redis‑кэш
...
***REMOVED******REMOVED*** 8) Контекстуализация и улучшения retrieval
...
***REMOVED******REMOVED*** 9) Оценка качества (offline)
...
***REMOVED******REMOVED*** 10) Наблюдаемость и алёрты
...
***REMOVED******REMOVED*** 11) Безопасность и лимиты
...
***REMOVED******REMOVED*** 12) Итоговая проверка (go‑live checklist)
...
***REMOVED******REMOVED*** Примечания по реализации
