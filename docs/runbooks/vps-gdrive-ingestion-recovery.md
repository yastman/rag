...
> those paths for local development. In production, replace these with
> your actual VPS paths.
...
```bash
for file in .env /etc/rag-fresh/rclone-sync.env; do  ***REMOVED*** /etc/rag-fresh/ is a VPS-specific path
