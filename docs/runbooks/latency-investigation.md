***REMOVED*** Runbook: LLM Latency Investigation

**Issue:** ***REMOVED***147 — Slow-thinking latency breakdown
**Last Updated:** 2026-02-12

***REMOVED******REMOVED*** Quick Commands

```bash
***REMOVED*** Check latency metrics (last 24h)
uv run python scripts/setup_langfuse_dashboards.py

***REMOVED*** Check with alert thresholds
uv run python scripts/setup_langfuse_dashboards.py --check-alerts

***REMOVED*** JSON output for automation
uv run python scripts/setup_langfuse_dashboards.py --json --hours 6

***REMOVED*** Run trace validation (full pipeline)
uv run python scripts/validate_traces.py --report
```

***REMOVED******REMOVED*** Step 1: Detect — Is There a Latency Problem?
...
***REMOVED******REMOVED******REMOVED*** Manual Detection (Langfuse UI)
...
***REMOVED******REMOVED*** Step 2: Find the Slow Trace

***REMOVED******REMOVED******REMOVED*** Via Langfuse UI
...
***REMOVED******REMOVED******REMOVED*** Non-LLM Bottlenecks
...
***REMOVED******REMOVED*** Step 4: Root Cause & Action

***REMOVED******REMOVED******REMOVED*** 4a. Hard Timeout (`llm_timeout = true`)
...
***REMOVED******REMOVED******REMOVED*** 4b. Queue-Bound (`llm_queue_ms > 1000ms`)
...
***REMOVED******REMOVED******REMOVED*** 4c. TTFT-Bound (`llm_ttft_ms > 2000ms`, queue low)
...
***REMOVED******REMOVED******REMOVED*** 4d. Decode-Bound (`llm_tps < 20`)
...
***REMOVED******REMOVED******REMOVED*** 4e. Response-Length-Bound (`llm_tps >= 20`, decode > 3s)
...
3. Review response length policy settings (***REMOVED***129)
4. Consider reducing `GENERATE_MAX_TOKENS` (default: 2048)

***REMOVED******REMOVED******REMOVED*** 4f. Stream Recovery (`llm_stream_recovery = true`)
...
***REMOVED******REMOVED*** Step 5: Verify Fix
