...
3. **Data corruption:** If the error references cache or graph state corruption,
   do not clear caches blindly from the bot container. Capture the traceback,
   inspect Redis with [`REDIS_CACHE_DEGRADATION.md`](REDIS_CACHE_DEGRADATION.md),
   and only run targeted cleanup after confirming the affected key family.
   ```bash
   COMPOSE_PROJECT_NAME=dev docker compose --env-file tests/fixtures/compose.ci.env -f compose.yml -f compose.dev.yml logs bot --since 30m 2>&1 | grep -iE 'cache|checkpoint|graph|redis'
   ```
