...
COMPOSE_PROJECT_NAME=dev docker compose --env-file tests/fixtures/compose.ci.env -f compose.yml -f compose.dev.yml exec minio sh -c 'test -n "$MINIO_ROOT_USER" && echo "MINIO_ROOT_USER: present" || echo "MINIO_ROOT_USER: MISSING"'
COMPOSE_PROJECT_NAME=dev docker compose --env-file tests/fixtures/compose.ci.env -f compose.yml -f compose.dev.yml exec langfuse-server sh -c 'test -n "$LANGFUSE_S3_EVENT_UPLOAD_ACCESS_KEY_ID" && echo "LANGFUSE_S3_EVENT_UPLOAD_ACCESS_KEY_ID: present" || echo "LANGFUSE_S3_EVENT_UPLOAD_ACCESS_KEY_ID: MISSING"'
```

If either variable is missing, treat the alert as configuration drift. Do not
print MinIO credentials or `.env` values in incident logs.
...
```bash
# Check credential presence only; do not print secret or access-key values
COMPOSE_PROJECT_NAME=dev docker compose --env-file tests/fixtures/compose.ci.env -f compose.yml -f compose.dev.yml exec minio sh -c 'test -n "$MINIO_ROOT_USER" && test -n "$MINIO_ROOT_PASSWORD" && echo "MinIO credentials: present" || echo "MinIO credentials: MISSING"'
COMPOSE_PROJECT_NAME=dev docker compose --env-file tests/fixtures/compose.ci.env -f compose.yml -f compose.dev.yml exec langfuse-server sh -c 'test -n "$LANGFUSE_S3_EVENT_UPLOAD_ACCESS_KEY_ID" && test -n "$LANGFUSE_S3_EVENT_UPLOAD_SECRET_ACCESS_KEY" && echo "Langfuse S3 credentials: present" || echo "Langfuse S3 credentials: MISSING"'
```
...
Do not remove the MinIO volume from this runbook. If the backup confirms that
the dev-only volume must be recreated, perform that as a separate approved
maintenance action with the exact volume name verified from `docker volume ls`.
...
# 3. If restart does not resolve, collect logs and escalate before volume changes
COMPOSE_PROJECT_NAME=dev docker compose --env-file tests/fixtures/compose.ci.env -f compose.yml -f compose.dev.yml logs minio --tail=300
```
