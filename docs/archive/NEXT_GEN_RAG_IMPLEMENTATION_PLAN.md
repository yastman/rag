***REMOVED*** Next-Gen RAG Implementation Plan - Ukrainian Civil Code
...
***REMOVED******REMOVED******REMOVED*** Текущий статус: PRODUCTION-GRADE BASELINE ✅
...
***REMOVED******REMOVED******REMOVED*** Next-Gen Upgrade: CONTEXTUAL + KNOWLEDGE GRAPH
...
***REMOVED******REMOVED*** 📊 Deep Analysis Results (12 Thoughts Sequential Thinking)

***REMOVED******REMOVED******REMOVED*** 1. Contextual Retrieval (Anthropic) - MAXIMUM PRIORITY
...
***REMOVED******REMOVED******REMOVED*** 2. Lightweight Knowledge Graph - HIGH PRIORITY
...
payload = {
    ...
    ***REMOVED*** Graph edges (relationships)
    "prev_article": 12,
    "next_article": 14,
    "related_articles": [25, 26, 27],  ***REMOVED*** Cross-references
...
***REMOVED******REMOVED******REMOVED*** 3. Enhanced Prompts - INTEGRATED APPROACH
...
***REMOVED*** Parse to extract:
context_text = extract_context(response)  ***REMOVED*** For embedding
metadata = extract_metadata_json(response)  ***REMOVED*** For Qdrant payload
```
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Core Implementation (2-3 days)

```
rag-fresh/
├── prompts.py                    ***REMOVED*** Enhanced prompts (DONE ✅)
├── contextualize.py              ***REMOVED*** Claude API + prompt caching
├── utils/
│   ├── structure_parser.py       ***REMOVED*** Extract book/section/chapter/article
│   └── relationship_extractor.py ***REMOVED*** Find related articles
├── ingestion_contextual_kg.py    ***REMOVED*** Full pipeline
├── create_collection_enhanced.py ***REMOVED*** Qdrant schema with KG
└── config.py                     ***REMOVED*** API keys, settings
```
            ...
            "content": [
                {
                    ...
                    "cache_control": {"type": "ephemeral"}  ***REMOVED*** Cache document!
                },
                {
                    "type": "text",
...
async def process_document_with_contextual_kg(
    ...
    ***REMOVED*** Step 1: Docling chunking
    doc_data = docling_chunk(pdf_path)
    doc_content = get_full_document_text(pdf_path)  ***REMOVED*** For caching
    chunks = doc_data['chunks']

    ***REMOVED*** Step 2: Process each chunk
    for idx, chunk in enumerate(chunks):
        chunk_text = chunk['text']

        ***REMOVED*** Generate context + extract metadata (Claude API)
        context_text, metadata_claude = await situate_context_with_metadata(
            doc_content, chunk_text
        )

        ***REMOVED*** Validate/augment with regex parsing
        metadata_regex = parse_legal_structure(chunk_text)
        metadata = merge_metadata(metadata_claude, metadata_regex)

        ***REMOVED*** Extract relationships
        ...
        ***REMOVED*** Store in Qdrant
        qdrant_upsert(
            ...
            payload={
                "text": chunk_text,  ***REMOVED*** Original for display
                "contextual_prefix": context_text,
                "embedded_text": embedded_text,
                **metadata,  ***REMOVED*** All KG metadata
                "source": pdf_path,
                "chunk_index": idx
            }
        )

        ***REMOVED*** Rate limiting for Claude API
        await asyncio.sleep(1.2)
```

---

***REMOVED******REMOVED******REMOVED*** Phase 2: Evaluation Framework (1-2 days)
...
async def run_comprehensive_evaluation():
    ...
    ***REMOVED*** Generate comparison report
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Optional Improvements (2-3 days)
...
```python
***REMOVED*** Current (ColBERT reranking):
search_payload = {
    "prefetch": [
...
***REMOVED*** Alternative (RRF fusion):
search_payload = {
    "prefetch": [
        {"query": sparse_vector, "using": "sparse", "limit": 20},
        {"query": dense_vector, "using": "dense", "limit": 20}
...
for query, lang in bilingual_queries:
    ...
    ***REMOVED*** Measure Recall@5 for both
```
...
def normalize_query(query: str) -> str:
    ...
    if detect_language(query) == "ru":
        return translate_ru_to_ua(query)  ***REMOVED*** LibreTranslate or Google Translate API
    return query
```
...
***REMOVED******REMOVED*** 📈 Expected Results & Validation

***REMOVED******REMOVED******REMOVED*** Target Metrics (Phase 1-2)
...
***REMOVED******REMOVED******REMOVED*** Cost Analysis
...
***REMOVED******REMOVED*** 🗓️ Implementation Timeline

***REMOVED******REMOVED******REMOVED*** Week 1: Core Implementation (Days 1-3)
...
***REMOVED******REMOVED******REMOVED*** Week 2: Evaluation & Optimization (Days 4-5)
...
***REMOVED******REMOVED******REMOVED*** Week 3: Optional Enhancements (Days 6-7+)
...
***REMOVED******REMOVED*** 🔧 Technical Requirements

***REMOVED******REMOVED******REMOVED*** Software Dependencies

```bash
***REMOVED*** Python packages
pip install anthropic              ***REMOVED*** Claude API
pip install qdrant-client          ***REMOVED*** Qdrant
pip install requests               ***REMOVED*** API calls
pip install numpy                  ***REMOVED*** Metrics calculation
pip install pandas                 ***REMOVED*** Data analysis
pip install asyncio                ***REMOVED*** Async processing
pip install python-dotenv          ***REMOVED*** Environment variables
pip install prometheus-client      ***REMOVED*** Monitoring (optional)
```
...
***REMOVED******REMOVED******REMOVED*** Official Documentation (via Context7 MCP)
...
***REMOVED******REMOVED******REMOVED*** Phase 1-2 (MUST ACHIEVE)
...
***REMOVED******REMOVED******REMOVED*** Phase 3 (NICE TO HAVE)
...
├── rag-fresh/
│   ├── __init__.py
│   ├── prompts.py                      ***REMOVED*** ✅ CREATED
│   ├── contextualize.py                ***REMOVED*** TODO
│   ├── utils/
│   │   ├── __init__.py
│   │   ├── structure_parser.py         ***REMOVED*** TODO
│   │   └── relationship_extractor.py   ***REMOVED*** TODO
│   ├── ingestion_contextual_kg.py      ***REMOVED*** TODO
│   ├── create_collection_enhanced.py   ***REMOVED*** TODO
│   ├── evaluation.py                   ***REMOVED*** TODO
│   └── config.py                       ***REMOVED*** TODO
├── evaluation_queries.json             ***REMOVED*** TODO
├── evaluation_results.json             ***REMOVED*** OUTPUT
├── NEXT_GEN_RAG_IMPLEMENTATION_PLAN.md ***REMOVED*** ✅ THIS FILE
└── NEXT_GEN_RAG_REPORT.md              ***REMOVED*** TODO (after testing)
```

***REMOVED******REMOVED*** Appendix B: Quick Start Commands

```bash
***REMOVED*** Step 1: Install dependencies
pip install anthropic qdrant-client requests numpy pandas python-dotenv

***REMOVED*** Step 2: Set up environment
cp .env.example .env
***REMOVED*** Edit .env and add ANTHROPIC_API_KEY

***REMOVED*** Step 3: Test on 5 chunks
python -m rag-fresh.ingestion_contextual_kg --test --max-chunks 5

***REMOVED*** Step 4: Full run
python -m rag-fresh.ingestion_contextual_kg --collection uk_civil_code_contextual_kg

***REMOVED*** Step 5: Evaluate
python -m rag-fresh.evaluation --run-all

***REMOVED*** Step 6: View results
