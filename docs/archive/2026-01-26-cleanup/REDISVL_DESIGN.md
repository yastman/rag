***REMOVED*** Adaptive Graph-Vector RAG 2026 - Design Plan
...
***REMOVED******REMOVED******REMOVED*** 2.1 Feature Matrix
...
***REMOVED******REMOVED******REMOVED*** 2.2 Embedding Models
...
***REMOVED******REMOVED*** 3. Components Detail

***REMOVED******REMOVED******REMOVED*** 3.1 RedisVL SemanticCache
...
semantic_cache = SemanticCache(
    ...
    ttl=48 * 3600,  ***REMOVED*** 48 hours
    distance_threshold=0.15,  ***REMOVED*** cosine distance (~85% similarity)
    vectorizer=vectorizer,
)

***REMOVED*** Usage - vectorizer handles embeddings internally
await semantic_cache.acheck(prompt=query)
await semantic_cache.astore(prompt=query, response=answer)
```

***REMOVED******REMOVED******REMOVED*** 3.2 CESC (Context-Enabled Semantic Cache)
...
class CESCPersonalizer:
    async def get_response(self, query: str, user_id: int) -> str:
        ***REMOVED*** 1. Check semantic cache
        cached = await self.semantic_cache.acheck(prompt=query)

        if cached:
            ***REMOVED*** 2. Get user context
            ctx = await self.cache.get_user_context(user_id)

            ***REMOVED*** 3. Personalize with lightweight LLM (gpt-4o-mini)
        ...
        ***REMOVED*** 4. Full RAG pipeline on cache miss
    ...
    async def personalize(self, cached_response: str, user_context: dict, query: str) -> str:
        ...
        """
        return await self.mini_llm.generate(prompt)  ***REMOVED*** gpt-4o-mini / Cerebras
```

***REMOVED******REMOVED******REMOVED*** 3.3 LightRAG Knowledge Graph
...
***REMOVED*** Index documents (extracts entities & relations automatically)
await rag.ainsert(documents)

***REMOVED*** Query with hybrid mode (vector + graph)
result = await rag.aquery(
    "Квартиры в Солнечном береге от застройщика Тера близко к морю",
    param=QueryParam(
        mode="hybrid",  ***REMOVED*** "naive" | "local" | "global" | "hybrid"
        top_k=10,
    )
)
```

***REMOVED******REMOVED******REMOVED*** 3.4 HyDE (Hypothetical Document Embeddings)
...
class HyDEQueryExpander:
    ...
    async def expand(self, query: str) -> tuple[str, list[float]]:
        ***REMOVED*** 1. Generate hypothetical ideal answer
        ...
        ***REMOVED*** 2. Embed the hypothetical (better semantic match than raw query)
        embedding = await self.embedder.embed(hypothetical)

        return hypothetical, embedding
```

***REMOVED******REMOVED******REMOVED*** 3.5 Qdrant Hybrid Search (BM42 + Dense + ColBERT)
...
***REMOVED*** Create collection with multiple vector types
await qdrant.create_collection(
    ...
    sparse_vectors_config={
        "sparse": SparseVectorParams(
            modifier=Modifier.IDF,  ***REMOVED*** BM42-style IDF weighting
        ),
    },
)
```
...
sparse_vec = await bge_m3.embed_sparse(text)  ***REMOVED*** or BM42
...
results = await qdrant.query_points(
    ...
    prefetch=[
        ...
        ***REMOVED*** Sparse retrieval (keyword matching)
        Prefetch(
            query=SparseVector(
...
***REMOVED******REMOVED*** 4. Implementation Phases

***REMOVED******REMOVED******REMOVED*** Phase 1: RedisVL SemanticCache Fix (Day 1-2)
...
***REMOVED******REMOVED******REMOVED*** Phase 2: User Context Storage (Day 2-3)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: CESC Personalization (Day 3-4)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: LightRAG Integration (Day 5-6)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: HyDE Query Expansion (Day 6-7)
...
***REMOVED******REMOVED******REMOVED*** Phase 6: Qdrant BM42 Hybrid (Day 7-8)
...
***REMOVED******REMOVED******REMOVED*** Phase 7: Testing & Validation (Day 9-10)
...
***REMOVED******REMOVED*** 5. Configuration
...
***REMOVED*** Cache TTLs (seconds)
SEMANTIC_CACHE_TTL=172800       ***REMOVED*** 48 hours
EMBEDDINGS_CACHE_TTL=604800     ***REMOVED*** 7 days
USER_CONTEXT_TTL=2592000        ***REMOVED*** 30 days

***REMOVED*** SemanticCache
...
services:
  redis-stack:
    ...
    ports:
      - "6379:6379"
      - "8001:8001"  ***REMOVED*** RedisInsight
    volumes:
      - redis_data:/data
    environment:
...
***REMOVED******REMOVED*** 6. Expected Performance
...
***REMOVED******REMOVED*** 7. Risks & Mitigations
...
***REMOVED******REMOVED*** 8. Success Criteria
