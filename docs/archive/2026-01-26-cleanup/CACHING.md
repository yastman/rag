***REMOVED*** Redis Caching - Полная Документация
...
***REMOVED******REMOVED******REMOVED*** 1. Semantic Cache (LLM Answers)
...
***REMOVED******REMOVED******REMOVED*** 2. Embeddings Cache (Query Vectors)
...
***REMOVED******REMOVED*** Tier 2: Medium Caching

***REMOVED******REMOVED******REMOVED*** 3. QueryAnalyzer Cache
...
***REMOVED******REMOVED******REMOVED*** 4. Search Cache (Qdrant Results)
...
cache_service = CacheService(
    ...
    semantic_cache_ttl=48 * 3600,        ***REMOVED*** 48 часов
    embeddings_cache_ttl=7 * 24 * 3600,  ***REMOVED*** 7 дней
    analyzer_cache_ttl=24 * 3600,        ***REMOVED*** 24 часа
    search_cache_ttl=2 * 3600,           ***REMOVED*** 2 часа
    distance_threshold=0.85,              ***REMOVED*** 85% сходства для semantic cache
)
```

***REMOVED******REMOVED*** Метрики

***REMOVED******REMOVED******REMOVED*** Автоматический сбор метрик

```python
metrics = cache_service.get_metrics()
***REMOVED*** {
***REMOVED***     "by_type": {
***REMOVED***         "semantic": {"hits": 10, "misses": 5, "hit_rate": 66.7},
***REMOVED***         "embeddings": {"hits": 50, "misses": 10, "hit_rate": 83.3},
***REMOVED***         "analyzer": {"hits": 20, "misses": 15, "hit_rate": 57.1},
***REMOVED***         "search": {"hits": 30, "misses": 20, "hit_rate": 60.0}
***REMOVED***     },
***REMOVED***     "total_hits": 110,
***REMOVED***     "total_misses": 50,
***REMOVED***     "total_requests": 160,
***REMOVED***     "overall_hit_rate": 68.8
***REMOVED*** }
```

***REMOVED******REMOVED******REMOVED*** Логирование

```python
cache_service.log_metrics()
***REMOVED*** Cache Metrics: 68.8% hit rate, 160 requests
***REMOVED***   semantic: 66.7% (10/15)
***REMOVED***   embeddings: 83.3% (50/60)
***REMOVED***   analyzer: 57.1% (20/35)
***REMOVED***   search: 60.0% (30/50)
```

***REMOVED******REMOVED*** Мониторинг

***REMOVED******REMOVED******REMOVED*** Просмотр логов

```bash
***REMOVED*** Все события кеширования
docker logs telegram-rag-bot -f | grep -E '(cache|Cache)'

***REMOVED*** Только HIT события
docker logs telegram-rag-bot -f | grep "cache HIT"

***REMOVED*** Только semantic cache
docker logs telegram-rag-bot -f | grep "Semantic cache"
```

***REMOVED******REMOVED******REMOVED*** Проверка Redis

```bash
***REMOVED*** Количество ключей по типам
...
***REMOVED******REMOVED*** Тестирование

***REMOVED******REMOVED******REMOVED*** Автоматические тесты
...
***REMOVED******REMOVED******REMOVED*** Ручное тестирование через Telegram
...
***REMOVED******REMOVED*** Настройка Threshold

***REMOVED******REMOVED******REMOVED*** Текущий: 0.85 (85% сходства)
...
***REMOVED******REMOVED******REMOVED*** Изменение threshold

```python
***REMOVED*** В telegram_bot/services/cache.py
cache_service = CacheService(
    redis_url=redis_url,
    distance_threshold=0.80  ***REMOVED*** Изменить здесь
)
```

***REMOVED******REMOVED*** Troubleshooting

***REMOVED******REMOVED******REMOVED*** Semantic Cache не работает

1. **Проверить Redis Stack:**
   ```bash
   docker exec ai-redis-secure redis-cli -a $REDIS_PASSWORD --no-auth-warning MODULE LIST
   ***REMOVED*** Должен быть: search (RediSearch)
   ```

2. **Проверить индекс:**
   ```bash
   docker exec ai-redis-secure redis-cli -a $REDIS_PASSWORD --no-auth-warning FT.INFO idx:rag:semantic_cache
   ***REMOVED*** Должно быть: num_docs > 0
   ```
...
***REMOVED******REMOVED******REMOVED*** Низкий Hit Rate
...
2. **Уменьшить threshold (если semantic cache):**
   ```python
   distance_threshold=0.80  ***REMOVED*** Было 0.85
   ```

3. **Увеличить TTL:**
   ```python
   semantic_cache_ttl=72 * 3600  ***REMOVED*** Было 48h
   ```

***REMOVED******REMOVED******REMOVED*** Проблемы с памятью Redis

```bash
***REMOVED*** Проверить использование памяти
docker exec ai-redis-secure redis-cli -a $REDIS_PASSWORD --no-auth-warning INFO memory

***REMOVED*** Очистить старые ключи вручную
