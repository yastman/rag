***REMOVED*** Deep Server Audit Report (Host + Docker + Code + Plan)
...
***REMOVED******REMOVED*** Runtime Snapshot (2026-02-06)

- Host load and resources:
  - `uptime`: load around `7.80, 5.61, 4.25`
  - RAM: `11 GiB` total, swap in use (`~2.2 GiB`)
  - Disk `/`: `74G/96G` (78% used)
- Open host ports: `80`, `443`, `22` (SSH), `3306` local-only.
- UFW: active, default deny incoming, only `80/443/22` allowed.
...
| C-03 | Critical | Backup / DR | `scripts/qdrant_backup.sh` exists, but no scheduler found in `/etc/cron*` or systemd; `/srv/backups` latest file is old test SQL dump | No verified automated data backups for live Qdrant/Postgres/Redis volumes; high data-loss risk on host/disk failure. | Add scheduled backups (Qdrant snapshots + pg_dump + Redis RDB/AOF policy), retention, and restore test. |
...
***REMOVED******REMOVED*** Plan Verification (`2026-02-01-rag-2026-tz.md`)
...
***REMOVED******REMOVED*** Context7 Cross-Check (Official SDK)
...
***REMOVED******REMOVED*** Priority Remediation Plan

***REMOVED******REMOVED******REMOVED*** 0-24h
...
***REMOVED******REMOVED******REMOVED*** 1-7 days
...
***REMOVED******REMOVED******REMOVED*** 1-4 weeks
