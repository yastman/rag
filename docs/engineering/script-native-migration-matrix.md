...
| `scripts/repo_cleanup.sh` | Former `Makefile` cleanup targets, tests, docs | `git fetch --prune origin`; `git branch --merged origin/$MAIN_BRANCH`; `git merge-base --is-ancestor`; `git branch -D`; `git branch -r --merged`; `gh pr list --head`; `git worktree prune`; `git stash list` | archived as `scripts/archive/repo_cleanup.sh`; no active callsites | DevEx | `make -n repo-cleanup repo-cleanup-force`; focused unit test |
