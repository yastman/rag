...
|---|---|---|---|---|---|
| `scripts/git_hygiene.py` | Former `Makefile` hygiene targets, tests, docs | `git fetch --prune origin`; `git branch --merged origin/$REPO_BASE_BRANCH`; `git for-each-ref`; `git worktree list --porcelain`; `git ls-files --others --exclude-standard` | deleted; git history is the archive (#2891); no active callsites | DevEx | `make -n git-hygiene git-hygiene-fix`; focused unit test |
| `scripts/repo_cleanup.sh` | Former `Makefile` cleanup targets, tests, docs | `git fetch --prune origin`; `git branch --merged origin/$MAIN_BRANCH`; `git merge-base --is-ancestor`; `git branch -D`; `git branch -r --merged`; `gh pr list --head`; `git worktree prune`; `git stash list` | deleted; git history is the archive (#2891); no active callsites | DevEx | `make -n repo-cleanup repo-cleanup-force`; focused unit test |
...
- Destructive local branch deletion re-checks `git merge-base --is-ancestor`
  against `origin/<base>` before `git branch -D`; it does not rely on
  `git branch -d` current-HEAD semantics.
