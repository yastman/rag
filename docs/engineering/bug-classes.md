...
| self-hosted runner scheduling drift | `tests/unit/test_trusted_heavy_workflow.py` pins PR self-hosted jobs to `pr-fast` and keeps `Heavy Contract Tests (shadow)` non-authoritative; `tests/unit/test_ci_deploy_workflow.py` pins nightly `nightly-heavy` routing, read-only permissions, cancellation, and timeouts; `scripts/check_self_hosted_runner.sh` verifies both runner label groups before workflows are considered live | [#1531](https://github.com/yastman/rag/issues/1531) | [#2310](https://github.com/yastman/rag/pull/2310), [#2311](https://github.com/yastman/rag/pull/2311) | 2026 | 2026 |
