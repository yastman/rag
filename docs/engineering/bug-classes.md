...
|---|---|---|---|---|---|
| Langfuse/OTEL/contextvars loss | Context propagation contract in `tests/contract/test_observability_contextvars_contract.py` and ingestion raw-thread coverage in `tests/contract/test_ingestion_threadhop_contract.py`; CI gate via `make test-contract` | [#2251](https://github.com/yastman/rag/issues/2251) | [#2246](https://github.com/yastman/rag/issues/2246), [#2301](https://github.com/yastman/rag/issues/2301) | 2025 | 2026 |
| Observability trace-coverage drift | End-to-end trace flow contract in `tests/contract/test_end_to_end_trace_flow_contract.py`; span metadata coverage in `tests/unit/test_observability_span_metadata.py`; trace continuity and log-correlation validation in `scripts/validate_traces.py`, `scripts/trace_continuity.py`, and `scripts/log_correlation.py`; CI gate via `make test-contract` | [#2215](https://github.com/yastman/rag/issues/2215) | [#2246](https://github.com/yastman/rag/issues/2246), [#2272](https://github.com/yastman/rag/issues/2272), [#2266](https://github.com/yastman/rag/issues/2266), [#2256](https://github.com/yastman/rag/issues/2256), [#2253](https://github.com/yastman/rag/issues/2253), [#2223](https://github.com/yastman/rag/issues/2223), [#2222](https://github.com/yastman/rag/issues/2222), [#2221](https://github.com/yastman/rag/issues/2221), [#2216](https://github.com/yastman/rag/issues/2216), [#2214](https://github.com/yastman/rag/issues/2214), [#2166](https://github.com/yastman/rag/issues/2166), [#2064](https://github.com/yastman/rag/issues/2064), [#2301](https://github.com/yastman/rag/issues/2301) | 2025 | 2026 |
...
| Docker/compose drift | `tests/unit/test_compose_runtime_contract.py` pins service profile expectations and BGE-M3 canonical endpoint/port contracts; `tests/unit/test_check_image_drift.py` locks the read-only runtime drift checker; `telegram_bot/preflight.py` validates BGE-M3 canonical port 8000 for local/native URLs before any network call; `tests/contract/test_qdrant_preflight_contract.py` locks Qdrant SDK transport fallback and non-empty startup diagnostics; `make verify-compose-runtime` compares running image digests and published host ports against Compose source | [#2185](https://github.com/yastman/rag/issues/2185) | [#2123](https://github.com/yastman/rag/issues/2123), [#2182](https://github.com/yastman/rag/issues/2182), [#2188](https://github.com/yastman/rag/issues/2188), [#2103](https://github.com/yastman/rag/issues/2103) duplicates [#2124](https://github.com/yastman/rag/issues/2124) (postgres `cap_drop` duplicate fix) | 2025 | 2026 |
| self-hosted runner scheduling drift | `tests/unit/test_trusted_heavy_workflow.py` pins PR self-hosted jobs to `pr-fast` and keeps `Fast Tests` and `Heavy Contract Tests` authoritative for trusted risk-path PRs; `tests/unit/test_ci_deploy_workflow.py` pins nightly `nightly-heavy` routing, read-only permissions, cancellation, and timeouts; `scripts/check_self_hosted_runner.sh` verifies both runner label groups before workflows are considered live | [#1531](https://github.com/yastman/rag/issues/1531) | [#2310](https://github.com/yastman/rag/pull/2310), [#2311](https://github.com/yastman/rag/pull/2311) | 2026 | 2026 |
| History-rewrite syntactic corruption | `make compile-python` runs compileall over every repo-tracked Python file to catch unterminated string literals and other syntactic breakage caused by secret-scrub history rewrites; `trusted-heavy.yml` Fast Tests and `nightly-heavy.yml` run it as early gates; `make baseline-compile` remains a compatibility alias for this repo-wide guardrail | [#2320](https://github.com/yastman/rag/issues/2320) | [#2319](https://github.com/yastman/rag/issues/2319), [#1982](https://github.com/yastman/rag/issues/1982), [#1580](https://github.com/yastman/rag/issues/1580) | 2026 | 2026 |
...
| Test service-dependency lane drift | Every `tests/integration/test_*.py` and `tests/smoke/test_*.py` file must carry exactly one file-level `no_services` or `requires_services` marker via `pytestmark`; both markers must be registered in `[tool.pytest.ini_options].markers`; contract coverage in `tests/contract/test_service_dependency_markers_contract.py`; CI gate via `make test-contract` | [#2324](https://github.com/yastman/rag/issues/2324) | [#2319](https://github.com/yastman/rag/issues/2319), [#2320](https://github.com/yastman/rag/issues/2320), [#2325](https://github.com/yastman/rag/issues/2325) | 2026 | 2026 |
