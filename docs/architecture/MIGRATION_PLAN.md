...
def log_ab_test(engine_name: str, results: dict, report_path: str):
    ...
    with mlflow.start_run(run_name=f"{engine_name}_evaluation"):
        ...
        ***REMOVED*** Log search config as nested params
...
def create_ragas_dataset(engine, queries_file: str, limit: int = 50):
    ...
    for query_data in queries:
        ...
        ***REMOVED*** Extract contexts
        contexts = [r.payload.get("text", "") for r in results]

        ***REMOVED*** Generate answer (top result)
        answer = results[0].payload.get("text", "") if results else ""

        ***REMOVED*** Ground truth (expected article text)
...
def run_ragas_evaluation(engine_name: str = "dbsf_colbert"):
    ...
    ***REMOVED*** Create dataset
    print("📊 Creating RAGAS dataset...")
    dataset = create_ragas_dataset(
        engine,
        queries_file="data/queries_testset.json",
        limit=50  ***REMOVED*** Start with 50 queries
    )

    ***REMOVED*** Initialize LLM for evaluation
    ...
    ***REMOVED*** Run evaluation
    ...
    with mlflow.start_run(run_name=f"ragas_{engine_name}"):
        ...
        ***REMOVED*** Log results as artifact
...
***REMOVED*** Helper function with native decorator
...
***REMOVED*** Manual spans for fine-grained control
...
***REMOVED*** Run and generate report
results = suite.run()
...
│   ├── langfuse_integration.py    ***REMOVED*** 🆕 Langfuse tracing
│   ├── giskard_smoke_tests.py     ***REMOVED*** 🆕 Giskard tests
│   ├── config_snapshot.py         ***REMOVED*** 📦 Deprecated → MLflow
│   ├── smoke_test.py              ***REMOVED*** 📦 Deprecated → Giskard
│   └── metrics_logger.py          ***REMOVED*** 📦 Deprecated → Langfuse
├── docker-compose.yml             ***REMOVED*** 🔄 + Langfuse service
├── .env                           ***REMOVED*** 🔄 + API keys
└── requirements.txt               ***REMOVED*** 🔄 Updated
