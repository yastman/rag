***REMOVED*** Deploy Checklist: Conversation Memory (***REMOVED***157)

**PR:** follow-up от ***REMOVED***154 (Conversation Memory)
...
***REMOVED******REMOVED*** Deploy Steps

```bash
***REMOVED*** 1. Build bot image
docker buildx bake bot --load

***REMOVED*** 2. Push to VPS
make k3s-push-bot

***REMOVED*** 3. Apply k3s manifests
make k3s-bot

***REMOVED*** 4. Verify pods
make k3s-status
***REMOVED*** Expected: bot pod Running, 1/1 Ready
```

***REMOVED******REMOVED*** Post-deploy Live Tests

***REMOVED******REMOVED******REMOVED*** Memory Continuity
...
***REMOVED******REMOVED******REMOVED*** /clear Command
1. Send: `/clear`
...
***REMOVED******REMOVED******REMOVED*** Redis Keys
```bash
***REMOVED*** On VPS:
ssh vps "docker exec redis redis-cli keys 'conversation:*'"
***REMOVED*** Expected: no conversation:* keys (legacy writes removed)
***REMOVED*** Semantic cache keys (sem:v3:*) should still exist
```

***REMOVED******REMOVED******REMOVED*** Langfuse Traces
