...
- Compose image: `qdrant/qdrant:v1.18.1` (pinned by digest in compose files)
...
### Audit payload indexes

Verify all expected payload indexes exist in the live collection:

```bash
make qdrant-audit-indexes
# or directly:
uv run python scripts/qdrant_audit_indexes.py
```

Prints `PASS` with count, or `FAIL` with each missing field name. Reads `QDRANT_URL` and `QDRANT_COLLECTION` from the environment (defaults: `http://localhost:6333` / `gdrive_documents_bge`). Requires Qdrant to be running.

Expected fields (from `src/ingestion/unified/cli.py` bootstrap + runtime filter contract):
`file_id`, `metadata.file_id`, `metadata.doc_id`, `metadata.source`, `metadata.file_name`,
`metadata.mime_type`, `metadata.source_type`, `metadata.topic`, `metadata.doc_type`,
`metadata.jurisdiction`, `metadata.audience`, `metadata.language`, `metadata.order`, `metadata.chunk_id`.
