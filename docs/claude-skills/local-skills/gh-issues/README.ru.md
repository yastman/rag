...
| Закрыть с комментарием | `gh issue close 123 -c "Fixed in ***REMOVED***456"` |
| Создать ветку | `gh issue develop 123 --checkout` |

***REMOVED******REMOVED******REMOVED*** Воркфлоу AI-контекста

```bash
***REMOVED*** Сохранить контекст в issue
gh issue comment 45 --body-file .ai-context.md

***REMOVED*** Загрузить контекст из issue
gh issue view 45 --json comments --jq '
  .comments[] | select(.body | contains("AI-CONTEXT")) | .body
'
```

***REMOVED******REMOVED******REMOVED*** Лейблы задач
