***REMOVED*** Langfuse Trace & Score Integrity Audit — ***REMOVED***476
...
***REMOVED******REMOVED*** 1. Enablement & Config

***REMOVED******REMOVED******REMOVED*** Mechanism
...
***REMOVED******REMOVED******REMOVED*** Graceful No-Op (LANGFUSE_ENABLED=False)
...
***REMOVED******REMOVED******REMOVED*** Startup Logging
...
***REMOVED******REMOVED******REMOVED*** Minor Finding
...
***REMOVED******REMOVED*** 2. Trace Coverage Map
...
***REMOVED******REMOVED******REMOVED*** Child Spans
...
***REMOVED******REMOVED******REMOVED*** Scores Written Per Path
...
***REMOVED******REMOVED******REMOVED*** Orphan Trace Risk

All 4 entry points use `propagate_attributes()` before invoking `@observe`-decorated functions. No orphan traces observed.

---

***REMOVED******REMOVED*** 3. Score Integrity

***REMOVED******REMOVED******REMOVED*** Idempotency Pattern
...
lf.create_score(
    ...
    name=name,
    value=value,
    id=f"{trace_id}-{name}",   ***REMOVED*** ← idempotency key
    **kwargs,
)
```
...
***REMOVED******REMOVED******REMOVED*** Cross-Trace Leakage (Issue ***REMOVED***435)
...
***REMOVED******REMOVED******REMOVED*** Finding F-01: Inconsistent Idempotency Key Parameter in `handle_feedback`
...
```python
***REMOVED*** handle_feedback uses score_id= (not id=)
lf_client.create_score(
    ...
    score_id=f"{trace_id}-user_feedback",  ***REMOVED*** ← should be id=
)
```
...
***REMOVED******REMOVED*** 4. Metadata Schema Consistency

***REMOVED******REMOVED******REMOVED*** Text Path (sdk_agent)
...
lf.update_current_trace(
    ...
    metadata=_build_trace_metadata(result),  ***REMOVED*** returns ~15 fields
)
```

**Metadata fields:** ~15 via `_build_trace_metadata()`

***REMOVED******REMOVED******REMOVED*** Finding F-02: Text Path Metadata Significantly Thinner Than Voice Path
...
***REMOVED******REMOVED******REMOVED*** `/history` Path
...
***REMOVED******REMOVED******REMOVED*** Finding F-03: Raw Integer `user_id` in `/history` Trace Metadata
...
```python
metadata={"user_id": user_id, ...}   ***REMOVED*** user_id is raw int (e.g. 523847291)
```

Even if PII masking were applied, `mask_pii()` does not handle integers (only strings). The Telegram user ID is PII under GDPR.

***REMOVED******REMOVED******REMOVED*** Session ID Format Consistency
...
***REMOVED******REMOVED*** 5. PII Masking

***REMOVED******REMOVED******REMOVED*** Architecture

```python
***REMOVED*** get_langfuse_client() — MASKED (used ONLY in handle_feedback)
def get_langfuse_client() -> Langfuse:
    return Langfuse(mask=mask_pii, flush_at=50, flush_interval=5)

***REMOVED*** get_client() — UNMASKED (used by ALL @observe spans)
get_client = _real_get_client   ***REMOVED*** SDK singleton, no mask= parameter
```

***REMOVED******REMOVED******REMOVED*** mask_pii() Coverage
...
***REMOVED******REMOVED******REMOVED*** Finding F-04 (CRITICAL): PII Masking Not Applied to @observe Spans
...
```python
***REMOVED*** In application startup, before @observe runs:
Langfuse(mask=mask_pii)  ***REMOVED*** initializes the singleton with masking
```
Or use `langfuse.configure(mask=mask_pii)` if the v3 SDK supports it.

***REMOVED******REMOVED******REMOVED*** Finding F-05 (MEDIUM): Integer PII Not Handled by mask_pii()
...
if isinstance(data, int):
    ...
    ***REMOVED*** or store as string: return "[USER_ID]" if re.match(r"^\d{9,10}$", s) else data
```

***REMOVED******REMOVED******REMOVED*** Finding F-06 (LOW): Truncation Threshold May Be Too Short
...
***REMOVED******REMOVED*** 6. Validation Results

***REMOVED******REMOVED******REMOVED*** validate_traces.py
...
***REMOVED******REMOVED******REMOVED*** make validate-traces-fast
...
***REMOVED******REMOVED*** 7. Findings Table
...
***REMOVED******REMOVED******REMOVED*** OBSERVABILITY: **CONDITIONAL GO**
...
- Score isolation via explicit `trace_id` is solid (***REMOVED***435 properly addressed) ✓
