***REMOVED*** Qdrant + Docling: Анализ и план оптимизации
...
***REMOVED******REMOVED*** 📊 ТЕКУЩАЯ КОНФИГУРАЦИЯ

***REMOVED******REMOVED******REMOVED*** Qdrant Collection: `legal_documents`
...
***REMOVED******REMOVED*** ✅ ЧТО УЖЕ ИСПОЛЬЗУЕТСЯ (ХОРОШО)

***REMOVED******REMOVED******REMOVED*** 1. Hybrid Search ✅
...
quantization_config = BinaryQuantization(
    binary={
        "encoding": "one_and_half_bits",  ***REMOVED*** 1.5-bit
        "always_ram": True
    }
)
```
...
***REMOVED******REMOVED******REMOVED*** 2. HNSW Delta Compression ❌
...
hnsw_config = {
    "m": 16,
    "ef_construct": 200,
    "compression": "delta"  ***REMOVED*** NEW!
}
```

***REMOVED******REMOVED******REMOVED*** 3. Strict Mode (NEW в v1.13.0) ❌
...
strict_mode_config = {
    "enabled": True,
    "max_batch_size": 1000,
    "max_payload_size": 1048576,  ***REMOVED*** 1MB
    "timeout": 30,
    "unindexed_filtering_retrieve": False
}
```

***REMOVED******REMOVED******REMOVED*** 4. Docling Optimizations ❌
...
pipeline_options = PdfPipelineOptions(
    do_ocr=False,  ***REMOVED*** Только для scanned PDFs
    do_table_structure=True,
    table_structure_options=TableFormerMode.FAST,  ***REMOVED*** 3-6x быстрее
    backend=DoclingParseV2DocumentBackend,  ***REMOVED*** 10x быстрее!
...
***REMOVED******REMOVED******REMOVED*** 5. Chunking Strategy ⚠️
...
**Решение**:
```python
***REMOVED*** Вариант 1: Увеличить chunk_size
DocumentChunker(
    chunk_size=1024,  ***REMOVED*** Было 512
    overlap=256,      ***REMOVED*** Было 128
    strategy=ChunkingStrategy.SEMANTIC  ***REMOVED*** Уважает структуру
)

***REMOVED*** Вариант 2: Docling HybridChunker (best)
...
chunker = HybridChunker(
    tokenizer=tokenizer,
    max_tokens=512,  ***REMOVED*** Token-aware, не chars
    merge_peers=True
)
```

***REMOVED******REMOVED******REMOVED*** 6. Payload Indexes ⚠️
...
***REMOVED******REMOVED*** 🚀 НОВЫЕ ФИЧИ QDRANT v1.15.x (НЕ ИСПОЛЬЗУЮТСЯ)

***REMOVED******REMOVED******REMOVED*** 1. GPU Indexing (v1.13.0+) 🔥
...
***REMOVED******REMOVED******REMOVED*** 2. MMR (Maximum Marginal Relevance) - v1.15.0 🆕
...
results = client.query_points(
    ...
    mmr_config=MMRConfig(
        lambda_param=0.7  ***REMOVED*** 0=max diversity, 1=max relevance
    )
)
```

***REMOVED******REMOVED******REMOVED*** 3. Server-Side Score Boosting - v1.14.0 🆕
...
***REMOVED******REMOVED******REMOVED*** 4. Custom Storage Engine (v1.13.0) 🆕
...
***REMOVED******REMOVED*** 📝 ПЛАН ОПТИМИЗАЦИИ

***REMOVED******REMOVED******REMOVED*** Приоритет 1: КРИТИЧНЫЕ (делать сейчас)
...
***REMOVED******REMOVED******REMOVED*** Приоритет 2: РЕКОМЕНДУЕМЫЕ (делать скоро)
...
***REMOVED******REMOVED******REMOVED*** Приоритет 3: ОПЦИОНАЛЬНЫЕ (по необходимости)
...
***REMOVED******REMOVED******REMOVED*** Качество RAG
...
***REMOVED******REMOVED******REMOVED*** Мониторинг
...
***REMOVED******REMOVED******REMOVED*** Тестирование
