# ADR-0007: Custom HyDEGenerator Retained (No LangChain-Native Replacement)
...
PR #1622 proposed replacing the custom `HyDEGenerator` (in `telegram_bot/services/query_preprocessor.py`) with LangChain's `HypotheticalDocumentEmbedder`. This ADR documents the SDK research that proves no viable LangChain 1.2-native replacement exists in the pinned dependency set.

### Pinned Environment (from `uv.lock`)
...
### Key Finding: LangChain 1.2 Restructuring
...
```python
# langchain 1.2.15 has no chains module
>>> import importlib.util
>>> importlib.util.find_spec("langchain.chains.hyde.base")
ModuleNotFoundError: No module named 'langchain.chains'

# HypotheticalDocumentEmbedder exists only in langchain-classic
>>> from langchain_classic.chains.hyde.base import HypotheticalDocumentEmbedder
# (works, but langchain-classic is not a direct dependency)
```
...
### 2. Fundamentally Different Contract
...
### 6. Cannot Use Dynamic Prompt Loading
...
```bash
# Confirms langchain.chains.hyde is NOT importable
uv run python -c "import importlib.util; print(importlib.util.find_spec('langchain.chains.hyde.base'))"
# Output: ModuleNotFoundError: No module named 'langchain.chains'
