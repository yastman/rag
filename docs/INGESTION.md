***REMOVED*** Document Ingestion Pipeline
...
```bash
***REMOVED*** 1. Setup Qdrant collection with payload indexes
make ingest-setup

***REMOVED*** 2. Ingest from local directory
make ingest-dir DIR=path/to/documents

***REMOVED*** 3. Or ingest from Google Drive
export GOOGLE_SERVICE_ACCOUNT_KEY=path/to/credentials.json
make ingest-gdrive FOLDER_ID=your_folder_id

***REMOVED*** 4. Check status
make ingest-status
```
...
***REMOVED******REMOVED******REMOVED*** Chunking Parameters
...
```bash
make ingest-setup    ***REMOVED*** Setup Qdrant collection with indexes
make ingest-dir      ***REMOVED*** Ingest from directory (DIR=path)
make ingest-gdrive   ***REMOVED*** Ingest from Google Drive (FOLDER_ID=xxx)
make ingest-status   ***REMOVED*** Show collection statistics
make ingest-test     ***REMOVED*** Run ingestion unit tests
```

***REMOVED******REMOVED*** Python API
...
***REMOVED******REMOVED******REMOVED*** Ingestion Stats
...
***REMOVED******REMOVED*** Postgres State Tracking (Optional)

For advanced state management, use the `cocoindex` database:

```bash
***REMOVED*** Database auto-created on Postgres startup
***REMOVED*** Tables: ingestion_state, ingestion_dead_letter
```
...
```bash
***REMOVED*** Unit tests (no Docker required)
make ingest-test

***REMOVED*** Integration tests (requires Docker services)
