***REMOVED*** Contextual RAG Pipeline - Полный Overview
...
```python
***REMOVED*** src/ingestion/csv_to_qdrant.py
indexer = CSVToQdrantIndexer()
records = indexer.read_csv("demo_BG.csv")
***REMOVED*** Каждая строка CSV → текст + metadata
***REMOVED*** "Недвижимость: Квартира... Город: Солнечный берег..."
```
...
```python
***REMOVED*** В indexer.py (обновлено 2025-11-05)
...
***REMOVED*** Returns: {
***REMOVED***   "dense_vecs": [1024],
***REMOVED***   "lexical_weights": {token_id: weight},
***REMOVED***   "colbert_vecs": [N, 1024]
***REMOVED*** }
```
...
PointStruct(
    ...
    vector={
        ...
        "colbert": [[N×1024] floats],     ***REMOVED*** Reranking
        "bm42": {                          ***REMOVED*** Keyword search
            "indices": [token_ids],
            "values": [weights]
        }
    },
    payload={
        "page_content": chunk.text,  ***REMOVED*** n8n LangChain format
        "metadata": {
            "document_name": "...",
            "article_number": "...",
            "chapter": "...",
...
***REMOVED******REMOVED******REMOVED*** Step 4: Retrieval (Поиск)

**Модуль:** `src/retrieval/`

**Два варианта (Variant A - default, Variant B - experimental):**

***REMOVED******REMOVED******REMOVED******REMOVED*** **Variant A: Hybrid RRF + ColBERT**
...
***REMOVED******REMOVED******REMOVED******REMOVED*** **Variant B: Hybrid DBSF + ColBERT**
...
```python
***REMOVED*** src/config/constants.py
...
***REMOVED******REMOVED******REMOVED*** Step 5: Cache (Redis)
...
***REMOVED*** Проверка кэша
cached = cache.get(query="как наказывается кража?")
if cached:
    return cached

***REMOVED*** ... выполнить поиск + LLM ...

***REMOVED*** Сохранить в кэш
cache.set(query="...", response="...", ttl=3600)
```

---

***REMOVED******REMOVED******REMOVED*** Step 6: LLM Processing (Contextualization)
...
***REMOVED******REMOVED******REMOVED*** Step 7: Observability (Мониторинг)
...
***REMOVED******REMOVED*** 🗂️ Структура проекта (детально)
...
***REMOVED******REMOVED*** 🚀 Как запустить каждый компонент

***REMOVED******REMOVED******REMOVED*** 1. Добавить PDF документы
...
***REMOVED******REMOVED******REMOVED*** 2. Добавить CSV данные
...
***REMOVED******REMOVED******REMOVED*** 3. Поисковый запрос (Variant A)
...
***REMOVED******REMOVED******REMOVED*** 4. Полный RAG pipeline
...
***REMOVED******REMOVED******REMOVED*** 5. Мониторинг
...
***REMOVED******REMOVED******REMOVED*** Settings (Python)
...
settings = Settings(
    ***REMOVED*** Search
    search_engine=SearchEngine.HYBRID_RRF_COLBERT,  ***REMOVED*** или DBSF_COLBERT
    top_k=10,
    score_threshold=0.7,
...
***REMOVED******REMOVED*** 📈 Метрики и Performance

***REMOVED******REMOVED******REMOVED*** Качество (RAGAS)

- **Faithfulness:** ≥ 0.85 (точность ответов)
- **Precision:** ≥ 0.80 (релевантность chunks)
- **Recall:** ≥ 0.90 (полнота покрытия)

***REMOVED******REMOVED******REMOVED*** Скорость
...
***REMOVED******REMOVED******REMOVED*** Экономия
...
***REMOVED******REMOVED******REMOVED*** Проблема: "Must provide API key" (Qdrant)

```python
***REMOVED*** Решение: Добавить API key
client = QdrantClient(
...
***REMOVED******REMOVED******REMOVED*** Проблема: Redis connection error

```bash
***REMOVED*** Проверить контейнер
docker ps | grep redis

***REMOVED*** Проверить пароль
echo $REDIS_PASSWORD
```

***REMOVED******REMOVED******REMOVED*** Проблема: Slow search

```python
***REMOVED*** Использовать Variant B (быстрее на 7%)
settings = Settings(
...
***REMOVED******REMOVED*** 🎓 Обучающие материалы

***REMOVED******REMOVED******REMOVED*** Для понимания компонентов:
...
```python
***REMOVED*** В src/config/constants.py
DEFAULT_SEARCH_ENGINE = SearchEngine.DBSF_COLBERT  ***REMOVED*** вместо HYBRID_RRF_COLBERT
```
...
***REMOVED******REMOVED*** ✅ Checklist: Что нужно знать
