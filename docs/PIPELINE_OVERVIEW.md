***REMOVED*** Contextual RAG Pipeline - Overview
...
***REMOVED******REMOVED*** Что это за система?
...
***REMOVED******REMOVED*** Архитектура High-Level
...
***REMOVED******REMOVED*** Data Flow: От источника до ответа

***REMOVED******REMOVED******REMOVED*** Step 1: Ingestion (Загрузка документов)
...
***REMOVED******REMOVED******REMOVED*** Step 2: Embeddings (Векторизация)

**Два варианта:**

***REMOVED******REMOVED******REMOVED******REMOVED*** Voyage AI (рекомендуется)

```python
from telegram_bot.services import VoyageService

service = VoyageService(
    model_docs="voyage-4-large",     ***REMOVED*** 1024-dim, для документов
    model_queries="voyage-4-lite",   ***REMOVED*** Для запросов (asymmetric)
...
***REMOVED******REMOVED******REMOVED******REMOVED*** BGE-M3 (локально, high RAM)

```python
from src.models import get_bge_m3_model

model = get_bge_m3_model()  ***REMOVED*** Singleton, экономит 4-6GB RAM
output = model.encode(
    texts,
    return_dense=True,
    return_sparse=True,
...
***REMOVED*** Returns: dense (1024-dim) + sparse (BM42) + ColBERT multi-vectors
```

---

***REMOVED******REMOVED******REMOVED*** Step 3: Indexing (Сохранение в Qdrant)
...
PointStruct(
    id=uuid.uuid4(),
    vector={
        "dense": [1024 floats],        ***REMOVED*** Semantic search
        "bm42": SparseVector(...),     ***REMOVED*** Keyword search
        "colbert": [[N×1024] floats],  ***REMOVED*** Reranking (optional)
    },
    payload={
...
qdrant = QdrantService(
    use_quantization=True,           ***REMOVED*** 40x faster
    quantization_rescore=True,       ***REMOVED*** Maintain accuracy
    quantization_oversampling=2.0,   ***REMOVED*** Fetch 2x, rescore top_k
)

***REMOVED*** A/B testing: disable per-request
results = await qdrant.hybrid_search_rrf(
    dense_vector=query_embedding,
    quantization_ignore=True,  ***REMOVED*** Skip quantization
)
```

---

***REMOVED******REMOVED******REMOVED*** Step 4: Query Processing

**Модуль:** `telegram_bot/services/`

***REMOVED******REMOVED******REMOVED******REMOVED*** QueryPreprocessor (Translit + RRF Weights)
...
***REMOVED***   "rrf_weights": {"dense": 0.2, "sparse": 0.8},  ***REMOVED*** Exact → sparse
...
***REMOVED******REMOVED******REMOVED******REMOVED*** QueryRouter (RAG Skipping)

```python
from telegram_bot.services import classify_query, QueryType, get_chitchat_response

query_type = classify_query("Привет!")  ***REMOVED*** QueryType.CHITCHAT
...
***REMOVED******REMOVED******REMOVED*** Step 5: Retrieval (Поиск)
...
***REMOVED*** 3-stage: Dense + Sparse → RRF → ColBERT rerank
...
***REMOVED******REMOVED******REMOVED*** Step 6: Cache (6-Tier)
...
cache = CacheService()
***REMOVED*** Auto-caches: embeddings, search results, rerank, analyzer
```

---

***REMOVED******REMOVED******REMOVED*** Step 7: LLM Contextualization
...
async for chunk in llm.stream_response(query, context):
    yield chunk  ***REMOVED*** Real-time streaming to Telegram
```
...
***REMOVED******REMOVED*** External Services
...
│   ├── ingestion/                ***REMOVED*** Document parsing, chunking, indexing
│   ├── retrieval/                ***REMOVED*** 4 search engine variants
│   ├── contextualization/        ***REMOVED*** LLM providers (Claude, OpenAI, Groq)
│   ├── cache/                    ***REMOVED*** RedisSemanticCache
│   ├── config/                   ***REMOVED*** Settings, enums
│   ├── evaluation/               ***REMOVED*** RAGAS, MLflow, Langfuse
│   ├── models/                   ***REMOVED*** BGE-M3 singleton
...
│
├── scripts/                      ***REMOVED*** Utilities
│   ├── test_quantization_ab.py   ***REMOVED*** Binary quantization A/B testing
│   └── index_contextual.py       ***REMOVED*** Document indexing
│
├── tests/                        ***REMOVED*** pytest (unit, integration, e2e, smoke, load)
├── docs/                         ***REMOVED*** Documentation
├── AGENTS.md                     ***REMOVED*** Codex workflow + instruction chain root
└── pyproject.toml                ***REMOVED*** Dependencies, Ruff, MyPy
```
...
***REMOVED******REMOVED******REMOVED*** Environment Variables (.env)
...
***REMOVED******REMOVED*** Performance (2026 Defaults)
