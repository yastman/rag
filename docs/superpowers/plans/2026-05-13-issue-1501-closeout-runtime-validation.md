***REMOVED*** Issue 1501 Closeout And Runtime Validation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Close the ***REMOVED***1501 code-complete work responsibly by linking the merged implementation evidence, separating unrelated broad-suite instability, and collecting fresh non-production Telegram HIT/MISS Langfuse trace evidence through tmux swarm execution.

**Architecture:** Treat ***REMOVED***1501 as a completed code change with one remaining runtime evidence lane. Keep GitHub issue management, runtime trace validation, and unrelated xdist investigation as separate bounded work items so workers do not conflate implementation correctness with environment/runtime validation. Runtime validation must use local/non-production credentials only and must produce redacted evidence suitable for a GitHub issue comment.
...
***REMOVED******REMOVED*** Linked Issue

- Primary issue: [***REMOVED***1501](https://github.com/yastman/rag/issues/1501) `perf: reduce pre-agent BGE latency in client-direct RAG traces`
- Merged implementation PRs:
  - [***REMOVED***1503](https://github.com/yastman/rag/pull/1503) `57e8f280` bot pre-agent deferral
  - [***REMOVED***1505](https://github.com/yastman/rag/pull/1505) `a3792366` dense helper + BGE span capture suppression
  - [***REMOVED***1506](https://github.com/yastman/rag/pull/1506) `ee015d07` `bge_model_processing_ms` propagation
- Existing evidence comment: <https://github.com/yastman/rag/issues/1501***REMOVED***issuecomment-4433184357>

***REMOVED******REMOVED*** Current Audit Decision
...
- If non-production Telegram credentials are available, collect fresh HIT/MISS traces and close ***REMOVED***1501 with evidence.
- If non-production credentials are not available, close or mark ***REMOVED***1501 code-complete only if the maintainer accepts focused test evidence; otherwise create a linked follow-up issue for runtime validation.
- Always create a separate issue for broad `PYTEST_ADDOPTS='-n auto --dist=worksteal' make test-unit` instability. It is not a ***REMOVED***1501 blocker unless a failure reproduces in ***REMOVED***1501 touched tests.

***REMOVED******REMOVED*** File Structure

This closeout plan is mostly issue/runtime work. Code edits are not expected unless the runtime worker finds a confirmed ***REMOVED***1501 regression.

- Create or update GitHub issue comments:
  - `***REMOVED***1501` final code-complete or validation-pending comment
...
***REMOVED******REMOVED*** tmux Swarm Execution
...
***REMOVED******REMOVED******REMOVED*** Swarm Constraints
...
***REMOVED******REMOVED******REMOVED*** Worker Wave 1: ***REMOVED***1501 Closeout Issue Operator
...
**Inputs:**
- ***REMOVED***1501 is open.
- Implementation PRs ***REMOVED***1503/***REMOVED***1505/***REMOVED***1506 are merged.
...
**Expected output:** A GitHub issue comment on ***REMOVED***1501 that links this plan and states the next runtime validation path.

***REMOVED******REMOVED******REMOVED*** Worker Wave 2: Non-Production Runtime HIT/MISS Trace Validation
...
**Purpose:** Produce fresh local Langfuse Telegram traces proving ***REMOVED***1501 runtime behavior.
...
***REMOVED******REMOVED******REMOVED*** Worker Wave 3: Broad Suite Instability Follow-Up
...
**Purpose:** Create a separate GitHub issue for broad `make test-unit` instability so ***REMOVED***1501 stays scoped to BGE/pre-agent changes.

**Expected output:** A linked issue number and comment back on ***REMOVED***1501.

---

***REMOVED******REMOVED******REMOVED*** Task 1: Record The Closeout Plan On ***REMOVED***1501

**Files:**
- Create: `docs/superpowers/plans/2026-05-13-issue-1501-closeout-runtime-validation.md`
- GitHub: comment on `***REMOVED***1501`

- [ ] **Step 1: Verify ***REMOVED***1501 state**
...
```markdown
***REMOVED******REMOVED*** Closeout / validation plan

Code for ***REMOVED***1501 is merged via ***REMOVED***1503, ***REMOVED***1505, and ***REMOVED***1506. The next step is tracked in the local plan:
...
Scope split:
- ***REMOVED***1501 code-complete evidence: focused tests on `origin/dev@ee015d07`.
- Runtime evidence lane: fresh non-production Telegram HIT/MISS Langfuse traces.
- Separate follow-up: broad `PYTEST_ADDOPTS='-n auto --dist=worksteal' make test-unit` instability, because it has not reproduced as a ***REMOVED***1501 touched-surface regression.
```
...
***REMOVED******REMOVED******REMOVED*** Task 2: Create Runtime Validation Follow-Up If Needed

**Files:**
- GitHub: create issue only if ***REMOVED***1501 will be closed without runtime traces, or if runtime credentials are not immediately available.
...
```bash
test -f .env && {
  grep -q '^TELEGRAM_BOT_TOKEN=[REDACTED-TELEGRAM-TOKEN] .env && echo TELEGRAM_BOT_TOKEN_PRESENT || true
...
RUNTIME_ISSUE_URL=$(gh issue create \
  --title "validation: capture fresh Telegram HIT/MISS Langfuse traces for ***REMOVED***1501" \
  --label "domain:observability" \
  --label "domain:bot" \
  --label "performance" \
  --label "testing" \
  --body-file /tmp/issue-1501-runtime-validation-followup.md)
RUNTIME_ISSUE_NUMBER="${RUNTIME_ISSUE_URL***REMOVED******REMOVED****/}"
printf 'Runtime validation issue: ***REMOVED***%s\n' "$RUNTIME_ISSUE_NUMBER"
```
...
Collect fresh non-production Telegram Langfuse traces for ***REMOVED***1501 after ***REMOVED***1503, ***REMOVED***1505, and ***REMOVED***1506 merged into `dev`.

***REMOVED******REMOVED*** Parent

Parent: ***REMOVED***1501

***REMOVED******REMOVED*** Required evidence
...
- [ ] **Step 3: Link the follow-up back to ***REMOVED***1501**

Run:

```bash
test -n "$RUNTIME_ISSUE_NUMBER"
gh issue comment 1501 --body "Runtime HIT/MISS trace validation split into follow-up: ***REMOVED***${RUNTIME_ISSUE_NUMBER}."
```

Expected: ***REMOVED***1501 links to the new validation issue.

***REMOVED******REMOVED******REMOVED*** Task 3: Create Broad Suite Instability Follow-Up
...
XDIST_ISSUE_NUMBER="${XDIST_ISSUE_URL***REMOVED******REMOVED****/}"
printf 'xdist instability issue: ***REMOVED***%s\n' "$XDIST_ISSUE_NUMBER"
```
...
During ***REMOVED***1501 final validation, broad local unit runs with:

```bash
PYTEST_ADDOPTS='-n auto --dist=worksteal' make test-unit
```

did not produce a stable pass. The observed failures were not in ***REMOVED***1501 touched surfaces.
...
Investigate broad local test scheduler/runtime instability separately from ***REMOVED***1501. Do not block ***REMOVED***1501 unless a failure reproduces in the focused ***REMOVED***1501 checks.

***REMOVED******REMOVED*** Suggested validation
...
- [ ] **Step 2: Link it back to ***REMOVED***1501**

Run:

```bash
test -n "$XDIST_ISSUE_NUMBER"
gh issue comment 1501 --body "Broad local unit-suite instability split into follow-up: ***REMOVED***${XDIST_ISSUE_NUMBER}. Not treated as a ***REMOVED***1501 blocker unless it reproduces in ***REMOVED***1501 focused checks."
```

Expected: ***REMOVED***1501 now has a clear separation between code-complete evidence and unrelated broad-suite instability.

***REMOVED******REMOVED******REMOVED*** Task 4: Run Non-Production Runtime HIT/MISS Validation
...
```markdown
***REMOVED******REMOVED*** Runtime validation evidence
...
***REMOVED******REMOVED******REMOVED*** Task 5: Final ***REMOVED***1501 Disposition

**Files:**
- GitHub: close or keep ***REMOVED***1501 open depending on runtime status.

- [ ] **Step 1: If runtime evidence exists, close ***REMOVED***1501**

Run:

```bash
gh issue close 1501 --comment "Implemented in ***REMOVED***1503, ***REMOVED***1505, and ***REMOVED***1506. Focused checks passed on origin/dev@ee015d07, runtime HIT/MISS Langfuse evidence attached above. Broad xdist instability is tracked separately."
```

Expected: ***REMOVED***1501 state is `CLOSED`.

- [ ] **Step 2: If runtime evidence is blocked, leave ***REMOVED***1501 open or mark validation pending**
...
gh issue comment 1501 --body "Code is complete and focused checks pass, but fresh runtime HIT/MISS traces are blocked pending non-production Telegram credentials. Runtime validation is tracked in ***REMOVED***${RUNTIME_ISSUE_NUMBER}."
```

Expected: ***REMOVED***1501 remains open with a concrete blocker.
...
```bash
test -n "$RUNTIME_ISSUE_NUMBER"
gh issue close 1501 --comment "Implemented in ***REMOVED***1503, ***REMOVED***1505, and ***REMOVED***1506. Focused checks passed on origin/dev@ee015d07. Fresh Telegram HIT/MISS runtime traces require non-production credentials and are tracked separately in ***REMOVED***${RUNTIME_ISSUE_NUMBER}."
```

Expected: ***REMOVED***1501 state is `CLOSED`, and runtime validation remains visible in the linked follow-up.

***REMOVED******REMOVED*** Verification Checklist

- [ ] ***REMOVED***1501 has a comment linking this plan.
- [ ] ***REMOVED***1501 has the focused verification evidence summarized.
- [ ] Runtime validation is either attached to ***REMOVED***1501 or split into a linked follow-up.
