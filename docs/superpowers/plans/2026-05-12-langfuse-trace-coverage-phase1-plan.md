***REMOVED*** Langfuse Trace Coverage Phase 1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the `***REMOVED***1485` text-only Langfuse trace gate deterministic and green for scenarios `0.1`, `6.3`, and `7.1`.
...
- Issue: `***REMOVED***1485`
- Parent issue: `***REMOVED***1307`
- Related PR: `***REMOVED***1491`

This plan does not implement:

- `***REMOVED***1486` voice-note fixture for scenario `8.1`;
- `***REMOVED***1487` `validate-traces-fast` / mini-app health fix;
- `***REMOVED***1488` Qdrant data preflight;
- `***REMOVED***1489` product-meaningful scenario checks;
- `***REMOVED***1490` latest-trace audit artifact.

***REMOVED******REMOVED*** Swarm Execution Contract
...
- `scripts/e2e/langfuse_trace_validator.py`
  ...
  - Add cache-check alias resolution if `***REMOVED***1491` is not already in the base.
...
***REMOVED******REMOVED******REMOVED*** Do Not Modify In Phase 1

- `Makefile` target `e2e-test-traces-core`; it still includes voice scenario
  `8.1` and belongs to Phase 2 after `***REMOVED***1486`.
...
***REMOVED******REMOVED*** Reserved Files
...
***REMOVED******REMOVED*** Task 0: Launch Swarm Implementation Worker
...
```markdown
***REMOVED*** Worker: ***REMOVED***1485 Langfuse Trace Contract
...
- If PR ***REMOVED***1491 is not present in this branch, implement equivalent alias support
  for cache-check/node-cache-check.
- If PR ***REMOVED***1491 behavior is already present, do not duplicate it. Keep the alias
...
***REMOVED******REMOVED*** Task 1: Validator Alias And Deterministic Branch Rules
...
`missing_spans`, unless PR `***REMOVED***1491` is already in the branch.
...
***REMOVED******REMOVED*** Task 2: Root Output Recording For Direct Streaming Delivery
...
***REMOVED******REMOVED*** Task 3: SDK-Agent DraftStreamer Output Gap
...
async def test_sdk_agent_draftstreamer_records_langfuse_root_output(self, mock_config):
    ***REMOVED*** Follow the existing test setup in TestResponseSentFlag.
    ***REMOVED*** Patch DraftStreamer.finalize to succeed.
    ***REMOVED*** Patch telegram_bot.bot.record_langfuse_response_output.
    ***REMOVED*** Exercise _handle_query_supervisor with response_text and private chat.
    ***REMOVED*** Assert record_langfuse_response_output called with response_text and chunk count.
```
...
***REMOVED******REMOVED*** Task 4: Focused Verification
...
Phase 1 pass/fail gate while `***REMOVED***1486` is open.

***REMOVED******REMOVED*** Task 5: Worker PR And Signal
...
gh pr create \
  ...
  --title "fix(observability): harden ***REMOVED***1307 text trace contract" \
  --body "$(cat <<'EOF'
...
***REMOVED******REMOVED*** Scope
Phase 1 for ***REMOVED***1485 only. Does not close ***REMOVED***1307 because ***REMOVED***1486/***REMOVED***1487/***REMOVED***1488/***REMOVED***1489/***REMOVED***1490 remain separate follow-ups.
...
***REMOVED******REMOVED*** Task 6: Orchestrator Review And Dedicated PR Review Worker
...
PR_NUMBER=1234  ***REMOVED*** replace with the actual PR number before running
...
- PR `***REMOVED***1491` behavior is present either by merge or equivalent implementation;
...
***REMOVED******REMOVED*** Phase 1 Runtime Gate
...
because it includes voice scenario `8.1`, which belongs to `***REMOVED***1486`.
