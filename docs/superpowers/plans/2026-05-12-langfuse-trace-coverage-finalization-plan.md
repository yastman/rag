***REMOVED*** Langfuse Trace Coverage Finalization Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Finish the remaining local Langfuse trace coverage work after PR `***REMOVED***1492` by making required app-owned traces structurally complete, then closing the open follow-up issues `***REMOVED***1486`, `***REMOVED***1487`, `***REMOVED***1488`, `***REMOVED***1489`, and `***REMOVED***1490`.
...
PR `***REMOVED***1492` is merged and fixed the root output writer for recent text traces.
...
- `***REMOVED***1307` is currently closed in GitHub, but the remaining acceptance work is
  represented by open issues `***REMOVED***1486` through `***REMOVED***1490`.
...
1. `cache-check` SDK observation repair for the post-`***REMOVED***1492` blocker.
2. `***REMOVED***1486`: voice note fixture and voice trace gate readiness.
3. `***REMOVED***1487`: mini-app frontend health so `validate-traces-fast` can run.
4. `***REMOVED***1488`: Qdrant data/schema preflight before Telethon trace E2E.
5. `***REMOVED***1489`: deterministic product-meaningful assertions in no-judge E2E.
6. `***REMOVED***1490`: sanitized latest-trace audit artifact after E2E.
...
***REMOVED******REMOVED*** Swarm Classification
...
***REMOVED******REMOVED******REMOVED*** Modify
...
***REMOVED******REMOVED******REMOVED*** Create
...
***REMOVED******REMOVED*** Task 0: Orchestration Setup For Each Wave
...
tmux display-message -p -t "$ORCH_PANE" '***REMOVED***{pane_id} ***REMOVED***{pane_dead}'
tmux display-message -p '***REMOVED***{window_id} ***REMOVED***{window_index}'
```
...
***REMOVED******REMOVED*** Task 1: Repair Pre-Agent Cache Decision Observation
...
***REMOVED******REMOVED*** Task 2: Voice Note Fixture For Scenario 8.1 (***REMOVED***1486)
...
***REMOVED******REMOVED*** Task 3: Mini-App Frontend Health For validate-traces-fast (***REMOVED***1487)
...
***REMOVED******REMOVED*** Task 4: Qdrant Data Preflight For E2E Trace Gate (***REMOVED***1488)
...
***REMOVED******REMOVED*** Task 5: Deterministic Product Assertions In No-Judge Mode (***REMOVED***1489)
...
***REMOVED******REMOVED*** Task 6: Latest Trace Audit Artifact (***REMOVED***1490)
...
e2e-audit-langfuse-latest: ***REMOVED******REMOVED*** Write sanitized latest Langfuse trace audit artifact
...
***REMOVED******REMOVED*** Task 7: Final Runtime Verification Wave
...
Only after `***REMOVED***1486` has a valid voice fixture:
...
Only after `***REMOVED***1487` is fixed:
...
```bash
gh issue comment <issue> --body "Fixed in PR ***REMOVED***<pr>. Verification: <commands or blocker>."
...
***REMOVED******REMOVED*** Required PR Review Gate
...
***REMOVED******REMOVED*** Final Acceptance Criteria
...
- `***REMOVED***1486` is fixed or explicitly blocked on a user-approved non-secret voice
  fixture;
- `***REMOVED***1487` no longer blocks `make validate-traces-fast` before trace validation;
- `***REMOVED***1488` fails early with actionable sanitized Qdrant data/schema preflight
  errors;
- `***REMOVED***1489` no-judge mode fails non-empty but unmeaningful responses;
- `***REMOVED***1490` produces sanitized latest-trace audit artifacts;
