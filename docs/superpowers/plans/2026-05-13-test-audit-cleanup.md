***REMOVED*** Test Audit Cleanup Implementation Plan
...
***REMOVED******REMOVED*** File Structure Map

```
tests/
├── conftest.py                    ***REMOVED*** MODIFY: add contract/baseline markers, split concerns
├── fixtures/                      ***REMOVED*** NEW: extracted fixtures
│   ├── __init__.py
│   ├── config.py                  ***REMOVED*** NEW: qdrant_url, redis_url, bge_m3_url, etc.
│   └── data.py                    ***REMOVED*** NEW: sample_context_chunks, sample_texts, etc.
├── unit/
│   ├── conftest.py                ***REMOVED*** MODIFY: receive ML mocks from root, add get_client fixture
│   ├── test_compat.py             ***REMOVED*** NEW: tests for src/_compat.py
│   ├── test_settings.py           ***REMOVED*** MODIFY: remove duplicated BotConfig tests (lines 239-275)
│   ├── test_bot_handlers.py       ***REMOVED*** MODIFY: call_count==0 → assert_not_called (line 720)
│   ├── test_bot_scores.py         ***REMOVED*** MODIFY: call_count==0 → assert_not_called (lines 903,923)
│   ├── core/
│   │   └── test_pipeline.py       ***REMOVED*** RECEIVE: e2e test_rag_pipeline.py content merged here
│   ├── evaluation/
│   │   └── test_config_snapshot.py ***REMOVED*** NEW: tests for src/evaluation/config_snapshot.py
│   ├── contextualization/
│   │   └── test_providers.py      ***REMOVED*** NEW: parametrized Claude/OpenAI/Groq (replaces 3 files)
│   ├── api/
│   │   └── test_schemas.py        ***REMOVED*** NEW: dedicated schemas tests
│   └── scripts/
│       └── test_kommo_seed.py     ***REMOVED*** MODIFY: random.seed() → random.Random()
├── e2e/
│   ├── conftest.py                ***REMOVED*** MODIFY: remove redundant pytest_collection_modifyitems
│   ├── test_rag_pipeline.py       ***REMOVED*** DELETE: moved to tests/unit/core/
│   └── test_core_flows_live.py    ***REMOVED*** DELETE: moved to tests/unit/core/
├── smoke/
│   ├── conftest.py                ***REMOVED*** MODIFY: fix asyncio.get_event_loop, reuse root fixtures
│   └── test_basic_connection.py   ***REMOVED*** NEW: moved from tests/integration/
├── integration/
│   ├── conftest.py                ***REMOVED*** MODIFY: remove redundant pytest_collection_modifyitems
│   ├── test_basic_connection.py   ***REMOVED*** DELETE: moved to tests/smoke/
│   ├── test_qdrant_connection.py  ***REMOVED*** DELETE: merged into test_basic_connection
│   └── test_qdrant_history.py     ***REMOVED*** MODIFY: uuid4() → fixed collection name
├── chaos/
│   ├── conftest.py                ***REMOVED*** MODIFY: remove redundant collection_modifyitems + chunk fixture
├── benchmark/
│   └── conftest.py                ***REMOVED*** MODIFY: remove redundant collection_modifyitems
├── load/
│   └── conftest.py                ***REMOVED*** MODIFY: remove redundant collection_modifyitems
├── contract/
│   └── conftest.py                ***REMOVED*** (already minimal, OK)
├── baseline/
│   └── conftest.py                ***REMOVED*** (already minimal, OK)
└── eval/
    └── __init__.py                ***REMOVED*** DELETE: empty directory

pyproject.toml                     ***REMOVED*** MODIFY: add contract/baseline markers, add .venv omit
src/_compat.py                     ***REMOVED*** UNCHANGED (we test it, don't change it)
src/evaluation/config_snapshot.py  ***REMOVED*** UNCHANGED (we test it, don't change it)
```

---

***REMOVED******REMOVED*** Phase 1: Critical Bugs

***REMOVED******REMOVED******REMOVED*** Task 1: Fix `asyncio.get_event_loop()` deprecation in smoke conftest
...
```python
    ...
    try:
        ...
        ***REMOVED*** Already in running event loop (nested asyncio.run scenario)
...
```bash
cd /repo && .venv/bin/python -m pytest tests/smoke/test_preflight.py -v --co 2>&1 | head -20
```
...
***REMOVED******REMOVED******REMOVED*** Task 2: Move mock-only E2E tests to unit/
...
```bash
cd /repo
***REMOVED*** Read the e2e file to understand what's being moved
...
```bash
***REMOVED*** First check what's in the existing unit test
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/core/test_pipeline.py -v --co 2>&1 | tail -30
```
...
***REMOVED******REMOVED******REMOVED*** Task 3: Add tests for `src/_compat.py`
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/test_compat.py -v
```
Expected: 5 passed
...
***REMOVED******REMOVED******REMOVED*** Task 4: Add tests for `src/evaluation/config_snapshot.py`
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/evaluation/test_config_snapshot.py -v
```
Expected: 10 passed
...
***REMOVED******REMOVED******REMOVED*** Task 5: Add `contract` and `baseline` directories to auto-marker hook
...
```bash
cd /repo && .venv/bin/python -m pytest tests/contract/ tests/baseline/ --collect-only -q 2>&1 | tail -5
```
...
```bash
cd /repo && .venv/bin/python -m pytest tests/ -m contract --collect-only -q 2>&1 | tail -5
```
...
***REMOVED******REMOVED******REMOVED*** Task 6: Exclude `telegram_bot/.venv/` from coverage
...
```bash
ls -la /repo/telegram_bot/.venv/ 2>/dev/null | head -5 || echo "NO_VENV"
```
...
```bash
cd /repo && .venv/bin/python -m coverage debug config 2>&1 | grep -A5 "omit"
```
...
***REMOVED******REMOVED*** Phase 2: Eliminate Duplications

***REMOVED******REMOVED******REMOVED*** Task 7: Remove redundant `pytest_collection_modifyitems` from 6 sub-conftest files
...
```bash
cd /repo && .venv/bin/python -m pytest tests/smoke/ tests/integration/ tests/chaos/ tests/load/ tests/benchmark/ tests/e2e/ --collect-only -q 2>&1 | tail -10
```
...
***REMOVED******REMOVED******REMOVED*** Task 8: Unify Redis URL fixtures — single source of truth with password support
...
```bash
cd /repo && .venv/bin/python -m pytest tests/smoke/ --collect-only -q 2>&1 | tail -5
```
...
***REMOVED******REMOVED******REMOVED*** Task 9: Extract `patch("get_client")` into a shared unit fixture
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/test_bot_handlers.py -v --timeout=60 -x 2>&1 | tail -30
```
...
***REMOVED******REMOVED******REMOVED*** Task 10: Remove duplicate BotConfig tests from test_settings.py
...
In `tests/unit/test_settings.py`, delete lines 236-275 (the comment `***REMOVED*** === BotConfig ...` and the entire `TestBotConfigIsPydanticSettings` class).
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/config/test_bot_config_settings.py -v 2>&1 | tail -15
```
...
***REMOVED******REMOVED******REMOVED*** Task 11: Unify `sample_context_chunks` — keep only root version, delete chaos copy
...
***REMOVED******REMOVED*** Phase 3: Architecture

***REMOVED******REMOVED******REMOVED*** Task 12: Move global ML mocks from root conftest to unit conftest
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/test_chunker.py --collect-only -q 2>&1 | tail -5
```
...
***REMOVED******REMOVED******REMOVED*** Task 13: Split root conftest into modular fixture files
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/test_chunker.py tests/smoke/test_preflight.py --collect-only -q 2>&1 | tail -5
```
...
***REMOVED******REMOVED******REMOVED*** Task 14: Move `test_basic_connection.py` from integration to smoke
...
```bash
cd /repo && .venv/bin/python -m pytest tests/smoke/test_basic_connection.py --collect-only -q 2>&1 | tail -5
```
...
***REMOVED******REMOVED******REMOVED*** Task 15: Additional architecture fixes
...
```bash
cd /repo && git rm tests/eval/__init__.py && rm -rf tests/eval/
```
...
***REMOVED******REMOVED*** Phase 4: Code Smells (Low Priority)

***REMOVED******REMOVED******REMOVED*** Task 16: Fix `call_count == 0` → `assert_not_called()` (3 instances)
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/test_bot_handlers.py::TestHandleQuery::test_handle_query_manager_skips_retry_on_checkpointer_error tests/unit/test_bot_scores.py -v --timeout=30 2>&1 | tail -15
```
Expected: all tests pass.
...
***REMOVED******REMOVED******REMOVED*** Task 17: Fix `random.seed()` to use `random.Random()` in kommo_seed tests
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/scripts/test_kommo_seed.py -v 2>&1 | tail -15
```
...
***REMOVED******REMOVED******REMOVED*** Task 18: Parametrize contextualization tests (Claude/OpenAI/Groq)
...
***REMOVED*** Provider metadata: (class, context_method_value, module_to_patch)
PROVIDERS = [
...
class TestContextualizeBasics:
    ...
    def test_contextualize_handles_api_error_gracefully(self, provider_name, cls, method_name, patch_target, sample_chunks):
        with patch(f"{patch_target}") as mock_client:
            ...
            result = instance.contextualize(sample_chunks)
            assert result is not None  ***REMOVED*** should not raise
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/contextualization/test_providers.py -v 2>&1 | tail -30
```
...
***REMOVED******REMOVED******REMOVED*** Task 19: Add dedicated tests for `src/api/schemas.py`
...
```bash
cd /repo && .venv/bin/python -m pytest tests/unit/api/test_schemas.py -v
```
...
***REMOVED******REMOVED******REMOVED*** Task 20: Fix `uuid4()` in test_qdrant_history.py (flaky collection name)
...
***REMOVED******REMOVED******REMOVED*** Task 21: Address `src/governance/` empty directory
...
***REMOVED******REMOVED*** Verification (After All Tasks)
