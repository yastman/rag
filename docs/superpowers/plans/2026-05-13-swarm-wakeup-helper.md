***REMOVED*** Swarm Wake-Up Helper Implementation Plan
...
***REMOVED******REMOVED******REMOVED*** New Helper
...
***REMOVED******REMOVED******REMOVED*** Contract Docs And Worker Skills
        ...
        textwrap.dedent(
            ...
            ***REMOVED***!/usr/bin/env bash
            ...
            if [ "$1" = "display-message" ]; then
              case "${{@: -1}}" in
                "***REMOVED***{{window_id}}") echo "@16" ;;
                "***REMOVED***{{window_name}}") echo "orch-test-20260513T000000-abcd" ;;
                "***REMOVED***{{session_name}}") echo "claude" ;;
...
def test_invalid_status_fails_before_tmux_send(tmp_path: Path) -> None:
    ...
    sent = tmp_path / "sent.jsonl"
    _write_fake_tmux(tmp_path, "***REMOVED***!/usr/bin/env bash\nprintf '%s\\n' \"$*\" >> '" + str(sent) + "'\n")
    ...
    _write_fake_tmux(
        ...
        textwrap.dedent(
            ...
            ***REMOVED***!/usr/bin/env bash
            ...
            if [ "$1" = "display-message" ]; then
              case "${{@: -1}}" in
                "***REMOVED***{{window_id}}") echo "@99" ;;
                "***REMOVED***{{window_name}}") echo "wrong-window" ;;
                "***REMOVED***{{session_name}}") echo "claude" ;;
              esac
              exit 0
            fi
...
***REMOVED******REMOVED*** Task 2: Implement The Notification Helper
...
def validate_route(args: argparse.Namespace) -> dict[str, str]:
    actual = {
        "window_id": tmux_display(args.target, "***REMOVED***{window_id}"),
        "window_name": tmux_display(args.target, "***REMOVED***{window_name}"),
        "session_name": tmux_display(args.target, "***REMOVED***{session_name}"),
    }
    if (
    ...
    if args.pane:
        try:
            pane_window_id = tmux_display(args.pane, "***REMOVED***{window_id}")
        except RuntimeError:
            pane_window_id = ""
...
***REMOVED******REMOVED*** Task 3: Update Contract Tests For Helper-Based Wake-Up
...
***REMOVED******REMOVED*** Task 4: Update Signal And Infrastructure Contracts
...
***REMOVED******REMOVED*** Task 5: Update Worker Contracts And OpenCode Skill
...
***REMOVED******REMOVED*** Task 6: Update Prompt Validator To Block New Raw Wake-Up Snippets
...
***REMOVED******REMOVED*** Task 7: Run Full Skill Test Suite
...
***REMOVED******REMOVED*** Task 8: Live Smoke Test The Helper
...
***REMOVED******REMOVED*** Task 9: Record Maintenance Evidence
...
```markdown
***REMOVED******REMOVED*** 2026-05-13TXX:XX:XX+00:00 — Canonical wake-up helper
...
tmux list-windows -a -F '***REMOVED***{window_name}' | rg '^W-wakeup-helper-smoke' || true
...
***REMOVED******REMOVED*** Task 10: Handoff Summary
