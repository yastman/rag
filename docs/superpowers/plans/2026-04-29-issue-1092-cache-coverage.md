***REMOVED*** Cache Layers Coverage Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Close issue ***REMOVED***1092 by adding deterministic unit coverage for cache policy decisions, Redis cache TTL/error behavior, and search cache key collision guards.
...
***REMOVED******REMOVED*** Smart Routing

- Routing: Full plan, because issue ***REMOVED***1092 is `lane:plan-needed` and spans cache policy plus Redis/RedisVL cache layer behavior.
...
***REMOVED******REMOVED*** Task 1: Cache Policy Coverage
...
***REMOVED******REMOVED*** Task 2: Cache Layer TTL, Redis, and Key Coverage
...
***REMOVED******REMOVED*** Task 3: Final Integration
...
gh pr create --base dev --head fix/1092-cache-coverage --title "test(cache): cover cache policy and layer guards" --body "Closes ***REMOVED***1092"
```
...
- `make check` passes.
- PR targets `dev` and closes ***REMOVED***1092.
