***REMOVED*** BGE-M3 Query Vector Bundle Implementation Plan
...
**Goal:** Fix issue ***REMOVED***1493 by caching BGE-M3 dense, sparse, and ColBERT query vectors as one Redis bundle so repeated retrieval misses do not make a second BGE request for ColBERT.
...
***REMOVED******REMOVED*** Current Issue Context

Tracked issue: https://github.com/yastman/rag/issues/1493

Related open issues reviewed:

- `***REMOVED***1472` verifies the Qdrant `v1.17.1` to `v1.18.0` and Redis `8.6.2` to `8.6.3` upgrade. This plan should not change collection schema, but final verification should include Qdrant retrieval tests because the bundle feeds the Qdrant hybrid/ColBERT path.
- `***REMOVED***1488` and `***REMOVED***1489` cover product-meaningful Telethon/Langfuse trace gates. After this change, those gates should be able to assert fewer BGE spans on repeat requests.
- `***REMOVED***1485` covers missing root output/scores in core traces. This plan includes a narrow scoring fix for cache-hit `results_count`/`no_results`, but should not take over the full trace-contract issue.
...
***REMOVED******REMOVED******REMOVED*** Wave Graph
...
***REMOVED******REMOVED******REMOVED*** Reserved Files By Worker
...
***REMOVED******REMOVED******REMOVED*** Worker Prompt Payloads

Every worker prompt must start with:

```text
***REMOVED******REMOVED*** STEP 0 POLICY GATE
...
```text
Issue: ***REMOVED***1493 Cache BGE-M3 dense/sparse/ColBERT query vectors as one Redis bundle
Base branch: dev
Integration branch: issue/1493-bge-m3-query-vector-bundle
...
- `w4-integration`: merge Waves 1-3, resolve conflicts without redesigning behavior, run the combined focused checks listed in Task 7 Step 3 and Step 4, then open/update one PR linked to `***REMOVED***1493`.
...
***REMOVED******REMOVED******REMOVED*** Launch Checklist

- [ ] Orchestrator confirms live pane:

```bash
ORCH_PANE=$(tmux display-message -p '***REMOVED***{pane_id}')
tmux display-message -p -t "$ORCH_PANE" '***REMOVED***{pane_dead}'
```
...
***REMOVED******REMOVED******REMOVED*** Task 1: Bundle Contract
...
***REMOVED******REMOVED******REMOVED*** Task 2: Redis Bundle Cache Tier
...
***REMOVED******REMOVED******REMOVED*** Task 3: Shared Embedding Core Uses Bundle First
...
***REMOVED******REMOVED******REMOVED*** Task 4: Remove Extra ColBERT Encode In Cache Check Paths
...
***REMOVED******REMOVED******REMOVED*** Task 5: Rewrite/Re-Embed Branches Use Bundle
...
***REMOVED******REMOVED******REMOVED*** Task 6: Langfuse Cache-Hit Scoring Regression
...
***REMOVED******REMOVED******REMOVED*** Task 7: Docs And Verification
...
```markdown
***REMOVED******REMOVED******REMOVED*** BGE-M3 Query Vector Bundle Cache
...
***REMOVED******REMOVED*** Acceptance Mapping
