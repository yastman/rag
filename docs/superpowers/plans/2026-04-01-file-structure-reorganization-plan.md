***REMOVED*** File Structure Reorganization Implementation Plan
...
***REMOVED******REMOVED*** Phase 1: Root Cleanup

***REMOVED******REMOVED******REMOVED*** Task 1: Delete tmp* directories and files at root
...
Run:
```bash
ls -la /repo/ | grep -E "tmp\.|^d.*tmp\.|^d.*uv-.*\.lock|^-.*tmp[0-9]"
```
...
***REMOVED******REMOVED******REMOVED*** Task 2: Move e2e_tester.session and .test_durations
...
mkdir -p /repo/data/test_artifacts/
```
...
Run:
```bash
mv /repo/e2e_tester.session /repo/tests/e2e/
```
...
Run:
```bash
mv /repo/.test_durations /repo/data/test_artifacts/
```
...
***REMOVED******REMOVED******REMOVED*** Task 3: Delete GitHub workflows disabled duplicates
...
Run:
```bash
ls /repo/.github/workflows.disabled/
```
...
Run:
```bash
rm /repo/.github/workflows.disabled/ci.yml \
   /repo/.github/workflows.disabled/ci.yml.disabled \
   /repo/.github/workflows.disabled/nightly-heavy.yml
```
...
***REMOVED******REMOVED*** Phase 2: Documentation Relocation

***REMOVED******REMOVED******REMOVED*** Task 4: Move AGENTS.override.md to .claude/rules/features/
...
Run:
```bash
mkdir -p /repo/.claude/rules/features/
```
...
Run:
```bash
mv /repo/src/ingestion/unified/AGENTS.override.md \
   /repo/.claude/rules/features/ingestion-unified.md
```
...
***REMOVED******REMOVED******REMOVED*** Task 5: Archive docs/plans/ and docs/reports/
...
Run:
```bash
mkdir -p /repo/docs/archive/plans/plans-2026-03/
mkdir -p /repo/docs/archive/reports/reports-2026-02/
mkdir -p /repo/docs/archive/reports/reports-2026-03/
```
...
Run:
```bash
mv /repo/docs/plans/* /repo/docs/archive/plans/plans-2026-03/
```
...
Run:
```bash
***REMOVED*** Move Feb 2026 files
mv /repo/docs/reports/2026-02* /repo/docs/archive/reports/reports-2026-02/
***REMOVED*** Move Mar 2026 files
mv /repo/docs/reports/2026-03* /repo/docs/archive/reports/reports-2026-03/
```
...
***REMOVED******REMOVED******REMOVED*** Task 6: Delete docs/documents/ .docx
...
Run:
```bash
grep -r "Кримінальний\|Кримінальний кодекс" /repo/docs/ --include="*.md"
```
...
Run:
```bash
rm "/repo/docs/documents/Кримінальний кодекс України - Кодекс України № 2341-III від 05.04.2001 - d82054-20250717.docx"
rmdir /repo/docs/documents/
```
...
***REMOVED******REMOVED*** Phase 3: Source Tree Cleanup

***REMOVED******REMOVED******REMOVED*** Task 7: Move Dockerfiles to docker/ and update compose.yml
...
Run:
```bash
mkdir -p /repo/docker/api/
mkdir -p /repo/docker/voice/
mkdir -p /repo/docker/ingestion/
```
...
Run:
```bash
mv /repo/src/api/Dockerfile /repo/docker/api/
mv /repo/src/voice/Dockerfile /repo/docker/voice/
mv /repo/Dockerfile.ingestion /repo/docker/ingestion/
```
...
***REMOVED******REMOVED******REMOVED*** Task 8: Rename duplicate search_engines.py files
...
Run:
```bash
mv /repo/src/evaluation/search_engines.py \
   /repo/src/evaluation/evaluation_search_engines.py
```
...
Run:
```bash
mv /repo/src/retrieval/search_engines.py \
   /repo/src/retrieval/retrieval_search_engines.py
```
...
Run:
```bash
cd /repo && uv run python -c "from src.evaluation.evaluation_search_engines import create_search_engine, BaselineSearchEngine; from src.retrieval.retrieval_search_engines import SearchEngine; print('OK')"
```
Expected: `OK`
...
***REMOVED******REMOVED******REMOVED*** Task 9: Archive legacy ingestion files
...
Run:
```bash
mkdir -p /repo/src/ingestion/legacy/
```

Write `src/ingestion/legacy/README.md`:
```markdown
***REMOVED*** Legacy Ingestion Files
...
Run:
```bash
mv /repo/src/ingestion/gdrive_flow.py \
   /repo/src/ingestion/legacy/
mv /repo/src/ingestion/gdrive_indexer.py \
   /repo/src/ingestion/legacy/
mv /repo/src/ingestion/service.py \
   /repo/src/ingestion/legacy/
```
...
***REMOVED******REMOVED******REMOVED*** Task 10: Delete empty src/governance/
...
***REMOVED******REMOVED*** Phase 4: Scripts and Final Polish

***REMOVED******REMOVED******REMOVED*** Task 11: Archive old scripts
...
***REMOVED******REMOVED******REMOVED*** Task 12: Delete empty settings.local.json
...
***REMOVED******REMOVED******REMOVED*** Task 13: Update .gitignore
...
```gitignore
***REMOVED*** Prevent temp files at root level
tmp*/
.lock
*.tmp
```
...
***REMOVED******REMOVED******REMOVED*** Task 14: Update README.md
...
***REMOVED******REMOVED******REMOVED*** Task 15: Add pre-commit hook for temp files
...
Run:
```bash
cat /repo/.pre-commit-config.yaml
```
...
```bash
***REMOVED*** Verify root has no tmp* or uv-*.lock
ls /repo/ | grep -E "^tmp|^uv-" && echo "FAIL: temp files remain" || echo "PASS: root clean"

***REMOVED*** Verify Dockerfiles moved
ls /repo/docker/api/Dockerfile
ls /repo/docker/voice/Dockerfile
ls /repo/docker/ingestion/Dockerfile.ingestion

***REMOVED*** Verify compose still works
docker compose config --quiet && echo "PASS: compose valid"

***REMOVED*** Verify imports still work
