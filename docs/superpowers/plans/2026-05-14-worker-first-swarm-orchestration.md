***REMOVED*** Worker-First Swarm Orchestration Implementation Plan
...
***REMOVED******REMOVED*** Source Documents
...
- Repo OpenCode agents: `/repo/.opencode/agents`
- Global OpenCode agents: `/home/USER/.config/opencode/agents`

***REMOVED******REMOVED*** Final Routing Decision
...
- `/repo/.opencode/agents/secretary-flash.md`
  - Read-mostly low-cost secretary agent using `opencode-go/deepseek-v4-flash`.
- `/repo/.opencode/agents/secretary-pro.md`
...
***REMOVED******REMOVED*** Task 1: Add Secretary Agent Files

**Files:**
- Create: `/repo/.opencode/agents/secretary-flash.md`
- Create: `/repo/.opencode/agents/secretary-pro.md`
...
def test_repo_secretary_agents_exist_with_expected_models() -> None:
    flash = Path("/repo/.opencode/agents/secretary-flash.md").read_text(encoding="utf-8")
    pro = Path("/repo/.opencode/agents/secretary-pro.md").read_text(encoding="utf-8")
...
```bash
git add /repo/.opencode/agents/secretary-flash.md /repo/.opencode/agents/secretary-pro.md
git commit -m "feat(swarm): add secretary opencode agents"
```

***REMOVED******REMOVED*** Task 2: Add Launcher Model Integrity Guard
...
model_match = re.search(r"(?m)^\s*model:\s*([^\s***REMOVED***]+)\s*$", frontmatter)
...
***REMOVED******REMOVED*** Task 3: Add Secretary Signal Validation
...
***REMOVED******REMOVED*** Task 4: Add Secretary Prompt Validation
...
***REMOVED******REMOVED*** SECRETARY PROMPT PAYLOAD
Target: issue ***REMOVED***123.
...
***REMOVED******REMOVED*** ORCHESTRATOR ROUTING
...
def test_secretary_prompt_requires_secretary_payload(tmp_path: Path) -> None:
    prompt = _valid_secretary_prompt().replace("***REMOVED******REMOVED*** SECRETARY PROMPT PAYLOAD", "***REMOVED******REMOVED*** OTHER")
    result = _run(_write_prompt(tmp_path, prompt), contract="quick")
    assert result.returncode == 1
    assert "secretary prompt missing ***REMOVED******REMOVED*** SECRETARY PROMPT PAYLOAD" in result.stderr
```
...
if is_secretary_prompt(text):
    if "***REMOVED******REMOVED*** SECRETARY PROMPT PAYLOAD" not in text:
        errors.append("secretary prompt missing ***REMOVED******REMOVED*** SECRETARY PROMPT PAYLOAD")
...
In `references/prompt-snippets.md`, add `***REMOVED******REMOVED*** Secretary Prompt` with a compact template using `secretary-flash`, `SECRETARY.md`, `secretary.json`, `NEXT_WORKER_PROMPT.md`, and `swarm_notify_orchestrator.py`.

- [ ] **Step 5: Add worker contract docs**

In `worker-contract.md`, add `***REMOVED******REMOVED*** Secretary Worker Fast Path`:
...
***REMOVED******REMOVED*** Task 5: Rewrite Routing Docs To Worker-First
...
def test_worker_first_secretary_routes_are_documented() -> None:
    ...
    assert "***REMOVED******REMOVED*** Worker-First Orchestrator Rule" in skill
...
Replace `***REMOVED******REMOVED*** Thin Orchestrator Rule` with `***REMOVED******REMOVED*** Worker-First Orchestrator Rule`.
...
Add `***REMOVED******REMOVED******REMOVED*** Secretary Flash` and `***REMOVED******REMOVED******REMOVED*** Secretary Pro` sections with allowed actions, forbidden actions, and artifact outputs. The Secretary Pro section must say it is not the default route in Phase 1.
...
***REMOVED******REMOVED*** Task 6: Delegate Review, SDK, And Knowledge Gates Without Swapping Core Routes
...
```markdown
***REMOVED******REMOVED*** Semantic Review
...
***REMOVED******REMOVED*** Task 7: End-To-End Secretary Smoke Prompts

**Files:**
- Create: `/repo/.codex/prompts/worker-secretary-flash-smoke.md`
- Create: `/repo/.codex/prompts/worker-secretary-pro-smoke.md`
- Modify: `/home/USER/.codex/skills/tmux-swarm-orchestration/references/prompt-snippets.md`
- Test: live tmux smoke, not committed test output

- [ ] **Step 1: Create secretary-flash smoke prompt**

Create `/repo/.codex/prompts/worker-secretary-flash-smoke.md` using the secretary prompt snippet. It should:
...
```bash
python3 /home/USER/.codex/skills/tmux-swarm-orchestration/scripts/validate_worker_prompt.py --contract quick /repo/.codex/prompts/worker-secretary-flash-smoke.md
```
...
Create `/repo/.codex/prompts/worker-secretary-pro-smoke.md`. It should:
...
```bash
python3 /home/USER/.codex/skills/tmux-swarm-orchestration/scripts/validate_worker_prompt.py --contract quick /repo/.codex/prompts/worker-secretary-pro-smoke.md
```
...
```bash
PROJECT_ROOT=/repo
WT_PATH=/repo-wt-secretary-flash-smoke
git worktree add "$WT_PATH" -b secretary/flash-smoke dev
mkdir -p "$WT_PATH/logs" "$WT_PATH/.signals" "$WT_PATH/.codex/prompts"
cp /repo/.codex/prompts/worker-secretary-flash-smoke.md "$WT_PATH/.codex/prompts/"
...
```bash
git add /repo/.codex/prompts/worker-secretary-flash-smoke.md /repo/.codex/prompts/worker-secretary-pro-smoke.md /home/USER/.codex/skills/tmux-swarm-orchestration/references/prompt-snippets.md
git commit -m "test(swarm): add secretary smoke prompts"
```

If not reusable, delete the smoke prompt files and do not commit them.

***REMOVED******REMOVED*** Task 8: Full Verification And Cleanup
...
git -C /repo diff -- .opencode/agents .codex/prompts
