***REMOVED*** Issue ***REMOVED***1074 Compose VPS Runtime Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Fix the current runtime-contract gaps behind `***REMOVED***1074` by correcting the base compose/env configuration that VPS and local runtime both rely on, while avoiding stale fixes for issue points that are no longer true.
...
***REMOVED******REMOVED******REMOVED*** Task 1: Lock the missing env/example contract
...
***REMOVED******REMOVED******REMOVED*** Task 2: Lock the compose runtime contract
...
***REMOVED******REMOVED******REMOVED*** Task 3: Verify VPS runtime shape and finish tracking
...
Run:
- `export POSTGRES_PASSWORD=x REDIS_PASSWORD=x TELEGRAM_BOT_TOKEN=[REDACTED-TELEGRAM-TOKEN] LITELLM_MASTER_KEY=x OPENAI_API_KEY=[REDACTED-OPENAI-KEY] NEXTAUTH_SECRET=x SALT=x ENCRYPTION_KEY=x LIVEKIT_API_KEY=x LIVEKIT_API_SECRET=x GDRIVE_SYNC_DIR=/tmp; COMPOSE_FILE=compose.yml:compose.vps.yml docker compose --compatibility config --services`
...
- move `***REMOVED***1074` out of `Plan needed`
