***REMOVED*** OpenCode Swarm Workers Implementation Plan
...
**Tech Stack:** Bash, tmux, OpenCode CLI `1.3.17`, Markdown OpenCode agents and skills, Python/pytest contract tests, existing Codex skill files under `/home/USER/.codex/skills/tmux-swarm-orchestration`, project OpenCode files under `/repo/.opencode`.

---

***REMOVED******REMOVED*** Scope Check
...
- `/repo/.opencode` and this plan file are in the `rag-fresh` repository. Commit only those repo-owned files when changed.
...
***REMOVED******REMOVED******REMOVED*** Operator Skill Files
...
***REMOVED******REMOVED******REMOVED*** Project OpenCode Files

- Create: `/repo/.opencode/agents/pr-review.md`
  - Project-local read-only PR review agent so swarm does not depend on global user config.
- Modify: `/repo/.opencode/agents/pr-worker.md`
...
- Modify: `/repo/.opencode/agents/pr-review-fix.md`
...
- Keep: `/repo/.opencode/skills/gh-pr-review/SKILL.md`
...
***REMOVED******REMOVED*** Task 1: Add Failing Launcher Contract Tests
...
***REMOVED******REMOVED*** Task 2: Add Failing Skill Documentation Contract Tests
...
***REMOVED******REMOVED*** Task 3: Implement the OpenCode tmux Launcher
...
if [[ -n "$opencode_required_skills" ]]; then
  ...
  for required_skill in "${required_skills[@]}"; do
    required_skill="${required_skill***REMOVED***"${required_skill%%[![:space:]]*}"}"
    required_skill="${required_skill%"${required_skill***REMOVED******REMOVED****[![:space:]]}"}"
...
***REMOVED******REMOVED*** Task 4: Harden Project OpenCode Agents For Required Skills
...
***REMOVED******REMOVED*** Task 5: Update Core Skill Documentation To OpenCode Workers
...
***REMOVED******REMOVED*** Task 6: Update Worker Contract And Prompt References
...
```text
***REMOVED******REMOVED*** REQUIRED OPENCODE SKILLS
...
***REMOVED******REMOVED*** Task 7: Update Worker Type And Review Verification References
...
***REMOVED******REMOVED*** Task 8: Update Tests To Lock The New Contract
...
***REMOVED******REMOVED*** Task 9: Smoke Test A Minimal OpenCode Worker

**Files:**
- Create temporary prompt: `/repo/.codex/prompts/worker-opencode-smoke.md`
- Create temporary worktree: `/repo-wt-opencode-smoke`
- Signal artifact: `/repo-wt-opencode-smoke/.signals/worker-opencode-smoke.json`
...
```bash
tmux display-message -p '***REMOVED***{pane_id} ***REMOVED***{pane_dead}'
```
...
```bash
cd /repo
git worktree add /repo-wt-opencode-smoke -b test/opencode-smoke-$(date +%Y%m%d%H%M%S)
mkdir -p .codex/prompts /repo-wt-opencode-smoke/.signals /repo-wt-opencode-smoke/logs
```
...
Create `/repo/.codex/prompts/worker-opencode-smoke.md` with:

```markdown
***REMOVED*** OpenCode Smoke Worker
...
SIGNAL_FILE: /repo-wt-opencode-smoke/.signals/worker-opencode-smoke.json
ORCH_PANE: {{ORCH_PANE}}
...
After writing valid JSON, submit `[DONE] W-opencode-smoke /repo-wt-opencode-smoke/.signals/worker-opencode-smoke.json` to ORCH_PANE with tmux `send-keys -l`, sleep 0.25, then `C-m`.
```
...
/home/USER/.codex/skills/tmux-swarm-orchestration/scripts/launch_opencode_worker.sh \
  W-opencode-smoke \
  /repo-wt-opencode-smoke \
  /repo/.codex/prompts/worker-opencode-smoke.md
```
...
```bash
python3 -m json.tool /repo-wt-opencode-smoke/.signals/worker-opencode-smoke.json >/dev/null
python3 - <<'PY'
import json
from pathlib import Path
p = Path("/repo-wt-opencode-smoke/.signals/worker-opencode-smoke.json")
...
git -C /repo worktree remove /repo-wt-opencode-smoke
git -C /repo branch --list 'test/opencode-smoke-*'
```

If the smoke branch remains and has no useful commits:

```bash
git -C /repo branch -D test/opencode-smoke-YYYYMMDDHHMMSS
```
...
***REMOVED******REMOVED*** Task 10: Final Verification And Handoff
...
Run from `/repo` or the dedicated worktree:
...
```bash
test -f /repo/.opencode/skills/gh-pr-review/SKILL.md
