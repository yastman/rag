***REMOVED*** Langfuse Trace Coverage Design

***REMOVED******REMOVED*** Context

GitHub issue `***REMOVED***1307` tracks local Langfuse bring-up and bot trace coverage.
The latest audit shows that local Langfuse, LiteLLM, Qdrant, BGE-M3, and the
Telegram E2E runner can run, but `***REMOVED***1307` is still not acceptable because the
core trace contract fails.

The current hard failure is tracked in `***REMOVED***1485`: required text scenarios receive
Telegram responses, but Langfuse validation reports missing root output,
missing cache-check coverage, and missing required scores. Open PR `***REMOVED***1491`
...
This design keeps the full `***REMOVED***1307` goal visible, but scopes the first
implementation phase to `***REMOVED***1485` after accounting for `***REMOVED***1491`.
...
- Make the `***REMOVED***1307` acceptance path explicit across follow-up issues.
...
- Do not solve voice fixture, mini-app health, Qdrant data preflight, and
  latest-trace audit artifact work inside the first `***REMOVED***1485` phase.
- Do not access VPS, production, secrets, SSH, cloud credentials, or real CRM
  write paths.

***REMOVED******REMOVED*** Current-State Findings

***REMOVED******REMOVED******REMOVED*** Issue Status

- `***REMOVED***1307` remains open and cannot be closed yet.
- `***REMOVED***1485` is the current hard blocker for required text scenarios.
- `***REMOVED***1486` blocks voice-note scenario `8.1` because `E2E_VOICE_NOTE_PATH` is not
  available.
- `***REMOVED***1487` blocks `make validate-traces-fast` because `mini-app-frontend` is
  unhealthy in the current compose path.
- `***REMOVED***1488`, `***REMOVED***1489`, and `***REMOVED***1490` cover Qdrant preflight, product-meaningful
  Telethon assertions, and post-E2E latest-trace audit artifacts.
- PR `***REMOVED***1491` is clean and narrowly fixes validator aliasing for
  `cache-check`/`node-cache-check`.

***REMOVED******REMOVED******REMOVED*** Existing SDK Surfaces
...
***REMOVED******REMOVED******REMOVED*** Trace Contract
...
Use a phased `***REMOVED***1307` design with Phase 1 scoped to `***REMOVED***1485`.

This is better than a narrow `***REMOVED***1485`-only design because the project retains one
clear acceptance path for `***REMOVED***1307`. It is also safer than a one-shot `***REMOVED***1307`
...
***REMOVED******REMOVED******REMOVED*** App-Owned Root Trace
...
***REMOVED******REMOVED******REMOVED*** Child Observations
...
***REMOVED******REMOVED******REMOVED*** Scores
...
***REMOVED******REMOVED*** Error Handling And Gates

***REMOVED******REMOVED******REMOVED*** Phase 1 Scenario Matrix
...
***REMOVED******REMOVED******REMOVED*** Phase 1 Hard Failures
...
after `***REMOVED***1491` lands or is otherwise incorporated.

***REMOVED******REMOVED******REMOVED*** Phase 1 Non-Blockers

Phase 1 should not fail because of:

- flat `litellm-acompletion` traces;
- missing LiveKit `voice-session`;
- missing voice-note scenario `8.1` while `***REMOVED***1486` is open;
- `mini-app-frontend` health while `***REMOVED***1487` is open;
- missing post-E2E latest-trace artifact while `***REMOVED***1490` is open.
...
Phase 2 resolves `***REMOVED***1486`.

***REMOVED******REMOVED******REMOVED*** Score Enforcement Decision
...
***REMOVED******REMOVED******REMOVED*** Blocked Runtime Behavior
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Core Text Trace Contract (`***REMOVED***1485`)

- Review and merge, or reproduce, PR `***REMOVED***1491` for cache-check aliasing.
...
- Do not use full `make e2e-test-traces-core` as a Phase 1 pass/fail gate while
  `***REMOVED***1486` is open, because it includes voice scenario `8.1`.

***REMOVED******REMOVED******REMOVED*** Phase 2: Voice Fixture (`***REMOVED***1486`)
...
***REMOVED******REMOVED******REMOVED*** Phase 3: Fast Trace Validation Runtime (`***REMOVED***1487`)
...
***REMOVED******REMOVED******REMOVED*** Phase 4: Qdrant Data Preflight (`***REMOVED***1488`)
...
***REMOVED******REMOVED******REMOVED*** Phase 5: Product-Meaningful Scenarios (`***REMOVED***1489`)
...
***REMOVED******REMOVED******REMOVED*** Phase 6: Latest-Trace Audit Artifact (`***REMOVED***1490`)
...
***REMOVED******REMOVED******REMOVED*** Runtime Validation
...
***REMOVED******REMOVED*** Rollout

Use small PRs aligned to the phase list:

1. `***REMOVED***1485` text trace contract.
2. `***REMOVED***1486` voice fixture.
3. `***REMOVED***1487` fast validation runtime.
4. `***REMOVED***1488` Qdrant data preflight.
5. `***REMOVED***1489` product-meaningful E2E assertions.
6. `***REMOVED***1490` latest-trace audit artifact.

Do not close `***REMOVED***1307` until the required local flow can show:
...
- Whether `***REMOVED***1491` should be merged as-is before Phase 1 implementation or
  folded into a broader `***REMOVED***1485` PR.
