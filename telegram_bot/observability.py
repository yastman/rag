***REMOVED*** telegram_bot/observability.py
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** PII masking (always available, used when Langfuse IS enabled)
***REMOVED*** ---------------------------------------------------------------------------
...
def mask_pii(data: Any) -> Any:
    ...
    if isinstance(data, str):
        ***REMOVED*** Mask Telegram user IDs (9-10 digits not part of larger number)
        data = re.sub(r"\b\d{9,10}\b", "[USER_ID]", data)
        ***REMOVED*** Mask phone numbers
        data = re.sub(r"\+?\d{10,15}", "[PHONE]", data)
        ***REMOVED*** Mask emails
        data = re.sub(r"[\w.-]+@[\w.-]+\.\w+", "[EMAIL]", data)
        ***REMOVED*** Truncate long texts
        if len(data) > 500:
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** No-op stubs (used when Langfuse is disabled)
***REMOVED*** ---------------------------------------------------------------------------
...
***REMOVED*** ---------------------------------------------------------------------------
***REMOVED*** Public API — conditional on LANGFUSE_ENABLED
***REMOVED*** ---------------------------------------------------------------------------
...
if LANGFUSE_ENABLED:
    ...
    def get_langfuse_client() -> Langfuse:
        ...
        return Langfuse(
            mask=mask_pii,  ***REMOVED*** type: ignore[arg-type]  ***REMOVED*** MaskFunction stub mismatch
            flush_at=50,
            flush_interval=5,
        )
    ...
    observe = _noop_observe  ***REMOVED*** type: ignore[assignment]  ***REMOVED*** noop stub for disabled Langfuse
    get_client = _noop_get_client  ***REMOVED*** type: ignore[assignment]  ***REMOVED*** noop stub for disabled Langfuse

    @contextmanager
    def _noop_propagate(**kwargs: Any):
        yield

    propagate_attributes = _noop_propagate  ***REMOVED*** type: ignore[assignment]  ***REMOVED*** noop stub

    def get_langfuse_client() -> None:  ***REMOVED*** type: ignore[misc]  ***REMOVED*** conditional return type
        """Langfuse disabled — return None."""
        return

    def create_callback_handler(**kwargs: Any):  ***REMOVED*** type: ignore[misc]  ***REMOVED*** noop stub
