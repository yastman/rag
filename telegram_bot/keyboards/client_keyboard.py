...
***REMOVED*** Fallback button text -> action ID mapping (used when YAML not yet loaded)
...
def build_client_keyboard(locale: str = "ru") -> ReplyKeyboardMarkup:
    ...
    if not buttons_map:
        ***REMOVED*** Fallback to hardcoded Russian buttons
        buttons_map = {v: k for k, v in MENU_BUTTONS.items()}
    texts = list(buttons_map.values())
    ***REMOVED*** Ensure exactly 6 buttons (pad or trim if YAML is malformed)
    while len(texts) < 6:
        texts.append("")
    texts = texts[:6]
...
def parse_menu_button(text: str) -> str | None:
    ...
    for locale in ("ru", "uk", "en"):
        buttons_map = get_menu_buttons(locale)  ***REMOVED*** {action_id: button_text}
