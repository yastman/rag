"""RAG agent — wraps existing RAG graph as a supervisor tool (***REMOVED***240 Task 3).
...
Since ***REMOVED***310 (supervisor-only mode), this tool also writes Langfuse pipeline
    ...
    async def rag_search(query: str, config: RunnableConfig) -> str:
        ...
        try:
            ...
            if isinstance(result, dict):
                ***REMOVED*** Compute wall-time metrics (***REMOVED***310)
                ...
                ***REMOVED*** Write full pipeline scores to Langfuse trace (***REMOVED***310)
                write_langfuse_scores(lf, result)

                ***REMOVED*** Online LLM-as-a-Judge sampling (***REMOVED***310)
