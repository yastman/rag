"""Kommo CRM supervisor tools (***REMOVED***312).
...
def create_crm_tools(
    *,
    ...
    async def crm_upsert_contact(
        ...
        last_name: str = "",
        email: str = "",
        config: RunnableConfig = None,  ***REMOVED*** type: ignore[assignment]
    ) -> str:
    ...
    async def crm_finalize_deal(query: str, config: RunnableConfig) -> str:
        ...
        ***REMOVED*** Idempotency check
        ...
        try:
            ***REMOVED*** Step 1: Extract deal data from chat history
            messages = await history_service.get_session_turns(user_id, session_id, limit=40)
            chat_text = "\n".join(
            ...
            ***REMOVED*** Step 4: Link contact to deal
            if contact:
                await kommo.link_contact_to_lead(lead_id=lead.id, contact_id=contact.id)

            ***REMOVED*** Step 5: Add note with chat summary
            ...
            ***REMOVED*** Step 6: Create follow-up task
            ...
            result_parts = [f"Сделка ***REMOVED***{lead.id} создана: {deal_name}."]
            if contact:
                result_parts.append(f"Контакт: {contact.name} (***REMOVED***{contact.id}).")
