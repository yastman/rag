...
async def _cache_check(
    ...
    ***REMOVED*** If caller pre-computed the embedding (pre-agent cache check), reuse it directly.
    ...
    if pre_computed_embedding:
        ...
        ***REMOVED*** Still warm the embedding cache so downstream hits benefit
        await cache.store_embedding(query, embedding)
        ***REMOVED*** Reuse pre-computed sparse + colbert so rag_pipeline avoids redundant BGE-M3 calls (***REMOVED***571)
