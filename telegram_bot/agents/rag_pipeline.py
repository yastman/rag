    ...
    for _attempt in range(config.max_rewrite_attempts + 1):
        ...
        ***REMOVED*** Step 3: Grade documents
        ...
        if grade_result["documents_relevant"]:
            ...
            ***REMOVED*** 15% gap → trim-to-1 was too aggressive for comprehensive queries
            ***REMOVED*** where all chunks are equally relevant (e.g. listing all ВНЖ types).
            if final_gap_confident and len(final_docs) > _MIN_FINAL_CHUNKS:
                final_docs = final_docs[:_MIN_FINAL_CHUNKS]

            ***REMOVED*** Small-to-big context expansion: fetch neighbor chunks from same document
            if config.small_to_big_mode != SmallToBigMode.OFF and final_docs:
                try:
                    ...
                    if expanded:
                        ***REMOVED*** Replace document texts with expanded versions
