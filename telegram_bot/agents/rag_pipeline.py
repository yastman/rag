...
***REMOVED*** top_k=3 for reranking. Saves ~20ms vs top_k=5 while capturing most relevant docs via ColBERT semantic similarity.
...
async def _cache_check(
    ...
    ***REMOVED*** Step 1: Get or compute dense embedding (prefer hybrid for efficiency)
    ***REMOVED*** If caller pre-computed the embedding (pre-agent cache check), reuse it directly.
    embedding_error: bool = False
    embedding_error_type: str | None = None
    colbert_query: list[list[float]] | None = None
    sparse: Any = None  ***REMOVED*** tracked for return so _hybrid_retrieve can skip re-fetch
    ...
    if pre_computed_embedding:
        ...
        embeddings_cache_hit = False  ***REMOVED*** embedding came from caller, not Redis
        ***REMOVED*** Pre-agent already stored embeddings in cache — no need to re-store (***REMOVED***633)
    ...
    ***REMOVED*** Step 2: Check semantic cache (allowlisted types only)
    cached = None
...
async def _hybrid_retrieve(
    ...
    ***REMOVED*** Step 3: Hybrid search via Qdrant SDK (RRF fusion or ColBERT server-side rerank)
...
async def _rerank(
    ...
    *,
    ...
    if reranker is not None:
        try:
            ...
            if _has_get_rerank:
                cached_reranked = await cache.get_rerank_results(query, documents, top_k)  ***REMOVED*** type: ignore[union-attr]
            ...
            if _has_store_rerank:
                await cache.store_rerank_results(query, documents, top_k, reranked)  ***REMOVED*** type: ignore[union-attr]
    ...
    ***REMOVED*** Fallback: sort by existing score, take top-k
    ...
    *,
    ...
    try:
        ...
        response = await llm.chat.completions.create(
            ...
            name="rewrite-query",  ***REMOVED*** type: ignore[call-overload]
        )
...
async def rag_pipeline(
    ...
    ***REMOVED*** let _hybrid_retrieve handle cache lookup for those new rewritten queries.
    if cache_key != query:
        ...
        ***REMOVED*** cache_result.colbert_query was computed for cache_key (original user text).
        ***REMOVED*** Reset and re-encode for the actual retrieval query to avoid query mismatch.
        colbert_query = None
        query_sparse: Any = None  ***REMOVED*** pre-computed sparse is for cache_key, not reformulated query
        if (
