"""Supervisor graph — routes queries to tools (***REMOVED***240 Task 5).
...
4. Tool call limit (***REMOVED***374): stops loop when tool_call_count >= max_tool_calls
"""
...
def build_supervisor_graph(
    *,
    ...
    async def supervisor_node(state: SupervisorState) -> dict[str, Any]:
        ...
        ***REMOVED*** Only update agent_used when a tool is actually selected
        tool_name = ""
        if hasattr(response, "tool_calls") and response.tool_calls:
            tool_name = response.tool_calls[0]["name"]
            update["agent_used"] = tool_name
            ***REMOVED*** Increment tool call count (***REMOVED***374)
