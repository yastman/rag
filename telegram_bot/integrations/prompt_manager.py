...
***REMOVED*** Default cache TTL in seconds (5 minutes)
DEFAULT_CACHE_TTL = 300

***REMOVED*** Module-level Langfuse client (lazy-initialized singleton)
...
def get_prompt(
    ...
    *,
    ...
    try:
        ***REMOVED*** Do not pass fallback to SDK: for missing prompts SDK logs noisy warnings on each call.
        ***REMOVED*** We handle fallback ourselves and cache "not found" locally.
        ...
        if _is_prompt_not_found(e):
            ***REMOVED*** Avoid hitting Langfuse on every request when prompt/label is absent.
            _missing_prompts_until[name] = time.monotonic() + cache_ttl
            logger.debug(
...
def _probe_prompt_available(client: Any, name: str) -> bool | None:
    ...
    try:
        ...
        ***REMOVED*** Avoid hard dependency on specific SDK exception type at import time.
