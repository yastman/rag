...
async def on_reset(
    ...
    with _start_filter_observation(
        ...
        ***REMOVED*** Resetting this dialog via RESET_STACK from inside its own callback can
        ***REMOVED*** leave the first redraw attached to stale context. Close the current
        ***REMOVED*** filter shell first, then open a clean instance above the catalog.
        msg = callback.message
        manager.show_mode = ShowMode.NO_UPDATE
        await manager.done()
