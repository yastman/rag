...
async def on_reset(
    ...
    ***REMOVED*** Clear Radio widget state directly: aiogram-dialog does not support
    ***REMOVED*** unchecking Radio via set_checked(None) and stores state in widget_data.
    with contextlib.suppress(Exception):
        widget_data = manager.current_context().widget_data
        for radio_id in _FIELD_TO_RADIO_ID.values():
            widget_data.pop(radio_id, None)
