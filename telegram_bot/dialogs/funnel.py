...
async def get_results_data(dialog_manager: DialogManager, **kwargs: Any) -> dict[str, Any]:
    ...
    ***REMOVED*** Build compact card data
...
async def on_summary_search(
    ...
    ***REMOVED*** Catalog keyboard (счётчик на кнопке — без дублирующего текста)
    shown = len(results)
...
funnel_dialog = Dialog(
    ...
    ***REMOVED*** Step 6: Results (list view)
    Window(
        Jinja("{{ title | safe }}\n\n"),
        List(
            Jinja(
