...
async def get_results_data(
    ...
    **kwargs: Any,
    ...
    return {
        ...
        "results_text": empty_msg,  ***REMOVED*** Placeholder — Phase 2 integrates RAG
        "btn_back": back,
    }
...
async def on_timeline_selected(
    ...
    ***REMOVED*** Persist scoring + CRM sync asynchronously on completed funnel answers.
    try:
...
funnel_dialog = Dialog(
    ***REMOVED*** Step 1: Property type (SPIN: Situation)
    Window(
        Format("{title}"),
        Column(
            Select(
    ...
    ***REMOVED*** Step 2: Budget (BANT: Budget)
    Window(
        Format("{title}"),
        Column(
            Select(
    ...
    ***REMOVED*** Step 3: Timeline (BANT: Timeline)
    Window(
        Format("{title}"),
        Column(
            Select(
