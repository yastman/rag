...
***REMOVED*** Display labels for summary
...
async def get_results_data(
    ...
    ***REMOVED*** Resolve apartments_service with property_bot fallback
    ...
    if svc is not None:
        try:
            ...
            if results:
                ...
                ***REMOVED*** Build zero-results recovery suggestions
                if total_count == 0:
...
async def on_summary_search(
    ...
    ***REMOVED*** Close dialog before sending cards
    await manager.done()

    ***REMOVED*** Store results in FSMContext for pagination
    ...
    ***REMOVED*** Footer
    shown = len(results)
    has_more = next_offset is not None
    remaining = max(total_count - shown, 0) if has_more else 0
    _ = remaining  ***REMOVED*** used for future "load more" logic
    await callback.message.answer(
...
funnel_dialog = Dialog(
    ...
    ***REMOVED*** Step 6: Results (SDK scroll with pagination)
    Window(
        Format("{title}\n\n{results_text}"),
        Column(
            Select(
