...
from aiogram_dialog.widgets.text import Const, Format  ***REMOVED*** noqa: F401
...
async def get_date_options(
    ...
    ***REMOVED*** Flip to (label, key) for Select widget
    return {
...
async def on_object_selected(
    ...
    ***REMOVED*** Check if already selected → remove (toggle)
...
async def _send_phone_reply_keyboard(callback: CallbackQuery) -> None:
    ...
    await callback.bot.send_message(  ***REMOVED*** type: ignore[union-attr]
...
async def on_phone_text_received(message: Message, widget: Any, manager: DialogManager) -> None:
    ...
    ***REMOVED*** Handle cancel from ReplyKeyboard
...
async def on_confirm(callback: CallbackQuery, button: Button, manager: DialogManager) -> None:
    ...
    try:
        ...
        ***REMOVED*** Build viewing_objects for note (reuse phone_collector format)
        ...
        ***REMOVED*** Build object summary for lead title and task
        obj_summary = ""
        if viewing_objects:
            names = [
        ...
        ***REMOVED*** Build note with date info
        date_label = DATE_LABELS.get(date_range, date_range)
        ...
        ***REMOVED*** Send confirmation + restore client menu keyboard
    ...
    ***REMOVED*** aiogram-dialog auto-answers callback; don't call callback.answer() manually
    await manager.done()
...
viewing_dialog = Dialog(
    ***REMOVED*** Step 1: Objects from favorites
    Window(
        Format("{title}"),
        Column(
            Select(
    ...
    ***REMOVED*** Step 1b: Free-text object input
    Window(
        Format("{title}"),
