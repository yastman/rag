...
async def handle_demo_example(
    ...
    **kwargs: Any,
    ...
    await callback.answer()
    idx = int(callback.data.split(":")[-1])  ***REMOVED*** type: ignore[union-attr]
    ...
    ***REMOVED*** Delegate search using callback.message as the reply target
    await _run_demo_search(query, callback.message, state, **kwargs)  ***REMOVED*** type: ignore[arg-type]
