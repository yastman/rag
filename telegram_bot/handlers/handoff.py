...
async def start_qualification(
    ...
    if hasattr(message_or_callback, "message"):
        ***REMOVED*** CallbackQuery
        await message_or_callback.message.answer(text, reply_markup=kb)
    else:
        await message_or_callback.answer(text, reply_markup=kb)


***REMOVED*** ── Qualification callback handler ──────────────────────────────

***REMOVED*** Stores in-progress qualification per user.  Cleared on completion.
***REMOVED*** Key: user_id → {"goal": ..., "budget": ...}
...
async def on_qual_callback(
    ...
    **kwargs: Any,
    ...
    if step == "goal":
        ...
        if value == "chat":
            ***REMOVED*** Trigger Forum Topics bridge — handled by bot.py integration
            await callback.message.edit_text(
                _t(i18n, "handoff-connecting", "Соединяю с менеджером...")
            )
        elif value == "phone":
            await callback.message.edit_text("...")
            ***REMOVED*** Delegate to PhoneCollector — handled by bot.py integration
