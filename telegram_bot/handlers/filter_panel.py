...
async def _handle_select(
    ...
    ***REMOVED*** For budget, reverse-map price_eur dict back to budget label
    if field == "budget" and isinstance(current_value, dict):
        current_value = _price_to_budget(current_value)

    ***REMOVED*** Load dynamic options for complex field
    ...
    ***REMOVED*** Build sub-menu header text
    _FIELD_LABELS = {
        "city": "📍 Выберите город",
...
async def _handle_set(
    ...
    if value == "":
        ***REMOVED*** Clear this specific filter
        filters.pop(field, None)
        filters.pop(_field_to_filter_key(field), None)
    else:
        ***REMOVED*** Map field name to filter key and coerce value
        filter_key = _field_to_filter_key(field)
...
async def _handle_apply(
    ...
    msg = callback.message  ***REMOVED*** type: ignore[union-attr]
