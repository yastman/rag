...
***REMOVED*** Fields that store integer values
...
async def on_filter_panel_select(
    ...
    try:
        ...
        await callback.message.edit_text(  ***REMOVED*** type: ignore[union-attr]
...
async def on_filter_panel_set(
    ...
    ***REMOVED*** Refresh panel with updated filters (count=0 until apply triggers re-search)
...
async def on_filter_panel_back(
    ...
    if callback_data.field:
        ***REMOVED*** Back from sub-menu → show main panel
        data = await state.get_data()
        filters: dict = data.get("apartment_filters") or {}
        count = data.get("apartment_total", 0)
        panel_text = build_filter_panel_text(filters=filters, count=count)
        ...
        ***REMOVED*** Back from main panel → close
        await callback.message.delete()  ***REMOVED*** type: ignore[union-attr]
