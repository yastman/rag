...
_MAX_CONTEXT_DOCS = 5
_STREAM_EDIT_INTERVAL = 0.3  ***REMOVED*** 300ms throttle for Telegram edit_text
...
async def generate_node(state: RAGState, *, message: Any | None = None) -> dict[str, Any]:
    ...
    ***REMOVED*** Extract current query (needed for style detection before prompt building)
    ...
    ***REMOVED*** Detect response style (C+ scoring, no LLM call, ~0ms) (***REMOVED***129)
    ***REMOVED*** Rollout-safe: disabled → legacy, shadow → compute metrics but legacy prompt/tokens
        ...
        ***REMOVED*** Streaming path: deliver directly to Telegram
        if message is not None and config.streaming_enabled:
            try:
                ...
                ***REMOVED*** Recovery is successful once fallback LLM response is produced,
                ***REMOVED*** even if edit delivery fails and respond_node sends later.
                stream_recovery = True
                ***REMOVED*** Edit existing message with fallback answer
                delivered = False
                try:
                ...
                ***REMOVED*** Fall back to non-streaming
                response_obj = await llm.chat.completions.create(
                    model=config.llm_model,
                    messages=llm_messages,
                    temperature=config.llm_temperature,
            ...
            ***REMOVED*** Non-streaming path (original)
            response_obj = await llm.chat.completions.create(
                model=config.llm_model,
                messages=llm_messages,
                temperature=config.llm_temperature,
