...
***REMOVED*** 5 context docs is the quality/latency sweet spot. Reducing to 3 saves ~50ms TTFT
***REMOVED*** but risks missing relevant context. Keep at 5 unless latency SLA requires <1s.
_MAX_CONTEXT_DOCS = 5
***REMOVED*** 0.5s edit interval: reduces Telegram API load and 429 risk.
***REMOVED*** Trade-off: slightly chunkier streaming UX vs 0.3s, but safer margin.
***REMOVED*** Telegram editMessageText counts 1 unit toward rate limit; 0.5s = max 120/min burst.
...
***REMOVED*** Fallback system prompt: ~130 tokens (Russian). Main token cost comes from
***REMOVED*** context docs (~5 * 500 chars) and history, not system prompt.
_GENERATE_FALLBACK = (
...
async def _generate_streaming(
    ...
    ***REMOVED*** Measure TTFT from request dispatch (includes provider pre-stream wait).
    ...
    ***REMOVED*** Legacy stream-only TTFT baseline kept for drift diagnostics.
    ...
    ***REMOVED*** Final edit with Markdown formatting
    try:
