...
***REMOVED*** Only these query types use semantic cache (check + store).
***REMOVED*** GENERAL uses a stricter threshold (0.08) to avoid false positives.
...
async def cache_check_node(
    ...
    if embedding is None:
        try:
            ...
            if _has_hybrid_colbert:
                ***REMOVED*** 3-way hybrid: dense + sparse + colbert in one call
                ...
                ***REMOVED*** Hybrid: get both dense + sparse in one call, cache both
    ...
    ***REMOVED*** ColBERT vectors are per-query token-level and not cached in Redis.
    ...
    ***REMOVED*** Step 2: Check semantic cache with query-type threshold (allowlisted types only).
    ***REMOVED*** Voice path has no user role — agent_role is intentionally omitted so that
    ***REMOVED*** voice responses are shared across roles within the same cache_scope="rag" bucket.
    cached = None
...
async def cache_store_node(
    ...
    ***REMOVED*** Store in semantic cache if we have both response and embedding (allowlisted types only)
