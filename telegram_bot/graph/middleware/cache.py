...
CACHEABLE_QUERY_TYPES: set[str] = _get_cacheable_types()
...
def _final_response_text(state: AgentState[Any] | dict[str, Any]) -> str:
    ...
    for message in reversed(messages):
        ...
        if message_type in {"ai", "assistant"}:
            ...
            if content:
                return str(content)
    # Fallback: if there are 2+ messages and the last has no type attr,
    # treat it as the assistant response (handles local Message stubs).
    if len(messages) >= 2:
        last = messages[-1]
        if getattr(last, "type", None) is None and not isinstance(last, dict):
            content = getattr(last, "content", None)
            if isinstance(content, str) and content:
                return content
