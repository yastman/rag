...
class PropertyBot:
    ...
    def __init__(self, config: BotConfig):
        ...
        ***REMOVED*** Agent checkpointer — MemorySaver to avoid Redis JSON serialization
        ***REMOVED*** issues with LangChain Message objects (***REMOVED***420)
    ...
    def _register_handlers(self):
        ...
        ***REMOVED*** Handoff qualification callbacks — qual:goal/budget (***REMOVED***730)
        self.dp.include_router(create_handoff_router())
        ***REMOVED*** Final qualification step — triggers handoff completion (***REMOVED***730 review)
        ...
        ***REMOVED*** ReplyKeyboard buttons — registered before catch-all F.text (***REMOVED***628)
    ...
    async def cmd_start(self, message: Message, dialog_manager: Any = None, i18n: Any = None):
        ...
        if role == "manager" and kommo_enabled and dialog_manager is not None:
            ...
            ***REMOVED*** Client: persistent ReplyKeyboard (***REMOVED***628)
    ...
    async def _handle_promotions(self, message: Message) -> None:
        """Show promotions from config (***REMOVED***628)."""
    ...
    ***REMOVED*** Mapping callback_data -> query text for RAG pipeline
    ...
    async def _on_qual_contact(self, callback: CallbackQuery, state: FSMContext) -> None:
        """Handle final qualification step — create topic + state (***REMOVED***730 review)."""
        await callback.answer()
        ...
        if value == "chat" and self._forum_bridge is not None:
            ...
            ***REMOVED*** Fallback — delegate to agent pipeline.
            if msg and hasattr(msg, "edit_text"):
                await msg.edit_text("Соединяю с менеджером...")
    ...
    async def handle_cta_callback(self, callback: CallbackQuery, state: FSMContext) -> None:
        ...
        if action == "get_offer":
            ...
            if self._forum_bridge is not None:
                ***REMOVED*** Forum Topics enabled — same qualification flow as menu (***REMOVED***730).
    ...
    async def handle_favorite_callback(
        ...
        """Handle favorite add/remove/viewing callbacks (***REMOVED***628)."""
        ...
        if action == "add" and property_id:
            ***REMOVED*** Build property_data from saved apartment results (***REMOVED***655, ***REMOVED***664)
            ...
            ***REMOVED*** Загрузить данные объекта из favorites
    ...
    async def handle_results_callback(
        ...
        if data == "results:more":
            ...
            ***REMOVED*** Funnel flow stores only the first page; lazily append more pages on demand.
            if new_offset >= len(results):
                ...
                if can_fetch_more:
                    ***REMOVED*** Qdrant may return None offset while more rows still exist in count().
                    ***REMOVED*** In that case, fetch a wider prefix from start and replace cached list.
                    ...
                    try:
                        (
                            ...
                            total_count,
                            next_offset,
                        ) = await apartments_service.scroll_with_filters(  ***REMOVED*** type: ignore[union-attr]
            ...
            for result in page:
                if callback.message:
                    await self._send_property_card(callback.message, result, callback.from_user.id)  ***REMOVED*** type: ignore[arg-type]
            shown = len(page)
            ...
            state_data = await state.get_data()
            results = state_data.get("apartment_results", [])
            ***REMOVED*** Первые 5 результатов как контекст для CRM заметки
            viewing_objs = []
            for r in results[:5]:
            ...
            if dialog_manager is not None:
                ...
                ***REMOVED*** Delete footer message to keep chat clean
                if callback.message:
                    with contextlib.suppress(Exception):
                        await callback.message.delete()  ***REMOVED*** type: ignore[union-attr]
    ...
    async def handle_card_callback(
        ...
        if action == "viewing":
            if dialog_manager is not None:
                ...
                ***REMOVED*** Delete footer message to avoid visual clutter during dialog
                footer_msg_id = state_data.get("apartment_footer_msg_id")
                if footer_msg_id and callback.message and callback.message.chat:
                    with contextlib.suppress(Exception):
                        await callback.bot.delete_message(callback.message.chat.id, footer_msg_id)  ***REMOVED*** type: ignore[union-attr]
    ...
    async def _handle_apartment_fast_path(
        ...
        ***REMOVED*** Store results in FSMContext and send property cards (***REMOVED***654)
        if state is not None and results:
            ...
            for result in page:
                await self._send_property_card(message, result, message.from_user.id)  ***REMOVED*** type: ignore[union-attr]
            shown = len(page)
            total = len(results)
    ...
    async def _handle_query_supervisor(
        ...
        with propagate_attributes(
            ...
            if query_type in CACHEABLE_QUERY_TYPES:
                try:
                    ...
                    if embedding is None:
                        ...
                        ***REMOVED*** Embedding cached but sparse may have expired (***REMOVED***637)
                        if sparse is None:
                            _has_hybrid_colbert = callable(
                                getattr(self._embeddings, "aembed_hybrid_with_colbert", None)
                            ) and asyncio.iscoroutinefunction(
                    ...
                    if cached:
                        ...
                        ***REMOVED*** Write Langfuse scores and trace metadata
                        lf = get_client()
                        ...
                        return cached
                    ***REMOVED*** MISS: stash all embeddings so rag_pipeline can skip recomputation (***REMOVED***571)
                    logger.debug("Pre-agent cache MISS (type=%s): %.60s", query_type, user_text)
                    rag_result_store["cache_key_embedding"] = embedding
                    rag_result_store["cache_key_sparse"] = sparse
                    rag_result_store["query_type"] = query_type
                    ***REMOVED*** Compute colbert if not yet available to avoid double embed in rag_pipeline (***REMOVED***634)
                    if colbert is None:
            ...
            ***REMOVED*** Build base tools list (client-only: rag_search)
            ...
            if role == "manager":
                ...
                ***REMOVED*** history_search is manager-only: search past conversations with clients/deals
                if self._history_service is not None:
                ...
                ***REMOVED*** Add direct CRM tools conditionally
            ...
            ***REMOVED*** Add utility tools for all roles (***REMOVED***445)
            ...
            ***REMOVED*** Skip if a tool already delivered the response via streaming (***REMOVED***428).
            if response_text and not ctx.response_sent:
                ...
                ***REMOVED*** Send with Markdown, fallback to plain text
    ...
    async def handle_feedback(self, callback: CallbackQuery):
        """Handle feedback button callback (***REMOVED***229, ***REMOVED***755)."""
        ...
        ***REMOVED*** Acknowledge "done" button silently
        if data == "fb:done":
            await callback.answer()
            return
        ...
        ***REMOVED*** Step 1 — dislike without reason: show 6-button reason keyboard
        ...
        ***REMOVED*** Step 2 — reason selected: write user_feedback=0 + user_feedback_reason={category}
        ***REMOVED*** Also handles like (value=1.0, reason=None) — single score path
    ...
    async def handle_menu_action(
        ...
        ***REMOVED*** Build tools list (mirrors _handle_query_supervisor tool assembly)
        base_tools: list[Any] = [rag_search, apartment_search]
        manager_tools: list[Any] = []
        if role == "manager":
            from .agents.manager_tools import (
    ...
    async def start(self):
        ...
        ***REMOVED*** Setup aiogram-dialog (***REMOVED***658: removed dead client_menu_dialog)
        ...
        ***REMOVED*** Set default Menu Button → opens commands list (***REMOVED***628)
