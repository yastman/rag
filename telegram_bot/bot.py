...
"""
    ...
    async def handle_query(self, message: Message):
        ...
        ***REMOVED*** 4. Search in Qdrant with cache (Tier 2)
        results = await self.cache_service.get_cached_search(query_vector, filters)
        if results is None:
            ***REMOVED*** Initial retrieval (more results for reranking)
            results = self.retriever_service.search(
                query_vector=query_vector,
                filters=filters if filters else None,
                top_k=self.config.search_top_k,  ***REMOVED*** Get more for reranking
                min_score=self.config.min_score,
            )

            ***REMOVED*** Rerank with Voyage AI (rerank-2.5, 32K context)
            if results and len(results) > 1:
                ***REMOVED*** Extract text content for reranking
                ...
                ***REMOVED*** Reorder results by rerank scores
