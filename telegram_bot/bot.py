...
class PropertyBot:
    ...
    def __init__(self, config: BotConfig):
        ...
        ***REMOVED*** Dense embedding provider (feature flag)
        ***REMOVED*** RETRIEVAL_DENSE_PROVIDER: bge_m3_api | voyage
        if config.retrieval_dense_provider == "bge_m3_api":
            ...
            self.voyage_service = None  ***REMOVED*** Not needed for dense
            logger.info("Using BgeM3DenseService for dense embeddings")
        else:
            ***REMOVED*** Voyage (default)
        ...
        ***REMOVED*** Rerank provider (feature flag)
        ***REMOVED*** RERANK_PROVIDER: colbert | none | voyage
        if config.rerank_provider == "colbert":
            ...
            ***REMOVED*** Voyage (default) - need VoyageService for rerank
        ...
        ***REMOVED*** Qdrant service with hybrid search, score boosting, MMR
        ...
        ***REMOVED*** BGE-M3 sparse embedding service (HTTP client, lazy init)
        ...
        ***REMOVED*** Query preprocessor (rule-based: translit, RRF weights, HyDE gating)
        self.query_preprocessor = QueryPreprocessor()

        ***REMOVED*** HyDE generator (optional, feature-flagged)
        ...
        ***REMOVED*** CESC: User context and personalization (configurable)
    ...
    def _setup_middlewares(self):
        ...
        ***REMOVED*** Rate limiting: 1.5 seconds between requests
...
"""
    ...
    async def handle_query(self, message: Message):
        ...
        ***REMOVED*** Scores accumulator - defaults are "miss" / "not applied"
        scores = {
        ...
        query_type = QueryType.SIMPLE  ***REMOVED*** Default for early exceptions

        try:
            ***REMOVED*** Context fingerprint for cache isolation + trace filtering
            ...
            ***REMOVED*** Update trace with user context
            ...
            ***REMOVED*** Query routing (2026 best practice) - skip RAG for chit-chat
            ...
            ***REMOVED*** Off-topic detection (Guardrails)
            ...
            ***REMOVED*** CESC: Lazy routing (2026 best practice) - only update context for personalized queries
            user_context = None
            ...
            ***REMOVED*** Initialize cache on first query if not done yet
            if not self._cache_initialized:
                await self.cache_service.initialize()
                self._cache_initialized = True

            ***REMOVED*** Show typing indicator
            await message.bot.send_chat_action(message.chat.id, "typing")

            ***REMOVED*** 0. Query preprocessing (translit + HyDE gating)
            ...
            ***REMOVED*** 0.1 RU/UK normalizer: strip greetings/politeness for better cache hits
            ...
            embed_text = clean_query  ***REMOVED*** Use normalized query for embedding

            ***REMOVED*** HyDE: generate hypothetical document for short/vague queries
            ...
            ***REMOVED*** === FAST-LANE: Dense embed → cache check (vector reuse) ===
            ...
            ***REMOVED*** Fast-Lane: check semantic cache with pre-computed vector (no vectorizer call)
            ...
            if cached_answer:
                ...
                ***REMOVED*** Observe vector distance from last cache hit
                ...
                ***REMOVED*** CESC: Personalize only if lazy routing determined it's needed
                ...
                return  ***REMOVED*** finally block writes scores

            ***REMOVED*** === MISS-LANE: parallel sparse + QueryAnalyzer, then search ===
            metrics.inc("cache_miss")
            t_miss = time.perf_counter()

            ***REMOVED*** Parallel: QueryAnalyzer + Sparse embedding (independent, both cached)
            ...
            ***REMOVED*** 4. Search in Qdrant with hybrid search
            t_search = time.perf_counter()
            results = await self.cache_service.get_cached_search(query_vector, filters)
            if results is None:
                ***REMOVED*** Hybrid RRF search (dense + sparse)
                ...
                ***REMOVED*** MMR diversity reranking (if enabled and enough results)
                if self.config.mmr_enabled and len(results) > self.config.rerank_top_k:
                    ***REMOVED*** Get embeddings for MMR calculation
                    result_embeddings = [
                        await self.dense_service.embed_query(r["text"])
                        for r in results[:20]  ***REMOVED*** Limit for performance
                    ]
                ...
                ***REMOVED*** Voyage rerank with cache (2026 best practice)
                ***REMOVED*** Skip rerank for simple queries, few results, or if rerank disabled
                if (
                    results
                    ...
                    ***REMOVED*** Check rerank cache first
                    ***REMOVED*** Hash embedding for cache key (same pattern as search cache)
                    ...
                    if cached_rerank:
                        ...
                        ***REMOVED*** Rebuild results from cached scores
                        ...
                        try:
                            ...
                            ***REMOVED*** Store rerank results in cache (dict format for JSON serialization)
                            rerank_cache_data = [
            ...
            ***REMOVED*** 5. Small-to-big context expansion (optional)
            ...
            ***REMOVED*** 6. Get conversation history for context-aware responses
            _conversation_history = await self.cache_service.get_conversation_history(
                user_id, last_n=3
            )

            ***REMOVED*** Store user query in conversation
            await self.cache_service.store_conversation_message(user_id, "user", query)

            scores["llm_used"] = 1.0
            t_llm = time.perf_counter()

            ***REMOVED*** 7. Generate answer with LLM
            temp_message = await message.answer("🔍 Генерирую ответ...")

            ***REMOVED*** Confidence scoring mode (non-streaming, returns ConfidenceResult)
            if self.config.enable_confidence_scoring:
                ...
                ***REMOVED*** Standard streaming mode
                accumulated_text = ""
                last_sent_text = ""  ***REMOVED*** Track what we last sent
                chunk_count = 0
                ...
                try:
                    async for chunk in self.llm_service.stream_answer(
                        ...
                        ***REMOVED*** Update message every 10 chunks or when punctuation appears
                        ***REMOVED*** Only if text actually changed
                        if (
                    ...
                    ***REMOVED*** Final update with complete answer (only if different)
                    if accumulated_text != last_sent_text:
                        try:
                            await temp_message.edit_text(accumulated_text, parse_mode="Markdown")
                        except Exception:
                            ***REMOVED*** Markdown parse failed — send without formatting
                            await temp_message.edit_text(accumulated_text)
                    answer = accumulated_text

                except Exception as e:
                    logger.error(f"Streaming error: {e}", exc_info=True)
                    ***REMOVED*** Fallback to non-streaming
            ...
            ***REMOVED*** 8. Store assistant answer in conversation
            await self.cache_service.store_conversation_message(user_id, "assistant", answer)

            ***REMOVED*** 9. Fire-and-forget: store in semantic cache with pre-computed vector
            ...
            ***REMOVED*** Record total pipeline time
            total_ms = (time.perf_counter() - start_time) * 1000
            metrics.record("total", total_ms)

            ***REMOVED*** Log summary every 50 queries
            qcount = metrics.inc_queries()
            if qcount % 50 == 0:
                metrics.log_summary()

            ***REMOVED*** Write all scores - guaranteed on ALL exit paths
            langfuse = _langfuse_client()
            if langfuse:
    ...
    async def _get_sparse_vector(self, text: str) -> dict:
        ...
        try:
            ...
            data = response.json()
            ***REMOVED*** BGE-M3 returns {"lexical_weights": [{"indices": [...], "values": [...]}]}
    ...
    def _format_results(self, results: list[dict]) -> str:
        ...
        for i, result in enumerate(results[:3], 1):  ***REMOVED*** Show top 3
            meta = result.get("metadata", {})
            score = result.get("score", 0)

            text = f"{i}. {meta.get('title', 'N/A')}\n"

            ***REMOVED*** Format price with thousands separator if it's a number
    ...
    async def start(self):
        ...
        ***REMOVED*** Initialize cache at startup (loads HF model, may take time on first run)
        if not self._cache_initialized:
        ...
        ***REMOVED*** Preflight dependency checks (raises PreflightError for CRITICAL failures)
    ...
    async def stop(self):
        ...
        ***REMOVED*** Close provider services (if they have close method and aren't VoyageService)
