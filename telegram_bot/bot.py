        ...
        if state is not None and results:
            ...
            for card_result in page:
                await self._send_property_card(
                    message,
                    card_result,
                    message.from_user.id,  ***REMOVED*** type: ignore[union-attr]
                )
            shown = len(page)
            total = len(results)
            ...
            if query_type in CACHEABLE_QUERY_TYPES:
                try:
                    ...
                    ***REMOVED*** MISS: stash all embeddings so rag_pipeline can skip recomputation (***REMOVED***571)
                    ...
                    rag_result_store["cache_key_sparse"] = sparse
                    rag_result_store["query_type"] = query_type
                    ***REMOVED*** Prefer the hybrid endpoint so BGE-M3 can return all query
                    ***REMOVED*** representations from one request when supported.
                    if colbert is None:
                        _has_hybrid_colbert = callable(
                            getattr(self._embeddings, "aembed_hybrid_with_colbert", None)
                        ) and asyncio.iscoroutinefunction(
                        ...
                        ***REMOVED*** Use the one-pass hybrid endpoint only when it can still
                        ***REMOVED*** fill in missing query representations. If dense+sparse
                        ***REMOVED*** are already cached, prefer standalone ColBERT to avoid
                        ***REMOVED*** recomputing embeddings we already have.
