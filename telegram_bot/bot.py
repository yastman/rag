...
class PropertyBot:
    ...
    ***REMOVED*** ReplyKeyboard button text → agent query mapping (***REMOVED***628)
    ...
    async def handle_menu_button(self, message: Message, locale: str = "ru"):
        """Handle ReplyKeyboard button press — translate to agent query (***REMOVED***628)."""
        action_id = parse_menu_button(message.text or "")
        if action_id is None:
            return
        ***REMOVED*** Replace button emoji text with natural-language query for pipeline
        query = self._MENU_BUTTON_QUERIES.get(action_id, message.text or "")
        ***REMOVED*** Use model_copy to avoid mutating the original message
