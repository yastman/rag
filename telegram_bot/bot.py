...
class PropertyBot:
    ...
    def _register_handlers(self):
        ...
        ***REMOVED*** Deep link: /start q_<uuid> — must be registered BEFORE generic /start
    ...
    async def _handle_deeplink_start(self, message: Message, uuid_str: str) -> None:
        ...
        ***REMOVED*** Atomic GET+DEL — prevents race condition with duplicate requests
        ...
        ***REMOVED*** Create / get forum topic for this expert
        try:
        ...
        if user_message:
            ***REMOVED*** Echo user question in the topic
            await self.bot.send_message(
                chat_id=message.chat.id,
                message_thread_id=topic_id,
                text=f"❝ {user_message} ❞",
            )
            ***REMOVED*** Trigger RAG pipeline: synthetic message in the topic
    ...
    async def _handle_catalog_dispatch(
        ...
        if catalog_action == "catalog_exit":
            ...
            return
        ***REMOVED*** Menu button while in catalog (e.g. "🏠 Главное меню")
        action_id = parse_menu_button(
            message.text or "",
            i18n_hub=getattr(self, "_i18n_hub", None),
        )
        if action_id == "menu":
            await self._handle_catalog_exit(message, state)
            return
        ***REMOVED*** Non-catalog, non-menu text in catalog mode — ignore (don't leak to RAG)
    ...
    async def _handle_catalog_filters(
        ...
        ***REMOVED*** Fallback: inline filter panel (если dialog_manager недоступен)
        import contextlib
    ...
    ***REMOVED*** Mapping callback_data -> query text for RAG pipeline
    ...
    async def handle_results_callback(
        ...
        ***REMOVED*** Resolve action from CallbackData or legacy string
    ...
    async def start(self):
        ...
        ***REMOVED*** Catalog mode router — intercepts ALL text when catalog_mode=True
        ***REMOVED*** Registered BEFORE catch-all so catalog buttons don't leak to RAG
