...
class PropertyBot:
    ...
    async def _handle_query_supervisor(self, message: Message, pipeline_start: float) -> None:
        """Handle query via supervisor graph (***REMOVED***240, ***REMOVED***310 — primary query path)."""
        ...
        ***REMOVED*** Resolve user role (***REMOVED***388)
        role = await self._resolve_user_role(user_id)

        ***REMOVED*** Build supervisor tools (base + role-specific)
        base_tools = [
            create_rag_agent(
                cache=self._cache,
        ...
        ***REMOVED*** Manager tools (***REMOVED***388) — activated when lead_service is available (***REMOVED***387)
        ...
        ***REMOVED*** Nurturing + analytics tools (***REMOVED***390) — manager-only
        ...
        ***REMOVED*** CRM tools are manager-only (issue ***REMOVED***389).
        ...
        ***REMOVED*** Lead score sync tool (***REMOVED***384) — available when scoring store is initialized
        ...
        ***REMOVED*** Build supervisor LLM (cheap model for routing)
        ...
        with propagate_attributes(
            ...
            ***REMOVED*** Send response to user
            if response_text and not result.get("response_sent"):
                await message.answer(response_text)

            ***REMOVED*** Wall-time for the full supervisor pipeline
            ...
            ***REMOVED*** Supervisor-specific scores
            lf.score_current_trace(
    ...
    async def start(self):
        ...
        ***REMOVED*** Initialize Kommo CRM client if enabled
