...
class PropertyBot:
    ...
    async def _handle_query_supervisor(self, message: Message, pipeline_start: float) -> None:
        ...
        ***REMOVED*** Build tools list
        tools: list[Any] = [rag_search]
        if self._history_service is not None:
            tools.append(history_search)

        ***REMOVED*** Add CRM tools conditionally
        if (
            role == "manager"
        ...
        with propagate_attributes(
            ...
            ***REMOVED*** Write Langfuse trace metadata
            ...
            if tid:
                ...
                ***REMOVED*** Tool call count (***REMOVED***374): count AIMessage tool_calls from agent output
                tool_calls = sum(
                    1
    ...
    async def start(self):
        ...
        ***REMOVED*** Agent/voice checkpointer — MemorySaver avoids redisvl JSON serialization
        ***REMOVED*** errors with LangChain Message objects (HumanMessage, AIMessage) (***REMOVED***420).
