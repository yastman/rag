...
class PropertyBot:
    ...
    def __init__(self, config: BotConfig):
        ...
        ***REMOVED*** i18n hub (fluentogram) — initialized in start()
    ...
    def _register_handlers(self):
        ...
        ***REMOVED*** Menu buttons MUST be registered before the generic F.text handler (***REMOVED***628)
        ...
        ***REMOVED*** Phone collection FSM router (***REMOVED***628)
        from .handlers.phone_collector import phone_router

        try:
            self.dp.include_router(phone_router)
        except RuntimeError:
            ***REMOVED*** Router may already be attached in test environments with multiple bot instances
    ...
    async def cmd_start(self, message: Message, dialog_manager: Any = None):
        """Handle /start command — role-aware routing (***REMOVED***628)."""
        assert message.from_user is not None
        role = await self._resolve_user_role(message.from_user.id)

        if role == "manager":
            ***REMOVED*** Manager: keep aiogram-dialog when available + kommo enabled
            ...
            ***REMOVED*** Client: persistent ReplyKeyboard + welcome text (***REMOVED***628)
    ...
    async def handle_menu_button(self, message: Message) -> None:
        """Route ReplyKeyboard button press to the appropriate handler (***REMOVED***628)."""
    ...
    async def _handle_search(self, message: Message) -> None:
        """Dispatch «Подбор апартаментов» to agent pipeline (***REMOVED***628)."""
    ...
    async def _handle_viewing(self, message: Message) -> None:
        """Dispatch «Запись на осмотр» to agent pipeline (***REMOVED***628)."""
    ...
    async def handle_menu_action_text(self, message: Message, query_text: str) -> None:
        """Dispatch a fixed query_text to the agent pipeline from a ReplyKeyboard context (***REMOVED***628).

        Mirrors handle_menu_action but accepts a Message instead of CallbackQuery.
        """
        original_text = message.text
        message.text = query_text  ***REMOVED*** type: ignore[assignment]
        try:
            await self.handle_query(message)
        finally:
            message.text = original_text  ***REMOVED*** type: ignore[assignment]

    async def handle_service_callback(self, callback: CallbackQuery) -> None:
        """Handle svc: inline button clicks — service list and service cards (***REMOVED***628)."""
        ...
        if action == "back":
            ...
            if svc and callback.message:
                ...
                await callback.message.edit_text(svc["card_text"], reply_markup=kb)  ***REMOVED*** type: ignore[union-attr]
            await callback.answer()
    ...
    async def handle_cta_callback(self, callback: CallbackQuery, state: Any = None) -> None:
        """Handle cta: CTA button clicks — get_offer starts phone collection, manager handoff (***REMOVED***628)."""
    ...
    async def handle_favorite_callback(self, callback: CallbackQuery, state: Any = None) -> None:
        """Handle fav: inline button clicks — add/remove/viewing favorites (***REMOVED***628)."""
        data = callback.data or ""
        parts = data.split(":", 2)  ***REMOVED*** fav:action:property_id
    ...
    async def start(self):
        ...
        ***REMOVED*** Set chat menu button to show commands list (***REMOVED***628)
