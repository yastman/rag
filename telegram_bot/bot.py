...
def _extract_current_turn(messages: list[Any]) -> list[Any]:
    """Extract only current-turn messages from checkpointer history (***REMOVED***507).

    The agent checkpointer returns ALL messages across turns. For scoring,
    we need only the messages from the current turn (after the last HumanMessage).
    """
    last_human_idx = -1
    for i in range(len(messages) - 1, -1, -1):
        msg_type = getattr(messages[i], "type", None)
        if msg_type == "human":
            last_human_idx = i
            break
    if last_human_idx < 0:
        return messages  ***REMOVED*** No HumanMessage found — return all (shouldn't happen)
...
class PropertyBot:
    ...
    async def cmd_start(self, message: Message, dialog_manager: Any = None):
        ...
        if dialog_manager is not None:
            ...
            ***REMOVED*** Fallback (dialog not initialized)
        ...
        with propagate_attributes(
            ...
            ***REMOVED*** Write Langfuse trace metadata
            ...
            if tid:
                ...
                ***REMOVED*** Extract current-turn messages only (***REMOVED***507): checkpointer returns
                ***REMOVED*** ALL history; we need only messages from the current turn
                ***REMOVED*** (everything after the last HumanMessage we sent).
