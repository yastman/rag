...
async def check_dependencies(config: BotConfig) -> dict[str, bool]:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ***REMOVED*** ----- Redis deep check (REDIS-01) -----
        try:
        ...
        ***REMOVED*** ----- Redis synthetic cache verification (REDIS-02) -----
        if results.get("redis"):
            ...
            ***REMOVED*** Skip cache verification when Redis itself is down
            results["redis_cache"] = False
            logger.warning("Preflight SKIP: Redis cache verify (Redis unreachable)")

        ***REMOVED*** Qdrant collection
        try:
        ...
        ***REMOVED*** BGE-M3 embedding service
        try:
        ...
        ***REMOVED*** LiteLLM proxy (/health/liveliness doesn't require auth)
        try:
