...
class RetrieverService:
    ...
    def __init__(
        ...
        ***REMOVED*** Initialize client with error handling
        try:
            if api_key:
    ...
    def _build_base_filter(self) -> "QdrantFilter | None":
        ...
        """
        ***REMOVED*** No base filter - search all documents in collection
        return None
    ...
    def _build_filter(self, filters: dict[str, Any]) -> "QdrantFilter | None":
        ...
        ***REMOVED*** Import lazily so returned models stay consistent with current qdrant_client module state.
        from qdrant_client import models

        conditions = []

        for field, value in filters.items():
            ***REMOVED*** Exact match for strings and integers
            ...
            ***REMOVED*** Range filter for numeric comparisons
        ...
        ***REMOVED*** Return None if no conditions were built
        if not conditions:
            return None

        return models.Filter(must=conditions)  ***REMOVED*** type: ignore[arg-type]
