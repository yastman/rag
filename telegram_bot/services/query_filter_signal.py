...
_FLOOR_RE = re.compile(r"\b(?:на\s*)?\d+\s*этаж\w*\b", re.IGNORECASE)
_DISTANCE_TO_SEA_RE = re.compile(
    r"\b(?:до|от)\s+мор[яе]\s*\d+\s*(?:м|метр\w*)\b",
    re.IGNORECASE,
)
_MAINTENANCE_RE = re.compile(
    r"\b(?:такс\w*\s+поддержк\w*|поддержк\w*)[^\d]{0,20}\d+\b",
    re.IGNORECASE,
)
_BATHROOMS_RE = re.compile(r"\b\d+\s*(?:санузл\w*|ванн\w*)\b", re.IGNORECASE)
_FURNITURE_RE = re.compile(r"\b(?:с\s+мебелью|мебел\w*)\b", re.IGNORECASE)
_YEAR_ROUND_RE = re.compile(r"\b(?:круглогодич\w*|year[- ]round)\b", re.IGNORECASE)
