...
- Верни ТОЛЬКО JSON без пояснений"""
    ...
    def __init__(self, cache_service: Any, llm_service: Any) -> None:
        ...
        self.context_ttl = 30 * 24 * 3600  ***REMOVED*** 30 days
    ...
    async def update_from_query(self, user_id: int, query: str) -> dict[str, Any]:
        ...
        ***REMOVED*** Update interaction stats
        context["interaction_count"] += 1
        context["last_queries"] = [query, *context["last_queries"][:4]]
        context["updated_at"] = datetime.now(timezone.utc).isoformat()

        ***REMOVED*** Extract preferences every 3rd query or if empty
    ...
    async def _extract_preferences(self, query: str, current: dict[str, Any]) -> dict[str, Any]:
        ...
        ***REMOVED*** Parse JSON from response
        response_clean = response.strip()
        ***REMOVED*** Handle markdown code blocks
        if response_clean.startswith("```"):
            lines = response_clean.split("\n")
            ***REMOVED*** Find content between ``` markers
            json_lines = []
            in_block = False
            for line in lines:
                if line.startswith("```"):
