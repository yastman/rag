...
***REMOVED*** Personal markers that indicate query needs personalization (Russian + English)
PERSONAL_MARKERS = [
    ***REMOVED*** Russian markers
    r"\bмне\b",
    r"\bя предпочитаю\b",
    ...
    r"\bучитывая мои\b",
    r"\bпод мои\b",
    ***REMOVED*** English markers (for bilingual users)
    r"\bfor me\b",
    r"\bi prefer\b",
    r"\bmy budget\b",
    r"\blike last time\b",
...
***REMOVED*** Compile regex patterns for efficiency
...
def is_personalized_query(query: str, user_context: dict[str, Any] | None = None) -> bool:
    ...
    """
    ***REMOVED*** Rule 1: Check for personal markers in query
    ...
    ***REMOVED*** Rule 2: Check if user has stored preferences
    if user_context:
...
Сохрани факты, адаптируй подачу. Русский язык."""
    ...
    def _build_prompt(self, cached_response: str, user_context: dict[str, Any]) -> str:
        ...
        return self.PERSONALIZATION_PROMPT.format(
            cached_response=cached_response[:500],  ***REMOVED*** Limit to 500 chars
    ...
    async def personalize(
        ...
        ***REMOVED*** If no preferences, return cached response as-is
        if not prefs:
        ...
        try:
            ...
            return cached_response  ***REMOVED*** Fallback to cached
