"""Load service content from YAML config (***REMOVED***628)."""
...
def load_services_config() -> dict[str, Any]:
    ...
    if path.exists():
        with open(path, encoding="utf-8") as f:
            return yaml.safe_load(f)  ***REMOVED*** type: ignore[no-any-return]
...
def get_service_card(service_key: str) -> dict[str, Any] | None:
    ...
    return config.get("services", {}).get(service_key)  ***REMOVED*** type: ignore[no-any-return]


def get_promotions() -> list[dict[str, Any]]:
    """Get promotions list from config."""
    config = load_services_config()
    return config.get("promotions", [])  ***REMOVED*** type: ignore[no-any-return]


def get_entry_point_config(key: str) -> dict[str, Any] | None:
    """Get entry point config by key (viewing, manager)."""
    config = load_services_config()
    return config.get("entry_points", {}).get(key)  ***REMOVED*** type: ignore[no-any-return]
