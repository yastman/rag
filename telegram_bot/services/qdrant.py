...
class QdrantService:
    ...
    async def hybrid_search_rrf(
        ...
        ***REMOVED*** Build search params with quantization
        ***REMOVED*** quantization_ignore: None=use default, True=skip quant, False=force quant
        search_params = None
        use_quant = self._use_quantization if quantization_ignore is None else not quantization_ignore
        if use_quant:
            search_params = models.SearchParams(
                quantization=models.QuantizationSearchParams(
                    ignore=False,  ***REMOVED*** Explicitly use quantization
                    rescore=self._quantization_rescore,
                    oversampling=self._quantization_oversampling,
                )
            )
        elif quantization_ignore is True:
            ***REMOVED*** Explicitly disable quantization for this request (A/B testing)
            search_params = models.SearchParams(
                quantization=models.QuantizationSearchParams(ignore=True)
            )

        ***REMOVED*** Execute RRF fusion search
    ...
    async def search_with_score_boosting(
        ...
        ***REMOVED*** Base search without boosting
        ...
        ***REMOVED*** Search with score boosting via Query API
        ***REMOVED*** Note: Qdrant Query API with formulas requires Qdrant 1.10+
        ***REMOVED*** For older versions, we do post-processing boosting
        try:
            result = await self._client.query_points(
                collection_name=self._collection_name,
                query=dense_vector,
                using=self._dense_vector_name,
                query_filter=self._build_filter(filters),
                limit=top_k * 2,  ***REMOVED*** Overfetch for boosting
                with_payload=True,
                search_params=search_params,
            )

            ***REMOVED*** Post-process with freshness boosting
            ...
            for point in points:
                ...
                ***REMOVED*** Get datetime from payload
                ...
                if created_at:
                    try:
                        ...
                        ***REMOVED*** Calculate exp_decay boost
                        age_seconds = (now - dt).total_seconds()
                        decay = np.exp(-age_seconds / scale_seconds)
                        boosted_score = base_score + 0.1 * decay  ***REMOVED*** Small boost
            ...
            ***REMOVED*** Sort by boosted score and take top_k
        ...
        ***REMOVED*** Iteratively select by MMR
        while len(selected_indices) < min(top_k, len(points)):
            ...
            for i in range(len(points)):
                ...
                ***REMOVED*** Relevance term (normalized score)
                relevance = points[i]["score"]

                ***REMOVED*** Max similarity to already selected
                ...
                ***REMOVED*** MMR: lambda * relevance - (1-lambda) * max_similarity
