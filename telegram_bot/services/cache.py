...
***REMOVED*** Lazy imports for redisvl (heavy dependency chain via voyageai SDK)
***REMOVED*** These are imported inside initialize() to speed up test collection
if TYPE_CHECKING:
...
***REMOVED*** Cache versioning - bump when changing cache structure or models
***REMOVED*** NOTE: SemanticMessageHistory index is versioned as:
***REMOVED***   rag_conversations:{CACHE_SCHEMA_VERSION}:{vectorizer_id}
***REMOVED***   e.g. rag_conversations:v2:userbase768 or rag_conversations:v2:voyage1024
***REMOVED*** Old indices are NOT deleted automatically; clean up manually if needed.
CACHE_SCHEMA_VERSION = "v2"

***REMOVED*** TTL strategy: pattern-based TTL for semantic cache entries
***REMOVED*** Price/availability queries change frequently → short TTL
PRICE_PATTERNS = re.compile(
    r"цен[аыу]|стоимост|ціна|вартіст|наличи[еи]|наявніст|скидк|знижк|акци[яию]|тариф",
    re.IGNORECASE,
)
***REMOVED*** FAQ/stable content queries → long TTL
...
def get_ttl_for_query(query: str, answer: str = "") -> int:
    ...
    if PRICE_PATTERNS.search(query):
        return 1800  ***REMOVED*** 30 min
    if FAQ_PATTERNS.search(query):
        return 86400  ***REMOVED*** 24h
...
class CacheService:
    ...
    def __init__(
        self,
        redis_url: str,
        semantic_cache_ttl: int = 48 * 3600,  ***REMOVED*** 48 hours
        ...
        distance_threshold: float | None = None,  ***REMOVED*** default from SEMANTIC_CACHE_THRESHOLD env
    ):
        ...
        ***REMOVED*** Resolve distance threshold: explicit arg > env var > 0.10
        if distance_threshold is None:
            distance_threshold = float(os.getenv("SEMANTIC_CACHE_THRESHOLD", "0.10"))

        ***REMOVED*** TTL configuration
        ...
        ***REMOVED*** Initialize Redis client for key-value operations (Tier 2)
        self.redis_client: redis.Redis | None = None

        ***REMOVED*** Native RedisVL SemanticCache (Tier 1)
        self.semantic_cache: SemanticCache | None = None

        ***REMOVED*** Native RedisVL EmbeddingsCache (Tier 1)
        self.embeddings_cache: EmbeddingsCache | None = None

        ***REMOVED*** Cosine distance threshold for semantic cache
        self.distance_threshold = distance_threshold

        ***REMOVED*** Native RedisVL SemanticMessageHistory (for conversation context)
        self.message_history: SemanticMessageHistory | None = None

        ***REMOVED*** Last semantic cache hit distance (for pipeline metrics observation)
    ...
    async def initialize(self):
        ...
        """
        ***REMOVED*** Lazy import redisvl components (heavy dependency chain)
        from redisvl.extensions.cache.embeddings import EmbeddingsCache
        from redisvl.extensions.cache.llm import SemanticCache
        from redisvl.extensions.message_history import SemanticMessageHistory
        ...
        try:
            ***REMOVED*** Initialize async Redis client for Tier 2 (key-value caches)
            ...
            ***REMOVED*** Supports local USER-base (best for RU) or Voyage API fallback
            ...
            try:
                if use_local:
                    ***REMOVED*** Fast-Lane: reuse BGE-M3 dense vector (1024-dim) for semantic cache.
                    ***REMOVED*** Vectors are passed via vector= parameter to acheck/astore,
                    ***REMOVED*** so the vectorizer is only used for Redis index schema creation.
            ...
            ***REMOVED*** Initialize native EmbeddingsCache (Tier 1)
            try:
    ...
    async def clear_semantic_index(self) -> int:
        ...
        ***REMOVED*** Count entries before clear (approximate)
        count = 0
        if self.redis_client:
    ...
    ***REMOVED*** ========== TIER 1: Semantic Cache (LLM Answers) ==========
    ...
    async def check_semantic_cache(
        ...
        try:
            ***REMOVED*** Lazy import Tag (redisvl dependency chain is heavy)
            from redisvl.query.filter import Tag

            start = time.time()

            ***REMOVED*** Use override if provided, otherwise use default
            effective_threshold = (
                threshold_override if threshold_override is not None else self.distance_threshold
            )

            ***REMOVED*** Build filter expression for multi-user isolation
            filter_expr = Tag("language") == language

            if user_id is not None:
                filter_expr = filter_expr & (Tag("user_id") == str(user_id))

            ***REMOVED*** Fast-Lane: pass pre-computed vector, skip internal vectorizer
            check_kwargs: dict = {
            ...
            ***REMOVED*** Timeout guard — cache miss is better than slow cache
            try:
    ...
    async def store_semantic_cache(
        ...
        try:
            ***REMOVED*** Build filters for multi-user isolation
            filters = {
                "language": language,
    ...
    ***REMOVED*** ========== TIER 1: Embeddings Cache (Native RedisVL) ==========
    ...
    ***REMOVED*** ========== TIER 1.5: Sparse Embedding Cache ==========
    ...
    async def get_cached_sparse_embedding(
        ...
        try:
            ...
            if cached:
                ...
                ***REMOVED*** Decode from Redis hash format
                return {
    ...
    async def store_sparse_embedding(
        ...
        ttl: int = 86400,  ***REMOVED*** 24 hours default
    ):
    ...
    ***REMOVED*** ========== TIER 2: QueryAnalyzer Cache ==========
    ...
    ***REMOVED*** ========== TIER 2: Qdrant Search Cache ==========
    ...
    async def get_cached_search(
        ...
        try:
            ***REMOVED*** Build cache key from embedding hash + filters + index version
            embedding_hash = self._hash_key(str(embedding[:10]))  ***REMOVED*** use first 10 dims
    ...
    ***REMOVED*** ============= Conversation Memory (Task 2.2) =============
    ...
    async def store_conversation_message(
        ...
        ***REMOVED*** Store as JSON string in Redis LIST
        await self.redis_client.lpush(key, message)
        ***REMOVED*** Keep only last N messages
    ...
    ***REMOVED*** ============= Semantic Message History (Task 4) =============
        ...
        try:
            ...
            ***REMOVED*** Count keys per known prefix using SCAN (non-blocking)
            ...
            for prefix in known_prefixes:
                ...
                async for _key in self.redis_client.scan_iter(match=f"{prefix}*", count=500):
                    count += 1
                ***REMOVED*** Use short display name (strip version suffix)
                display_name = prefix.rstrip(":")
                prefix_counts[display_name] = count

            ***REMOVED*** Total keys in DB
