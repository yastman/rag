...
class CacheService:
    ...
    def __init__(
        ...
        distance_threshold: float = 0.85,  ***REMOVED*** semantic similarity threshold
    ):
        ...
        ***REMOVED*** Initialize Redis client for key-value operations
        self.redis_client: Optional[redis.Redis] = None

        ***REMOVED*** Initialize RedisVL Embeddings cache (Tier 1)
        self.embeddings_cache: Optional[EmbeddingsCache] = None

        ***REMOVED*** Distance threshold for semantic cache
    ...
    async def initialize(self):
        ...
        try:
            ***REMOVED*** Initialize async Redis client for Tier 2
            ...
            ***REMOVED*** Initialize Semantic Cache using Redis vector search
            ***REMOVED*** Создаем векторный индекс для KNN поиска похожих ответов
            try:
            ...
            ***REMOVED*** Initialize Embeddings Cache (Tier 1)
            try:
    ...
    ***REMOVED*** ========== TIER 1: Semantic Cache (LLM Answers) ==========
    ...
    async def check_semantic_cache(self, query_embedding: list[float]) -> Optional[str]:
        ...
        try:
            ...
            ***REMOVED*** Convert embedding to bytes for Redis vector search
            vector_bytes = np.array(query_embedding, dtype=np.float32).tobytes()

            start = time.time()

            ***REMOVED*** KNN search: find most similar cached answer
            ***REMOVED*** Using COSINE distance, threshold = distance_threshold (0.85 = high similarity)
            ...
            ***REMOVED*** Result format: [count, key1, [field1, value1, field2, value2, ...]]
            if result and len(result) > 1 and int(result[0]) > 0:
                ***REMOVED*** Parse field-value pairs from result
                fields = result[2]
                field_dict = {}
                for i in range(0, len(fields), 2):
                    field_dict[fields[i]] = fields[i + 1]

                ***REMOVED*** Get similarity score (cosine distance: 0 = identical, 1 = opposite)
                score = float(field_dict.get("score", 1.0))
                similarity = 1 - score  ***REMOVED*** Convert distance to similarity
    ...
    async def store_semantic_cache(self, query: str, query_embedding: list[float], answer: str):
        ...
        try:
            ...
            ***REMOVED*** Generate unique key for this cache entry
            key = f"rag:semantic:{self._hash_key(query)}"

            ***REMOVED*** Convert embedding to bytes for Redis vector field
            vector_bytes = np.array(query_embedding, dtype=np.float32).tobytes()

            ***REMOVED*** Store as Redis Hash with vector field
            mapping = {
                "query_vector": vector_bytes,
                "answer": answer,
                "timestamp": int(time.time()),
                "query": query,  ***REMOVED*** Store original query for debugging
            }

            await self.redis_client.hset(key, mapping=mapping)

            ***REMOVED*** Set TTL (48 hours)
