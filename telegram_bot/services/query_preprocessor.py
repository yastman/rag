...
class HyDEGenerator:
    ...
    ***REMOVED*** System prompt for generating hypothetical documents
...
"""
    ...
    def __init__(
        ...
        """
        self.api_key = api_key or "not-needed"  ***REMOVED*** LiteLLM proxy may not need key
        ...
        try:
            response = await self.client.post(
                ...
                json={
                    ...
                    "temperature": 0.7,  ***REMOVED*** Some creativity for diverse results
                    "max_tokens": 200,  ***REMOVED*** Short hypothetical doc
                },
            )
            ...
            ***REMOVED*** Fallback: return original query
            return query
    ...
    ***REMOVED*** Patterns requiring strict cache threshold
    ...
    def normalize_translit(self, query: str) -> str:
        ...
        for latin, cyrillic in self.TRANSLIT_MAP.items():
            ***REMOVED*** Case-insensitive replacement
    ...
    def get_rrf_weights(self, query: str) -> tuple[float, float]:
        ...
        for pattern in self.EXACT_PATTERNS:
            if re.search(pattern, query, re.IGNORECASE):
                ...
                return (0.2, 0.8)  ***REMOVED*** Favor sparse for exact matches

        return (0.6, 0.4)  ***REMOVED*** Default: favor dense for semantic queries
    ...
    def get_cache_threshold(self, query: str) -> float:
        ...
        for pattern in self.STRICT_CACHE_PATTERNS:
            if re.search(pattern, query, re.IGNORECASE):
                ...
                return 0.05  ***REMOVED*** 95% similarity required

        return 0.10  ***REMOVED*** Default 90% similarity
    ...
    def should_use_hyde(self, query: str, min_words: int = 5) -> bool:
        ...
        """
        ***REMOVED*** Don't use HyDE for exact identifier queries
        if self.has_exact_identifier(query):
            logger.debug("HyDE skipped: query has exact identifiers")
            return False

        ***REMOVED*** Use HyDE for short queries (< min_words)
    ...
    def analyze(
        ...
        ***REMOVED*** Determine if HyDE should be used for this specific query
