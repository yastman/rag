...
***REMOVED*** Patterns for simple queries (light RAG, no rerank)
SIMPLE_PATTERNS = [
    ***REMOVED*** Price questions
    r"^сколько стоит\b",
    r"^какая цена\b",
    r"^price\b",
    ***REMOVED*** Single property queries
    r"^\d+\s*(комнат|спал)",  ***REMOVED*** "2 комнаты"
...
def needs_rerank(query_type: QueryType, result_count: int) -> bool:
    ...
    """
    ***REMOVED*** Skip rerank for simple queries
    if query_type == QueryType.SIMPLE:
        logger.debug("Skipping rerank for SIMPLE query")
        return False

    ***REMOVED*** Skip rerank if too few results
    if result_count <= 2:
