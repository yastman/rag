...
class LLMService:
    ...
    async def generate_answer(
        ...
        Wrapped in ``@observe`` (***REMOVED***1660) so the auto-traced generation produced
        ...
        Per the audit correction on ***REMOVED***1660 the wrapper is a *plain span*, not
    ...
    async def stream_answer(
        ...
        Wrapped in ``@observe`` (***REMOVED***1660) as a *plain span* — NOT
        ``as_type="generation"``. Per the audit on ***REMOVED***1660: ``langfuse.openai``
        ...
        try:
            ...
            stream = await self.client.chat.completions.create(
                ...
                name="stream-answer",  ***REMOVED*** type: ignore[call-overload]  ***REMOVED*** langfuse kwarg
            )
    ...
    async def generate(self, prompt: str, max_tokens: int = 200) -> str:
        ...
        Wrapped in ``@observe`` (***REMOVED***1660) as a plain span; ``langfuse.openai``
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[{"role": "user", "content": prompt}],  ***REMOVED*** type: ignore[arg-type]
                temperature=0.3,
                max_tokens=max_tokens,
                name="generate-simple",  ***REMOVED*** type: ignore[call-overload]  ***REMOVED*** langfuse kwarg
            )
