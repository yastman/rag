...
***REMOVED*** Default confidence threshold for triggering fallback response
...
class LLMService:
    ...
    async def generate_answer(
        ...
        ***REMOVED*** Track generation start
        ...
        try:
            ***REMOVED*** Build context from chunks
...
Ответь на вопрос на основе контекста выше."""
            ...
            ***REMOVED*** Call LLM API
            response = await self.client.post(
                ...
                json={
                    ...
                    "messages": messages,
                    "temperature": 0.7,
                    "max_tokens": 4096,  ***REMOVED*** Reasoning models need headroom beyond thinking tokens
                },
            )
            ...
            ***REMOVED*** Support reasoning models: content may be in "reasoning_content"
            raw_answer = (
                message.get("content")
            ...
            ***REMOVED*** Parse confidence if requested
            if with_confidence:
                result = self._parse_confidence_response(raw_answer, question, context_chunks)
                ***REMOVED*** Track completion with confidence
                langfuse.update_current_generation(
                    output={
            ...
            ***REMOVED*** Track completion with usage
            langfuse.update_current_generation(
...
Если контекст не содержит релевантной информации, установи confidence < 0.5."""
    ...
    def _parse_confidence_response(
        ...
        try:
            ...
            ***REMOVED*** Handle cases where LLM wraps JSON in markdown code blocks
            json_match = re.search(r"```(?:json)?\s*(\{.*?\})\s*```", raw_response, re.DOTALL)
            if json_match:
                json_str = json_match.group(1)
            else:
                ***REMOVED*** Try to find raw JSON
            ...
            ***REMOVED*** Clamp confidence to valid range
            ...
            ***REMOVED*** Return raw response with medium confidence (parsing failed)
            return ConfidenceResult(
                answer=raw_response,
                confidence=0.5,
...
Ответь на вопрос на основе контекста выше.""",
            ...
            ***REMOVED*** Stream LLM response
            async with self.client.stream(
                ...
                json={
                    ...
                    "temperature": 0.7,
                    "max_tokens": 4096,  ***REMOVED*** Reasoning models need headroom beyond thinking tokens
                    "stream": True,  ***REMOVED*** Enable streaming
                },
                timeout=60.0,
            ) as response:
                response.raise_for_status()

                ***REMOVED*** Parse SSE stream
                async for line in response.aiter_lines():
                    if not line.strip():
                        continue

                    ***REMOVED*** SSE format: "data: {...}"
                    if line.startswith("data: "):
                        data_str = line[6:]  ***REMOVED*** Remove "data: " prefix

                        ***REMOVED*** Skip [DONE] marker
                        if data_str == "[DONE]":
                            break
                        ...
                        try:
                            ...
                            ***REMOVED*** Support both regular and reasoning models:
                            ***REMOVED*** reasoning models (e.g. zai-glm-4.7) send tokens
                            ***REMOVED*** as "reasoning_content" or "reasoning" instead of "content"
                            content = (
                                delta.get("content")
    ...
    def _format_context(self, chunks: list[dict[str, Any]]) -> str:
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            ***REMOVED*** Add metadata info if available
            meta_str = ""
    ...
    def _get_fallback_answer(self, question: str, context_chunks: list[dict[str, Any]]) -> str:
        ...
        ***REMOVED*** Format first 3 results as simple text
