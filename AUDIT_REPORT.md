***REMOVED*** Отчет: ревью / аудит репозитория `rag-fresh` (восстановлен и дополнен)
...
***REMOVED******REMOVED*** 2) Сильные стороны (что уже хорошо)
...
***REMOVED******REMOVED*** 3) Критические находки (P0 — исправить немедленно)

***REMOVED******REMOVED******REMOVED*** P0.1 Секреты/ключи и `.env` файлы
...
***REMOVED******REMOVED******REMOVED*** P0.2 CI выключен
...
***REMOVED******REMOVED******REMOVED*** P0.3 Redis образ не соответствует использованию RedisVL (функциональный риск)
...
***REMOVED******REMOVED******REMOVED*** P0.4 Docker dev-стек может “не подниматься” из-за healthcheck/depends_on
...
***REMOVED******REMOVED*** 4) Высокий приоритет (P1 — 1–2 недели)

***REMOVED******REMOVED******REMOVED*** P1.1 Конфигурация: import-time сайд-эффекты и “ломкие” импорты
...
***REMOVED******REMOVED******REMOVED*** P1.2 Dependency management: рассинхрон `pyproject.toml` vs `requirements.txt`
...
***REMOVED******REMOVED******REMOVED*** P1.3 Несостыковки Docker-документации и compose
...
***REMOVED******REMOVED******REMOVED*** P1.4 Тяжелые eager-import’ы в `telegram_bot/services`
...
***REMOVED******REMOVED******REMOVED*** P1.5 Тесты и observability: не герметичный OTEL export
...
***REMOVED******REMOVED*** 5) Средний приоритет (P2 — планово)

***REMOVED******REMOVED******REMOVED*** P2.1 Логи/ошибки: `print()` в коде
...
***REMOVED******REMOVED******REMOVED*** P2.2 Prompt-injection / контентные атаки
...
***REMOVED******REMOVED******REMOVED*** P2.3 Лицензирование и “чистота” репозитория
...
***REMOVED******REMOVED******REMOVED*** P2.4 Дублирование компонент (два “мира”)
...
***REMOVED******REMOVED*** 6) SDK-first рекомендации (в сторону “меньше кастома”)

***REMOVED******REMOVED******REMOVED*** 6.1 LLM: уйти от ручного HTTP/SSE в пользу SDK
...
***REMOVED******REMOVED******REMOVED*** 6.2 RedisVL: сделать окружение корректным (Redis Stack)

Если semantic cache — часть продукта, то Redis Stack обязателен (иначе RedisVL vector/semantic функции не гарантируются).

***REMOVED******REMOVED******REMOVED*** 6.3 Qdrant: закрепить один gateway
...
***REMOVED******REMOVED*** 7) Дорожная карта (приоритеты)

Ниже — приоритизированный план работ “как серия PR”, с критериями готовности (Definition of Done).

***REMOVED******REMOVED******REMOVED*** P0 (сегодня/завтра) — блокеры/риски
...
***REMOVED******REMOVED******REMOVED*** P1 (1–2 недели) — воспроизводимость/поддерживаемость
...
***REMOVED******REMOVED******REMOVED*** P2 (1–2 месяца) — SDK-first и консолидация
...
***REMOVED******REMOVED*** 8) Быстрые проверки (чеклист)
...
***REMOVED******REMOVED*** 9) Docker/Compose контейнеры (дополнение: консолидировано из `DOCKER_CONTAINERS_REPORT.md`)

***REMOVED******REMOVED******REMOVED*** 9.1 Снимок окружения (на машине аудита)
...
***REMOVED******REMOVED******REMOVED*** 9.2 Инвентаризация: что обнаружено в репозитории
...
***REMOVED******REMOVED******REMOVED*** 9.3 Текущее состояние контейнеров (по факту на машине аудита)
...
***REMOVED******REMOVED******REMOVED*** 9.4 Порты наружу (поверхность атаки + конфликты)
...
***REMOVED******REMOVED******REMOVED*** 9.5 Лимиты ресурсов: применяются не везде
...
***REMOVED******REMOVED******REMOVED*** 9.6 Dev-секреты/дефолты (важно даже в dev)
...
***REMOVED******REMOVED******REMOVED*** 9.7 Healthchecks и зависимости (`depends_on`)
...
***REMOVED******REMOVED******REMOVED*** 9.8 Postgres + LiteLLM: миграции схемы
...
***REMOVED******REMOVED******REMOVED*** 9.9 `.dockerignore` и риск утечки секретов в build context
...
***REMOVED******REMOVED******REMOVED*** 9.10 Рекомендации (приоритет)
