...
***REMOVED*** Set specific loggers
logging.getLogger("src.ingestion.indexer").setLevel(logging.INFO)
logging.getLogger("httpx").setLevel(logging.WARNING)  ***REMOVED*** Suppress HTTP logs
...
async def main():
    ...
    ***REMOVED*** Initialize Docling DocumentConverter and HybridChunker
    print("Initializing Docling DocumentConverter and HybridChunker...")
    doc_converter = DocumentConverter()
    chunker = HybridChunker(
        tokenizer="BAAI/bge-m3",  ***REMOVED*** HybridChunker accepts model ID directly
    ...
    ***REMOVED*** Process all files
    ...
    for file_path_str in args.files:
        ...
        ***REMOVED*** CSV files: use row-based chunking (1 row = 1 chunk)
        ...
        ***REMOVED*** Other files: use Docling + HybridChunker
        print(f"📄 Converting: {file_path.name}...")
        result = doc_converter.convert(file_path)
        dl_doc = result.document

        ***REMOVED*** Export to markdown to get character count
        ...
        ***REMOVED*** Convert HybridChunker chunks to our Chunk format
