...
***REMOVED*** Add src/ to path for Langfuse import
...
try:
    ...
    ***REMOVED*** Dummy decorator if Langfuse not available
...
class ContextualRetrievalZAIAsync:
    ...
    def __init__(
        ...
        max_retries: int = 3,
        rate_limit_delay: float = 0.5,  ***REMOVED*** Reduced from 1.2s
        max_concurrent: int = 10,  ***REMOVED*** Parallel requests
    ):
        """
        ...
        ***REMOVED*** GLM Coding Plan endpoint
    ...
    async def situate_context_with_metadata(
        ...
        """
        ***REMOVED*** Update Langfuse metadata (if available)
        if LANGFUSE_AVAILABLE:
            try:
                ...
                pass  ***REMOVED*** Silently fail if Langfuse not configured

        async with self.semaphore:  ***REMOVED*** Rate limiting
            for attempt in range(self.max_retries):
                try:
                    ***REMOVED*** OPTIMIZED prompt: only chunk + minimal context
...
Важливо: JSON має бути валідний (без коментарів, без trailing commas)."""
                    ...
                    ***REMOVED*** Track tokens
                    if "usage" in result:
                        ...
                        ***REMOVED*** Update Langfuse with usage (if available)
                        if LANGFUSE_AVAILABLE:
                            try:
                                langfuse = get_client()
                                langfuse.update_current_generation(
                    ...
                    ***REMOVED*** Small delay for rate limiting
