...
def detect_tables_heuristic(text: str) -> bool:
    ...
    ***REMOVED*** NEGATIVE indicators: Legal document structure (NOT tables)
    legal_keywords = [
        "стаття",
        "статья",  ***REMOVED*** Article
        "розділ",
        "раздел",  ***REMOVED*** Section
        "глава",  ***REMOVED*** Chapter
        "книга",  ***REMOVED*** Book
        "кодекс",
        "кодекса",  ***REMOVED*** Code
    ]

    ***REMOVED*** If document is clearly legal structure → NOT a table document
    legal_markers = sum(1 for kw in legal_keywords if kw in text_lower)
    if legal_markers > 3:
        ***REMOVED*** High density of legal markers → skip table detection
        return False

    ***REMOVED*** POSITIVE indicators: Real tables
    table_keywords = [
        "таблиця",
        "таблица",
        "table",
        ...
        "+---+",
        "+===+",  ***REMOVED*** ASCII table markers
    ]

    ***REMOVED*** Strong table indicators
    has_table_keywords = any(kw in text_lower for kw in table_keywords)

    if has_table_keywords:
        return True

    ***REMOVED*** Check for aligned columns (but only if NOT legal document)
    ***REMOVED*** Real tables: many short aligned lines
    ***REMOVED*** Legal docs: few aligned lines (section headers)
    lines = text.split("\n")
    aligned_lines = 0
    short_aligned_lines = 0  ***REMOVED*** Lines < 80 chars

    for i in range(len(lines) - 2):
        len1, len2, len3 = len(lines[i]), len(lines[i + 1]), len(lines[i + 2])

        ***REMOVED*** Similar length lines
        if abs(len1 - len2) < 10 and abs(len2 - len3) < 10 and 20 < len1 < 150:
            aligned_lines += 1

            ***REMOVED*** Short lines more likely tables
            if len1 < 80:
                short_aligned_lines += 1

    ***REMOVED*** Real tables typically have many short aligned lines
    ***REMOVED*** Legal docs have few long aligned sections
...
def detect_pdf_complexity(pdf_path: str) -> dict:
    ...
    try:
        ...
        ***REMOVED*** Check up to 3 pages for fast detection
        pages_to_check = min(3, len(doc))

        ***REMOVED*** Check 1: Text layer presence
        sample_text = ""
        ...
        ***REMOVED*** Check 2: Image count (scans, photos)
        image_count = 0
        for page_num in range(pages_to_check):
        ...
        ***REMOVED*** Check 3: Table detection (heuristic)
        ...
        ***REMOVED*** On error, default to Docling (safer)
        return {
            "use_docling": True,
...
def docling_chunk(pdf_path: str, max_retries: int = 3) -> dict:
    ...
    ***REMOVED*** For DOCX files, detect if it's a legal document
    if file_ext == ".docx":
        print("  ✅ DOCX format detected → Checking document type...")

        ***REMOVED*** Check if it's a Ukrainian legal document (has "Стаття" pattern)
        try:
            import fitz
            ...
            ***REMOVED*** Check for Ukrainian legal markers
        ...
        ***REMOVED*** PHASE 1.1: Smart Detection for PDF
    ...
    ***REMOVED*** If simple text document → skip Docling entirely!
    if not detection["use_docling"]:
        ...
        try:
            ...
            ***REMOVED*** Continue to Docling below

    ***REMOVED*** Docling path (for complex PDFs or if PyMuPDF failed)
    ***REMOVED*** Adaptive timeout based on file size
    ***REMOVED*** Rule: ~1 minute per 1 MB + 2 min base
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                ***REMOVED*** All Docling attempts failed → Use PyMuPDF fallback
                ...
                try:
                    ...
                    ***REMOVED*** Convert to Docling-compatible format
                    return {
                        "chunks": chunks,
                        "processing_time": processing_time,
            ...
            if attempt == max_retries - 1:
                ***REMOVED*** Try PyMuPDF fallback for any other error
...
def generate_chunk_id(chunk_text: str, source: str, chunk_index: int) -> str:
    ...
    """
    ***REMOVED*** Include source + content for hash (allows cross-document dedup)
    content = f"{source}::{chunk_text}"
    hash_obj = hashlib.sha256(content.encode("utf-8"))

    ***REMOVED*** Use first 32 hex chars as UUID
    ***REMOVED*** This gives us 2^128 possible IDs (collision probability ~0)
...
async def process_chunk_async(
    ...
    try:
        ...
        ***REMOVED*** Generate stable UUID from content hash (enables auto-deduplication)
        chunk_id = generate_chunk_id(chunk_text, pdf_path, chunk_index)

        ***REMOVED*** Generate context + metadata (ASYNC, NO document context!)
        ...
        ***REMOVED*** IMPORTANT: Preserve metadata from chunker (article_number, etc.)
        ***REMOVED*** Chunker metadata has priority over contextualizer metadata
        for key in [
            "article_number",
            "article_title",
            "chapter",
        ...
        ***REMOVED*** Add graph edges
        metadata = add_graph_edges(metadata)

        ***REMOVED*** Prepare contextualized text
        embedded_text = f"{context_text}\n\n{chunk_text}"

        ***REMOVED*** Embed with BGE-M3 (sync call, but fast ~100ms)
        embeddings = bge_m3_encode(embedded_text)

        ***REMOVED*** Prepare payload
        payload = {
            "text": chunk_text,
            "chunk_id": chunk_id,  ***REMOVED*** Store hash for reference/debugging
        ...
        ***REMOVED*** Insert into Qdrant (sync call)
        qdrant_upsert(collection_name, chunk_id, vectors, payload)

        return {"status": "success", "chunk_id": chunk_id}

    except Exception as e:
        ***REMOVED*** chunk_id might not be defined if error happens early
...
async def process_document_contextual_kg_async(
    ...
    ***REMOVED*** Initialize MLflow if enabled
    mlflow_logger = None
    mlflow_run = None
    if enable_mlflow:
        try:
            mlflow_logger = MLflowRAGLogger(experiment_name="contextual_rag_ingestion")
            doc_short_name = Path(pdf_path).stem[:30]  ***REMOVED*** Truncate for run name
            ...
            mlflow_run.__enter__()  ***REMOVED*** Enter context manager
    ...
    ***REMOVED*** Initialize async contextualizer
    print_step(0, "INITIALIZATION")
    try:
        contextualizer = ContextualRetrievalZAIAsync(
            api_key=ZAI_API_KEY,
            rate_limit_delay=0.5,  ***REMOVED*** Reduced delay
    ...
    ***REMOVED*** Chunking (Smart Detection → Docling or PyMuPDF)
    ...
    try:
        ...
        ***REMOVED*** Show detection metrics if available
        ...
        ***REMOVED*** Show if Docling was skipped (fast path)
        ...
        ***REMOVED*** Show fallback reason if applicable
    ...
    ***REMOVED*** Process with progress tracking
    completed = 0
    ...
    print()  ***REMOVED*** New line after progress
    ...
    ***REMOVED*** Speedup calculation
    old_time_per_chunk = 8.21  ***REMOVED*** From original test
    ...
    ***REMOVED*** Print contextualizer stats
    ...
    ***REMOVED*** Log metrics to MLflow
    if enable_mlflow and mlflow_logger:
        ***REMOVED*** Calculate metadata coverage
        sample_chunks_for_coverage = chunks[: min(100, len(chunks))]  ***REMOVED*** Sample first 100
        ...
        ***REMOVED*** Calculate avg chunk size
        ...
        ***REMOVED*** Log sample chunks as artifact
        sample_chunks = chunks[:5]  ***REMOVED*** First 5 chunks
...
if __name__ == "__main__":
    ...
    ***REMOVED*** PDF file path - supports both positional and --pdf flag
    parser.add_argument(
    ...
    ***REMOVED*** Determine PDF path: --pdf flag takes priority, then positional, then default
    if args.pdf_flag:
    ...
    ***REMOVED*** Auto-generate collection name from PDF filename if not specified
    if not args.collection:
        pdf_basename = os.path.basename(args.pdf)
        ***REMOVED*** Remove extension and convert to lowercase with underscores
    ...
    ***REMOVED*** Auto-extract document name if not specified
    if not args.document_name:
        pdf_basename = os.path.basename(args.pdf)
        ***REMOVED*** Extract readable name from filename
        doc_name = os.path.splitext(pdf_basename)[0]
        ***REMOVED*** Clean up: remove dates, "as_of", etc.
    ...
    ***REMOVED*** Validate PDF file exists
    if not os.path.exists(args.pdf):
        print(f"❌ ERROR: PDF file not found: {args.pdf}")
        sys.exit(1)

    ***REMOVED*** Check API keys
    if not ZAI_API_KEY:
