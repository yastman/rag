...
class PyMuPDFChunker:
    ...
    ***REMOVED*** Ukrainian legal document patterns
    PATTERNS = {
        "book": re.compile(
        ...
        ***REMOVED*** Russian patterns (for Criminal Code RU)
        "book_ru": re.compile(
    ...
    def __init__(
        ...
        overlap_percent: float = 0.12,  ***REMOVED*** 12% overlap
    ):
        """
        Initialize chunker.
    ...
    def chunk_pdf(self, pdf_path: str) -> list[dict]:
        ...
        ***REMOVED*** Normalize text
        normalized_text = self.normalize_text(full_text)

        ***REMOVED*** Detect structure and chunk
    ...
    def normalize_text(self, text: str) -> str:
        ...
        """
        ***REMOVED*** 1. NFC normalization
        text = unicodedata.normalize("NFC", text)

        ***REMOVED*** 2. Remove soft hyphens
        text = text.replace("\u00ad", "")
        text = text.replace("\xad", "")  ***REMOVED*** Another variant

        ***REMOVED*** 3. Join broken lines (word continues on next line)
        ***REMOVED*** Pattern: "слово-\nслово" → "словослово"
        text = re.sub(r"(\w)-\s*\n\s*(\w)", r"\1\2", text)

        ***REMOVED*** 4. Join lines without punctuation
        ***REMOVED*** Pattern: "слово\nслово" → "слово слово"
        ***REMOVED*** BUT: DO NOT join if next line starts with legal structure markers
        ***REMOVED*** Use negative lookahead to prevent joining before Статья, Глава, etc.
        text = re.sub(
        ...
        ***REMOVED*** 5. Clean multiple spaces
        text = re.sub(r" +", " ", text)

        ***REMOVED*** 6. Clean multiple newlines
        text = re.sub(r"\n{3,}", "\n\n", text)

        ***REMOVED*** 7. Remove common page footers/headers patterns
        text = re.sub(r"\n\s*\d+\s*\n", "\n", text)  ***REMOVED*** Standalone page numbers
        text = re.sub(r"\n\s*Стор\.\s*\d+\s*\n", "\n", text)  ***REMOVED*** "Стор. 123"
    ...
    def _chunk_by_articles(
        ...
        for i, article_match in enumerate(articles):
            ...
            ***REMOVED*** Estimate tokens (rough: 1 token ≈ 4 chars for Ukrainian)
            tokens = len(article_text) / 4

            if tokens <= self.max_chunk_size:
                ***REMOVED*** Article fits in one chunk
                chunk = {
                    "text": article_text,
                ...
                ***REMOVED*** Extract metadata from context before article
                ...
                ***REMOVED*** Article too long → split with overlap
        ...
        ***REMOVED*** Split by paragraphs
        ...
        for para in paragraphs:
            ...
            if current_tokens + para_tokens > self.target_chunk_size and current_chunk:
                ...
                ***REMOVED*** Overlap: keep last paragraph
    ...
    def _chunk_by_size(self, text: str, pages_info: list[dict]) -> list[dict]:
        ...
        ***REMOVED*** Split by double newlines (paragraphs)
    ...
    def _extract_metadata_from_context(self, context: str) -> dict:
        ...
        ***REMOVED*** Try Ukrainian patterns
        ...
        ***REMOVED*** Try Russian patterns if nothing found
    ...
    def _extract_number(self, text: str) -> int:
        ...
        ***REMOVED*** Try arabic number first
        number_match = re.search(r"\d+", text)
        if number_match:
            return int(number_match.group())

        ***REMOVED*** Try roman numerals
        roman_map = {"I": 1, "V": 5, "X": 10, "L": 50, "C": 100, "D": 500, "M": 1000}
        if all(c in roman_map for c in text.upper()):
            return self._roman_to_int(text.upper())

        ***REMOVED*** Try word numbers (Ukrainian/Russian)
        word_numbers = {
            "перша": 1,
            "перший": 1,
            "первая": 1,
