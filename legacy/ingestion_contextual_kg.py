...
def get_full_document_text(pdf_path: str) -> str:
    ...
    ***REMOVED*** For now, use Docling to get full text
    ***REMOVED*** In production, might want to cache this separately
    with open(pdf_path, "rb") as f:
        response = requests.post(
            f"{DOCLING_URL}/v1/chunk/hybrid/file", files={"files": f}, timeout=DOCLING_TIMEOUT
        )
    ...
    ***REMOVED*** Concatenate all chunks to get full document
...
def process_document_contextual_kg(
    ...
    ***REMOVED*** Get full document text (for prompt caching)
    ...
    ***REMOVED*** Step 3: Process each chunk
    ...
    for idx, chunk in enumerate(chunks):
        ...
        try:
            ...
            ***REMOVED*** Prepare contextualized text for embedding
            embedded_text = f"{context_text}\n\n{chunk_text}"

            ***REMOVED*** Embed with BGE-M3
            embeddings = bge_m3_encode(embedded_text)

            ***REMOVED*** Prepare Qdrant payload
            payload = {
                ***REMOVED*** Original content (for display)
                "text": chunk_text,
                ...
                **metadata,  ***REMOVED*** All KG metadata (book, section, chapter, article, etc.)
    ...
    print()  ***REMOVED*** Complete progress line
