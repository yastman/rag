...
async def example_cached_rag_query():
    ...
    ***REMOVED*** Initialize cache
    cache = RedisSemanticCache(
        index_version="1.0.0",  ***REMOVED*** Match your Qdrant collection version
        embedding_ttl_days=30,  ***REMOVED*** Cache embeddings for 30 days
        response_ttl_minutes=30,  ***REMOVED*** Cache responses for 30 minutes
    )
    ...
    ***REMOVED*** Step 1: Try to get cached response (fastest)
    ...
    ***REMOVED*** Step 2: Try to get cached embedding
    ...
    if query_embedding:
        ...
        ***REMOVED*** In real code, call BGE-M3:
        ***REMOVED*** query_embedding = await bge_m3_client.embed(test_query)

        ***REMOVED*** For example, use dummy embedding
        query_embedding = [0.1] * 1024

        ***REMOVED*** Cache it for future use
        await cache.set_embedding(test_query, query_embedding)
        print("   ✅ Embedding cached for 30 days")

    ***REMOVED*** Step 3: Search Qdrant (real search would go here)
    print("\n3️⃣  Searching Qdrant...")

    ***REMOVED*** In real code:
    ***REMOVED*** results = await qdrant_client.search(
    ***REMOVED***     collection_name="contextual_rag_criminal_code_v1",
    ***REMOVED***     query_vector=query_embedding,
    ***REMOVED***     limit=10
    ***REMOVED*** )

    ***REMOVED*** For example, use dummy results
    results = [
        {
            "id": "article_121",
    ...
    ***REMOVED*** Step 4: Cache the full response
    ...
    ***REMOVED*** Step 5: Show cache stats
...
def main():
    ...
    ***REMOVED*** Check if we can connect (rough check)
