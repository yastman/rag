...
Usage Example 2: Manual spans for RAG (Retrieval + Generation):
    ...
    with langfuse.start_as_current_span(name="rag-pipeline") as trace:
        ...
        ***REMOVED*** Generation span (if using LLM)
        with trace.start_as_current_span(
            name="generation",
...
***REMOVED*** ============================================================================
***REMOVED*** Helper Functions for RAG Search Tracing
***REMOVED*** ============================================================================
...
def trace_search_with_decorator(
    ...
    ***REMOVED*** Update trace with metadata
    langfuse.update_current_trace(
        input={"query": query, "engine": engine_name},
        user_id=user_id,
    ...
    ***REMOVED*** Execute search (automatically captured as nested span if search_fn is also decorated)
    start_time = time.time()
    results = search_fn(query)
    latency_ms = (time.time() - start_time) * 1000

    ***REMOVED*** Calculate evaluation metrics
    metrics = {
    ...
    if expected_article is not None and results:
        ***REMOVED*** Extract article numbers from results
        retrieved_articles = [
            r.payload.get("article_number") if hasattr(r, "payload") else None for r in results
        ]

        ***REMOVED*** Calculate precision@1 and recall@10
        ...
        ***REMOVED*** Log scores to Langfuse
    ...
    ***REMOVED*** Update trace with output
...
def trace_search_with_spans(
    ...
    ***REMOVED*** Create main trace
    with langfuse.start_as_current_span(name="rag-search") as trace:
        ...
        ***REMOVED*** Evaluation span (if expected article provided)
        if expected_article is not None and results:
            with trace.start_as_current_span(
                name="evaluation", input={"expected_article": expected_article}
            ) as eval_span:
                ***REMOVED*** Extract article numbers
                retrieved_articles = [
...
if __name__ == "__main__":
    ...
    ***REMOVED*** Example 1: Decorator-based tracing (recommended)
    print("\n" + "=" * 80)
    ...
    def example_search(query: str):
        ...
        ***REMOVED*** Simulate search
        time.sleep(0.1)  ***REMOVED*** Simulate latency
    ...
    ***REMOVED*** Example 2: Manual spans for RAG
    print("\n" + "=" * 80)
