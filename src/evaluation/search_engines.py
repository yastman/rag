...
import numpy as np
import requests  ***REMOVED*** type: ignore[import-untyped]
...
class BaselineSearchEngine(SearchEngine):
    ...
    def search(self, query: str, top_k: int = 10) -> list[dict]:
        ...
        ***REMOVED*** Search using dense vector only
        search_payload = {
...
class HybridSearchEngine(SearchEngine):
    ...
    def search(self, query: str, top_k: int = 10) -> list[dict]:
        ...
        ***REMOVED*** Convert sparse to Qdrant format
        ***REMOVED*** lexical_weights is a dict, need to convert to lists
        lexical_weights = query_embeddings["lexical_weights"]
        if hasattr(lexical_weights, "indices"):
            ***REMOVED*** Scipy sparse format
            sparse_indices = lexical_weights.indices.tolist()
            sparse_values = lexical_weights.values.tolist()
        else:
            ***REMOVED*** Dict format - keys are strings, need to convert to ints!
        ...
        ***REMOVED*** NOTE: Testing without ColBERT first - will add back if dense+sparse works
        search_payload = {
            "prefetch": [
                ***REMOVED*** Prefetch 1: Dense vector search
                {
                    "query": query_embeddings["dense_vecs"].tolist(),
                    "using": "dense",
                    "limit": 100,  ***REMOVED*** Get more candidates for fusion
                },
                ***REMOVED*** Prefetch 2: Sparse BM25 search
                {
                    "query": {
                        "values": sparse_values,  ***REMOVED*** values BEFORE indices (Qdrant API requirement)
                        "indices": sparse_indices,
                    },
                    "using": "sparse",
                    "limit": 100,
                },
                ***REMOVED*** Temporarily disabled ColBERT for testing
                ***REMOVED*** {
                ***REMOVED***     "query": query_embeddings["colbert_vecs"].tolist(),
                ***REMOVED***     "using": "colbert",
                ***REMOVED***     "limit": 100
                ***REMOVED*** }
            ],
            "query": {
        ...
        ***REMOVED*** Convert all numpy types to Python types for JSON serialization
        ...
        if response.status_code != 200:
            ***REMOVED*** Print detailed error for debugging
        ...
        ***REMOVED*** Query API returns dict with 'points' key, not a list
...
class HybridDBSFColBERTSearchEngine(SearchEngine):
    ...
    def __init__(self, collection_name: str, embedding_model):
        ...
        ***REMOVED*** Import config parameters
        sys.path.append("/srv/contextual_rag")
        from config import (
    ...
    def search(self, query: str, top_k: int = 10) -> list[dict]:
        ...
        ***REMOVED*** Build 3-stage query with DBSF fusion + ColBERT reranking
        search_payload = {
            "prefetch": [
                ***REMOVED*** Stage 1a: Dense vector search (prefetch 100 candidates)
                {
                    "prefetch": [
                        {
                            "query": query_embeddings["dense_vecs"].tolist(),
                        ...
                        ***REMOVED*** Stage 1b: Sparse BM25 search (prefetch 100 candidates)
                        {
                    ...
                    ***REMOVED*** Stage 2: DBSF fusion combines dense + sparse results
                    "query": {
                        "fusion": "dbsf"  ***REMOVED*** Distribution-Based Score Fusion
                    },
                }
            ],
            ***REMOVED*** Stage 3: ColBERT multivector reranking on fused results
            "query": query_embeddings["colbert_vecs"].tolist(),
            "using": "colbert",
            "limit": top_k,
            "score_threshold": self.score_threshold,
            "params": {
                "hnsw_ef": self.hnsw_ef  ***REMOVED*** Higher precision for ColBERT search
            },
            "with_payload": self.payload_fields,
            "with_vector": False,
        }

        ***REMOVED*** Convert numpy types to Python types
        search_payload = convert_to_python_types(search_payload)

        ***REMOVED*** Execute query using Qdrant Query API
        response = requests.post(
            f"{self.qdrant_url}/collections/{self.collection_name}/points/query",
            json=search_payload,
            headers=self.headers,
                ...
                ***REMOVED*** Stage 1a: Dense vector search (prefetch 100 candidates)
                {
                    ...
                    ***REMOVED*** Stage 2: RRF fusion combines dense + sparse results
                    "query": {
                        "fusion": "rrf"  ***REMOVED*** Reciprocal Rank Fusion (OFFICIAL METHOD)
                    },
                }
            ],
            ***REMOVED*** Stage 3: ColBERT multivector reranking on fused results
