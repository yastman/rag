...
class RAGExperimentRunner:
    ...
    async def _run_variant(
        ...
        with self.logger.start_run(
            ...
            ***REMOVED*** Log variant config
            self.logger.log_config(variant_config, prefix=f"variant_{variant_name}.")

            ***REMOVED*** Simulate evaluation (replace with actual evaluation logic)
            ***REMOVED*** TODO: Integrate with actual RAG pipeline
    ...
    async def _simulate_evaluation(
        ...
        Returns:
            Simulated metrics
        """
        ***REMOVED*** Simulate processing time
        await asyncio.sleep(0.1 * len(test_queries))

        ***REMOVED*** Simulate metrics (placeholder)
        ***REMOVED*** TODO: Replace with actual evaluation
        base_recall = 0.90
        base_ndcg = 0.95
        base_latency = 400

        ***REMOVED*** Adjust based on config
        if config.get("enable_contextualization"):
            base_recall += 0.04  ***REMOVED*** Context improves recall
            base_latency += 50  ***REMOVED*** But adds latency

        if config.get("chunk_size", 600) < 500:
            base_recall -= 0.02  ***REMOVED*** Smaller chunks hurt recall
    ...
    def _compare_variants(
        ...
        Returns:
            Comparison results
        """
        ***REMOVED*** Primary metric: Recall@1
        recall_diff = results_a["recall_at_1"] - results_b["recall_at_1"]

        if abs(recall_diff) < 0.01:  ***REMOVED*** Statistical tie
            ***REMOVED*** Secondary: Latency (lower is better)
...
async def main():
    ...
    ***REMOVED*** Example test queries (replace with real queries)
    test_queries = [
        {"query": "Стаття 121 УК", "expected_article": 121},
        {"query": "Конституція України", "expected_article": 1},
    ...
    ***REMOVED*** Test 2: Chunk Size Comparison
