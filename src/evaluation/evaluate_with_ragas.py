...
***REMOVED*** Load environment variables and add parent to path before local imports
load_dotenv()
sys.path.insert(0, str(Path(__file__).parent))

from mlflow_integration import MLflowRAGLogger  ***REMOVED*** noqa: E402
from search_engines import (  ***REMOVED*** noqa: E402
...
def evaluate_with_ragas(
    ...
    ***REMOVED*** Check for RAGAS and datasets
    try:
    ...
    ***REMOVED*** Check for OpenAI API key
    ...
    ***REMOVED*** Initialize search engine
    ...
    ***REMOVED*** Initialize MLflow logger if requested
    mlflow_logger = None
    if use_mlflow:
    ...
    ***REMOVED*** Prepare RAGAS dataset
    ...
    for i, query_item in enumerate(queries_data, 1):
        ...
        ***REMOVED*** Search for documents
        try:
            results = engine.search(query, limit=10)

            ***REMOVED*** Extract contexts (retrieved document texts)
            contexts = []
            for result in results[:5]:  ***REMOVED*** Use top 5 for context
                text = result.text  ***REMOVED*** SearchResult.text already contains page_content
                contexts.append(text)

            ***REMOVED*** Generate answer using LLM (simplified - just use first doc)
            ***REMOVED*** In production, you'd use your full RAG pipeline here
    ...
    ***REMOVED*** Convert to RAGAS dataset
    ...
    ***REMOVED*** Log to MLflow
    if mlflow_logger:
        ...
        with mlflow_logger.start_run(run_name=run_name, tags={"engine": engine_name}):
            ...
            ***REMOVED*** Log dataset as artifact
