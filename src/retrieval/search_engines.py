...
class BaselineSearchEngine(BaseSearchEngine):
    ...
    def search(
        ...
        results = self.client.search(
            ...
            ***REMOVED*** Oversampling + rescoring for quantization accuracy
            search_params={
                "quantization": {
                    "rescore": True,  ***REMOVED*** Rescore with original vectors
                    "oversampling": 3.0,  ***REMOVED*** Retrieve 3x more for rescoring
                }
            },
        )
...
class HybridRRFSearchEngine(BaseSearchEngine):
    ...
    def _search_hybrid(
        ...
        ***REMOVED*** Build hybrid search with RRF using query API
        search_payload = {
            "prefetch": [
                ...
                ***REMOVED*** Prefetch 2: Sparse BM42 search (better than BM25 for chunks)
                {
                    "query": {
                        "values": sparse_values,
                        "indices": sparse_indices,
                    },
                    "using": "bm42",  ***REMOVED*** Using BM42 instead of "sparse"
                    "limit": 100,
                },
            ],
        ...
        ***REMOVED*** Execute hybrid search via Qdrant query API
        ...
        if response.status_code != 200:
            ***REMOVED*** Fallback to dense-only on error
            print(
        ...
        ***REMOVED*** Query API returns dict with 'points' key
...
class HybridRRFColBERTSearchEngine(BaseSearchEngine):
    ...
    def _search_hybrid_colbert(
        ...
        ***REMOVED*** Build 3-stage query: Prefetch → RRF → ColBERT rerank
        search_payload = {
            "prefetch": [
                {
                    "prefetch": [
                        ...
                        ***REMOVED*** Stage 1b: BM42 sparse search (better for short chunks)
                        {
                            "query": {
                                "values": sparse_values,
                                "indices": sparse_indices,
        ...
        ***REMOVED*** Convert all numpy types to Python types
        search_payload = convert_to_python_types(search_payload)

        ***REMOVED*** Execute 3-stage query via Qdrant query API
        response = requests.post(
            f"{self.settings.qdrant_url}/collections/{self.settings.collection_name}/points/query",
            json=search_payload,
            headers={"api-key": self.settings.qdrant_api_key or ""},
        ...
        if response.status_code != 200:
            ***REMOVED*** Fallback to RRF without ColBERT on error
            print(
                f"WARNING: ColBERT rerank failed ({response.status_code}), falling back to RRF only"
            )
            ***REMOVED*** Create temporary HybridRRFSearchEngine for fallback
...
class DBSFColBERTSearchEngine(BaseSearchEngine):
    ...
    def _search_hybrid_colbert(
        ...
        ***REMOVED*** Build 3-stage query: Prefetch → DBSF → ColBERT rerank
        search_payload = {
            "prefetch": [
                {
                    "prefetch": [
                    ...
                    ***REMOVED*** Stage 2: DBSF fusion (statistical normalization)
                    "query": {"fusion": "dbsf"},
                }
            ],
            ***REMOVED*** Stage 3: ColBERT multivector rerank
        ...
        ***REMOVED*** Execute 3-stage query via Qdrant query API
        ...
        if response.status_code != 200:
            ***REMOVED*** Fallback to RRF variant on error
            print(
                f"WARNING: DBSF ColBERT rerank failed ({response.status_code}), falling back to RRF"
            )
            ***REMOVED*** Create temporary HybridRRFColBERTSearchEngine for fallback
...
def create_search_engine(
    ...
    ***REMOVED*** Default to Variant A (best performance)
    return HybridRRFColBERTSearchEngine(settings)
