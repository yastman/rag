...
class DBSFColBERTSearchEngine(BaseSearchEngine):
    ...
    def search(
        ...
        ***REMOVED*** Stage 1: Dense retrieval (initial candidates)
        dense_results = self.client.search(
            collection_name=self.settings.collection_name,
            query_vector=query_embedding,
        ...
        ***REMOVED*** Stage 2: Compute DBSF scores
        dbsf_results = self._compute_dbsf_scores(dense_results)

        ***REMOVED*** Stage 3: Rerank and filter
    ...
    def _compute_dbsf_scores(
        ...
        for rank, result in enumerate(results):
            ***REMOVED*** Compute density boost (based on neighborhood)
    ...
    def _compute_density_boost(
        ...
        """
        ***REMOVED*** Find k nearest neighbors
        neighbors_scores = []
        for other in all_results[:k_neighbors]:
            if other.id != result.id:
                ***REMOVED*** Simple similarity: use score difference
                neighbors_scores.append(1 - abs(other.score - result.score))

        ***REMOVED*** Density boost = average neighbor similarity
        if neighbors_scores:
