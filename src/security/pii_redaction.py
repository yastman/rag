...
class PIIRedactor:
    ...
    ***REMOVED*** Replacement tokens keyed by pattern name
    ...
    def __init__(self) -> None:
        ***REMOVED*** Order matters: passport and tax_id before user_id to avoid overlap on 10-digit numbers;
        ***REMOVED*** user_id before phone so 9-10 digit IDs are not consumed by the broader phone pattern.
        self.patterns: dict[str, re.Pattern[str]] = {
            "passport": re.compile(r"\b[А-ЯІЇЄҐ]{2}\d{6}\b"),
            "tax_id": re.compile(r"\b\d{10}\b"),  ***REMOVED*** РНОКПП (10 digits, applied before user_id)
            "user_id": re.compile(r"\b\d{9,10}\b"),  ***REMOVED*** Telegram user IDs (9-10 digits)
            "phone": re.compile(r"\+?\d{10,15}"),  ***REMOVED*** International phone numbers
