...
Usage:
    python -m src.ingestion.gdrive_flow --once    ***REMOVED*** Run once
    python -m src.ingestion.gdrive_flow --watch   ***REMOVED*** Continuous mode
"""
...
class GDriveFlowConfig:
    ...
    ***REMOVED*** Sync directory (where rclone puts files)
    sync_dir: str = field(
    ...
    ***REMOVED*** Default to binary collection for best latency; override via env if needed.
    collection_name: str = field(
        default_factory=lambda: os.getenv("GDRIVE_COLLECTION_NAME", "gdrive_documents_bge")
    )

    ***REMOVED*** Docling settings
...
class ProcessedFile:
    ...
    file_path: str
    file_id: str  ***REMOVED*** Hash of relative path
    content_hash: str  ***REMOVED*** Hash of file content
    chunks_count: int
...
class GDriveFileProcessor:
    ...
    def __init__(self, config: GDriveFlowConfig | None = None):
        ...
        ***REMOVED*** Docling client for chunking
        self.docling_config = DoclingConfig(
            base_url=self.config.docling_url,
            max_tokens=self.config.chunk_max_tokens,
        )

        ***REMOVED*** Indexer for Qdrant
        self.indexer = GDriveIndexer(
            qdrant_url=self.config.qdrant_url,
            voyage_api_key=self.config.voyage_api_key,
        )

        ***REMOVED*** Track processed files (PERSISTENT):
        ***REMOVED*** Use sqlite3 (stdlib) to persist file_id -> (source_path, content_hash, last_indexed_at)
        ***REMOVED*** so restarts do not trigger full reindex, and so we can detect deletions.
    ...
    async def process_file(self, path: Path) -> ProcessedFile | None:
        ...
        try:
            ***REMOVED*** Step 1: Chunk via docling
            ...
            ***REMOVED*** Add file metadata to chunks
            ...
            ***REMOVED*** Step 2: Index to Qdrant (with replace semantics)
    ...
    async def process_directory(self) -> list[ProcessedFile]:
        ...
        ***REMOVED*** Track current file_ids to enable deletion detection
        ...
        ***REMOVED*** Reconcile deletions/renames:
        ***REMOVED*** - load known file_ids from sqlite state
        ***REMOVED*** - for file_ids not present in current_file_ids: delete from Qdrant and mark deleted
        ***REMOVED*** (This is required because rclone sync can delete/rename files without a "delete event".)
...
def main():
    ...
    load_dotenv()  ***REMOVED*** Load .env before config initialization
