...
1. **Current path** (`unified/`) — incremental pipeline with deterministic file identity and replace semantics.
...
| `src.ingestion.unified.flow` `build_flow()` / `run_once()` / `run_watch()` | Flow assembly and execution |
...
| [`unified/flow.py`](./unified/flow.py) | Flow definition |
...
| [`unified/targets/qdrant_hybrid_target.py`](./unified/targets/qdrant_hybrid_target.py) | Custom target connector |
...
- **PostgreSQL** — ingestion flow state database
