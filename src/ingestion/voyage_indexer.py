...
class VoyageIndexer:
    ...
    ***REMOVED*** Voyage API batch size limit
    ...
    def __init__(
        ...
        ***REMOVED*** Qdrant client with longer timeout for batch operations
        ...
        ***REMOVED*** Voyage AI unified service for dense embeddings
        self.voyage_service = VoyageService(
            api_key=self.voyage_api_key,
            model_docs=voyage_model,
        )

        ***REMOVED*** FastEmbed BM42 for sparse embeddings
    ...
    def create_collection(self, collection_name: str, recreate: bool = False) -> bool:
        ...
        try:
            ...
            pass  ***REMOVED*** Collection doesn't exist

        ***REMOVED*** Create collection with Voyage-optimized config
        ***REMOVED*** No ColBERT - use Voyage rerank-2.5 instead
        self.client.create_collection(
            collection_name=collection_name,
            vectors_config={
                "dense": VectorParams(
                    size=1024,  ***REMOVED*** Voyage voyage-4-large dimension
    ...
    async def index_chunks(
        ...
        ***REMOVED*** Process in batches with rate limiting
        ...
        for i, batch_idx in enumerate(range(0, len(chunks), batch_size)):
            ...
            ***REMOVED*** Rate limit delay (skip for last batch)
    ...
    async def _index_batch(self, chunks: list[Chunk], collection_name: str) -> None:
        ...
        try:
            ***REMOVED*** Get texts for embedding
            texts = [chunk.text for chunk in chunks]

            ***REMOVED*** Generate Voyage dense embeddings
            dense_embeddings = await self.voyage_service.embed_documents(texts)

            ***REMOVED*** Generate BM42 sparse embeddings
...
async def reindex_from_existing(
    ...
    ***REMOVED*** Initialize indexer with voyage-4-large
    indexer = VoyageIndexer(voyage_model="voyage-4-large")

    ***REMOVED*** Create target collection
    indexer.create_collection(target_collection, recreate=True)

    ***REMOVED*** Read all points from source
    ...
    ***REMOVED*** Scroll through all points
    all_chunks = []
    offset = None
    ...
    while True:
        ...
        for point in points:
            ...
            ***REMOVED*** Create chunk from existing data
            ***REMOVED*** Use text_for_embedding if available, otherwise page_content
