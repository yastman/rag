...
***REMOVED*** Qdrant default max payload size (32 MB).  Points exceeding this are skipped.
...
class QdrantHybridWriter:
    ...
    async def upsert_chunks(
        ...
        try:
            ***REMOVED*** Step 1: Delete existing (replace semantics)
            stats.points_deleted = await self.delete_file(file_id, collection_name)

            ***REMOVED*** Step 2: Extract texts
            texts = [chunk.text for chunk in chunks]

            ***REMOVED*** Step 3: Generate embeddings (local BGE-M3 or Voyage API)
            ...
            ***REMOVED*** Step 5: Payload size guard
    ...
    def upsert_chunks_sync(
        ...
        try:
            ...
            ***REMOVED*** Step 3: Generate embeddings — single hybrid call when local BGE-M3
            ...
            ***REMOVED*** Step 5: Payload size guard — skip oversized batches
            payload_size = len(_json.dumps([p.payload for p in points]).encode())
            if payload_size > QDRANT_MAX_PAYLOAD_BYTES:
                msg = (
                    f"Payload {payload_size / 1024 / 1024:.1f}MB exceeds "
