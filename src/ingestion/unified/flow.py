...
***REMOVED*** MIME types for supported extensions
MIME_TYPES = {
    ".pdf": "application/pdf",
...
def unified_ingestion_flow(flow_builder: cocoindex.FlowBuilder, config: UnifiedConfig) -> None:
    ...
    ***REMOVED*** Supported file patterns
    included_patterns = [
        "**/*.pdf",
        "**/*.docx",
        "**/*.doc",
    ...
    ***REMOVED*** Exclude hidden files and temp files
    excluded_patterns = [
        "**/.*",
        "**/*~",
        "**/*.tmp",
    ...
    ***REMOVED*** Source: LocalFile with change detection
    files = flow_builder.source(
        cocoindex.sources.LocalFile(
            path=sync_dir,
            included_patterns=included_patterns,
            excluded_patterns=excluded_patterns,
            binary=True,  ***REMOVED*** Get file content as bytes
        )
    )

    ***REMOVED*** Collector: extract metadata and compute file_id
    ...
    ***REMOVED*** Export to Qdrant via custom target
...
def build_flow(config: UnifiedConfig | None = None) -> cocoindex.Flow:
    ...
    ***REMOVED*** Initialize CocoIndex with Postgres backend
    cocoindex.init(
...
def run_watch(config: UnifiedConfig | None = None) -> None:
    ...
    ***REMOVED*** Live updater for continuous watching
