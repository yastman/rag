...
class QdrantHybridTargetSpec(TargetSpec):
    ...
    ***REMOVED*** Source
    sync_dir: str = ""  ***REMOVED*** Base directory for resolving relative paths
...
class QdrantHybridTargetValues:
    ...
    source_path: str  ***REMOVED*** Relative path (abs_path computed from spec.sync_dir)
    file_name: str
    mime_type: str
    file_size: int
...
class QdrantHybridTargetConnector:
    ...
    ***REMOVED*** Shared resources (initialized lazily)
    ...
    def _handle_delete(cls, spec: QdrantHybridTargetSpec, file_id: str) -> None:
        ...
        ***REMOVED*** State manager needs async - use a dedicated event loop
    ...
    def _handle_upsert(
        ...
        ***REMOVED*** State operations need async handling
        loop = asyncio.new_event_loop()

        try:
            ***REMOVED*** Check if processing needed (skip unchanged)
