...
***REMOVED******REMOVED*** Context: What Already Exists
...
| create_agent SDK with tool routing | Production (***REMOVED***413) |
...
Active issues define the next milestone: ***REMOVED***447 (menu), ***REMOVED***443 (HITL), ***REMOVED***444 (i18n), ***REMOVED***445 (new tools), ***REMOVED***446 (E2E), ***REMOVED***439 (guard).

---

***REMOVED******REMOVED*** Feature Landscape

***REMOVED******REMOVED******REMOVED*** Table Stakes (Users Expect These)
...
| **Guard on text path** | Voice path is guarded; text path has 3 bypass vectors (issue ***REMOVED***439 — HIGH risk classified) | LOW | Missing — critical security gap | Pre-agent `detect_injection(message.text)` call in bot.py before `agent.ainvoke()` |
| **Free text + buttons as unified path** | Users expect seamless switch between typing and tapping buttons | LOW | Architecture defined (***REMOVED***447 plan) — buttons generate text intents → same agent | Intent mapper: `callback_id → text_intent → agent.ainvoke()` |
| **User-facing "my deals" / "my tasks"** | Client: see own applications; Manager: see own pipeline | MEDIUM | Missing — needs `crm_get_my_leads`, `crm_get_my_tasks` tools (***REMOVED***443) | Kommo API: filter by responsible_user_id and contact_id |
| **Mortgage calculator** | Real estate bots universally provide annuity payment calculation | LOW | Missing (***REMOVED***445) | Pure math, no external API needed: `M = P * r(1+r)^n / ((1+r)^n - 1)` |
| **Manager handoff (live agent)** | When bot can't help, escalate to human — standard in CRM bots | MEDIUM | Missing (***REMOVED***445) | Notify manager via Telegram notification; flag conversation for live takeover |
...
***REMOVED******REMOVED******REMOVED*** Differentiators (Competitive Advantage)
...
| **Agent reformulation keeps original text for guard** | Guard currently checks reformulated query (gap ***REMOVED***4 in ***REMOVED***439) — checking original prevents semantic laundering attacks | LOW | Missing | Store `original_user_query` in BotContext; pass to guard before agent |
| **Manager daily summary** | LLM-synthesized daily CRM digest (deals updated today, tasks due, hot leads) | MEDIUM | Missing (***REMOVED***445) | `crm_get_leads(updated_today)` → LLM summarize → send via bot |
| **Session summary → CRM note** | When session idle 30+ min, auto-summarize conversation → add as note to Kommo lead | MEDIUM | Missing (***REMOVED***445) — `SessionSummaryWorker` described in plan | APScheduler job: idle detection → gpt-4o-mini summarize → `crm_add_note` |
...
| **CRM submenu from agent response** | Inline keyboard attached to CRM results (contact/deal cards) with quick actions | MEDIUM | Missing — defined in ***REMOVED***447 plan | `crm_get_deal` / `crm_get_contacts` response → parse → attach InlineKeyboardMarkup |
| **Follow-up inline buttons after lead creation** | "Book showing / See similar / Remind later" inline keyboard in 24h follow-up | MEDIUM | Missing — described in ***REMOVED***447 background automation section | Callback → agent intent; integrates with NurturingScheduler |
| **crm_search_leads for manager** | Full-text search across deals/contacts from manager menu | MEDIUM | Missing (***REMOVED***443) | Kommo API search endpoint; manager enters query via TextInput window |

***REMOVED******REMOVED******REMOVED*** Anti-Features (Deliberately NOT Build)
...
***REMOVED******REMOVED*** Feature Dependencies

```
[Guard fix (***REMOVED***439)]
    └──required-before──> [Text path agent invocation] (any feature using agent)

[Pipeline refactor (***REMOVED***442)]
    └──required-before──> [HITL (***REMOVED***443)]
    └──required-before──> [Menu skeleton (***REMOVED***447)]
    └──required-before──> [New tools (***REMOVED***445)]

[HITL (***REMOVED***443)]
    └──required-before──> [Menu skeleton full (***REMOVED***447)] (buttons 4, 6 need HITL tools)
    └──required-before──> [CRM submenu (***REMOVED***447)] (write actions need HITL)
    └──provides──> [crm_search_leads, crm_get_my_leads, crm_get_my_tasks, crm_update_contact]

[Menu skeleton (***REMOVED***447)]
    └──required-before──> [E2E tests (***REMOVED***446)] (E2E tests navigate menu)
    └──requires──> [i18n (***REMOVED***444)] for localized button text

[i18n (***REMOVED***444)]
    └──enhances──> [Menu skeleton (***REMOVED***447)] (button text from locale files)
    └──enhances──> [System prompt] (per-locale prompt variants)
    └──required-by──> [Settings language switch] (already partial)

[New tools (***REMOVED***445)]
    └──provides──> [mortgage_calculator, daily_summary, handoff_to_manager, SessionSummaryWorker]
    └──required-before──> [Menu buttons 5, 8 client / 6, 8 manager (***REMOVED***447)]
    └──required-before──> [E2E full coverage (***REMOVED***446)]
...
[E2E tests (***REMOVED***446)]
    └──requires──> All phases merged
    └──requires──> Mocked agent.ainvoke() pattern established

[Guard on text path (***REMOVED***439)]
    └──independent──> All other features (can be fixed in parallel with ***REMOVED***442)
...
***REMOVED******REMOVED******REMOVED*** Dependency Notes

- **Guard (***REMOVED***439) is independent**: Can be merged in parallel with Phase 1 (***REMOVED***442). Should ship first or concurrently because it fixes a HIGH-risk security gap present in production now.
- *****REMOVED***442 gates ***REMOVED***443, ***REMOVED***444, ***REMOVED***447, ***REMOVED***445**: Pipeline refactor is the critical-path blocker for all downstream phases.
...
***REMOVED******REMOVED*** MVP Definition

The active phases (***REMOVED***442→***REMOVED***443→***REMOVED***444→***REMOVED***445→***REMOVED***446) define the milestone MVP.

***REMOVED******REMOVED******REMOVED*** Phase ordering matches dependency graph:

- [ ] **Phase 0 (guard fix, ***REMOVED***439)** — ship immediately; HIGH security risk in production
- [ ] **Phase 1 (***REMOVED***442)** — pipeline refactor; unblocks everything downstream
- [ ] **Phase 2 (***REMOVED***443)** — 4 new CRM tools + HITL; unblocks menu write buttons and E2E HITL tests
- [ ] **Phase 3 (***REMOVED***444/***REMOVED***447)** — menu expansion to 9+9 buttons + i18n system prompt; user-facing impact
- [ ] **Phase 4 (***REMOVED***445)** — mortgage, daily_summary, handoff, background workers; completes feature set
- [ ] **Phase 5 (***REMOVED***446)** — E2E tests + cleanup + epic close

***REMOVED******REMOVED******REMOVED*** Launch With (milestone complete when):
...
- [ ] Guard runs on text path before agent (closes ***REMOVED***439)
...
***REMOVED******REMOVED*** Feature Prioritization Matrix

| Feature | User Value | Implementation Cost | Priority |
|---------|------------|---------------------|----------|
| Guard text path fix (***REMOVED***439) | HIGH (security) | LOW | P0 — ship immediately |
| Pipeline refactor (***REMOVED***442) | MEDIUM (internal) | MEDIUM | P1 — unblocks all |
| HITL on write CRM ops (***REMOVED***443) | HIGH | MEDIUM | P1 |
| Menu 9+9 expansion (***REMOVED***444/***REMOVED***447) | HIGH | MEDIUM | P1 |
| i18n system prompt (***REMOVED***444) | MEDIUM | LOW | P1 |
| Mortgage calculator (***REMOVED***445) | HIGH | LOW | P1 |
| Manager daily summary (***REMOVED***445) | HIGH (manager UX) | MEDIUM | P1 |
| Handoff to manager (***REMOVED***445) | HIGH (safety net) | MEDIUM | P1 |
| SessionSummaryWorker (***REMOVED***445) | MEDIUM | MEDIUM | P1 |
| E2E tests (***REMOVED***446) | MEDIUM (confidence) | MEDIUM | P1 |
...
***REMOVED******REMOVED*** Competitor Feature Analysis
...
***REMOVED******REMOVED*** Implementation Notes Per Feature

***REMOVED******REMOVED******REMOVED*** HITL Pattern (HIGH confidence — LangGraph official docs verified)

```python
***REMOVED*** Inside @tool function (e.g., crm_create_lead):
...
async def crm_create_lead(name: str, budget: str, config: RunnableConfig) -> str:
    ...
    if decision.get("action") == "approve":
        ***REMOVED*** execute actual Kommo API call
        ...
    return "Операция отменена"

***REMOVED*** Bot side: callback handler for hitl:confirm / hitl:cancel
***REMOVED*** → graph.invoke(Command(resume={"action": "approve"}), config=config)
```
...
***REMOVED******REMOVED******REMOVED*** i18n Approach (MEDIUM confidence — official aiogram docs verified)
...
***REMOVED******REMOVED******REMOVED*** Guard Text Path Fix (HIGH confidence — issue ***REMOVED***439 documented the exact gaps)
...
***REMOVED******REMOVED******REMOVED*** E2E Testing Pattern (MEDIUM confidence — community pattern, no canonical aiogram E2E framework)

No canonical E2E framework exists for aiogram bots. The production pattern is:
```python
***REMOVED*** Mocked aiogram Message
msg = AsyncMock(spec=Message)
msg.from_user = MagicMock(id=12345)
msg.text = "Покажи каталог"

***REMOVED*** Patch agent.ainvoke
...
This approach (already used in `tests/unit/agents/`) is the right direction. Issue ***REMOVED***446 extends it to cover menu navigation, BANT funnel flow, and HITL confirm/cancel.

***REMOVED******REMOVED******REMOVED*** Menu Architecture (HIGH confidence — defined in ***REMOVED***447 plan, matches aiogram-dialog patterns)
...
- Project codebase: `telegram_bot/dialogs/`, `telegram_bot/agents/`, `docs/plans/`, GitHub issues ***REMOVED***439–***REMOVED***447 [HIGH confidence]
