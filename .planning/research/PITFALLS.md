...
- Agent-reformulated queries reaching guard instead of original user text (issue ***REMOVED***430 root cause — agent reformulates before `rag_search`, so guard inside pipeline sees cleaned text).
...
***REMOVED******REMOVED******REMOVED*** Pitfall 2: LangGraph HITL Requires a Persistent Checkpointer — MemorySaver Loses State Across Processes
...
**Why it happens:**
`self._agent_checkpointer = create_fallback_checkpointer()` (bot.py:1144) uses MemorySaver by design because `redisvl` JSON serialization fails for LangChain `Message` objects (issue ***REMOVED***420). HITL interrupts require the same checkpointer that holds the interrupted graph state to be available at resume time.
...
***REMOVED******REMOVED******REMOVED*** Pitfall 3: Pipeline Refactor (11→6 nodes) Silently Loses Observability and Score Coverage
...
***REMOVED******REMOVED******REMOVED*** Pitfall 5: aiogram-dialog Menu Conflicts with Free-Text Handler Registration Order
...
***REMOVED******REMOVED******REMOVED*** Pitfall 6: Kommo OAuth2 Race Condition Under Concurrent Tool Calls
...
***REMOVED******REMOVED******REMOVED*** Pitfall 7: APScheduler v3 API Is Not Forward-Compatible with v4
...
**Why it happens:**
APScheduler v4.0 is published to PyPI under the `APScheduler` package name but with a different API. The Renovate dashboard (Issue ***REMOVED***11) tracks all deps. If no version constraint pins `<4`, a weekly Renovate batch may include this upgrade.

**How to avoid:**
- Pin `apscheduler>=3.10,<4` in `pyproject.toml`.
- Add a comment: `***REMOVED*** v4 has breaking API changes — migration requires rewrite of NurturingScheduler`.
...
***REMOVED******REMOVED*** Technical Debt Patterns

| Shortcut | Immediate Benefit | Long-term Cost | When Acceptable |
|----------|-------------------|----------------|-----------------|
| `MemorySaver` for agent checkpointer | Avoids Redis JSON serialization bug (***REMOVED***420) | HITL state lost on restart; no cross-process state | OK until HITL is implemented; must be fixed in Phase 2 |
...
| `detect_injection()` only runs on voice path | Guard was built for voice pipeline | Text path is unguarded (issue ***REMOVED***439) | Never acceptable in production |
...
***REMOVED******REMOVED*** Integration Gotchas
...
***REMOVED******REMOVED*** Performance Traps
...
| Semantic cache never hits (agent reformulation) | Cache hit rate ~0% for text path; embedding costs 3x expected | Fix query key to use `original_user_query`, not reformulated agent query | Immediately — issue ***REMOVED***430 |
...
***REMOVED******REMOVED*** Security Mistakes

| Mistake | Risk | Prevention |
|---------|------|------------|
| No pre-agent guard on text path (issue ***REMOVED***439) | Prompt injection attacks bypass all safety rules; system prompt leakage | Add `detect_injection(message.text)` before `agent.ainvoke()` in `_handle_query_supervisor` |
| Agent system prompt contains zero safety instructions (issue ***REMOVED***439, Gap ***REMOVED***2) | Agent responds to jailbreak attempts without resistance | Add refusal rules and safety instructions to `DEFAULT_SYSTEM_PROMPT` |
...
| Guard checks agent-reformulated query, not original user input (issue ***REMOVED***430, ***REMOVED***439) | Injection patterns in original text are cleaned by agent before guard sees them | Guard must run on original `message.text` before any LLM processing |
...
| HITL confirmation shows raw parameter dict | Manager sees `{"lead_id": 42, "name": "...", "budget": 50000}` — not human-readable | Format preview as natural language: "Update deal ***REMOVED***42 (Ivan Petrov) — change budget to 50,000 EUR?" |
...
***REMOVED******REMOVED*** "Looks Done But Isn't" Checklist
...
***REMOVED******REMOVED*** Recovery Strategies
...
***REMOVED******REMOVED*** Pitfall-to-Phase Mapping

| Pitfall | Prevention Phase | Verification |
|---------|------------------|--------------|
| Guard bypass on text path (issue ***REMOVED***439) | Phase 1 (pipeline refactor) | Unit test: injection text → `_handle_query_supervisor` → assert response is blocked, `agent.ainvoke` never called |
...
| Semantic cache broken by reformulation (issue ***REMOVED***430) | Phase 1 (pipeline refactor) | Integration test: two identical queries → assert `cache_hit=True` on second |
| aiogram-dialog vs agent handler conflict | Phase 1 (menu skeleton, issue ***REMOVED***447) | Unit test: active dialog + free-text → dialog handler responds, not agent |
| HITL persistence (MemorySaver) | Phase 2 (HITL implementation, issue ***REMOVED***443) | Test: interrupt → simulate restart → confirm → assert stateless recovery OR state survives |
| HITL callback user_id validation | Phase 2 (HITL implementation, issue ***REMOVED***443) | Test: forge confirm callback with wrong user_id → assert rejected |
| Kommo OAuth2 race condition | Phase 2 (CRM expansion, issue ***REMOVED***443) | Concurrency test: parallel `get_valid_token` with expired token → `force_refresh` called once |
...
| E2E test isolation (Telegram polling conflict) | Phase 5 (E2E tests, issue ***REMOVED***446) | Verify test uses HTTP mock layer, not live polling; no `getUpdates` conflict |

---

***REMOVED******REMOVED*** Sources

- **Issue ***REMOVED***439** — Guard bypass audit (3 critical gaps confirmed in production codebase)
- **Issue ***REMOVED***430** — Semantic cache broken by agent reformulation (confirmed root cause)
- **Issue ***REMOVED***428** — Streaming coordination broken (`response_sent` flag removed in ***REMOVED***413)
- **Issue ***REMOVED***443** — HITL design spec (Phase 2 plan)
- **Issue ***REMOVED***420** — MemorySaver chosen due to redisvl JSON serialization failure (tracked reason)
- **LangGraph docs** — `interrupt()` requires persistent checkpointer, suspends at node level, resumes via `Command(resume=...)`
- **LangGraph GitHub ***REMOVED***6241** — `InjectedState` vs `InjectedConfig` tool parameter handling edge cases
...
- **OAuth2 race condition (oauth2-proxy ***REMOVED***1992, SO discussion)** — confirmed pattern: parallel refresh requests with single-use refresh tokens cause failures; fix is locking
