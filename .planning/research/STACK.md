...
***REMOVED******REMOVED*** Context: What Already Exists (Do Not Re-research)
...
- ***REMOVED***442 — async RAG pipeline simplification
- ***REMOVED***441 — CRM tool test coverage
- ***REMOVED***439 — guard node on all paths
- ***REMOVED***447 — menu skeleton (client/manager dialogs)
- ***REMOVED***443 — HITL confirmation
- ***REMOVED***444 — i18n

---

***REMOVED******REMOVED*** Recommended Stack (New Additions Only)

***REMOVED******REMOVED******REMOVED*** 1. Pipeline Simplification (***REMOVED***442)
...
***REMOVED*** In Telegram handler — resume with user's choice:
...
***REMOVED******REMOVED******REMOVED*** 2. CRM Test Coverage (***REMOVED***441)
...
***REMOVED******REMOVED******REMOVED*** 3. Guard Node on All Paths (***REMOVED***439)
...
```python
***REMOVED*** history_graph: guard before retrieve
workflow.add_edge(START, "guard")
workflow.add_edge("guard", "retrieve")  ***REMOVED*** or respond if blocked

***REMOVED*** For CRM tools: wrap tool inputs with detect_injection() before LLM call
***REMOVED*** (reuse existing detect_injection() from guard.py — no import cost)
...
***REMOVED******REMOVED******REMOVED*** 4. Menu System — Client/Manager Dialogs (***REMOVED***447)
...
class ClientMenuSG(StatesGroup):
    main = State()       ***REMOVED*** Main menu
    search = State()     ***REMOVED*** RAG search
    history = State()    ***REMOVED*** Conversation history

class ManagerMenuSG(StatesGroup):
    main = State()       ***REMOVED*** Manager main
    crm = State()        ***REMOVED*** CRM tools
    confirm = State()    ***REMOVED*** HITL confirmation

***REMOVED*** Sub-dialog result handling:
async def on_confirmation_result(start_data, result, manager: DialogManager):
    if result.get("confirmed"):
        ***REMOVED*** execute CRM action
        pass
...
***REMOVED******REMOVED******REMOVED*** 5. HITL Confirmation (***REMOVED***443)

**No new dependencies. LangGraph interrupt + aiogram-dialog sub-dialog.**

Two complementary patterns depending on context:

***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern A: LangGraph `interrupt()` — for pipeline actions (CRM writes, lead creation)

```python
***REMOVED*** In CRM tool or graph node:
...
```python
***REMOVED*** In aiogram callback handler — user pressed "Yes"
await graph.ainvoke(
    Command(resume=True),
...
***REMOVED******REMOVED******REMOVED******REMOVED*** Pattern B: aiogram-dialog sub-dialog — for menu-driven confirmations
...
***REMOVED******REMOVED******REMOVED*** 6. i18n (***REMOVED***444)
...
```python
***REMOVED*** locales/ru/main.ftl
menu-main = Главное меню
menu-search = Поиск по базе
menu-history = История чата
crm-confirm = Подтвердить действие: { $action }?

***REMOVED*** In aiogram-dialog window:
from aiogram_dialog.widgets.text import Format
Window(
    Format("{menu_main}"),  ***REMOVED*** value from fluentogram translator passed via getter
    ...
)
```
...
***REMOVED******REMOVED*** Supporting Libraries Summary
...
***REMOVED******REMOVED*** E2E Testing Stack
...
***REMOVED******REMOVED*** Installation (New Dependencies Only)

```bash
***REMOVED*** Only new addition:
uv add respx --dev

***REMOVED*** All existing deps already in pyproject.toml — no changes needed
```
...
***REMOVED******REMOVED*** What NOT to Use
...
***REMOVED******REMOVED*** Stack Patterns by Scenario
...
***REMOVED******REMOVED*** Version Compatibility
