***REMOVED*** Project Research Summary
...
This is a subsequent milestone on a production system that already ships hybrid RAG search (BGE-M3 + RRF + ColBERT), 8 CRM tools against Kommo API, voice STT via Whisper, and Langfuse observability with 35 observations per trace. The milestone targets six open issues: guard coverage gaps (***REMOVED***439), async pipeline simplification (***REMOVED***442), CRM test coverage (***REMOVED***441), menu system expansion (***REMOVED***447), HITL confirmation for write operations (***REMOVED***443), and i18n system prompt support (***REMOVED***444). All research confirms that the existing stack is the correct foundation — the only new dependency is `respx 0.22.0` for CRM test mocking; everything else is already in the codebase.

The recommended approach is a five-phase sequential delivery ordered by dependency graph. Pipeline refactor (***REMOVED***442) is the critical-path blocker because it is where the guard text-path fix (***REMOVED***439), the broken semantic cache (***REMOVED***430), and the new async pipeline function all converge. Guard fix and history sub-graph guard (***REMOVED***432) can run as parallel tracks since they share no code changes. Menu skeleton (***REMOVED***447) can proceed in parallel with the pipeline work because the navigation structure is independent of the pipeline internals. HITL confirmation (***REMOVED***443) unblocks after both menu and pipeline are stable, because it requires both the dialog FSM state machine and the new pipeline's async function surface.

The highest-risk area is HITL state persistence: the current `MemorySaver` checkpointer (chosen because redisvl JSON serialization fails for LangChain Message objects — issue ***REMOVED***420) will silently lose HITL confirmation state on any bot restart. The architecture research is unambiguous on this: the correct approach is a stateless HITL pattern where pending CRM actions are stored in Redis TTL keys rather than in LangGraph checkpoint state. This avoids the serialization bug, survives restarts, and matches the existing `feedback.py` keyboard pattern. A second high-risk area is the security gap in ***REMOVED***439: three independent bypass vectors exist on the text path in production right now, and the guard fix must ship before or alongside the pipeline refactor, not after it.

---

***REMOVED******REMOVED*** Key Findings

***REMOVED******REMOVED******REMOVED*** Recommended Stack
...
***REMOVED******REMOVED******REMOVED*** Expected Features
...
- Guard on text path (***REMOVED***439, P0) — three bypass vectors confirmed in production; security gap ships with every deploy until fixed
- Role-based menu expansion to 9+9 buttons (***REMOVED***447) — skeleton exists with 3 buttons; users expect full navigation
- HITL confirmation for write CRM operations (***REMOVED***443) — standard UX for any bot that modifies CRM data
- BANT funnel wired to RAG and CRM (***REMOVED***447 + existing funnel.py) — funnel steps exist but results window is a placeholder
- i18n system prompt per locale (***REMOVED***444) — dialogs already have i18n getters; LLM still replies in hardcoded RU

**Should have (differentiators this milestone):**
- `mortgage_calculator` tool (***REMOVED***445) — universal real estate feature, pure math, no external API
- Manager daily summary (***REMOVED***445) — LLM-synthesized CRM digest, high manager UX value
- `handoff_to_manager` tool (***REMOVED***445) — safety net when bot cannot help; sends notification to manager
- `SessionSummaryWorker` background job (***REMOVED***445) — auto-summarize idle sessions to Kommo notes
- CRM test coverage (***REMOVED***441) — `respx` fixtures for all 8 tools; currently only partial coverage
...
- CRM submenu inline keyboard on deal/contact cards — depends on stable ***REMOVED***447 menu
...
5. CRM `@tools` (agents/crm_tools.py) — 8 tools + 4 new (***REMOVED***443); HITL via inline keyboard + Redis TTL, not LangGraph interrupt
...
***REMOVED******REMOVED******REMOVED*** Critical Pitfalls

1. **Guard bypass on text path (issue ***REMOVED***439)** — `detect_injection(message.text)` must be called BEFORE `agent.ainvoke()` in `_handle_query_supervisor`, not inside the pipeline. Guard must also check `original_user_query`, not the agent-reformulated string. Fix in Phase 1 before any other new code ships.

2. **HITL state lost on restart (MemorySaver + issue ***REMOVED***420)** — Do not use `interrupt()` with `MemorySaver` for HITL. Use stateless pattern: CRM write tools send inline keyboard, store `{action_id → {tool_name, args, user_id}}` in Redis with 5-minute TTL, callback handler reads Redis and executes directly. Validate `user_id` ownership to prevent HITL forgery attacks.
...
4. **Semantic cache never hits (issue ***REMOVED***430)** — Agent LLM reformulates queries before calling `rag_search`, so the semantic cache key (based on embedding of the tool-call query string) never matches the original question. Fix: store `original_user_query` in `BotContext` at the `message.text` read point in `bot.py`; use it as the cache lookup key in `rag_pipeline()`.
...
***REMOVED******REMOVED*** Implications for Roadmap
...
***REMOVED******REMOVED******REMOVED*** Phase 0: Guard Fix (Issue ***REMOVED***439)
...
**Delivers:** `detect_injection()` before `agent.ainvoke()` on text path; `original_user_query` stored in BotContext; safety instructions in `DEFAULT_SYSTEM_PROMPT`; history sub-graph guard node (***REMOVED***432).
...
***REMOVED******REMOVED******REMOVED*** Phase 1: Pipeline Refactor + Test Infrastructure (Issues ***REMOVED***442, ***REMOVED***441)
**Rationale:** `rag_pipeline()` is the critical-path blocker for HITL, funnel-to-RAG wiring, and E2E tests. CRM test coverage (***REMOVED***441) is best done alongside pipeline work because `respx` fixtures establish the test infrastructure pattern used in all subsequent phases.
...
**Avoids:** Observability loss pitfall (***REMOVED***3), semantic cache broken pitfall (***REMOVED***4)

***REMOVED******REMOVED******REMOVED*** Phase 2: HITL Confirmation + 4 New CRM Tools (Issues ***REMOVED***443, ***REMOVED***441 extension)
...
**Avoids:** HITL persistence pitfall (***REMOVED***2), Kommo OAuth2 race condition pitfall (***REMOVED***6)

***REMOVED******REMOVED******REMOVED*** Phase 3: Menu Expansion + i18n System Prompt (Issues ***REMOVED***447, ***REMOVED***444)
...
**Avoids:** Dialog vs agent handler conflict pitfall (***REMOVED***5); dialog state pollution anti-pattern

***REMOVED******REMOVED******REMOVED*** Phase 4: Additional Tools + Background Workers (Issue ***REMOVED***445)
...
**Avoids:** APScheduler v4 breakage pitfall (***REMOVED***7)

***REMOVED******REMOVED******REMOVED*** Phase 5: E2E Tests + Epic Close (Issue ***REMOVED***446)
...
***REMOVED******REMOVED******REMOVED*** Phase Ordering Rationale
...
- **Phase 0 (Guard fix):** Exact code locations and changes are documented in issue ***REMOVED***439 and confirmed in PITFALLS.md. No research needed — just implement.
...
***REMOVED******REMOVED*** Confidence Assessment
...
| Features | HIGH | Issues ***REMOVED***439-***REMOVED***447 are the authoritative source; codebase confirms current state of each feature. Competitor analysis is MEDIUM (generic sources) but does not affect core feature decisions. |
...
| Pitfalls | HIGH | Pitfalls 1-4 sourced from confirmed GitHub issues (***REMOVED***439, ***REMOVED***430, ***REMOVED***420, ***REMOVED***428) with root causes in codebase. Pitfalls 5-7 sourced from official library docs and codebase audit. |

**Overall confidence:** HIGH

***REMOVED******REMOVED******REMOVED*** Gaps to Address

- **HITL implementation choice:** Whether to use stateless Redis TTL HITL (recommended) or pursue fixing issue ***REMOVED***420 to enable LangGraph interrupt-based HITL is a decision that affects Phase 2 complexity. Recommend deciding before Phase 2 planning starts and adding the chosen approach to the phase specification.
...
***REMOVED******REMOVED******REMOVED*** Primary (HIGH confidence)
...
- GitHub issues ***REMOVED***439, ***REMOVED***430, ***REMOVED***420, ***REMOVED***428, ***REMOVED***443, ***REMOVED***447 — confirmed bugs and design specs
...
***REMOVED******REMOVED******REMOVED*** Secondary (MEDIUM confidence)
...
- OAuth2 race condition documented patterns (oauth2-proxy ***REMOVED***1992, SO) — concurrent refresh lock fix
- APScheduler v3→v4 migration docs — confirmed breaking API rewrite

***REMOVED******REMOVED******REMOVED*** Tertiary (LOW confidence)
