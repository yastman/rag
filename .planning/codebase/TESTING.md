***REMOVED*** Testing Patterns

**Analysis Date:** 2026-02-19

***REMOVED******REMOVED*** Test Framework
...
```bash
***REMOVED*** All unit tests (fast, ~5 min with -n auto)
uv run pytest tests/unit/ -v

***REMOVED*** Parallel execution (4x speedup, recommended)
uv run pytest tests/unit/ -n auto

***REMOVED*** Watch mode (continuous re-run)
...
make test-cov                    ***REMOVED*** Opens htmlcov/index.html in browser
...
***REMOVED*** Integration tests (requires Docker)
uv run pytest tests/integration/ -v

***REMOVED*** Graph path tests (6 deterministic routing paths, no Docker)
uv run pytest tests/integration/test_graph_paths.py -v
```

***REMOVED******REMOVED*** Test File Organization
...
```
tests/
├── unit/                       ***REMOVED*** Fast unit tests (~5 min, -n auto)
│   ├── conftest.py             ***REMOVED*** Isolation fixture (disable Langfuse/OTEL)
│   ├── test_bot_scores.py      ***REMOVED*** 14 tests
│   ├── test_query_preprocessor.py
│   ├── graph/                  ***REMOVED*** 31 tests (nodes, edges, assembly)
│   │   ├── test_retrieve_node.py
│   │   ├── test_agentic_nodes.py
│   │   ├── test_edges.py
│   │   ├── test_graph.py
│   │   └── conftest.py         ***REMOVED*** Graph fixture helpers
│   ├── services/               ***REMOVED*** 21 tests (kommo, qdrant, etc.)
│   │   ├── test_kommo_client_*.py
│   │   └── test_qdrant_service.py
│   └── contextualization/      ***REMOVED*** 12 tests (LLM providers)
│       ├── test_openai.py
│       ├── test_groq.py
│       └── test_claude.py
├── integration/                ***REMOVED*** Deterministic graph path tests
│   ├── test_graph_paths.py     ***REMOVED*** 6 routing paths
│   └── conftest.py
├── chaos/                      ***REMOVED*** Resilience tests (service failures)
...
│   └── conftest.py
├── baseline/                   ***REMOVED*** Regression detection
│   ├── test_manager.py         ***REMOVED*** 8 tests
│   ├── test_collector.py       ***REMOVED*** 4 tests
│   └── thresholds.yaml         ***REMOVED*** Regression thresholds
└── legacy/                     ***REMOVED*** Pre-LangGraph tests (excluded from CI)
...
def test_scores_written_full_pipeline():
    ...
    ***REMOVED*** ARRANGE: Set up data and mocks
    mock_lf = MagicMock()
    result = {**FULL_PIPELINE_RESULT, "checkpointer_overhead_proxy_ms": 39.0}

    ***REMOVED*** ACT: Call the function under test
    _run_score_writer(result, mock_lf)

    ***REMOVED*** ASSERT: Verify behavior
...
async def test_hybrid_search_returns_results():
    ...
    ***REMOVED*** asyncio_mode=auto, no decorator needed
...
***REMOVED*** Async function returning value
...
```python
***REMOVED*** GOOD: Mock the service factory
patch("telegram_bot.graph.config.GraphConfig.create_llm", return_value=mock_llm)

***REMOVED*** AVOID: Mocking low-level libraries
***REMOVED*** AVOID: patch("openai.AsyncOpenAI") — patches too low

***REMOVED*** GOOD: Mock get_client() wrapper
...
with patch("service.KommoClient") as mock_kommo, \
    ...
    ***REMOVED*** Both mocks available

***REMOVED*** Patch with side_effect (callable or exception)
mock_client.request = AsyncMock(side_effect=httpx.TimeoutError())

***REMOVED*** Patch with side_effect function
...
***REMOVED******REMOVED*** Fixtures and Factories
...
```python
***REMOVED*** Typical graph result for a full RAG pipeline
...
```python
***REMOVED*** No decorator needed — asyncio_mode="auto"
...
def test_kommo_retries_on_429():
    ...
    ***REMOVED*** tenacity retry should convert 429 to success on second attempt
    ***REMOVED*** (actual test uses the @_kommo_retry decorator on _request method)
...
@pytest.mark.asyncio
async def test_async_pattern_old():  ***REMOVED*** Not needed with asyncio_mode="auto"
    pass

@pytest.mark.timeout(60)  ***REMOVED*** Override default 30s
...
@pytest.mark.xfail(reason="Known issue ***REMOVED***123")
...
***REMOVED******REMOVED*** Isolation & Test Hygiene
...
```python
***REMOVED*** FORBIDDEN at module level
sys.modules["foo"] = MagicMock()

***REMOVED*** OK in fixture (uses monkeypatch)
@pytest.fixture
def mock_module(monkeypatch):
    monkeypatch.setitem(sys.modules, "foo", MagicMock())

***REMOVED*** OK in conftest pytest_configure hook (with cleanup)
...
```python
***REMOVED*** tests/unit/conftest.py
...
***REMOVED******REMOVED*** CI Pipeline & Sharding
...
```bash
***REMOVED*** Run shard 2 of 4 (as in CI)
uv run pytest tests/unit/ --splits 4 --group 2 -n auto --dist loadscope

***REMOVED*** Regenerate durations after refactors
