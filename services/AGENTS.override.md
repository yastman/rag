...
## Local Rules
- Each subservice (`bge-m3-api/`, `docling/`, ...) owns its own Dockerfile and, where applicable, service-local dependency manifest.
...
- Run subservice-scoped unit tests, e.g. `uv run pytest tests/unit/test_bge_m3_endpoints.py tests/unit/test_docling*.py -q`.
