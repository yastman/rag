...
local-pre-push: ***REMOVED******REMOVED*** Fast local pre-push sanity gate (check + compose validate)
...
local-pr-ready: ***REMOVED******REMOVED*** Full PR readiness gate (fast gate + unit tests) - run manually
...
test-fast: ***REMOVED******REMOVED*** Run unit tests in parallel (xdist, loadscope)
...
test-all-fast: ***REMOVED******REMOVED*** Run ALL test suites in parallel (unit + integration + smoke)
...
test-profile: ***REMOVED******REMOVED*** Profile slowest tests (find bottlenecks)
...
***REMOVED*** Remote Docker host for offloading local container runtime to the MacBook.
...
remote-docker-status: ***REMOVED******REMOVED*** Show remote MacBook Docker/Colima status
...
remote-docker-ps: ***REMOVED******REMOVED*** Show remote MacBook compose service status
	@$(REMOTE_SSH) '$(REMOTE_COMPOSE_CMD) --profile full ps --format "table {{.Name}}\t{{.Status}}\t{{.Ports}}"'

remote-compose-config: ***REMOVED******REMOVED*** Render remote MacBook compose services
	@$(REMOTE_SSH) '$(REMOTE_COMPOSE_CMD) --profile full config --services'

remote-local-up: ***REMOVED******REMOVED*** Start the normal local-service subset on remote MacBook Docker
	@$(REMOTE_SSH) '$(REMOTE_COMPOSE_CMD) up -d $(LOCAL_SERVICES)'

remote-active-up: ***REMOVED******REMOVED*** Start the same service set currently used on the workstation
	@$(REMOTE_SSH) '$(REMOTE_COMPOSE_CMD) --profile ml --profile voice up -d $(REMOTE_ACTIVE_SERVICES)'

remote-bot-up: ***REMOVED******REMOVED*** Start the bot container separately on remote MacBook Docker
	@$(REMOTE_SSH) '$(REMOTE_COMPOSE_CMD) --profile bot up -d bot'

remote-bot-logs: ***REMOVED******REMOVED*** Show recent remote MacBook bot logs
	@$(REMOTE_SSH) '$(REMOTE_COMPOSE_CMD) --profile bot logs --tail 120 bot'

remote-full-up: ***REMOVED******REMOVED*** Start the full profile stack on remote MacBook Docker
	@$(REMOTE_SSH) '$(REMOTE_COMPOSE_CMD) --profile full up -d'

remote-local-down: ***REMOVED******REMOVED*** Stop the remote MacBook compose stack
...
remote-service-health: ***REMOVED******REMOVED*** Check selected remote MacBook service endpoints on the Docker host
...
deploy-bot:  ***REMOVED******REMOVED*** Show official deploy flow: push dev/feature, open PR, merge to main for auto-deploy
...
deploy-vps-local:  ***REMOVED******REMOVED*** Fallback/manual deploy: sync local workspace to VPS via rsync + rebuild
...
validate-traces-fast: ***REMOVED******REMOVED*** No rebuild; trace validation fails if required trace families are missing
	...
	LANGFUSE_PUBLIC_KEY=[REDACTED-LANGFUSE-KEY] $(LANGFUSE_PUBLIC_KEY),[REDACTED-LANGFUSE-KEY] \
	LANGFUSE_SECRET_KEY=[REDACTED-LANGFUSE-KEY] $(LANGFUSE_SECRET_KEY),[REDACTED-LANGFUSE-KEY] \
	uv run python scripts/validate_traces.py --report
	@echo "$(GREEN)Validation complete — see docs/reports/$(NC)"
