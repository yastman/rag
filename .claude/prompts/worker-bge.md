W-BGE: BGE-M3 cold start fix Phase A (***REMOVED***106)
...
ПЛАН: /repo/docs/plans/2026-02-11-bge-cold-start-plan.md
...
3. Коммиты: perf(bge-m3): add startup warmup via lifespan (***REMOVED***106) и perf(bot): add keep-warm encode to preflight (***REMOVED***106)

ЛОГИРОВАНИЕ в /repo/logs/worker-bge.log (APPEND):
Используй Bash tool: echo "[START] $(date +%H:%M:%S) Task: description" >> /repo/logs/worker-bge.log
