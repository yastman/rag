W-EMBED: Tenacity retry + granular timeout for BGEM3HybridEmbeddings (***REMOVED***210)
...
Step 6: Коммит (ТОЛЬКО эти файлы):
  ...
  git commit -m "fix(embeddings): add tenacity retry + granular timeout for BGE-M3 ***REMOVED***210
...
ЛОГИРОВАНИЕ в /repo/logs/worker-embed-retry.log (APPEND):
Перед каждым шагом пиши в лог через Bash:
  echo "[START] $(date +%H:%M:%S) Step N: description" >> /repo/logs/worker-embed-retry.log
После каждого шага:
  echo "[DONE] $(date +%H:%M:%S) Step N: result" >> /repo/logs/worker-embed-retry.log
В конце:
  echo "[COMPLETE] $(date +%H:%M:%S) Worker finished" >> /repo/logs/worker-embed-retry.log
