W-TRIAGE: Backlog audit and issue triage (Issue ***REMOVED***175)
...
КОНТЕКСТ:
Issue ***REMOVED***175: backlog audit v2 — review all open issues, dedupe, reprioritize.
Уже сделано: первый triage pass (комментарий в ***REMOVED***175 с решениями по каждому issue).
Подтверждено код-ревью: ***REMOVED***155, ***REMOVED***164, ***REMOVED***166, ***REMOVED***167, ***REMOVED***168 — реально сломаны/открыты.
***REMOVED***174 уже закрыт как дуп ***REMOVED***166.

Приоритетный порядок из triage:
1. ***REMOVED***71 (epic)
2. ***REMOVED***164 (chaos tests)
3. ***REMOVED***166 (validation retry)
4. ***REMOVED***167 (baseline contamination)
5. ***REMOVED***168 (validation thresholds)
6. ***REMOVED***155 (redisvl vectorizer)
7. ***REMOVED***162 (SDK hooks)
8. ***REMOVED***147 (latency dashboards)
...
4. Закрой очевидные дупликаты с комментарием (duplicate of ***REMOVED***XX)
...
ЛОГИРОВАНИЕ в /repo/logs/worker-triage.log (APPEND):
Формат: echo "[START/DONE] $(date +%H:%M:%S) Task N: description" >> /repo/logs/worker-triage.log
В конце: echo "[COMPLETE] $(date +%H:%M:%S) Worker triage finished" >> /repo/logs/worker-triage.log
