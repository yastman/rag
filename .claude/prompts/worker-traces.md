...
2. Запустить trace validation:
   uv run python -m scripts.validate_traces --collection gdrive_bge --report 2>&1 | tee /home/user/projects/rag-fresh/logs/trace-validation-report.log
...
ЛОГИРОВАНИЕ в /home/user/projects/rag-fresh/logs/worker-traces.log (APPEND):
echo "[START] $(date +%H:%M:%S) Step N: description" >> /home/user/projects/rag-fresh/logs/worker-traces.log
echo "[DONE] $(date +%H:%M:%S) Step N: result" >> /home/user/projects/rag-fresh/logs/worker-traces.log
echo "[COMPLETE] $(date +%H:%M:%S) Worker finished" >> /home/user/projects/rag-fresh/logs/worker-traces.log
