...
ЛОГИРОВАНИЕ в /repo/logs/worker-rebuild.log (APPEND):
echo "[START] $(date +%H:%M:%S) Step N: description" >> /repo/logs/worker-rebuild.log
echo "[DONE] $(date +%H:%M:%S) Step N: result" >> /repo/logs/worker-rebuild.log
echo "[COMPLETE] $(date +%H:%M:%S) Worker finished" >> /repo/logs/worker-rebuild.log
