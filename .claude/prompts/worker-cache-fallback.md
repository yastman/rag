...
ЛОГИРОВАНИЕ в /home/user/projects/rag-fresh/logs/worker-cache-fallback.log (APPEND):
Перед каждым шагом:
  echo "[START] $(date +%H:%M:%S) Step N: description" >> /home/user/projects/rag-fresh/logs/worker-cache-fallback.log
После каждого шага:
  echo "[DONE] $(date +%H:%M:%S) Step N: result" >> /home/user/projects/rag-fresh/logs/worker-cache-fallback.log
В конце:
  echo "[COMPLETE] $(date +%H:%M:%S) Worker finished" >> /home/user/projects/rag-fresh/logs/worker-cache-fallback.log
