...
ЛОГИРОВАНИЕ в /repo/logs/worker-t1-hybrid.log (APPEND):
echo "[START] $(date +%H:%M:%S) Step N: description" >> /repo/logs/worker-t1-hybrid.log
echo "[DONE] $(date +%H:%M:%S) Step N: result" >> /repo/logs/worker-t1-hybrid.log
В конце: echo "[COMPLETE] $(date +%H:%M:%S) Worker T1 finished" >> /repo/logs/worker-t1-hybrid.log
