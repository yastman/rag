...
Работай из /repo
GitHub Issue: ***REMOVED***44 — Task 9: E2E bot test
...
4. Запусти E2E тесты:
   uv run python scripts/e2e/runner.py --skip-judge 2>&1 | tee /repo/logs/e2e-tests.txt
5. Если runner.py не работает — попробуй:
   uv run pytest tests/e2e/ -v --tb=short 2>&1 | tee /repo/logs/e2e-tests.txt
6. Запиши результат
...
Если E2E вообще не может запуститься (нет Telegram session) — запиши причину в лог и выполни хотя бы smoke тесты повторно с живыми Docker:
   uv run pytest tests/smoke/ -v --tb=short 2>&1 | tee /repo/logs/e2e-smoke-fallback.txt
   uv run pytest tests/integration/ -v --tb=short 2>&1 | tee -a /repo/logs/e2e-smoke-fallback.txt

НЕ делай git commit.

Логирование в /repo/logs/worker-e2e.log (APPEND):
[START] timestamp
[DONE] timestamp
[COMPLETE] timestamp
