W-AUDIT: Audit remediation CI + smoke tests (***REMOVED***91)
...
3. Коммит: fix(ci): consolidate dev deps + remove legacy_api markers (***REMOVED***91)
4. ВАЖНО: pyproject.toml также меняется в ***REMOVED***121 (redis version bump). Твоя область — секция [dependency-groups] и [project.optional-dependencies].dev. НЕ трогай версию redis.
