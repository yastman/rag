W-TTFT: TTFT variance + provider metadata (***REMOVED***124)
...
ПЛАН: /repo/docs/plans/2026-02-11-ttft-variance-plan.md
Работай из /repo
...
- ВАЖНО: generate.py уже изменён в ***REMOVED***103 (error spans в except блоках). НЕ трогай error span код (get_client().update_current_span). Твоя область — основной flow: response.model, TTFT замер, return dict.
- ВАЖНО: state.py уже изменён в ***REMOVED***108 (score_improved field). Добавляй llm_* поля ПОСЛЕ score_improved.

ПРАВИЛА:
1. git commit — ТОЛЬКО конкретные файлы. НЕ git add -A.
2. Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com> в каждом коммите.
3. Коммит: feat(observability): add provider metadata + TTFT tracking (***REMOVED***124)

ЛОГИРОВАНИЕ в /repo/logs/worker-ttft.log (APPEND):
echo "[START] $(date +%H:%M:%S) Step N: description" >> /repo/logs/worker-ttft.log
echo "[DONE] $(date +%H:%M:%S) Step N: result" >> /repo/logs/worker-ttft.log
echo "[COMPLETE] $(date +%H:%M:%S) Worker finished" >> /repo/logs/worker-ttft.log
