W-COLL: Baseline collector — per-trace metrics computation (***REMOVED***167)
...
ЛОГИРОВАНИЕ в /repo/logs/worker-baseline-collector.log (APPEND):
В начале каждого таска: echo "[START] $(date +%H:%M:%S) Task N: description" >> /repo/logs/worker-baseline-collector.log
В конце каждого: echo "[DONE] $(date +%H:%M:%S) Task N: result" >> /repo/logs/worker-baseline-collector.log
После всех задач: echo "[COMPLETE] $(date +%H:%M:%S) Worker finished" >> /repo/logs/worker-baseline-collector.log
