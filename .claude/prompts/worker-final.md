...
Работай из /repo
GitHub Issue: ***REMOVED***44 — Task 12: Final verification
...
1. Unit тесты (полный прогон):
   uv run pytest tests/unit/ -q --tb=short 2>&1 | tee /repo/logs/final-unit.txt

2. Smoke тесты:
   uv run pytest tests/smoke/ -q --tb=short 2>&1 | tee /repo/logs/final-smoke.txt

3. Integration тесты (если Docker up):
   uv run pytest tests/integration/ -q --tb=short 2>&1 | tee /repo/logs/final-integration.txt

4. Chaos тесты:
   uv run pytest tests/chaos/ -q --tb=short 2>&1 | tee /repo/logs/final-chaos.txt

5. Создай финальный отчёт /repo/logs/final-verification-report.txt:
...
Логирование в /repo/logs/worker-final.log (APPEND):
[START] timestamp
[DONE] timestamp
[COMPLETE] timestamp
