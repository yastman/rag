...
ЛОГИРОВАНИЕ в /repo/logs/worker-validate-run.log (APPEND):
echo "[START] $(date +%H:%M:%S) Task N: description" >> /repo/logs/worker-validate-run.log
echo "[DONE] $(date +%H:%M:%S) Task N: result" >> /repo/logs/worker-validate-run.log
echo "[COMPLETE] $(date +%H:%M:%S) Worker finished" >> /repo/logs/worker-validate-run.log
