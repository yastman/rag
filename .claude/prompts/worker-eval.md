W-EVAL: Fix Settings env-independence in tests + evaluation lazy init (***REMOVED***191)
...
ПЛАН: /repo/docs/plans/2026-02-12-ci-green-sweep-design.md
Работай из /repo
Ветка: fix/191-ci-green-sweep (уже checkout). НЕ ПЕРЕКЛЮЧАЙСЯ на другие ветки.

ЗАДАЧИ (выполняй по порядку):

Task 4: Fix RAGPipeline test env-independence (***REMOVED***186)
...
Task 5a: Lazy Settings() in extract_ground_truth.py (***REMOVED***187)
  ...
  Фикс:
  1. Удали строку sys.path.append("/srv/contextual_rag") — legacy VPS artifact.
...
Task 5b: Lazy Settings() in search_engines.py (***REMOVED***188)
  ...
  1. Удали sys.path.append("/srv/contextual_rag").
...
Commit message: fix(ci): decouple test pipeline from env + lazy eval Settings (***REMOVED***191)
...
ЛОГИРОВАНИЕ в /repo/logs/worker-eval.log (APPEND):
