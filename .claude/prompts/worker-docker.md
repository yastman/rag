...
5. Запости статус в ***REMOVED***120:
   gh issue comment 120 --body "***REMOVED******REMOVED*** Phase 3: Docker Validation

   ***REMOVED******REMOVED******REMOVED*** Services Status
   (вставь вывод docker compose ps)

   ***REMOVED******REMOVED******REMOVED*** Bot Logs (last 20 lines)
   (вставь ключевые строки)

   ***REMOVED******REMOVED******REMOVED*** Result
   Bot healthy / Bot FAILED (опиши проблему)"

ЛОГИРОВАНИЕ в /repo/logs/worker-docker.log (APPEND):
echo "[TAG] $(date +%H:%M:%S) message" >> /repo/logs/worker-docker.log
