W-CHAOS: Fix chaos LLM fallback tests (Issue ***REMOVED***164, PR ***REMOVED***180)
...
КОНТЕКСТ:
Issue ***REMOVED***164: Все 16 chaos тестов в tests/chaos/test_llm_fallback.py падают с TypeError.
Тесты написаны под старый LLMService API (api_key=..., client=...).
Текущий LLMService имеет другой конструктор.
PR ***REMOVED***180 уже создан на ветке fix/164-chaos-llm-api, но CI lint ПАДАЕТ.
...
3. Прочитай PR ***REMOVED***180 diff: git diff main..origin/fix/164-chaos-llm-api -- tests/chaos/
...
ФАЗА 2 — ВЫПОЛНЕНИЕ:
1. Обнови тесты чтобы использовать текущий LLMService API (смотри что сделано в PR ***REMOVED***180 как reference)
...
ЛОГИРОВАНИЕ в /repo/logs/worker-chaos.log (APPEND):
Формат: echo "[START/DONE] $(date +%H:%M:%S) Task N: description" >> /repo/logs/worker-chaos.log
В конце: echo "[COMPLETE] $(date +%H:%M:%S) Worker chaos finished" >> /repo/logs/worker-chaos.log
