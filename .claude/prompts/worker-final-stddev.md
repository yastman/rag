...
Работай из /repo. Ветка: fix/168-validation-thresholds (уже в ней). Tasks 1-4 уже завершены.
...
7. В generate_report, обнови Go/No-Go таблицу (в секции "if go_no_go:"):
   -- Заголовок: "| ***REMOVED*** | Criterion | Target | Actual | Stddev | Status |"
   -- Разделитель: "|---|-----------|--------|--------|--------|--------|"
...
11. Коммит: feat(validation): add stddev column to Go/No-Go report ***REMOVED***168

== Task 6: Update docs ==

1. Прочитай .claude/rules/observability.md

2. В секции "***REMOVED******REMOVED*** Thresholds (regression detection)":
...
3. В секции "***REMOVED******REMOVED*** Trace Validation (***REMOVED***110, ***REMOVED***143)":
   -- Удали упоминание "Reference trace c2b95d86 --- anomalous (5213s), not reproducible."
   -- Добавь: "Go/No-Go thresholds are config-driven via tests/baseline/thresholds.yaml go_no_go section (***REMOVED***168)."

4. Коммит: docs(observability): update thresholds docs for ***REMOVED***168
...
ЛОГИРОВАНИЕ в /repo/logs/worker-final-stddev.log (APPEND):
echo "[START] $(date +%H:%M:%S) Task N Step M: description" >> /repo/logs/worker-final-stddev.log
echo "[DONE] $(date +%H:%M:%S) Task N Step M: result" >> /repo/logs/worker-final-stddev.log
echo "[COMPLETE] $(date +%H:%M:%S) W-FINAL finished" >> /repo/logs/worker-final-stddev.log
