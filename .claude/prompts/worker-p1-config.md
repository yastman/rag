W-P1-CONFIG: Fix 3 P1/P2 issues (***REMOVED***116, ***REMOVED***118, ***REMOVED***119)
...
Task 4.1 (***REMOVED***116): Sync rewrite_max_tokens default
...
- Commit: git add telegram_bot/graph/config.py && commit с "Closes ***REMOVED***116"

Task 4.2 (***REMOVED***118): test_redis_cache legacy imports
...
- Commit: git add tests/integration/test_redis_cache.py && commit с "Closes ***REMOVED***118"

Task 4.4 (***REMOVED***119): mypy duplicate module name
...
- Commit: git add измененные файлы && commit с "Closes ***REMOVED***119"
...
ЛОГИРОВАНИЕ в /repo/logs/worker-p1-config.log (APPEND):
Каждое действие логируй через: echo "[TAG] $(date +%H:%M:%S) message" >> /repo/logs/worker-p1-config.log
Теги: [START], [DONE], [ERROR], [COMPLETE]
В конце: echo "[COMPLETE] $(date +%H:%M:%S) Worker P1-CONFIG finished" >> /repo/logs/worker-p1-config.log
