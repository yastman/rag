...
ЛОГИРОВАНИЕ в /repo/logs/worker-docker-rebuild.log (APPEND):
echo "[START] $(date +%H:%M:%S) Task N: description" >> /repo/logs/worker-docker-rebuild.log
echo "[DONE] $(date +%H:%M:%S) Task N: result" >> /repo/logs/worker-docker-rebuild.log
echo "[COMPLETE] $(date +%H:%M:%S) Worker finished" >> /repo/logs/worker-docker-rebuild.log
