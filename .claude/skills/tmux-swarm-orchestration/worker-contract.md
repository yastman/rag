...
***REMOVED******REMOVED*** Финал part-worker (контракт B-part — параллельная часть)
...
Orch использует GrepAI MCP tools ДО классификации для понимания scope и зависимостей:
    ...
    ***REMOVED*** 1. Semantic search — найти код, затронутый issue:
    grepai_search(query="{issue_description}", limit=5, format="toon", compact=true)

    ***REMOVED*** 2. Call graph — impact analysis:
    ...
    ***REMOVED*** 3. Полный граф (для COMPLEX+):
...
**Когда:** COMPLEX / VERY COMPLEX — исследование + план, БЕЗ кода.
    ...
    ***REMOVED******REMOVED*** Группы выполнения
    ***REMOVED******REMOVED******REMOVED*** Группа A (worker-1): задачи 1, 3, 5
    Файлы: src/models/user.py, tests/test_user.py
    ***REMOVED******REMOVED******REMOVED*** Группа B (worker-2): задачи 2, 4
    Файлы: src/services/cache.py, tests/test_cache.py
    ***REMOVED******REMOVED******REMOVED*** Группа C (финальная, после A+B): задачи 6, 7
