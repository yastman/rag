***REMOVED*** Инфраструктура

Механика tmux, worktree, сигналов, таймаутов, очистки.

***REMOVED******REMOVED*** Идентификация оркестратора
...
***REMOVED******REMOVED*** Инициализация

    PROJECT_ROOT=$(git rev-parse --show-toplevel)
    mkdir -p logs .signals

***REMOVED******REMOVED*** Task List координация (cross-session)

Shared Task List — основной канал координации orch ↔ workers. Работает через `CLAUDE_CODE_TASK_LIST_ID`.

    ***REMOVED*** Orch обычно запускается БЕЗ CLAUDE_CODE_TASK_LIST_ID → получает случайный UUID.
    ***REMOVED*** Шаг 1: Создать хотя бы 1 задачу (чтобы task list появился на диске):
    TaskCreate(subject="...", description="...", metadata={issue: N, contract: "A"})

    ***REMOVED*** Шаг 2: Обнаружить свой task list ID (самая свежая директория):
    TASK_LIST_ID=$(ls -t ~/.claude/tasks/ | head -1)

    ***REMOVED*** Шаг 3: DAG зависимости:
    TaskUpdate(taskId="2", addBlockedBy=["1"])

    ***REMOVED*** Шаг 4: Worker спавнится с тем же ID (см. Запуск воркеров)
    ***REMOVED*** Worker видит задачи через TaskList, берёт через TaskUpdate(in_progress)
    ***REMOVED*** Orch АВТОМАТИЧЕСКИ видит изменения через system-reminder — БЕЗ polling

    ***REMOVED*** АЛЬТЕРНАТИВА: если orch запущен С CLAUDE_CODE_TASK_LIST_ID (named):
    ***REMOVED*** TASK_LIST_ID="$CLAUDE_CODE_TASK_LIST_ID"  ***REMOVED*** уже задан

***REMOVED******REMOVED******REMOVED*** Как orch узнаёт о прогрессе

System-reminder **автоматически** пушит состояние task list в контекст orch:

    ***REMOVED*** Orch НЕ вызывает TaskList — система сама показывает:
    ***REMOVED*** > ***REMOVED***1 [completed] Fix auth bug
    ***REMOVED*** > ***REMOVED***2 [in_progress] Add tests  [owner: W-AUTH]
    ***REMOVED*** > ***REMOVED***3 [pending] Integration check  [blocked by ***REMOVED***1, ***REMOVED***2]

    ***REMOVED*** Orch видит completed → сразу переходит к Phase 5 (code review)
    ***REMOVED*** Orch видит in_progress → продолжает свою работу (другие workers, SDK research)
...
***REMOVED******REMOVED******REMOVED*** Named task lists (для multi-session)

    ***REMOVED*** Вместо UUID — именованный ID:
    export CLAUDE_CODE_TASK_LIST_ID="sprint-42"
    ***REMOVED*** Хранится в ~/.claude/tasks/sprint-42/
    ***REMOVED*** Полезно для: возобновление после крэша, аудит
...
**КРИТИЧНО:** Untracked файлы (планы, промты) НЕ попадают в worktree. `git worktree add` копирует только tracked (committed) файлы. Если промт ссылается на untracked файл — скопировать вручную:
    ...
    ***REMOVED*** После создания worktree — копировать untracked файлы, на которые ссылается промт:
    WT="${PROJECT_ROOT}-wt-{name}"
    cp "${PROJECT_ROOT}/docs/plans/{plan}.md" "${WT}/docs/plans/" 2>/dev/null
    ***REMOVED*** Или: закоммитить план ДО создания worktree (предпочтительно)

**НИКОГДА `claude --worktree`** для tmux workers — удаляет worktree при exit, коммиты ПОТЕРЯНЫ.

***REMOVED******REMOVED******REMOVED*** Изоляция секретов

`.env` gitignored → НЕ копируется в worktree автоматически. Это правильно.

    ***REMOVED*** Для тестов — только тестовые значения:
...
***REMOVED******REMOVED*** Запуск воркеров
...
**КРИТИЧНО:** `env VAR=val command` вместо `VAR=val command` — tmux send-keys может разбить длинную строку, и shell увидит `VAR=val` как отдельную команду. `env` решает это.
    ...
    ***REMOVED*** Sonnet (Контракт A/B) — worktree:
    ...
    ***REMOVED*** Opus (Контракт C) — корень проекта:
    ...
    ***REMOVED*** Opus Соло (Контракт D) — worktree:
    TMUX="" tmux new-window -n "W-{NAME}" -c "$WT_PATH"
    sleep 5
    TMUX="" tmux send-keys -t "W-{NAME}" C-c
    sleep 0.5
...
***REMOVED******REMOVED*** Сигнализация

***REMOVED******REMOVED******REMOVED*** Файловые сигналы (основной метод)

Worker → orch через JSON-файлы в `.signals/`. **Атомарная запись** (write .tmp → mv) предотвращает partial reads:

    ***REMOVED*** Worker пишет при завершении:
...
**Основной метод:** system-reminder автоматически пушит состояние task list в контекст orch. Orch **не поллит** — он продолжает работу (SDK research, спавн других workers), и система сама уведомляет о смене статусов.
    ...
    ***REMOVED*** Orch НЕ делает sleep/polling. Вместо этого:
    ***REMOVED*** 1. Спавнит worker'ов
    ***REMOVED*** 2. Продолжает свою работу (Phase 2.7 для следующего issue, etc.)
    ***REMOVED*** 3. System-reminder показывает: "***REMOVED***1 [completed]" → orch переходит к Phase 5
    ***REMOVED***
    ***REMOVED*** Если orch хочет явно проверить:
    TaskList  ***REMOVED*** → текущее состояние всех задач
...
***REMOVED******REMOVED******REMOVED*** Чтение сигналов через context-mode

При обработке сигнала — используй context-mode для верификации (output не засоряет контекст):

    ***REMOVED*** Проверить все сигналы за один вызов:
    ...
    ***REMOVED*** Code review diff через context-mode (Фаза 5.0):
    mcp execute(language="shell", code="""
      cd '{WT_PATH}' && git diff dev...HEAD 2>&1
    """, intent="code review: verify changes match issue ***REMOVED***{N}")

***REMOVED******REMOVED******REMOVED*** tmux send-keys (запасной метод)
...
***REMOVED******REMOVED*** Таймауты

Watchdog запускается параллельно с каждым worker:

    ***REMOVED*** Запуск watchdog (сразу после запуска worker):
    ( sleep ${TIMEOUT} && \
...
***REMOVED******REMOVED*** Маркеры скиллов в логах
...
***REMOVED******REMOVED*** Центральный лог оркестратора

Orch логирует ВСЕ события в `.signals/orch-log.jsonl`:

    ***REMOVED*** Хелпер (в начале сессии):
...
***REMOVED******REMOVED*** Двухволновой запуск (COMPLEX+)

1. **Волна 1:** Opus C → план → сигнал с `execution` решением
2. **Волна 2:** Orch читает сигнал и действует:

***REMOVED******REMOVED******REMOVED*** execution: sequential (по умолчанию)

    ***REMOVED*** 1 Sonnet B с полным планом:
    git worktree add "${PROJECT_ROOT}-wt-${issue}" -b "fix/${issue}-impl"
    ***REMOVED*** Запуск Контракт B → 1 PR

***REMOVED******REMOVED******REMOVED*** execution: parallel (Opus решил)

    ***REMOVED*** Orch парсит группы из плана и запускает волнами:

    ***REMOVED*** Волна 2a: независимые группы — параллельно
    ...
    ***REMOVED*** Sonnet B в каждом worktree со СВОЕЙ частью плана

    ***REMOVED*** Волна 2b: зависимые группы — после завершения тех, от кого зависят
    ***REMOVED*** Orch ждёт сигналы от grpA/grpB → запускает grpC

    ***REMOVED*** Волна 2c: финальная интеграция
    ***REMOVED*** Последний worker: rebase всех part-веток в основную ветку
    git worktree add "${PROJECT_ROOT}-wt-${issue}-final" -b "fix/${issue}"
    ***REMOVED*** В промте финального worker:
    ***REMOVED***   git merge --no-ff origin/fix/${issue}-part-a
    ***REMOVED***   git merge --no-ff origin/fix/${issue}-part-b
    ***REMOVED***   Разрешить конфликты если есть → make check → 1 PR
...
***REMOVED******REMOVED*** Безопасность tmux

- НИКОГДА `tmux new-session` (отключает юзера)
- ВСЕГДА `TMUX=""` (вложенные операции)
- Сокет сломался: `pkill -USR1 tmux`

***REMOVED******REMOVED*** VPS удалённый запуск

    SESSION=$(TMUX="" tmux list-sessions -F "***REMOVED***{session_name}" | head -1)
