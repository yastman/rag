...
***REMOVED******REMOVED*** ORCH Identity (Phase 5.1)

    ***REMOVED*** ОДИН вызов — discover + generate + rename + save:
    ORCH_ID=$(uv run python scripts/tmux_orch_identity.py init)
    ***REMOVED*** → ORCH-a1b2c3d4 (stdout) + .claude/orch-identity.json (state)

    ***REMOVED*** Позже:
    uv run python scripts/tmux_orch_identity.py get              ***REMOVED*** → ORCH-a1b2c3d4
...
***REMOVED******REMOVED*** Worktree Setup (Phase 5.2)
...
***REMOVED******REMOVED*** Spawn Commands (Phase 5.4)
...
***REMOVED******REMOVED*** Signal: tmux wait-for (Phase 4→5 bridge)

**Механизм:** true push IPC через tmux. 0 polling, 0 tokens, instant notification.

**Orch сторона** (после каждого spawn):

    ***REMOVED*** Блокирующий wait в background — Claude получит <task-notification> при сигнале
    Bash(
        command='TMUX="" tmux wait-for worker-{name}-done && echo "W-{NAME} COMPLETE"',
        run_in_background=True
    )
...
***REMOVED******REMOVED*** Cleanup после signal

    TMUX="" tmux kill-window -t "$SESSION:W-{NAME}" 2>/dev/null

***REMOVED******REMOVED*** Merge + Cleanup
...
***REMOVED******REMOVED*** Железное правило
