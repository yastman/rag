...
***REMOVED******REMOVED******REMOVED*** OpenCode: pane оркестратора
...
Если пользователь вызвал skill в текущем окне оркестратора и worker'ы запускаются через `opencode`, НЕ полагайся на имя или номер окна. Сохрани точный pane id:
    ...
    tmux display-message -p -t "$ORCH_PANE" '***REMOVED***{pane_id}'
...
***REMOVED******REMOVED*** Координация orch ↔ workers
...
***REMOVED******REMOVED******REMOVED*** OpenCode worker
...
`opencode run --file` используется только как fallback, если TUI `--prompt` не работает.
    ...
    ***REMOVED*** Primary: visible TUI with a short prompt-file handoff.
...
Fallback only if `opencode --prompt` is impossible:
    ...
    TMUX="" tmux new-window -t "$(tmux display-message -p '***REMOVED***{session_name}')" -n "W-{NAME}" -c "$WT_PATH" \
...
Paste fallback only if both native prompt modes are impossible. Target the exact pane id and use bracketed paste:
    ...
    PANE=$(TMUX="" tmux new-window -P -F '***REMOVED***{pane_id}' -t "$(tmux display-message -p '***REMOVED***{session_name}')" -n "W-{NAME}" -c "$WT_PATH" 'opencode')
    sleep 8
...
- paste fallback: только target pane id (`new-window -P -F '***REMOVED***{pane_id}'`) + `paste-buffer -p`
...
***REMOVED******REMOVED******REMOVED*** OpenCode wake-up (основной метод)
...
***REMOVED******REMOVED******REMOVED*** Orch мониторинг (event-driven через signals)
...
***REMOVED******REMOVED******REMOVED*** Context budget guardrails
...
***REMOVED******REMOVED******REMOVED*** tmux send-keys wake-up
...
***REMOVED******REMOVED*** Feedback ledger
...
***REMOVED******REMOVED******REMOVED*** execution: sequential (по умолчанию)
    ...
    ***REMOVED*** 1 OpenCode B с полным планом:
    git worktree add "${PROJECT_ROOT}-wt-${issue}" -b "fix/${issue}-impl"
    ***REMOVED*** Запуск Контракт B → 1 PR

***REMOVED******REMOVED******REMOVED*** execution: parallel (Codex решил)
    ...
    ***REMOVED*** OpenCode B-part в каждом worktree со СВОЕЙ частью плана
