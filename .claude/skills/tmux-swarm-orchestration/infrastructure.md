...
Промты → `.claude/prompts/worker-{name}.md`. **НИКОГДА** inline.
    ...
    ***REMOVED*** Как Контракт C, но -c "$WT_PATH"
...
Worker → orch через JSON-файлы в `.signals/`:
    ...
    ***REMOVED*** Контракт C — вместо PR указывает путь к плану:
    echo '{"status":"done","worker":"W-{NAME}","plan":"{path}","ts":"'$(date -Iseconds)'"}' \
      > {PROJECT_ROOT}/.signals/worker-{name}.json

***REMOVED******REMOVED******REMOVED*** Orch мониторинг

    ***REMOVED*** Вариант 1: inotifywait (мгновенный, если доступен)
    ...
    ***REMOVED*** Вариант 2: polling (универсальный, 30 сек)
