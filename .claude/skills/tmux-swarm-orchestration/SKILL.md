...
***REMOVED******REMOVED*** Low-Token OpenCode Protocol
...
1. Зафиксируй pane оркестратора ДО запуска worker'ов:
       ...
       ORCH_PANE=$(tmux display-message -p '***REMOVED***{pane_id}')
       tmux display-message -p -t "$ORCH_PANE" '***REMOVED***{pane_id}'  ***REMOVED*** sanity check
...
***REMOVED******REMOVED*** GitHub + Notion Control Plane
...
***REMOVED******REMOVED******REMOVED*** Project Bootstrap
...
***REMOVED******REMOVED******REMOVED*** Phase 0: Issue Intake → Notion Task
...
4. Найти существующую task в `Tasks Tracker`:
   - сначала query `Task name` contains `GH ***REMOVED***123`;
...
5. Если task не найдена — создать page в `Tasks Tracker`:
       ...
       Task name: "GH ***REMOVED***123: {issue title}"
       Status: "Not started"
       Source: "Codex"
       Owner / Agent: "Codex"
...
7. Page body обязан иметь или получить эти секции:
       ...
       ***REMOVED******REMOVED*** AI Brief
       Source: GitHub issue ***REMOVED***123
       URL:
       Goal:
       Acceptance criteria:
       Reserved files:
       ...
       ***REMOVED******REMOVED*** Orchestrator State
       Status: Queued
       Branch:
       Worktree:
       Worker agent/model:
...
***REMOVED******REMOVED******REMOVED*** Notion Status Updates
...
**Фаза 2.7: SDK КОНТЕКСТ** — **ОБЯЗАТЕЛЬНА** если `docs/engineering/sdk-registry.md` или `.codex/rules/sdk-registry.md` существует:
    ...
    ***REMOVED*** 1. Проверить наличие реестра:
    SDK_REGISTRY=""
    ...
    ***REMOVED*** 2. Если есть — прочитать и матчить triggers:
    Read "$SDK_REGISTRY"
    Для каждого SDK: сравнить triggers с issue body keywords
    Матч = SDK затронут → запомнить context7_id + как_у_нас + gotchas

    ***REMOVED*** 3. При матче — Codex собирает актуальный SDK context до запуска worker:
    ...
    ***REMOVED*** 4. Собрать {sdk_registry_excerpt} — релевантные блоки из реестра для промта worker'а
    ***REMOVED*** Включает: как_у_нас + паттерны + gotchas для каждого совпавшего SDK

    ***REMOVED*** 5. Даже без матча — список SDK (имена + triggers) для awareness в промте worker'а
    ***REMOVED*** 6. Нет реестра → поведение как раньше (по усмотрению orch)

Переиспользование: одно исследование → N воркеров.

**Фаза 2.9: КОММИТ/КОПИРОВАНИЕ АРТЕФАКТОВ** — перед созданием worktree:

    ***REMOVED*** Планы, промты — всё что нужно worker'у — либо закоммитить, либо явно скопировать в worktree:
    git add docs/plans/{plan}.md .codex/prompts/worker-{name}.md
    git commit -m "chore: add plan and worker prompts for issue ***REMOVED***{N}"
    ***REMOVED*** Без commit/copy worktree НЕ будет содержать эти файлы (git worktree = snapshot коммита)
...
ПЕРЕД спавном КАЖДОГО OpenCode worker'а orch ОБЯЗАН выполнить ВСЕ шаги. Пропуск = сломанный worker.
    ...
    ***REMOVED*** ШАГ 0: Изоляция worker'а
    ***REMOVED*** Создать отдельный git worktree для ЛЮБОГО worker'а (A/B/C/D/part/final):
    git worktree add "${PROJECT_ROOT}-wt-{name}" -b "{branch_name}"
    mkdir -p "${PROJECT_ROOT}-wt-{name}/logs"
    ***REMOVED*** Reserved files НЕ должны пересекаться с другими активными worker'ами.
    ***REMOVED*** Если пересекаются — сделать DAG/последовательную волну, НЕ параллелить.

    ***REMOVED*** ШАГ 1: Написать prompt в файл (см. worker-contract.md)
    ***REMOVED*** Промт ОБЯЗАН содержать:
    ***REMOVED*** - "***REMOVED******REMOVED*** IMPLEMENTATION BRIEF" с goal, acceptance criteria, exact files, non-goals, tests
    ***REMOVED*** - "***REMOVED******REMOVED*** WORKTREE ISOLATION" с worktree, branch, reserved files
    ***REMOVED*** - "***REMOVED******REMOVED*** SUPERPOWERS WORKFLOW" с обязательным порядком skill'ов
    ***REMOVED*** - "***REMOVED******REMOVED*** VERIFICATION LADDER" с focused-first проверками и условиями для broad suite
    ***REMOVED*** - "***REMOVED******REMOVED*** SIGNALING" с ORCH_PANE и SIGNAL_FILE
    ***REMOVED*** - "***REMOVED******REMOVED*** DONE JSON" со схемой результата

    ***REMOVED*** ШАГ 2: Спавнить видимый OpenCode TUI через tmux + short file-handoff --prompt (см. infrastructure.md)
    ***REMOVED*** Основной режим: отдельное окно, opencode TUI сразу получает короткую ссылку на prompt file.
    ***REMOVED*** НЕ использовать paste-buffer для основного пути.
...
БЕЗ секции "***REMOVED******REMOVED*** IMPLEMENTATION BRIEF" → worker будет додумывать задачу и часто сделает не то.
БЕЗ секции "***REMOVED******REMOVED*** SUPERPOWERS WORKFLOW" → worker может пропустить план/TDD/review/verify/finish.
БЕЗ секции "***REMOVED******REMOVED*** VERIFICATION LADDER" → worker либо недотестирует, либо зря гоняет broad suite на каждом small task.
...
Пример {sdk_registry_excerpt} для issue "добавить кнопку в меню":
    ...
    ***REMOVED******REMOVED*** Другие SDK в проекте (awareness):
...
**Фаза 2.3: CODEBASE КОНТЕКСТ** — orch использует 3 системы:
    ...
    ***REMOVED*** 1. GrepAI — semantic search + call graph:
    ...
    grepai_trace_graph(symbol="{key_symbol}", depth=1, format="toon")  ***REMOVED*** полный граф
    grepai_index_status(format="toon")  ***REMOVED*** проверить актуальность

    ***REMOVED*** 2. LSP — точные типы, сигнатуры, references:
    LSP(operation="documentSymbol", filePath="{file}")  ***REMOVED*** структура файла
    LSP(operation="hover", filePath="{file}", line=N, character=M)  ***REMOVED*** тип + docstring
    LSP(operation="findReferences", filePath="{file}", line=N, character=M)  ***REMOVED*** все ссылки
    LSP(operation="incomingCalls", filePath="{file}", line=N, character=M)  ***REMOVED*** кто вызывает

    ***REMOVED*** 3. context-mode — сбор контекста без засорения окна:
...
Codex анализирует задачи, зависимости и file overlap ДО запуска worker'ов и выбирает `"execution": "sequential"` или `"parallel"`.
    ...
    ***REMOVED*** Orch читает SIGNAL_FILE path из `[DONE] W-{NAME} {absolute_signal_file}`:
...
***REMOVED******REMOVED******REMOVED*** Уровень 0: Code Review (orch лично — ОБЯЗАТЕЛЬНО)

Orch **лично** читает diff через context-mode. Worker может пройти тесты, но решить не ту проблему.

    ***REMOVED*** Для кодовых контрактов (A/B/D) — diff в sandbox, summary в контекст:
    mcp execute(language="shell", code="""
      cd '{WT_PATH}'
      base=$(gh pr view "{pr}" --json baseRefName --jq .baseRefName)
      git fetch origin "$base"
      git diff "origin/$base"...HEAD 2>&1
    """, intent="code review: verify changes match issue ***REMOVED***{N} requirements")

    ***REMOVED*** Для контракта C — план в sandbox:
...
***REMOVED******REMOVED******REMOVED*** Уровень 1: Артефактная проверка
...
***REMOVED******REMOVED******REMOVED*** Уровень 2: Маркеры скиллов (дополнительный)
...
***REMOVED******REMOVED******REMOVED*** Решение при провале
...
***REMOVED******REMOVED******REMOVED*** Review-Fix Loop
...
***REMOVED******REMOVED******REMOVED*** Фаза 5.2: Обновление SDK реестра (orch лично)

После PASS верификации — **orch сам** анализирует diff и обновляет реестр. Worker'ы НЕ участвуют в мета-анализе.

    ***REMOVED*** 1. Orch анализирует diff через context-mode (уже прочитан в Phase 5.0):
    mcp execute(language="shell", code="""
      cd '{WT_PATH}'
      echo '=== Новые импорты ==='
      base=$(gh pr view "{pr}" --json baseRefName --jq .baseRefName)
    ...
    ***REMOVED*** 2. Orch ЛИЧНО сверяет с реестром:
    Read "$SDK_REGISTRY"
    ***REMOVED*** Для каждого нового import/dep:
    ***REMOVED***   - Есть в реестре? → проверить: новый паттерн использования?
    ***REMOVED***   - Нет в реестре? → добавить секцию по шаблону
    ***REMOVED*** Для изменённых файлов:
    ***REMOVED***   - Новый паттерн SDK (отличается от как_у_нас)? → обновить

    ***REMOVED*** 3. При обнаружении обновлений — orch редактирует реестр в dev:
    git stash  ***REMOVED*** если есть незакоммиченное в dev
    Edit "$SDK_REGISTRY"
    git add "$SDK_REGISTRY"
    git commit -m "docs(sdk): update registry - {краткое описание}"
    git stash pop  ***REMOVED*** вернуть если было
...
***REMOVED******REMOVED*** Контекст-бюджет
...
***REMOVED******REMOVED*** Фаза 6: Финальный отчёт

После завершения всех воркеров orch генерирует:

    ***REMOVED******REMOVED*** Отчёт сессии
...
После каждого run append findings в `${CODEX_HOME:-$HOME/.codex}/swarm-feedback/{repo}.md`: worker stalls, prompt gaps, local env blockers, skill fixes.
    |-------|---------|----------|--------|-------|----|--------|
    | ***REMOVED***{N}  | CLEAR   | A        | W-{name} | {m}m{s}s | ***REMOVED***{pr} | done |
    | ***REMOVED***{N}  | COMPLEX | B        | W-{name} | {m}m | ***REMOVED***{pr} | done |
    | ***REMOVED***{N}  | MEDIUM  | A        | W-{name} | — | — | timeout→escalate |
...
***REMOVED******REMOVED*** Вспомогательные файлы
