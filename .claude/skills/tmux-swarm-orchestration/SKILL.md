...
***REMOVED******REMOVED******REMOVED*** Фаза 5.2: Обновление SDK реестра

После PASS верификации — orch проверяет `sdk_updates` из сигнала worker'а и diff.

    ***REMOVED*** 1. Читать sdk_updates из сигнала:
    ...
    ***REMOVED*** 2. Если есть updates ИЛИ diff затрагивает новую зависимость:
    mcp execute(language="shell", code="""
      cd '{WT_PATH}'
      ***REMOVED*** Новые импорты из diff:
      git diff dev...HEAD | grep -E '^\+.*from .* import|^\+.*import ' | head -20
      ***REMOVED*** Изменения в pyproject.toml:
      git diff dev...HEAD -- pyproject.toml 2>/dev/null | head -20
    """, intent="check for new SDK patterns or dependencies in worker diff")

    ***REMOVED*** 3. При обнаружении обновлений — orch редактирует реестр:
    test -f .claude/rules/sdk-registry.md && {
      ***REMOVED*** Обновить существующую секцию SDK (новый паттерн/gotcha)
      ***REMOVED*** ИЛИ добавить новый SDK по шаблону в конце реестра
