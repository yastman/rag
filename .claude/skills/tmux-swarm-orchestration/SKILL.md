...
description: Use when given GitHub issues to solve, problems to fix, or "review all open issues". Also for running heavy commands on VPS through user's terminal. Triggers on "реши issue", "fix ***REMOVED***123", "изучи все открытые", "parallel workers", "swarm", "spawn-claude", "tmux воркеры", "VPS команда"
---

***REMOVED*** tmux Swarm Оркестрация v10
...
**Фаза 2.7: SDK КОНТЕКСТ** — **ОБЯЗАТЕЛЬНА** если `.claude/rules/sdk-registry.md` существует:
    ...
    ***REMOVED*** 3. При матче — Sonnet субагент для актуальной документации:
...
**Фаза 2.9: КОММИТ АРТЕФАКТОВ** — перед созданием worktree:
    ...
    ***REMOVED*** Планы, промты — всё что нужно worker'у — ЗАКОММИТИТЬ:
    git add docs/plans/{plan}.md .claude/prompts/worker-{name}.md
    git commit -m "chore: add plan + worker prompts for issue ***REMOVED***{N}"
    ***REMOVED*** Без этого worktree НЕ будет содержать эти файлы (git worktree = snapshot коммита)
...
ПЕРЕД спавном КАЖДОГО worker'а orch ОБЯЗАН выполнить ВСЕ 4 шага. Пропуск = сломанный worker.
    ...
    ***REMOVED*** ШАГ 1: Создать задачи в Task List
    TaskCreate(subject="...", description="...", metadata={issue: N, contract: "A"})
    ***REMOVED*** DAG если нужно:
    TaskUpdate(taskId="2", addBlockedBy=["1"])

    ***REMOVED*** ШАГ 2: Найти TASK_LIST_ID
    TASK_LIST_ID=$(ls -t ~/.claude/tasks/ | head -1)

    ***REMOVED*** ШАГ 3: Написать промт в файл (см. worker-contract.md)
    ***REMOVED*** Промт ОБЯЗАН содержать секцию "***REMOVED******REMOVED*** TASK LIST" с taskId и алгоритмом

    ***REMOVED*** ШАГ 4: Спавнить через tmux (см. infrastructure.md)
...
БЕЗ секции "***REMOVED******REMOVED*** TASK LIST" в промте → worker не знает свой taskId → не обновляет статус.
...
После завершения всех воркеров orch генерирует:
    ...
    ***REMOVED******REMOVED*** Отчёт сессии
    ...
    |-------|---------|----------|--------|-------|-----------|-----|--------|
    | ***REMOVED***{N}  | CLEAR   | A        | W-{name} | {m}m{s}s | ~$3 | ***REMOVED***{pr} | done |
    | ***REMOVED***{N}  | COMPLEX | C→B      | W-{name} | {m}m | ~$8 | ***REMOVED***{pr} | done |
    | ***REMOVED***{N}  | MEDIUM  | A        | W-{name} | — | ~$3 | — | timeout→escalate |
