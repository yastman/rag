...
```
k8s/
├── base/                          ***REMOVED*** Base manifests
│   ├── kustomization.yaml         ***REMOVED*** 22 resources, labels
│   ├── namespace.yaml             ***REMOVED*** Namespace: rag
│   ├── configmaps/                ***REMOVED*** postgres-init, litellm-config
│   ├── postgres/                  ***REMOVED*** PVC + Deployment + Service
│   ├── redis/                     ***REMOVED*** Deployment + Service
│   ├── qdrant/                    ***REMOVED*** PVC + Deployment + Service
│   ├── docling/                   ***REMOVED*** PVC + Deployment + Service
│   ├── bge-m3/                    ***REMOVED*** Deployment + Service (hostPath)
│   ├── user-base/                 ***REMOVED*** Deployment + Service (hostPath)
│   ├── litellm/                   ***REMOVED*** Deployment + Service
│   ├── bot/                       ***REMOVED*** Deployment (5 init containers)
│   └── ingestion/                 ***REMOVED*** Deployment (replicas: 0)
├── overlays/
│   ├── core/                      ***REMOVED*** postgres, redis, qdrant (11 resources)
│   ├── bot/                       ***REMOVED*** core + ML + litellm + bot (19 resources)
│   ├── ingest/                    ***REMOVED*** core + docling + bge-m3 + ingestion (17 resources)
│   └── full/                      ***REMOVED*** Everything (23 resources)
├── secrets/
│   └── .env.example               ***REMOVED*** Template (gitignored .env)
├── images-prepull.txt             ***REMOVED*** Public images for k3s pre-pull
└── k3s-config.yaml                ***REMOVED*** Server config template
```

***REMOVED******REMOVED*** Makefile Targets
...
***REMOVED******REMOVED*** Docker Bake (Parallel Builds)

```bash
docker buildx bake --load           ***REMOVED*** All 5 images (parallel)
docker buildx bake bot --load       ***REMOVED*** Single target
docker buildx bake stack-bot --load ***REMOVED*** Bot + bge-m3 + user-base
```
...
***REMOVED******REMOVED*** Image Transfer to VPS

```bash
***REMOVED*** Single image
make k3s-push-bot    ***REMOVED*** docker save | ssh vps 'k3s ctr import'

***REMOVED*** Manual (with progress)
docker save rag/bot:latest | pv | ssh vps 'sudo k3s ctr -n k8s.io images import -'
```

***REMOVED******REMOVED*** k3s Server Config
...
***REMOVED******REMOVED*** Pre-pull Manifest
...
***REMOVED******REMOVED*** Resource Requirements (VPS)
...
***REMOVED******REMOVED*** LiteLLM ConfigMap Sync
...
***REMOVED******REMOVED*** Kustomize Notes
...
***REMOVED******REMOVED*** VPS Setup (Phase 1)

```bash
***REMOVED*** 1. Install k3s
sudo mkdir -p /etc/rancher/k3s
sudo cp k8s/k3s-config.yaml /etc/rancher/k3s/config.yaml
curl -sfL https://get.k3s.io | sh -

***REMOVED*** 2. Pre-pull public images
sudo cp k8s/images-prepull.txt /var/lib/rancher/k3s/agent/images/rag-prepull.txt

***REMOVED*** 3. Build + transfer custom images (from WSL2)
docker buildx bake --load
for img in bot bge-m3 user-base docling ingestion; do
  make k3s-push-$img
done

***REMOVED*** 4. Create secrets + deploy
make k3s-secrets
make k3s-bot
make k3s-status
```

***REMOVED******REMOVED*** CRM Env Vars (bot deployment)

Added in ***REMOVED***384/***REMOVED***390/***REMOVED***402: `KOMMO_LEAD_SCORE_FIELD_ID`, `KOMMO_LEAD_BAND_FIELD_ID`, `NURTURING_ENABLED`, `NURTURING_INTERVAL_MINUTES`, `FUNNEL_ROLLUP_CRON`. Add to `k8s/secrets/.env` before deploying.
