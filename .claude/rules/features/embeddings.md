...
***REMOVED******REMOVED*** LangChain Wrappers (LangGraph integration)

***REMOVED******REMOVED******REMOVED*** BGEM3HybridEmbeddings (preferred — single API call)
...
***REMOVED*** Shared httpx.AsyncClient with connection pooling (reused across calls)
...
***REMOVED******REMOVED******REMOVED*** BGEM3Embeddings (dense) — legacy, use BGEM3HybridEmbeddings instead
...
***REMOVED*** Async (used by cache_check_node)
vector = await emb.aembed_query("search text")            ***REMOVED*** list[float], 1024-dim
vectors = await emb.aembed_documents(["doc1", "doc2"])     ***REMOVED*** list[list[float]]

***REMOVED*** Sync (for non-async code)
vector = emb.embed_query("search text")
```

Wraps BGE-M3 `/encode/dense` endpoint (fixed from legacy `/encode`). Batching via `batch_size=32`.

***REMOVED******REMOVED******REMOVED*** BGEM3SparseEmbeddings (sparse) — legacy, use BGEM3HybridEmbeddings instead
...
***REMOVED*** Async (used by retrieve_node)
sv = await sparse.aembed_query("search text")      ***REMOVED*** dict with sparse vector
svs = await sparse.aembed_documents(["d1", "d2"])   ***REMOVED*** list[dict]
```

Wraps BGE-M3 `/encode/sparse` endpoint (fixed from legacy `/encode`). Returns `lexical_weights` format.

***REMOVED******REMOVED*** Embedding Models
...
***REMOVED******REMOVED*** BGE-M3 API Endpoints
...
pytest tests/unit/integrations/test_embeddings.py -v  ***REMOVED*** LangChain wrappers
```
...
***REMOVED******REMOVED******REMOVED*** Adding new embedding model
