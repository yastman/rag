...
with propagate_attributes(session_id=..., user_id=..., tags=["telegram", "rag"]):
    ...
    _write_langfuse_scores(lf, result)  ***REMOVED*** 12 scores
```

***REMOVED******REMOVED*** Supervisor Path (USE_SUPERVISOR=true)
...
***REMOVED*** 3. Langfuse scores: agent_used (CATEGORICAL), supervisor_latency_ms (NUMERIC), supervisor_model (CATEGORICAL)
```
