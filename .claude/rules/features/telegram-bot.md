...
| `telegram_bot/scoring.py` | `score()`, `write_langfuse_scores()`, `write_history_scores()`, `write_crm_scores()` (***REMOVED***310, ***REMOVED***451, ***REMOVED***452) |
| `telegram_bot/main.py` | Entry point |
| `telegram_bot/config.py` | BotConfig (pydantic-settings BaseSettings) |
| `telegram_bot/agents/rag_pipeline.py` | 6-step async RAG pipeline: `_cache_check → _hybrid_retrieve → _grade_documents → _rerank → _rewrite_query → _cache_store` (***REMOVED***442) |
| `telegram_bot/agents/agent.py` | `create_bot_agent()` — `langchain.agents.create_agent` SDK, `ChatOpenAI` client, `get_prompt` (***REMOVED***413) |
...
| `telegram_bot/agents/hitl.py` | `hitl_guard()` — LangGraph `interrupt()` for HITL confirmation on CRM write ops (***REMOVED***443) |
| `telegram_bot/agents/manager_tools.py` | `build_tools_for_role()` — role-gating (client vs manager), `sync_pending_lead_scores` tool |
| `telegram_bot/agents/utility_tools.py` | `mortgage_calculator`, `daily_summary`, `handoff` @tools (***REMOVED***445) |
...
| `telegram_bot/pipelines/client.py` | `run_client_pipeline()` — deterministic client fast-path: classify → intent gate → cache → RAG → generate → post-process (***REMOVED***567) |
...
| `telegram_bot/services/response_style_detector.py` | `ResponseStyleDetector` — zero-latency regex style/difficulty classifier (***REMOVED***129) |
...
| `telegram_bot/middlewares/i18n.py` | I18nMiddleware — locale detection, injects `i18n`, `locale`, `property_bot`, `apartments_service` (***REMOVED***660) |
...
| `telegram_bot/agents/apartment_tools.py` | `apartment_search` @tool — agent escalation for complex queries (***REMOVED***632) |

***REMOVED******REMOVED*** Text RAG Pipeline (6 async steps, ***REMOVED***442)
...
**`skip_rewrite`** (***REMOVED***567): When `True`, bypasses the rewrite loop entirely. Used by client pipeline for FAQ queries where rewrite adds latency without improving retrieval.

***REMOVED******REMOVED*** Voice LangGraph Pipeline (11 nodes)
...
***REMOVED******REMOVED******REMOVED*** Edges (conditional routing)
...
|---------|---------|-------------|
| `/start` | cmd_start | Welcome via i18n key `welcome-text` (fallback: `services.yaml`) + localized ReplyKeyboard (***REMOVED***660) |
...
| (menu button) | handle_menu_button | Routes 6 ReplyKeyboard buttons to dedicated handlers (***REMOVED***628) |
| (callback `svc:`) | handle_service_callback | Service cards, back/menu navigation (***REMOVED***628) |
| (callback `cta:`) | handle_cta_callback | CTA actions: get_offer → phone FSM, manager (***REMOVED***628) |
| (callback `fav:`) | handle_favorite_callback | Favorites: add/remove/viewing/viewing_all (***REMOVED***628) |
| (callback `results:`) | handle_results_callback | Results: more/refine/viewing (***REMOVED***628) |
| (callback) | handle_feedback | Like/dislike feedback (***REMOVED***229) |
| (callback `cc:`) | handle_clearcache_callback | Clear selected cache tier (***REMOVED***566) |
...
***REMOVED******REMOVED******REMOVED*** Callback Routing (inline keyboards)
...
Promotions section: `config/services.yaml` → `promotions:` list with `emoji`, `title`, `text` per item (***REMOVED***628).

***REMOVED******REMOVED******REMOVED*** Funnel → Hybrid Search (***REMOVED***628)
...
***REMOVED******REMOVED******REMOVED*** PhoneCollector → Kommo CRM (***REMOVED***628)
...
***REMOVED******REMOVED******REMOVED*** Handler Registration Order (`_register_handlers`)
...
***REMOVED******REMOVED******REMOVED*** SDK-backed Constraints (aiogram 3)
...
| `supervisor_model` | `SUPERVISOR_MODEL` | `gpt-4o-mini` | Model for agent routing decisions (***REMOVED***413: create_agent SDK) |
...
***REMOVED******REMOVED******REMOVED*** GraphConfig (pipeline tuning)
...
| `show_sources` | `SHOW_SOURCES` | `false` | Source attribution footer + inline citations in responses (***REMOVED***225) |
...
self._embeddings = self._hybrid  ***REMOVED*** primary embeddings provider
...
***REMOVED******REMOVED*** handle_voice Flow
...
***REMOVED******REMOVED*** handle_query Flow (dual-path routing)
...
***REMOVED******REMOVED******REMOVED*** sdk_agent path (manager role)
...
***REMOVED******REMOVED*** Client Pipeline (`pipelines/client.py`, ***REMOVED***567)
...
run_client_pipeline(user_text, user_id, session_id, message, cache, ...) → PipelineResult
  ...
  5. Generate: generate_response(message=message) with streaming (***REMOVED***571: message forwarded for chunked edits)
...
**Embedding passthrough (***REMOVED***571):** Pre-agent computes dense+sparse+ColBERT in one `aembed_hybrid_with_colbert()` call. All three are stashed in `rag_result_store` (on both HIT and MISS) and passed to `rag_pipeline()` via `pre_computed_sparse`/`pre_computed_colbert` params, eliminating redundant BGE-M3 calls.
...
***REMOVED******REMOVED*** generate_response Service
...
generate_response(query, documents, message?, config?, llm?, ...) →
  ...
  5. Streaming path: asyncio.gather(LLM stream, placeholder send) → chunks (300ms throttle) → finalize Markdown
...
When `STREAMING_ENABLED=true` (default), `generate_response` streams LLM output directly to Telegram. Placeholder send and LLM stream creation run in parallel via `asyncio.gather` (***REMOVED***675), saving 100-300ms Telegram round-trip. Chunks throttled at 300ms, finalized with Markdown parse_mode. Sets `response_sent=True` → `respond_node` skips duplicate send.
...
**asyncio.gather safety (***REMOVED***675):** Uses `return_exceptions=True` — if placeholder send fails (Telegram rate-limit), LLM stream is preserved and error propagates to non-streaming fallback. Prevents `UnboundLocalError` on `sent_msg`.
...
```bash
pytest tests/unit/test_bot_handlers.py -v                ***REMOVED*** Bot handlers + client direct pipeline routing
pytest tests/unit/test_bot_menu_handlers.py -v           ***REMOVED*** Client menu: 47 tests for buttons, callbacks, FSM guard (***REMOVED***628)
pytest tests/unit/pipelines/test_client_pipeline.py -v   ***REMOVED*** Client pipeline: classify, intent, cache, RAG, generate
pytest tests/unit/test_middlewares.py -v
pytest tests/unit/graph/ -v                              ***REMOVED*** All graph tests (incl. test_transcribe_node.py)
pytest tests/unit/agents/ -v                             ***REMOVED*** Agent tests (factory, context, tools, CRM, history, streaming, skip_rewrite)
pytest tests/unit/services/test_generate_response.py -v  ***REMOVED*** Shared generation service
pytest tests/unit/dialogs/test_menu_routing.py -v        ***REMOVED*** Menu routing dialog tests
pytest tests/unit/keyboards/ -v                          ***REMOVED*** Keyboard builder tests
pytest tests/unit/handlers/test_phone_collector.py -v    ***REMOVED*** Phone collector FSM + CRM lead tests
pytest tests/unit/handlers/test_phone_crm_integration.py -v  ***REMOVED*** CRM integration tests (***REMOVED***628)
pytest tests/unit/dialogs/test_funnel.py -v              ***REMOVED*** Funnel dialog + hybrid search tests (***REMOVED***628)
pytest tests/unit/dialogs/test_funnel_results.py -v      ***REMOVED*** Funnel filter building + results getter (***REMOVED***628)
pytest tests/unit/services/test_content_loader_promotions.py -v  ***REMOVED*** Promotions config tests (***REMOVED***628)
pytest tests/unit/services/test_favorites_service.py -v  ***REMOVED*** Favorites service tests
pytest tests/integration/test_graph_paths.py -v          ***REMOVED*** Graph path tests incl. voice flow (~5s, no Docker)
pytest tests/smoke/test_langgraph_pipeline.py -v         ***REMOVED*** Smoke tests
```
...
***REMOVED******REMOVED*** CRM Integration (***REMOVED***384, ***REMOVED***390, ***REMOVED***402, ***REMOVED***413)

***REMOVED******REMOVED******REMOVED*** Agent CRM Tools (8 @tool, Kommo API v4)
...
***REMOVED******REMOVED******REMOVED*** Kommo Infrastructure (***REMOVED***413)
...
***REMOVED******REMOVED******REMOVED*** Lead Scoring
...
- **Lifecycle:** `HotLeadNotifier` wired in bot startup (***REMOVED***402)

***REMOVED******REMOVED******REMOVED*** Nurturing & Funnel Analytics
...
***REMOVED******REMOVED******REMOVED*** CRM Config
...
***REMOVED******REMOVED******REMOVED*** Langfuse Scores (CRM — `write_crm_scores()` ***REMOVED***455)
