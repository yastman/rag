...
***REMOVED******REMOVED******REMOVED*** FilterDialog — Dynamic Hub + Back Buttons
...
When `STREAMING_ENABLED=true` (default), `generate_response` streams LLM output to Telegram via native `sendMessageDraft` (Bot API 9.5, ***REMOVED***744). Draft updates sent every 200ms with smooth animation. After streaming completes, `message.answer()` persists the final message in chat history.
...
pytest tests/unit/dialogs/test_filter_constants.py -v    ***REMOVED*** Shared filter constants + build_filters_dict
pytest tests/unit/dialogs/test_filter_dialog.py -v       ***REMOVED*** FilterDialog hub, select handlers, on_apply/reset
