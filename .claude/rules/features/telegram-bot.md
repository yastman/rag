...
Telegram interface for domain-configurable search (default: Bulgarian property) with async RAG pipeline (***REMOVED***442), LangGraph voice path, streaming-compatible responses, and rate limiting.
...
| `telegram_bot/scoring.py` | `write_langfuse_scores()` + `write_crm_scores()` (***REMOVED***440) |
| `telegram_bot/main.py` | Entry point |
| `telegram_bot/config.py` | BotConfig (pydantic-settings BaseSettings) |
| `telegram_bot/agents/rag_pipeline.py` | `rag_pipeline()` — 6-step async pipeline: cache → retrieve → grade → rerank → rewrite → cache_store (***REMOVED***442) |
...
| `telegram_bot/agents/agent.py` | `create_bot_agent()` — wraps `langchain.agents.create_agent` SDK (***REMOVED***413) |
| `telegram_bot/agents/context.py` | `BotContext` dataclass (13 fields) — DI via `context_schema` into tools |
| `telegram_bot/agents/rag_tool.py` | `rag_search` @tool — wraps `rag_pipeline()` (***REMOVED***442), returns formatted context |
| `telegram_bot/agents/history_tool.py` | `history_search` @tool — wraps 5-node history sub-graph (with guard ***REMOVED***432) |
| `telegram_bot/agents/crm_tools.py` | 8 CRM @tools for Kommo API (get/create/update deals, contacts, notes, tasks) |
| `telegram_bot/agents/history_graph/` | History sub-graph: guard → retrieve → grade → rewrite → summarize (5 nodes ***REMOVED***432) |
...
```python
***REMOVED*** Download .ogg → bytes → inject into state
voice = message.voice
...
***REMOVED*** Same graph.ainvoke(state) — transcribe_node runs first via route_start
```
...
***REMOVED******REMOVED*** handle_query Flow (create_agent SDK since ***REMOVED***413)

```python
***REMOVED*** handle_query always delegates to _handle_query_supervisor
***REMOVED*** 1. Build tools list
tools = [rag_search]  ***REMOVED*** @tool from agents/rag_tool.py
if history_service: tools.append(history_search)  ***REMOVED*** @tool from agents/history_tool.py
if kommo_enabled: tools.extend(get_crm_tools())   ***REMOVED*** 8 @tools from agents/crm_tools.py

***REMOVED*** 2. Create agent via SDK
agent = create_bot_agent(model=config.supervisor_model, tools=tools, checkpointer=...)

***REMOVED*** 3. Build BotContext for DI (injected into tools via context_schema)
ctx = BotContext(telegram_user_id=..., session_id=..., kommo_client=..., embeddings=..., ...)

***REMOVED*** 4. Invoke with CallbackHandler for Langfuse
...
pytest tests/unit/agents/ -v                             ***REMOVED*** Agent tests (factory, context, tools, CRM, history, streaming)
...
***REMOVED******REMOVED******REMOVED*** Langfuse Scores (CRM)
...
***REMOVED******REMOVED******REMOVED*** Adding new graph node
