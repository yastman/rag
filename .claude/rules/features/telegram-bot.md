...
| `telegram_bot/graph/config.py` | GraphConfig dataclass (service factories, `skip_rerank_threshold=0.85`, `generate_max_tokens=2048`, `max_rewrite_attempts=1`, `rewrite_model`/`rewrite_max_tokens`, `streaming_enabled`) |
...
***REMOVED******REMOVED*** LangGraph Pipeline (9 nodes)
...
pytest tests/unit/graph/ -v                    ***REMOVED*** All graph tests (~106 tests)
pytest tests/integration/test_graph_paths.py -v          ***REMOVED*** 6 graph path integration tests (~5s, no Docker)
