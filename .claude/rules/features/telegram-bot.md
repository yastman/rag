...
| `telegram_bot/scoring.py` | `write_langfuse_scores()` + `compute_checkpointer_overhead_proxy_ms()` (extracted ***REMOVED***310) |
...
| `supervisor_model` | `SUPERVISOR_MODEL` | `gpt-4o-mini` | Model for supervisor routing decisions (***REMOVED***310: supervisor-only since v3.3) |
...
***REMOVED******REMOVED*** handle_query Flow (supervisor-only since ***REMOVED***310)

```python
***REMOVED*** handle_query always delegates to _handle_query_supervisor
***REMOVED*** 1. Build tools (rag_search wraps existing RAG graph, direct_response, optionally history_search)
tools = [create_rag_agent(...), direct_response]
if history_service: tools.append(create_history_search_tool(history_service))

***REMOVED*** 2. Supervisor LLM routes to appropriate tool
...
***REMOVED*** 3. Langfuse trace + scores
lf = get_client()
lf.update_current_trace(input=..., output=..., metadata=...)
***REMOVED*** Scores: agent_used (CATEGORICAL), supervisor_latency_ms (NUMERIC), supervisor_model (CATEGORICAL)
```
...
pytest tests/unit/agents/ -v                             ***REMOVED*** Supervisor tests (state, tools, routing, observability)
...
***REMOVED******REMOVED*** CRM Integration (***REMOVED***384, ***REMOVED***390, ***REMOVED***402)

***REMOVED******REMOVED******REMOVED*** Supervisor CRM Tools
