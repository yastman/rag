...
```
Server-side ColBERT path (preferred, ***REMOVED***569):
...
***REMOVED******REMOVED*** Server-Side ColBERT Reranking (***REMOVED***569)
...
***REMOVED*** Flow: search cache → sparse cache → ColBERT path or RRF fallback → cache results
result = await retrieve_node(state, cache=cache, sparse_embeddings=sparse, qdrant=qdrant)
***REMOVED*** Returns: {documents, search_results_count, sparse_embedding, rerank_applied, latency_stages}
```
