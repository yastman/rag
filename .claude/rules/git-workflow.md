...
***REMOVED*** Git & PR Workflow

***REMOVED******REMOVED*** PR Discipline
...
| **Linking** | Always include `Closes ***REMOVED***N` or `Related ***REMOVED***N` in PR body |
...
***REMOVED******REMOVED*** Bug Handling During PR Review

| Found during review | Action |
|---------------------|--------|
| Bug **in PR code** (new/changed lines) | PR review comment → author fixes in same PR |
| **Pre-existing** bug (existed before PR) | New issue with `Found during ***REMOVED***PR` in description |
...
***REMOVED******REMOVED*** Merge Discipline
...
***REMOVED******REMOVED*** Renovate PR Handling
...
| **Langfuse Docker** | Pinned (disabled) | No — upstream bug ***REMOVED***11924 |
...
***REMOVED******REMOVED*** Branch Cleanup

```bash
make git-hygiene          ***REMOVED*** Audit report: stale branches, orphan worktrees
make git-hygiene-fix      ***REMOVED*** Auto-cleanup merged branches
```
...
***REMOVED******REMOVED*** Parallel Sessions & Git Worktrees
...
***REMOVED******REMOVED******REMOVED*** Method 1: `claude --worktree` (recommended)

```bash
***REMOVED*** User sessions — each terminal gets isolated worktree
claude --worktree feature-auth    ***REMOVED*** → .claude/worktrees/feature-auth/, branch worktree-feature-auth
claude --worktree bugfix-123      ***REMOVED*** → .claude/worktrees/bugfix-123/, branch worktree-bugfix-123
claude --worktree                 ***REMOVED*** → random name (e.g. bright-running-fox)
```
...
***REMOVED******REMOVED******REMOVED*** Method 2: Manual `git worktree` (custom paths/branches)

```bash
***REMOVED*** Create worktree based on dev (not main!)
git worktree add /repo-wt-{name} -b feat/{name} dev

***REMOVED*** Agent works in its own directory — no branch conflicts
***REMOVED*** After done: merge feat branch into dev, then clean up
git checkout dev && git merge feat/{name}
git worktree remove /repo-wt-{name}
```

***REMOVED******REMOVED******REMOVED*** When to use what
