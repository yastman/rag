...
***REMOVED******REMOVED*** Full Stack (14 containers)
...
***REMOVED******REMOVED*** Docker Profiles (Fast Startup)
...
***REMOVED******REMOVED******REMOVED*** Combining Profiles

```bash
***REMOVED*** Bot + observability
docker compose -f docker-compose.dev.yml --profile bot --profile obs up -d

***REMOVED*** Using COMPOSE_PROFILES env var
COMPOSE_PROFILES=bot,obs docker compose -f docker-compose.dev.yml up -d
```

***REMOVED******REMOVED******REMOVED*** Startup Times (warm images)
...
***REMOVED******REMOVED*** VPS Stack (Local Embeddings)
...
***REMOVED*** With ingestion profile
...
**Feature flags:**
```bash
RETRIEVAL_DENSE_PROVIDER=bge_m3_api  ***REMOVED*** bge_m3_api | voyage
RERANK_PROVIDER=colbert              ***REMOVED*** colbert | none | voyage
BGE_M3_URL=http://bge-m3:8000
```

**Collection:** `gdrive_documents_bge` (1024-dim BGE-M3 dense + BGE-M3 sparse, field name "bm42" kept)

***REMOVED******REMOVED******REMOVED*** VPS Code Deployment (Hot Reload)

Volume mounts позволяют обновлять код без rebuild:

```yaml
***REMOVED*** docker-compose.vps.yml (ingestion service)
volumes:
  - ./src:/app/src:ro
...
***REMOVED*** 2. Restart (НЕ rebuild)
ssh vps "docker restart vps-ingestion"
***REMOVED*** Готово за 5 секунд
```
...
***REMOVED*** Очистка (осторожно!)
ssh vps "docker builder prune -f"           ***REMOVED*** Build cache
ssh vps "docker image prune -f"             ***REMOVED*** Dangling images
***REMOVED*** НИКОГДА: docker system prune --volumes    ***REMOVED*** Удалит данные!
```

***REMOVED******REMOVED******REMOVED*** VPS Troubleshooting
...
***REMOVED******REMOVED*** LLM Gateway (LiteLLM)
...
```bash
***REMOVED*** Start full dev stack (Qdrant, Redis, Langfuse, MLflow, bot)
docker compose -f docker-compose.dev.yml up -d

***REMOVED*** Build and restart bot only
docker compose -f docker-compose.dev.yml build bot
docker compose -f docker-compose.dev.yml up -d bot

***REMOVED*** Check bot logs
docker logs dev-bot -f

***REMOVED*** Verify bot health
...
```bash
make monitoring-up          ***REMOVED*** Start Loki, Promtail, Alertmanager
make monitoring-down        ***REMOVED*** Stop monitoring stack
make monitoring-status      ***REMOVED*** Check health
make monitoring-test-alert  ***REMOVED*** Send test alert to Telegram
make monitoring-logs        ***REMOVED*** View logs
```
...
***REMOVED******REMOVED*** Parallel Claude Workers
...
***REMOVED******REMOVED*** Docker Hardening (Feb 2026)
...
docker inspect dev-litellm --format '{{json .HostConfig.LogConfig}}'  ***REMOVED*** max-size: 50m
...
RUN uv sync --frozen --no-dev --no-install-project  ***REMOVED*** deps layer
COPY . .
RUN uv sync --frozen --no-dev                        ***REMOVED*** code layer
...
***REMOVED******REMOVED*** Loki Rules Mount (Feb 2026)
...
```yaml
***REMOVED*** Before (broken)
- ./docker/monitoring/rules:/etc/loki/rules:ro
***REMOVED*** After (working)
- ./docker/monitoring/rules:/etc/loki/rules/fake:ro
```

**Verification:**
```bash
curl -sS http://localhost:3100/loki/api/v1/rules | head -5  ***REMOVED*** Should return YAML, not 400
```
