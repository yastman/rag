...
***REMOVED******REMOVED******REMOVED*** Profile-gated services
...
***REMOVED******REMOVED******REMOVED*** Hot Reload (no rebuild)

Volume mounts allow code updates without image rebuild:
```bash
rsync -avz src/ vps:/opt/rag-fresh/src/
ssh vps "docker restart vps-bot"   ***REMOVED*** ~5 seconds
```
...
***REMOVED*** Cleanup (careful — never prune volumes):
...
***REMOVED******REMOVED*** Building Custom Images
...
make monitoring-down        ***REMOVED*** Stop
...
***REMOVED******REMOVED*** Security & Environment
...
**Image drift check:**
```bash
make verify-compose-images   ***REMOVED*** Check running containers match pinned digests
```

***REMOVED******REMOVED*** PostgreSQL Schema (Init Scripts)
