...
make baseline-check                          ***REMOVED*** Smoke + compare against main-latest
```

***REMOVED******REMOVED*** Langfuse UI Workflow

**Local URL:** http://localhost:3001

***REMOVED******REMOVED******REMOVED*** Debugging a Slow Query
...
***REMOVED******REMOVED******REMOVED*** Comparing Baseline Runs
...
***REMOVED******REMOVED******REMOVED*** Finding Cache Problems
...
***REMOVED******REMOVED******REMOVED*** Tracking Costs
...
***REMOVED******REMOVED******REMOVED*** Session ID Format
...
***REMOVED******REMOVED******REMOVED*** Key Filters
...
| LLM calls | +15% | Alert if call count increases (widened from 5% per ***REMOVED***168) |
...
| `bot.py` _handle_query_supervisor | `telegram-rag-supervisor` | Supervisor root span (always-on since ***REMOVED***310) |

***REMOVED******REMOVED******REMOVED*** Supervisor Spans (***REMOVED***242, ***REMOVED***310)

Supervisor is the only query path (monolith removed in ***REMOVED***310). Trace tree:
```
...
***REMOVED******REMOVED******REMOVED*** Entry Point Pattern (Orphan Prevention)
...
***REMOVED******REMOVED******REMOVED*** Graph Nodes (9 nodes, all covered)
...
***REMOVED******REMOVED******REMOVED*** Payload Bloat Prevention (***REMOVED***143)
...
***REMOVED******REMOVED******REMOVED*** Error Span Tracking (***REMOVED***103 P1.2)
...
***REMOVED******REMOVED******REMOVED*** Cache (9 methods)
...
***REMOVED******REMOVED******REMOVED*** LLM Calls (auto-traced via langfuse.openai.AsyncOpenAI)
...
***REMOVED******REMOVED******REMOVED*** OTEL Configuration

```yaml
OTEL_SERVICE_NAME: rag-bot  ***REMOVED*** Set in docker-compose.dev.yml bot service
```
...
| `user_feedback` | 0.0/1.0 | User like/dislike via inline button (***REMOVED***229) |

***REMOVED******REMOVED******REMOVED*** LLM-as-a-Judge Scores (async, online sampling)
...
***REMOVED******REMOVED******REMOVED*** CRM Scores (***REMOVED***384, ***REMOVED***390)
...
***REMOVED******REMOVED*** Langfuse Prompt Management
...
```yaml
LANGFUSE_PUBLIC_KEY: ${LANGFUSE_PUBLIC_KEY:-}   ***REMOVED*** optional, empty disables tracing
LANGFUSE_SECRET_KEY: ${LANGFUSE_SECRET_KEY:-}
LANGFUSE_HOST: http://langfuse:3000
```

***REMOVED******REMOVED*** Trace Validation (***REMOVED***110, ***REMOVED***143)
...
Go/No-Go thresholds are config-driven via `tests/baseline/thresholds.yaml` `go_no_go` section (***REMOVED***168).

Go/No-Go criterion `generate_p50_lt_2s` (renamed from `ttft_p50_lt_2s` in ***REMOVED***143) measures full generation latency in non-streaming validation mode. True TTFT requires a streaming phase (see ***REMOVED***144).

***REMOVED******REMOVED*** Baseline Module (***REMOVED***167)
...
```
tests/baseline/
├── collector.py       ***REMOVED*** LangfuseMetricsCollector + SessionMetrics (per-trace computation)
├── manager.py         ***REMOVED*** BaselineManager + BaselineSnapshot (compare logic)
├── cli.py             ***REMOVED*** CLI: compare (--baseline-tag, --current-session, --output), set-baseline
├── thresholds.yaml    ***REMOVED*** Regression detection thresholds
├── conftest.py        ***REMOVED*** Fixtures
└── test_*.py          ***REMOVED*** Tests (25 passing)
```
