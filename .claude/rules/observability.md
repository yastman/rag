...
***REMOVED******REMOVED******REMOVED*** Graph Nodes (11 nodes — 9 core + 2 voice-only)
...
| `llm_ttft_ms` | float | Time to first token (ms); streaming: first chunk, non-streaming: total LLM call time (***REMOVED***571) |
| `llm_tps` | float/None | Tokens per second; streaming: from decode phase, non-streaming: from total call + usage.completion_tokens (***REMOVED***571) |
...
***REMOVED******REMOVED*** Trace Contract (***REMOVED***626)
