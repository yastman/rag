...
***REMOVED******REMOVED******REMOVED*** Hybrid Search with ColBERT Reranking
...
***REMOVED******REMOVED******REMOVED*** Zero-Cost Apartment Search
...
***REMOVED******REMOVED******REMOVED*** Agentic RAG with CRM Tools
...
***REMOVED******REMOVED******REMOVED*** Multi-Level Semantic Cache
...
***REMOVED******REMOVED******REMOVED*** Voice Assistant
...
***REMOVED******REMOVED******REMOVED*** Full Observability
...
***REMOVED******REMOVED******REMOVED*** 1. Install & Configure
...
`make test-bot-health` validates the published local prerequisites used by native bot runs. The authoritative startup preflight still runs in [`telegram_bot/preflight.py`](/repo-issue-1198/telegram_bot/preflight.py) when the bot starts. That runtime preflight still owns the repo-local BGE-M3 contract because BGE-M3 is a service this repository depends on directly, not a generic upstream SDK health path.

***REMOVED******REMOVED******REMOVED*** 3. Or Run Everything in Docker

```bash
make docker-bot-up     ***REMOVED*** Core + bot (production-like)
make docker-full-up    ***REMOVED*** All 23 services
```
...
```
telegram_bot/              ***REMOVED*** Telegram bot (aiogram 3 + aiogram-dialog)
├── handlers/              ***REMOVED***   Message & callback handlers
├── services/              ***REMOVED***   Business logic (search, LLM, cache, apartments, CRM)
├── graph/                 ***REMOVED***   LangGraph voice RAG pipeline (11 nodes)
├── agents/                ***REMOVED***   Sub-agents (history search, HITL)
├── dialogs/               ***REMOVED***   Dialog UI (menus, filters, settings)
├── integrations/          ***REMOVED***   External clients (Redis, Postgres, Langfuse)
├── middlewares/           ***REMOVED***   Throttling, i18n, error handling
└── locales/{ru,uk,en}/    ***REMOVED***   Fluent .ftl translations

src/
├── ingestion/unified/     ***REMOVED***   CocoIndex ingestion pipeline
├── retrieval/             ***REMOVED***   Search engines & evaluation
├── voice/                 ***REMOVED***   LiveKit voice agent
└── api/                   ***REMOVED***   FastAPI RAG API

mini_app/                  ***REMOVED*** Telegram Mini App (React + TypeScript)
k8s/                       ***REMOVED*** Kubernetes manifests (k3s deployment)
src/evaluation/            ***REMOVED*** RAG evaluation (RAGAS, A/B testing)
```
